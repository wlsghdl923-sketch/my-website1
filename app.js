...
let _memoTemplates=[],_memoNameHistory=[],_markerPresets=[];
let _selMarkerColor='',_selMarkerName='';
let _memoEditId=null; // null=신규, id=수정중
...
async function _loadMemoData(){
  try{
    ...
    const r2=await _sbRest.selectOne('site_data','memo_names');
    if(r2&&Array.isArray(r2.value))_memoNameHistory=r2.value;
    else if(r2&&typeof r2.value==='string')_memoNameHistory=JSON.parse(r2.value);
...
async function _saveMemoData(){
  try{
    ...
    await _sbRest.upsert('site_data',{key:'memo_names',value:_memoNameHistory,updated_at:new Date().toISOString()});
...
async function openMemoInput(){
  ...
  document.getElementById('memoInputName').value='';
  document.getElementById('memoInputDesc').value='';
  document.getElementById('memoMarkerName').value='';
  _selMarkerColor='';_selMarkerName='';
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
  _renderMemoNameChips();_renderMemoTemplateChips();_renderMarkerPresetChips();
  document.getElementById('memoInputOverlay').style.display='block';
  setTimeout(()=>document.getElementById('memoInputName').focus(),100);
}

// 표 안에서 메모 추가: 선택 영역을 새로 저장
function openMemoInputFromTable(){
  document.getElementById('tblCtxMenu').style.display='none';
  const sel=window.getSelection();
  if(sel&&!sel.isCollapsed){_memoSavedRange=sel.getRangeAt(0).cloneRange();_memoSavedText=sel.toString().trim();}
  else{_memoSavedText='(표 내 메모)';_memoSavedRange=null;}
  openMemoInput();
}
...
async function openMemoEdit(id){
  ...
  document.getElementById('memoInputName').value=el.dataset.memoName||'';
...
function applyMemoEdit(id){
  ...
  const name=(document.getElementById('memoInputName').value||'').trim();
  const mn=document.getElementById('memoMarkerName').value.trim();
  _selMarkerName=mn;
  const desc=document.getElementById('memoInputDesc').value||'';
  // 데이터 저장
  if(name&&!_memoNameHistory.includes(name)){_memoNameHistory.push(name);if(_memoNameHistory.length>10)_memoNameHistory=_memoNameHistory.slice(-10);}
  if(desc.trim()&&!_memoTemplates.includes(desc.trim())){_memoTemplates.push(desc.trim());if(_memoTemplates.length>10)_memoTemplates=_memoTemplates.slice(-10);}
  if(_selMarkerColor&&_selMarkerName&&!_markerPresets.some(p=>p.color===_selMarkerColor&&p.name===_selMarkerName)){_markerPresets.push({color:_selMarkerColor,name:_selMarkerName});if(_markerPresets.length>10)_markerPresets=_markerPresets.slice(-10);}
  _saveMemoData();
  el.dataset.memoName=name||'메모';el.dataset.memoDesc=desc;el.dataset.memoMc=_selMarkerColor;el.dataset.memoMn=_selMarkerName;
  // 하이라이트 색 갱신
...
function _renderMemoNameChips(){
  const el=document.getElementById('memoNameChips');if(!el)return;
  if(!_memoNameHistory.length){el.innerHTML='';return;}
  el.innerHTML=_memoNameHistory.slice(-10).reverse().map(n=>`<span class="memo-tmpl-chip" onclick="document.getElementById('memoInputName').value=unescape('${escape(n)}')">${n.length>12?n.slice(0,12)+'…':n}</span>`).join('');
}
function _renderMemoTemplateChips(){
  ...
  el.innerHTML=_memoTemplates.slice(-10).reverse().map(t=>`<span class="memo-tmpl-chip" onclick="document.getElementById('memoInputDesc').value=unescape('${escape(t)}')">${t.length>13?t.slice(0,13)+'…':t}</span>`).join('');
}
function _renderMarkerPresetChips(){
  ...
  el.innerHTML=_markerPresets.slice(-10).reverse().map(p=>`<span class="mkr-preset-chip" onclick="applyMarkerPreset('${p.color}','${encodeURIComponent(p.name)}')"><span class="mkr-preset-dot" style="background:${p.color};"></span>${p.name||'마커'}</span>`).join('');
}
function applyMarkerPreset(color,enc){
  const name=decodeURIComponent(enc);
  _selMarkerColor=color;
...
function closeMemoInput(){document.getElementById('memoInputOverlay').style.display='none';_memoSavedRange=null;_memoSavedText='';_memoEditId=null;}

function saveMemoToSelection(){
  if(!_memoSavedRange){closeMemoInput();return;}
  const name=(document.getElementById('memoInputName').value||'').trim();
  const mn=document.getElementById('memoMarkerName').value.trim();
  _selMarkerName=mn;
  const desc=document.getElementById('memoInputDesc').value||'';
  ...
  span.dataset.memoid=id;span.dataset.memoName=name||'메모';span.dataset.memoDesc=desc;span.dataset.memoDate=dateStr;
  span.dataset.memoResolved='false';span.dataset.memoMc=_selMarkerColor;span.dataset.memoMn=_selMarkerName;
  const bg=_selMarkerColor?_hexRgba(_selMarkerColor,.3):'rgba(255,213,79,.38)';
  const bord=_selMarkerColor||'#F57F17';
  ...
  closeMemoInput();setTimeout(refreshMemoPanel,80);toast('메모가 저장되었습니다.');
}
...
const CARD_MIN_H=92,CARD_GAP=6;

function refreshMemoPanel(){
  const editor=document.getElementById('reEditor');
  const panel=document.getElementById('reMemoPanel');
  const list=document.getElementById('reMemoList');
  const mPanel=document.getElementById('reMarkerPanel');
  const mList=document.getElementById('reMarkerList');
  const mStatus=document.getElementById('reMarkerStatus');
  const wrap=document.getElementById('reEditorWrap');
  const lResizer=document.getElementById('markerPanelResizer');
  const rResizer=document.getElementById('memoPanelResizer');
  if(!editor||!panel||!list)return;
  const memos=Array.from(editor.querySelectorAll('.re-memo'));
  if(!memos.length){panel.style.display='none';if(mPanel)mPanel.style.display='none';if(lResizer)lResizer.style.display='none';if(rResizer)rResizer.style.display='none';return;}
  panel.style.display='block';if(rResizer)rResizer.style.display='block';
  // 마커 집계
  ...
  if(mPanel){
    const hasM=Object.keys(markerMap).length>0;
    mPanel.style.display=hasM?'block':'none';if(lResizer)lResizer.style.display=hasM?'block':'none';
    if(hasM&&mList)mList.innerHTML=Object.values(markerMap).map(mk=>`<div class="marker-row"><div class="marker-dot" style="background:${mk.color};"></div><div class="marker-label">${mk.name}</div><div class="marker-count">${mk.count}</div></div>`).join('');
    if(mStatus)mStatus.innerHTML=`<div class="marker-status-row"><span>완료</span><span class="status-done">${doneCount}개</span></div><div class="marker-status-row"><span>미완료</span><span class="status-todo">${todoCount}개</span></div>`;
  }
  // 메모 카드 배치
  const edRect=editor.getBoundingClientRect();
  const scroll=wrap?wrap.scrollTop:0;
  const HEADER_H=36;
  let nextMinY=HEADER_H+8;
  const positions=[];
  memos.forEach((m,i)=>{
    const id=m.dataset.memoid||('m'+i);m.dataset.memoid=id;
    const mRect=m.getBoundingClientRect();
    let y=mRect.top-edRect.top+scroll;
    y=Math.max(nextMinY,y);nextMinY=y+CARD_MIN_H+CARD_GAP;
    positions.push({id,y,m});
  });
  list.style.minHeight=(positions[positions.length-1].y+CARD_MIN_H+40)+'px';
  const validIds=new Set();
  positions.forEach(({id,y,m})=>{
    validIds.add('mc_'+id);
    const isRes=m.dataset.memoResolved==='true';
    const name=m.dataset.memoName||'메모',desc=m.dataset.memoDesc||'',date=m.dataset.memoDate||'';
    ...
    let card=document.getElementById('mc_'+id);
    if(!card){card=document.createElement('div');card.id='mc_'+id;list.appendChild(card);}
    card.className='memo-card'+(isRes?' resolved':'');
    card.style.top=y+'px';
    ...
    card.innerHTML=
      `<div class="memo-card-header"><span class="memo-card-name">${name}</span><span class="memo-card-time">${date}</span></div>`+
      markerHtml+
      `<div class="memo-card-quote">"${quote}"</div>`+
      (desc||isRes?`<div class="memo-card-desc">${isRes?'<span class="memo-done-badge">✓ 완료</span>'+(desc?'<br><span style="color:#6b7280;font-size:.74rem;">'+desc+'</span>':''):desc}</div>`:'')+
      `<div class="memo-card-btns">`+
      (!isRes?`<button class="mresolve" onclick="event.stopPropagation();toggleResolveMemo('${id}')">✓</button>`:
       `<button class="munresolve" onclick="event.stopPropagation();toggleResolveMemo('${id}')">↩</button>`)+
      `<button class="medit" onclick="event.stopPropagation();openMemoEdit('${id}')">수정</button>`+
      `<button class="mdel" onclick="event.stopPropagation();deleteMemoCard('${id}')">삭제</button>`+
      `</div>`;
    // 카드 클릭 → 원문으로 스크롤
    card.onclick=function(e){if(!e.target.closest('button')){highlightMemoCard(m);scrollToMemo(m);}};
  });
  Array.from(list.querySelectorAll('.memo-card')).forEach(c=>{if(!validIds.has(c.id))c.remove();});
}

function scrollToMemo(memoEl){
  if(!memoEl)return;
  const wrap=document.getElementById('reEditorWrap');if(!wrap)return;
  memoEl.scrollIntoView({behavior:'smooth',block:'center'});
}

function highlightMemoCard(memoEl){
  document.querySelectorAll('.re-memo').forEach(m=>m.classList.remove('active'));
  document.querySelectorAll('.memo-card').forEach(c=>c.classList.remove('active'));
  if(!memoEl)return;memoEl.classList.add('active');
  const card=document.getElementById('mc_'+memoEl.dataset.memoid);
  if(card){card.classList.add('active');card.scrollIntoView({behavior:'smooth',block:'nearest'});}
}
function toggleResolveMemo(id){
  const el=document.querySelector(`[data-memoid="${id}"]`);if(!el)return;
  const isRes=el.dataset.memoResolved==='true';
  el.dataset.memoResolved=isRes?'false':'true';
...
function _renderMmgrList(){
  ...
  el.innerHTML=_markerPresets.map((p,i)=>`<div class="mmgr-item"><div class="mmgr-dot" style="background:${p.color};"></div><div style="flex:1;font-size:.82rem;font-weight:600;color:#1e3a5f;">${p.name||'마커'}</div><button class="mbtn-c" style="padding:2px 8px;font-size:.72rem;color:#E53935;" onclick="deleteMarkerPreset(${i})">삭제</button></div>`).join('');
}
async function addMarkerPreset(){
  const name=document.getElementById('mmgrMarkerName').value.trim();
  if(!_mmgrSelColor){toast('색을 먼저 선택하세요.');return;}
  if(!name){toast('마커명을 입력하세요.');return;}
  if(_markerPresets.some(p=>p.color===_mmgrSelColor&&p.name===name)){toast('이미 있는 조합입니다.');return;}
  _markerPresets.push({color:_mmgrSelColor,name});
  if(_markerPresets.length>20)_markerPresets=_markerPresets.slice(-20);
  await _saveMemoData();_renderMmgrList();
  document.getElementById('mmgrMarkerName').value='';toast('마커 추가됨');
}
async function deleteMarkerPreset(idx){
  _markerPresets.splice(idx,1);await _saveMemoData();_renderMmgrList();toast('마커 삭제됨');
}
...
function startPanelResize(e,side){
  e.preventDefault();
  const body=document.getElementById('reBody');if(!body)return;
  const panel=document.getElementById(side==='left'?'reMarkerPanel':'reMemoPanel');
  _panelRsz={side,startX:e.clientX,startW:panel.offsetWidth,panel};
  document.body.style.cursor='col-resize';document.body.style.userSelect='none';
}
document.addEventListener('mousemove',function(e){
  if(!_panelRsz)return;
  const dx=side==='left'?e.clientX-_panelRsz.startX:_panelRsz.startX-e.clientX;
  // left: 오른쪽으로 드래그하면 넓어짐
  // right: 왼쪽으로 드래그하면 넓어짐
  const {side}=_panelRsz;
  const dxx=side==='left'?(e.clientX-_panelRsz.startX):(_panelRsz.startX-e.clientX);
  const newW=Math.max(100,Math.min(300,_panelRsz.startW+dxx));
  _panelRsz.panel.style.width=newW+'px';
}.bind(null));
document.addEventListener('mouseup',function(){
  if(!_panelRsz)return;_panelRsz=null;document.body.style.cursor='';document.body.style.userSelect='';
});
...
function showTblCtxMenu(e,td){
  selectTableCell(td);const menu=document.getElementById('tblCtxMenu');
  menu.style.display='block';menu.style.left=Math.min(e.clientX,window.innerWidth-200)+'px';menu.style.top=Math.min(e.clientY,window.innerHeight-400)+'px';
  const close=ev=>{if(!ev.target.closest('#tblCtxMenu')){menu.style.display='none';document.removeEventListener('mousedown',close);}};
  setTimeout(()=>document.addEventListener('mousedown',close),10);
}
function tblCmd(cmd){
  ...
  setTimeout(()=>_updateTableColHandles(),80);
}
...
/* ════════════════════════════════════════
   표 세로선 — Google Sheets 방식
   오버레이 div에 절대좌표 핸들 배치
   핸들 드래그 → 해당 열 전체 너비 변경
════════════════════════════════════════ */
let _colDragState=null;

function _updateTableColHandles(){
  const overlay=document.getElementById('tblColOverlay');
  const editor=document.getElementById('reEditor');
  const wrap=document.getElementById('reEditorWrap');
  if(!overlay||!editor||!wrap)return;
  // 드래그 중이면 갱신 보류
  ...
  editor.querySelectorAll('table').forEach(table=>{
    ...
    const tblTop=tblRect.top-wrapRect.top+scroll;
    const tblH=table.offsetHeight;
    const firstCells=Array.from(rows[0].querySelectorAll('td,th'));
    firstCells.forEach((cell,ci)=>{
      if(ci===firstCells.length-1)return;
      const cellRect=cell.getBoundingClientRect();
      const handleX=cellRect.right-wrapRect.left;
      const h=document.createElement('div');
      h.className='tbl-col-handle';
      h.style.cssText=`left:${handleX}px;top:${tblTop}px;height:${tblH}px;`;
      h.addEventListener('mousedown',function(e){
        e.preventDefault();e.stopPropagation();
        const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
        _colDragState={cells:colCells,startX:e.clientX,startW:cell.offsetWidth,handle:h,wrap,cell};
...
function _initTableResize(){
  const editor=document.getElementById('reEditor');
  const wrap=document.getElementById('reEditorWrap');
  if(!editor||editor._tblRszSet)return;
  editor._tblRszSet=true;
  _updateTableColHandles();
  // 행 높이 드래그
  let _rowEdge=null,_rowDrag=null;
  editor.addEventListener('mousemove',function(e){
    if(_colDragState||_rowDrag)return;
  ...
  document.addEventListener('mousemove',function(e){
    if(_colDragState){
      ...
      const newW=Math.max(1,_colDragState.startW+dx);
      _colDragState.cells.forEach(c=>{
      ...
      if(wrap){
        ...
        const cr=_colDragState.cell.getBoundingClientRect();
        _colDragState.handle.style.left=(cr.right-wr.left)+'px';
      }
    }
    if(_rowDrag){
  ...
  document.addEventListener('mouseup',function(){
    if(_colDragState){
      _colDragState.handle.classList.remove('dragging');
      _colDragState=null;document.body.style.cursor='';document.body.style.userSelect='';
      setTimeout(_updateTableColHandles,60);
    }
    if(_rowDrag){_rowDrag=null;if(editor)editor.style.cursor='';}
  });
  // 스크롤/리사이즈/내용변경
  if(wrap)wrap.addEventListener('scroll',_updateTableColHandles);
  window.addEventListener('resize',_updateTableColHandles);
  const obs=new MutationObserver(()=>{clearTimeout(obs._t);obs._t=setTimeout(_updateTableColHandles,300);});
...
function doPdfExport(){
  ...
  let memoSection='';
  if(withMemo){
    const memos=[];editor.querySelectorAll('.re-memo').forEach(m=>memos.push({text:(m.textContent||'').replace(/\u200B/g,''),name:m.dataset.memoName||'메모',desc:m.dataset.memoDesc||'',resolved:m.dataset.memoResolved==='true',mc:m.dataset.memoMc||'',mn:m.dataset.memoMn||''}));
    if(memos.length){memoSection='<div style="margin-top:32px;border-top:2px solid #BFDBFE;padding-top:16px;"><div style="font-weight:800;font-size:.95rem;margin-bottom:10px;color:#1e3a5f;">📝 메모 목록</div>';memos.forEach(m=>{const dot=m.mc?`<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${m.mc};margin-right:4px;vertical-align:middle;"></span>`:'';memoSection+=`<div style="margin-bottom:8px;background:${m.resolved?'#F0FDF4':'#EFF6FF'};border-left:3px solid ${m.resolved?'#86EFAC':m.mc||'#BFDBFE'};padding:8px 12px;border-radius:0 6px 6px 0;font-size:.85rem;"><div style="font-weight:700;color:#1e3a5f;">${dot}${m.name}${m.mn?' ('+m.mn+')':''}${m.resolved?' ✓':''}</div><div style="color:#6b7280;font-size:.78rem;margin:2px 0;">"${m.text}"</div>${m.desc?`<div style="color:#374151;margin-top:3px;">${m.desc}</div>`:''}</div>`;});memoSection+='</div>';}
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title}</title><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700;800&display=swap" rel="stylesheet"><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Noto Sans KR',sans-serif;font-size:14px;line-height:1.9;color:#191F28;padding:40px 48px;}h1{font-size:1.9rem;font-weight:800;margin:1.2em 0 .4em;}h2{font-size:1.4rem;font-weight:800;}h3{font-size:1.1rem;font-weight:700;}p{margin:.4em 0;}ul,ol{padding-left:1.5em;margin:.4em 0;}li{margin:.15em 0;}blockquote{border-left:4px solid #D91F3E;padding:.5em 1em;background:#FFF3F5;border-radius:0 6px 6px 0;margin:.6em 0;}.re-callout{display:flex;gap:10px;border-radius:8px;padding:12px 14px;margin:.6em 0;}table{border-collapse:collapse;width:100%;margin:.6em 0;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}img{max-width:100%;}.re-img-wrap{display:inline-block;}.re-img-handle,.tbl-col-handle{display:none!important;}@media print{body{padding:20px;}}</style></head><body><h1 style="border-bottom:2px solid #E5E8EB;padding-bottom:10px;margin-bottom:20px;">${title}</h1>${clone.innerHTML}${memoSection}<script>window.onload=function(){window.print();}<\/script></body></html>`);
  win.document.close();
}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('contextmenu',function(e){
      ...
      if(td){e.preventDefault();showTblCtxMenu(e,td);return;}
      const sel=window.getSelection();
    ...
    editor.addEventListener('click',function(e){
      ...
      if(memo&&!e.target.closest('button')){e.stopPropagation();highlightMemoCard(memo);}
    });
    ...
    const wrap=document.getElementById('reEditorWrap');
    if(wrap&&!wrap._scrollSet){wrap._scrollSet=true;wrap.addEventListener('scroll',function(){debouncedMemoPanel();_updateTableColHandles();});}
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
    ...
    reUpdateToolbar();setTimeout(()=>{_initImgResize();_initTableResize();initColorPalettes();_initMemoMarkerPalette();},300);
...
function reInsertTable(){
  const rows=3,cols=3;
  let html='<table style="border-collapse:collapse;width:100%;margin:.8em 0;border:1px solid #E5E8EB;" contenteditable="false"><colgroup>';
  for(let c=0;c<cols;c++)html+='<col style="width:'+Math.floor(100/cols)+'%">';
  html+='</colgroup><tbody>';
  for(let r=0;r<rows;r++){html+='<tr>';for(let c=0;c<cols;c++){const isH=r===0;const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;':'padding:10px 14px;border:1px solid #E5E8EB;';html+='<'+(isH?'th':'td')+' contenteditable="true" style="'+st+'">'+(isH?'제목'+(c+1):'')+'</'+(isH?'th':'td')+'>';}html+='</tr>';}
  html+='</tbody></table><p><br></p>';
  reExec('insertHTML',html);
  setTimeout(()=>_initTableResize(),100);
