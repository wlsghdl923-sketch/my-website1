...
/* ══ 커스텀 이모지 — Supabase에 저장 (어떤 환경에서도 사용 가능) ══ */
let _coCustomEmojis=[];
...
function _coEmojiGridHTML(selEm){
  const customs=_coCustomEmojis.map((em,i)=>{
    ...
    return `<span class="co-em-btn${em===selEm?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">
      ${inner}
      <span class="co-em-del" onclick="event.stopPropagation();delCoCustomEmoji(${i})" title="삭제">✕</span>
    </span>`;
  });
  const base=CO_EMOJIS.map(em=>`<span class="co-em-btn${em===selEm?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${em}</span>`);
  const none=`<span class="co-em-btn co-em-none${selEm===''?' sel':''}" data-em="" onclick="pickCoEmoji(this)">없음</span>`;
...
async function openCalloutPicker(){
  ...
  const cg=document.getElementById('coColorRow');
  const eg=document.getElementById('coEmojiGrid');
...
function pickCoColor(el){
  document.querySelectorAll('#coColorRow .co-sw').forEach(s=>s.classList.remove('sel'));
  el.classList.add('sel');_coPick.color=CO_COLORS[parseInt(el.dataset.i)];
}
function pickCoEmoji(el){
  const grid=el.closest('#coEmojiGrid,#coEmojiChangeGrid');
  if(grid)grid.querySelectorAll('.co-em-btn').forEach(s=>s.classList.remove('sel'));
  el.classList.add('sel');_coPick.emoji=el.dataset.em||'';
}
async function handleCoCustomImg(e){
  ...
  reader.onload=async ev=>{
    _coCustomEmojis.push(ev.target.result);
    await _saveCoCustomEmojis();
    const sel=_coPick.emoji;
    ['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{
      const g=document.getElementById(id);
      if(g&&document.getElementById(id.replace('Grid','Overlay')||'coPickerOverlay')?.style.display==='block')
        g.innerHTML=_coEmojiGridHTML(sel);
      else if(g)g.innerHTML=_coEmojiGridHTML(sel);
    });
    toast('커스텀 이모지가 저장되었습니다. (어떤 기기에서도 사용 가능)');
  };
  reader.readAsDataURL(file);
}
async function delCoCustomEmoji(idx){
  await _loadCoCustomEmojis();
  _coCustomEmojis.splice(idx,1);
  await _saveCoCustomEmojis();
  const sel=_coPick.emoji;
...
function _buildCalloutHtml(color,emoji){
  ...
  const emojiHtml=emoji
    ?(isImg?`<img src="${emoji}" data-coem="${emoji}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;" contenteditable="false">`
            :`<span data-coem="${emoji}" contenteditable="false" style="font-size:1.2rem;flex-shrink:0;margin-top:1px;">${emoji}</span>`)
    :'';
  ...
  return `<div class="re-callout" contenteditable="false" style="${st}">${emojiHtml}<div contenteditable="true" style="flex:1;min-width:0;outline:none;">내용을 입력하세요</div></div><p><br></p>`;
}
function doInsertCallout(){
  closeCalloutPicker();
  reExec('insertHTML',_buildCalloutHtml(_coPick.color,_coPick.emoji));
}

/* ── 기존 콜아웃 이모지 변경 (더블클릭) ── */
let _coChangeTarget=null;
async function openCoEmojiChange(calloutEl){
  _coChangeTarget=calloutEl;
  await _loadCoCustomEmojis();
  const cur=calloutEl.querySelector('[data-coem]');
  const curEm=cur?cur.dataset.coem:'';
  _coPick.emoji=curEm;
  const g=document.getElementById('coEmojiChangeGrid');
  if(g)g.innerHTML=_coEmojiGridHTML(curEm);
...
function applyCoEmojiChange(){
  ...
  const isImg=_coPick.emoji&&(_coPick.emoji.startsWith('data:')||_coPick.emoji.startsWith('http'));
  // 기존 이모지 요소 완전 제거
  const oldEm=_coChangeTarget.querySelector('[data-coem]');
  if(oldEm)oldEm.remove();
  // 새 이모지 삽입 (없음 선택 시 건너뜀)
  if(_coPick.emoji){
    const newEl=document.createElement(isImg?'img':'span');
    newEl.setAttribute('data-coem',_coPick.emoji);
    newEl.contentEditable='false';
    if(isImg){
      newEl.src=_coPick.emoji;
      newEl.style.cssText='width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;';
    }else{
      newEl.style.cssText='font-size:1.2rem;flex-shrink:0;margin-top:1px;';
      newEl.textContent=_coPick.emoji;
    }
    // 텍스트 div 앞에 삽입
    const body=_coChangeTarget.querySelector('div[contenteditable="true"]');
    if(body)_coChangeTarget.insertBefore(newEl,body);
    else _coChangeTarget.prepend(newEl);
  }
  closeCoEmojiChange();
}
// 더블클릭으로 이모지 변경 팝업 열기
...
function selectTableCell(td){
  clearTableCellSel();
  td.classList.add('tbl-sel');
  _tblTd=td;
}
function clearTableCellSel(){
  document.querySelectorAll('.tbl-sel').forEach(c=>c.classList.remove('tbl-sel'));
}
function showTblCtxMenu(e,td){
  ...
  const close=ev=>{
    if(!ev.target.closest('#tblCtxMenu')){menu.style.display='none';document.removeEventListener('mousedown',close);}
  };
...
function tblCmd(cmd){
  ...
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;min-width:50px;';
  const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;min-width:50px;';
  ...
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    ...
    // 병합 해제: colspan/rowspan 원복 + 빈 셀 생성
    ...
    for(let r2=ri;r2<ri+rs;r2++){
      ...
      for(let c2=startC;c2<ci+cs;c2++){
        ...
        const targetRow=rows[r2];
        if(targetRow){const sib=Array.from(targetRow.querySelectorAll('td,th'))[c2];targetRow.insertBefore(nc,sib||null);}
      }
    }
  }else if(cmd==='splitToGrid'){
    // 선택한 셀을 지정 행/열로 분할 (내부 서브테이블)
    const sr=parseInt(prompt('분할할 행 수:','2'))||2;
    const sc=parseInt(prompt('분할할 열 수:','2'))||2;
    td.innerHTML='';
    let sub='<table style="width:100%;border-collapse:collapse;margin:0;">';
    for(let r2=0;r2<sr;r2++){sub+='<tr>';for(let c2=0;c2<sc;c2++){sub+='<td contenteditable="true" style="border:1px dashed #ccc;padding:6px;min-width:20px;"></td>';}sub+='</tr>';}
    sub+='</table>';
    td.innerHTML=sub;
...
function openCellColorPicker(){
  ...
  if(g)g.innerHTML=CELL_COLORS30.map(c=>
    `<div class="clr-chip${c===cur?' sel':''}" style="background:${c};${c==='#FFFFFF'?'border-color:#ccc;':''}" title="${c}" onclick="pickCellClr(this,'${c}')"></div>`
  ).join('');
  _selCellClr=cur;
...
function pickCellClr(el,color){
  document.querySelectorAll('#cellClrGrid .clr-chip').forEach(c=>c.classList.remove('sel'));
  el.classList.add('sel');_selCellClr=color;
}
function applyCellClr(color){
  if(_tblTd)_tblTd.style.background=color||'';
  closeCellClr();_tblTd=null;clearTableCellSel();
}
...
h1{font-size:1.9rem;font-weight:800;margin:1.2em 0 .4em;}
h2{font-size:1.4rem;font-weight:800;margin:1em 0 .35em;}
h3{font-size:1.1rem;font-weight:700;margin:.9em 0 .3em;}
p{margin:.4em 0;}
ul,ol{padding-left:1.5em;margin:.4em 0;}
li{margin:.15em 0;}
blockquote{border-left:4px solid #D91F3E;padding:.5em 1em;background:#FFF3F5;border-radius:0 6px 6px 0;margin:.6em 0;color:#4E5968;}
...
.re-img-wrap{display:inline-block;vertical-align:top;}
.re-img-handle{display:none;}
...
async function importBackup(event){
  ...
  try{
    ...
    if(!obj.data){toast('올바른 파일이 아닙니다.');return;}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('click',function(e){
      const td=e.target.closest&&e.target.closest('td,th');
      if(td){selectTableCell(td);}
      else{clearTableCellSel();}
    });
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
...
function reInsertTable(){
  ...
  for(let r=0;r<rows;r++){
    ...
    for(let c=0;c<cols;c++){
      const isH=r===0;
      const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;min-width:50px;':'padding:10px 14px;border:1px solid #E5E8EB;min-width:50px;';
  ...
  toast('3×3 표가 삽입되었습니다. 셀을 우클릭하면 행/열 추가, 병합 등을 할 수 있습니다.');
}
