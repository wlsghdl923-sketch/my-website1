...
.notion-callout{display:block;background:#F0F4FF;border:1px solid #C7D7FF;border-radius:8px;padding:.9em 1.1em;margin:.8em 0;color:#1A3A8F;line-height:1.8;}
...
/* ══ 붙여넣기 핸들러 — aside → notion-callout 변환 ══ */
function rePasteHandler(e){
  e.preventDefault();
  const html=e.clipboardData.getData('text/html');
  if(html){
    // <aside> 태그를 콜아웃 div로 변환 (브라우저가 aside를 제거하기 전에 처리)
    let clean=html
      .replace(/<aside[^>]*>/gi,'<div class="notion-callout">')
      .replace(/<\/aside>/gi,'</div>');
    // 노션 끊긴 이미지 제거
    clean=clean.replace(/<img[^>]*(notion-static|amazonaws)[^>]*>/gi,'[이미지: 노션에서 직접 저장 후 재업로드 필요]');
...
function reInsertCallout(){
  document.execCommand('insertHTML',false,'<div class="notion-callout">💡 여기에 내용을 입력하세요</div><p><br></p>');
  document.getElementById('reEditor').focus();
}
