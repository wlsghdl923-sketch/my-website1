...
function openCalDocSettings(){
  ...
  // 표기 설정 초기화
  const curMode=cfg.labelMode||'task';
  _cdLabelPickedMode=curMode;
  document.querySelectorAll('#cdLabelOptGrid .cal-label-opt').forEach(el2=>{
    el2.classList.toggle('active',el2.dataset.mode===curMode);
  });
  updateCdLabelPreview(curMode);
  calDocSetTab('display');
...
let _cdLabelPickedMode=null;
function pickCdLabelMode(el){
  document.querySelectorAll('#cdLabelOptGrid .cal-label-opt').forEach(e=>e.classList.remove('active'));
  el.classList.add('active');
  _cdLabelPickedMode=el.dataset.mode;
  updateCdLabelPreview(_cdLabelPickedMode);
}
function updateCdLabelPreview(mode){
  ...
  const el=document.getElementById('cdLabelPreviewText');
...
function calDocSetTab(id){
  const tabIds=['display','tags'];
  document.querySelectorAll('#calDocSettingsOverlay .m-stab').forEach((b,i)=>b.classList.toggle('active',tabIds[i]===id));
...
async function saveCalDocSettings(){
  ...
  CAL_DOC_CFG[_curDocId].labelMode=_cdLabelPickedMode||'task';
...
function getEvLabel(ev){var task=ev.task||ev.name||'';var proj=ev.project||'';var proc=ev.process||'';var ep=ev.episode||'';var SEP=' · ';var mode='${(settings&&settings.labelMode)||'task'}';switch(mode){case 'task':return task||ep||proj;case 'process':return proc||task;case 'project':return proj||task;case 'project+process':return[proj,proc].filter(Boolean).join(SEP)||task;case 'process+task':return[proc,task].filter(Boolean).join(SEP);case 'all':return[proj,proc,task].filter(Boolean).join(SEP)||ep;case 'episode':return ep||task;case 'name':return ev.name||task;default:return task||ev.name;}}
