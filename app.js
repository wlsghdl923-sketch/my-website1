...
function initColorPalettes(){
  ...
  fg.innerHTML=TEXT_COLORS30.map(c=>{
    const isLight=(c==='#FFFFFF'||c==='#CFD8DC'||c==='#FFE082'||c==='#F9F9F9');
    return `<div class="tc${isLight?' tc-light':''}" style="background:${c};" title="${c}" onmousedown="event.preventDefault();applyFgColor('${c}')"></div>`;
  }).join('');
  bg.innerHTML=BG_COLORS30.map(c=>{
    const isTransp=c==='transparent';
    const sty=isTransp?'background:linear-gradient(135deg,#fff 45%,#f44 45%,#f44 55%,#fff 55%);':'background:'+c+';';
    const isLight=(c==='#FFFFFF'||c==='#F9F9F9'||isTransp);
    return `<div class="tc${isLight?' tc-light':''}" style="${sty}" title="${isTransp?'없음':c}" onmousedown="event.preventDefault();applyBgColor('${c}')"></div>`;
  }).join('');
  // 외부 클릭 시 팔레트 닫기
...
function toggleColorPalette(id){
  ...
  if(!isOpen)p.style.display='flex';
}

function _restoreSel(){
  if(_savedSel){const s=window.getSelection();s.removeAllRanges();s.addRange(_savedSel.r);}
}
...
function applyBgColor(color){
  ...
  if(color!=='transparent'){const f=document.getElementById('reBgColorFill');if(f)f.setAttribute('fill',color);}
  document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');
  _savedSel=null;
}

/* ══ 메모 기능 ══ */
let _memoSel=null;   // 메모 달 선택 영역
let _memoTarget=null; // 현재 보고 있는 메모 span
...
function showEditorCtxMenu(e){
  ...
  const close=ev=>{
    if(!ev.target.closest('#editorCtxMenu')){menu.style.display='none';document.removeEventListener('mousedown',close);}
  };
...
function openMemoInput(){
  ...
  const sel=window.getSelection();
  if(!sel||sel.isCollapsed){toast('텍스트를 드래그하여 선택한 뒤 우클릭하세요.');return;}
  _memoSel={r:sel.getRangeAt(0).cloneRange()};
...
function saveMemoToSelection(){
  ...
  try{
    ...
    // surroundContents 실패 시 (부분 태그 교차) extractContents 방식 사용
...
function showMemoView(e,memoEl){
  _memoTarget=memoEl;
  const popup=document.getElementById('memoViewPopup');
  document.getElementById('memoViewText').textContent=memoEl.dataset.memo||'';
...
function editCurrentMemo(){
  ...
  const newText=prompt('메모 수정:',_memoTarget.dataset.memo||'');
  if(newText===null)return;
  if(newText.trim()){
    _memoTarget.dataset.memo=newText.trim();
    _memoTarget.title='📝 '+newText.trim();
    document.getElementById('memoViewText').textContent=newText.trim();
    toast('메모가 수정되었습니다.');
  }
  closeMemoView();
}
...
function deleteCurrentMemo(){
  if(!_memoTarget)return;
  const parent=_memoTarget.parentNode;
  while(_memoTarget.firstChild)parent.insertBefore(_memoTarget.firstChild,_memoTarget);
  parent.removeChild(_memoTarget);
  closeMemoView();toast('메모가 삭제되었습니다.');
}
...
async function openCalloutPicker(){
  ...
  const cg=document.getElementById('coColorRow');const eg=document.getElementById('coEmojiGrid');
...
async function handleCoCustomImg(e){
  ...
  reader.onload=async ev=>{
    _coCustomEmojis.push(ev.target.result);await _saveCoCustomEmojis();
    const sel=_coPick.emoji;
    ['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});
    toast('커스텀 이모지 저장됨');
...
async function openCoEmojiChange(calloutEl){
  ...
  const cur=calloutEl.querySelector('[data-coem]');const curEm=cur?cur.dataset.coem:'';
  _coPick.emoji=curEm;
...
function applyCoEmojiChange(){
  ...
  if(_coPick.emoji){
    ...
    const el=document.createElement(isImg?'img':'span');
    el.setAttribute('data-coem',_coPick.emoji);el.contentEditable='false';
...
function tblCmd(cmd){
  ...
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    ...
    const nx=td.nextElementSibling;if(nx){td.colSpan=(td.colSpan||1)+(nx.colSpan||1);td.innerHTML+=nx.innerHTML;nx.remove();}
  }else if(cmd==='mergeDown'){
    const nr=rows[ri+1];if(nr){const bl=Array.from(nr.querySelectorAll('td,th'))[ci];if(bl){td.rowSpan=(td.rowSpan||1)+(bl.rowSpan||1);td.innerHTML+=bl.innerHTML;bl.remove();}}
  }else if(cmd==='splitCell'){
    const cs=td.colSpan||1,rs=td.rowSpan||1;
    if(cs<=1&&rs<=1){toast('병합된 셀이 아닙니다.');_tblTd=null;return;}
    td.colSpan=1;td.rowSpan=1;
    for(let r2=ri;r2<ri+rs;r2++){
      const startC=(r2===ri)?ci+1:ci;
      for(let c2=startC;c2<ci+cs;c2++){
        const nc=document.createElement('td');nc.contentEditable='true';nc.style.cssText=TDS;
        const targetRow=rows[r2];if(targetRow){const sib=Array.from(targetRow.querySelectorAll('td,th'))[c2];targetRow.insertBefore(nc,sib||null);}
      }
    }
  }else if(cmd==='splitH'){
    const colspan=td.colSpan||1;const newTr=document.createElement('tr');
    cells.forEach((cell,i)=>{
      if(i===ci){const nc=document.createElement('td');nc.contentEditable='true';nc.style.cssText=cell.style.cssText;nc.colSpan=colspan;newTr.appendChild(nc);}
      else{cell.rowSpan=(cell.rowSpan||1)+1;}
    });
...
const CELL_COLORS30=['#FFFFFF','#F5F6FA','#FFECEC','#FFF3E0','#FFFDE7','#F1F8E9',
  '#E8F5E9','#E0F7FA','#E1F5FE','#E8EAF6','#F3E5F5','#FCE4EC',
  '#FFCDD2','#FFE0B2','#FFF9C4','#DCEDC8','#B2DFDB','#B3E5FC',
  '#C5CAE9','#E1BEE7','#F8BBD9','#D7CCC8','#CFD8DC','#FFAB91',
  '#FFD54F','#AED581','#4DB6AC','#4FC3F7','#7986CB','#F06292'];
let _selCellClr='';
...
const TBL_BORDER_COLORS=[
  '#E5E8EB','#191F28','#D91F3E','#FF7A00','#0064FF','#2E9E50','#9C27B0','#8B95A1',
  '#F44336','#E91E63','#673AB7','#3F51B5','#2196F3','#03A9F4','#00BCD4','#009688',
  '#4CAF50','#8BC34A','#CDDC39','#FFEB3B','#FFC107','#FF9800','#FF5722','#795548',
  '#607D8B','#BDBDBD','#90CAF9','#A5D6A7','#FFCC80','#CE93D8',
  '#E8D5D5','#E8E0D5','#E8E8D5','#D5E8D5','#D5E8E8','#D5D5E8','#E8D5E8','#E2E8D5',
  '#D5E2E8','#E8DDD5','#DDE8D5','#D5DDE8','#E8D5DD','#E5DDE8','#D8E8E5',
];
let _selTblBorderClr='#E5E8EB';
function openTblBorderColorPicker(){
  if(!_tblTd){toast('셀을 먼저 클릭하여 선택하세요.');return;}
  const table=_tblTd.closest('table');
  const cur=(table&&table.style.borderColor)||'#E5E8EB';
...
function pickTblBorderClr(el,color){document.querySelectorAll('#tblBorderClrGrid .clr-chip').forEach(c=>c.classList.remove('sel'));el.classList.add('sel');_selTblBorderClr=color;}
function applyTblBorderClr(color){
  if(!_tblTd)return;
  const table=_tblTd.closest('table');if(!table)return;
  table.style.border='1px solid '+color;
  table.querySelectorAll('td,th').forEach(cell=>{cell.style.border='1px solid '+color;});
  closeTblBorderClr();
}

/* ══ 표 선 드래그 리사이즈 — 핵심: document 레벨에서 mousemove 처리 ══ */
let _tblRsz=null;
...
function _initTableResize(){
  ...
  // 마우스 커서 변경 — 셀 오른쪽/아래쪽 6px 이내
  editor.addEventListener('mousemove',function(e){
    if(_tblRsz)return;
    const cell=e.target.closest&&e.target.closest('td,th');
    if(!cell){if(editor.style.cursor!=='')editor.style.cursor='';return;}
    const rect=cell.getBoundingClientRect();
    const rx=e.clientX-rect.left;
    const ry=e.clientY-rect.top;
    if(rx>=rect.width-6){editor.style.cursor='col-resize';}
    else if(ry>=rect.height-6){editor.style.cursor='row-resize';}
    else{if(editor.style.cursor!=='')editor.style.cursor='';}
  });

  // mousedown — 경계에서 드래그 시작
  editor.addEventListener('mousedown',function(e){
    // 색상 팔레트 위에서는 무시
    if(e.target.closest('.re-color-palette'))return;
    const cell=e.target.closest&&e.target.closest('td,th');
    if(!cell)return;
    const rect=cell.getBoundingClientRect();
    const rx=e.clientX-rect.left;
    ...
    if(rx>=rect.width-6){
      e.preventDefault();
      e.stopPropagation();
      _tblRsz={type:'col',cell,startX:e.clientX,startW:cell.offsetWidth};
    }else if(ry>=rect.height-6){
      e.preventDefault();
      e.stopPropagation();
      const tr=cell.closest('tr');
      _tblRsz={type:'row',tr,startY:e.clientY,startH:tr.offsetHeight};
    }
  },true); // capture 단계에서 처리하여 다른 핸들러보다 먼저 실행

  // mousemove — document 레벨 (캡처)
  document.addEventListener('mousemove',function(e){
    if(!_tblRsz)return;
    e.preventDefault();
    if(_tblRsz.type==='col'){
      const newW=Math.max(1,_tblRsz.startW+(e.clientX-_tblRsz.startX));
      _tblRsz.cell.style.width=newW+'px';
      _tblRsz.cell.style.minWidth='0';
      _tblRsz.cell.style.maxWidth='none';
    }else{
      const newH=Math.max(10,_tblRsz.startH+(e.clientY-_tblRsz.startY));
      _tblRsz.tr.style.height=newH+'px';
      Array.from(_tblRsz.tr.querySelectorAll('td,th')).forEach(c=>{c.style.height=newH+'px';});
    }
  },true);
...
function exportDocAsPdf(){
  ...
  // 메모 span을 PDF용 노트로 변환
  const clone=editor.cloneNode(true);
  clone.querySelectorAll('.re-memo').forEach(m=>{
    const noteEl=document.createElement('span');
    noteEl.style.cssText='background:#FFD54F;border-radius:3px;border-bottom:2px solid #F57F17;position:relative;';
    noteEl.innerHTML=m.innerHTML;
    const note=document.createElement('sup');
    note.style.cssText='font-size:.65rem;background:#F57F17;color:#fff;border-radius:3px;padding:1px 4px;margin-left:2px;vertical-align:super;';
    note.textContent='📝';
    note.title=m.dataset.memo||'';
    noteEl.appendChild(note);
    m.replaceWith(noteEl);
  });
  // 메모 목록 생성
  const memos=[];
  editor.querySelectorAll('.re-memo').forEach((m,i)=>{
    memos.push({text:m.textContent,memo:m.dataset.memo||''});
  });
  let memoSection='';
  if(memos.length>0){
...
async function exportBackup(){
  ...
  try{
    ...
    a.href=url;a.download='backup_'+new Date().toISOString().slice(0,16).replace('T','_').replace(':','-')+'.json';
    document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
    toast('백업 완료 — '+rows.length+'개');
  }catch(e){toast('백업 실패');console.error(e);}
}
async function importBackup(event){
  ...
  }catch(e){toast('복원 실패');console.error(e);}
}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('click',function(e){
      ...
      if(memo){e.preventDefault();e.stopPropagation();showMemoView(e,memo);}
    });
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
...
function reInsertTable(){
  const rows=3,cols=3;
  let html='<table style="border-collapse:collapse;width:100%;margin:.8em 0;border:1px solid #E5E8EB;table-layout:fixed;"><colgroup>';
  ...
  toast('3×3 표 삽입. 셀 경계 드래그로 크기 조절, 우클릭으로 편집');
}
