...
function tblCtxMemo() {
  ...
  const sel = window.getSelection();
  if (!sel || sel.isCollapsed || !sel.toString().trim()) {
    toast('메모를 추가하려면 텍스트를 먼저 드래그해 선택하세요.');
    return;
  }
  _memoCtxRange = sel.getRangeAt(0).cloneRange();
  memoCtxOpen();
...
function tblCmd(cmd) {
  ...
  if (cmd === 'addRowAbove' || cmd === 'addRowBelow') {
    ...
    table.remove();
  } else if (cmd === 'mergeCell') {
    _tblMergeSelected(table, td);
  } else if (cmd === 'splitCellH') {
    _tblSplitH(td);
  }
  _lastCtxTd = null;
  // 리사이즈 핸들 갱신
...
/* ── 셀 병합 ── */
let _tblSelStart=null, _tblSelEnd=null, _tblSelecting=false;

function _tblMergeSelected(table, fallbackTd) {
  const sels = table.querySelectorAll('.tbl-sel');
  if (sels.length < 2) {
    toast('Shift+클릭으로 병합할 셀 2개 이상을 선택하세요.');
    return;
  }
  const tds = Array.from(sels);
  const rows = Array.from(table.querySelectorAll('tr'));
  ...
  tds.forEach(td => {
    const tr = td.closest('tr');
    const ri = rows.indexOf(tr);
    // 해당 행에서 이 셀 이전까지의 시각적 열 위치 계산
    let visualCol = 0;
    const siblings = Array.from(tr.querySelectorAll('td,th'));
    for (const sib of siblings) {
      if (sib === td) break;
      visualCol += (parseInt(sib.colSpan) || 1);
    }
    const cs = parseInt(td.colSpan) || 1;
    const rs = parseInt(td.rowSpan) || 1;
    cellPositions.push({ td, ri, ci: visualCol, cs, rs });
  });

  // 직사각형 영역 계산
  let minR = Infinity, maxR = -1, minC = Infinity, maxC = -1;
  cellPositions.forEach(p => {
    minR = Math.min(minR, p.ri);
    maxR = Math.max(maxR, p.ri + p.rs - 1);
    minC = Math.min(minC, p.ci);
    maxC = Math.max(maxC, p.ci + p.cs - 1);
  });

  // 정렬: 왼쪽 위가 첫 번째
  cellPositions.sort((a, b) => a.ri - b.ri || a.ci - b.ci);

  const firstTd = cellPositions[0].td;
  let mergedText = '';

  // 첫 번째 외 나머지 제거
  for (let i = 1; i < cellPositions.length; i++) {
    const t = cellPositions[i].td;
    if (t.textContent.trim()) mergedText += ' ' + t.textContent.trim();
    t.remove();
  }

  firstTd.colSpan = maxC - minC + 1;
  firstTd.rowSpan = maxR - minR + 1;
  if (mergedText) firstTd.innerHTML += mergedText;
  firstTd.classList.remove('tbl-sel');

  // 나머지 선택 표시도 제거
  table.querySelectorAll('.tbl-sel').forEach(c => c.classList.remove('tbl-sel'));
  toast('셀이 병합되었습니다.');
}

/* ── 셀 분리 ── */
function _tblSplitH(td) {
  const cs = parseInt(td.colSpan) || 1;
  const rs = parseInt(td.rowSpan) || 1;
  if (cs <= 1 && rs <= 1) {
    toast('이미 분리된 셀입니다.');
  ...
  const tr = td.closest('tr');
  const ri = rows.indexOf(tr);
  const cellStyle = 'padding:8px 12px;border:1px solid #E5E8EB;';

  // colspan 해제 → 같은 행에 셀 추가
  const origCs = cs;
  const origRs = rs;
  td.colSpan = 1;
  td.rowSpan = 1;

  // 같은 행에 (origCs - 1)개 셀 추가
  let lastInserted = td;
  for (let c = 1; c < origCs; c++) {
    const nc = document.createElement(td.tagName.toLowerCase());
    nc.style.cssText = cellStyle;
    nc.setAttribute('contenteditable', 'true');
    lastInserted.after(nc);
    lastInserted = nc;
  }

  // rowspan 해제 → 아래 행에 셀 추가
  for (let r = 1; r < origRs; r++) {
    const targetRow = rows[ri + r];
    if (!targetRow) continue;
    // 이 행에서 삽입할 위치 찾기: td의 시각적 열 위치 계산
    let visualCol = 0;
    const siblings = Array.from(tr.querySelectorAll('td,th'));
    for (const sib of siblings) {
      if (sib === td) break;
    ...
    // 대상 행에서 해당 시각적 위치에 셀 삽입
    const targetCells = Array.from(targetRow.querySelectorAll('td,th'));
    let insertBefore = null;
    let runningCol = 0;
    for (const cell of targetCells) {
      if (runningCol >= visualCol) { insertBefore = cell; break; }
      runningCol += (parseInt(cell.colSpan) || 1);
    }
    for (let c = 0; c < origCs; c++) {
...
function _bindTblResize(editor) {
  ...
  editor.addEventListener('click', e => {
    ...
    if (cb && cb.closest('.re-check-item')) {
      const li = cb.closest('.re-check-item');
      const span = li.querySelector('span');
      if (cb.checked) {
        li.classList.add('checked');
        if (span) { span.style.textDecoration = 'line-through'; span.style.color = '#8B95A1'; }
      } else {
        li.classList.remove('checked');
        if (span) { span.style.textDecoration = 'none'; span.style.color = ''; }
      }
    }
  });

  // ★ 표 활성 표시 (7번) + Shift+클릭 셀 선택 (병합용)
  editor.addEventListener('click', e => {
    // 모든 표의 활성 표시 해제
    ...
    if (td && td.closest('.re-editor')) {
      ...
      if (e.shiftKey) {
        e.preventDefault();
        td.classList.toggle('tbl-sel');
      } else {
        // Shift 없이 클릭하면 모든 셀 선택 해제
        editor.querySelectorAll('.tbl-sel').forEach(c => c.classList.remove('tbl-sel'));
      }
    }
  });
  ...
  // 마우스 다운으로 열/행 경계 감지 → 리사이즈 시작
  let _resizing = false, _resizeType = '', _resizeTarget = null, _resizeStartPos = 0, _resizeStartSize = 0;
  ...
  editor.addEventListener('mousedown', e => {
    ...
    // 오른쪽 경계 6px 이내 → 열 리사이즈
    if (Math.abs(e.clientX - rect.right) < 6) {
      e.preventDefault();
      _resizing = true; _resizeType = 'col'; _resizeTarget = td;
      _resizeStartPos = e.clientX;
      _resizeStartSize = td.offsetWidth;
      document.body.style.cursor = 'col-resize';
      document.body.style.userSelect = 'none';
    }
    // 하단 경계 6px 이내 → 행 리사이즈
    else if (Math.abs(e.clientY - rect.bottom) < 6) {
      e.preventDefault();
      _resizing = true; _resizeType = 'row'; _resizeTarget = td;
      _resizeStartPos = e.clientY;
      _resizeStartSize = td.offsetHeight;
  ...
  document.addEventListener('mousemove', e => {
    if (!_resizing || !_resizeTarget) return;
    if (_resizeType === 'col') {
      const dx = e.clientX - _resizeStartPos;
      const newW = Math.max(30, _resizeStartSize + dx);
      _resizeTarget.style.width = newW + 'px';
      _resizeTarget.style.minWidth = newW + 'px';
    } else if (_resizeType === 'row') {
      const dy = e.clientY - _resizeStartPos;
      const newH = Math.max(20, _resizeStartSize + dy);
      _resizeTarget.style.height = newH + 'px';
    }
  });

  document.addEventListener('mouseup', () => {
    if (_resizing) {
      _resizing = false; _resizeTarget = null;
      document.body.style.cursor = '';
      document.body.style.userSelect = '';
    }
  });

  // 마우스 커서 모양 변경 (열/행 경계 근처에서)
  editor.addEventListener('mousemove', e => {
    if (_resizing) return;
    const td = e.target.closest('td,th');
    if (!td || !td.closest('.re-editor')) return;
    const rect = td.getBoundingClientRect();
    if (Math.abs(e.clientX - rect.right) < 6) {
...
async function openRichEditor(docId=''){
  ...
  _asActive=!!docId; _asSetUI('idle'); _asLastSaveTime=null;
  // 이벤트 바인딩 (한 번만)
...
function saveRichDoc(){
  ...
  if(_reEditDocId){
    ...
    _reEditDocId=id; // 신규 → 기존 모드로 전환 (이후 자동저장 가능)
    _asActive=true;
    _docTargetFolder=null;
  }
  saveAll();
...
function execTblInsert() {
  ...
  closeTblInsert();
  document.getElementById('reEditor').focus();
  let html = '<div class="re-tbl-wrap" contenteditable="false" style="margin:.8em 0;position:relative;"><table style="border-collapse:collapse;width:100%;table-layout:fixed;" class="re-tbl"><tbody>';
  for (let r = 0; r < rows; r++) {
    html += '<tr>';
    for (let c = 0; c < cols; c++) {
      const tag = r === 0 ? 'th' : 'td';
      html += `<${tag} style="padding:8px 12px;border:1px solid #E5E8EB;${r === 0 ? 'background:#F5F6FA;font-weight:700;' : ''}" contenteditable="true">${r === 0 ? `열 ${c + 1}` : ''}</${tag}>`;
    }
    html += '</tr>';
  }
...
function reInsertCheckbox() {
  const editor = document.getElementById('reEditor');
  editor.focus();
  const html = '<ul class="re-checklist"><li class="re-check-item"><input type="checkbox" onclick="reToggleCheck(this)"><span>할 일 항목</span></li></ul><p><br></p>';
  reExec('insertHTML', html);
}
function reToggleCheck(cb) {
...
th{background:#F5F6FA;font-weight:700;}
.re-checklist{list-style:none;padding-left:.3em;}
.re-check-item{display:flex;align-items:flex-start;gap:5px;margin:.2em 0;}
.re-check-item input[type=checkbox]{width:14px;height:14px;margin-top:4px;}
.re-check-item.checked span{text-decoration:line-through;color:#8B95A1;}
...
function reOnKeydown(e){
    ...
    if(_asActive&&_reEditDocId){
      ...
      if(title&&content){
        ...
        _sbRest.upsert('site_data',{key:`hub_doc_${_reEditDocId}`,value:content,updated_at:new Date().toISOString()})
          .then(()=>_asSetUI('saved')).catch(()=>_asSetUI('error'));
      }
    }
    return;
  }
  ...
  if(e.key==='Enter'){
    ...
    if(chkItem){
      const span=chkItem.querySelector('span');
      if(!span||!span.textContent.trim()){
        // 빈 체크 아이템이면 체크리스트 종료
        e.preventDefault();
        const ul=chkItem.closest('.re-checklist');
        if(ul){
          chkItem.remove();
      ...
      newLi.className='re-check-item';
      newLi.innerHTML='<input type="checkbox" onclick="reToggleCheck(this)"><span></span>';
      chkItem.after(newLi);
      const r2=document.createRange();
      r2.setStart(newLi.querySelector('span'),0);r2.collapse(true);
      sel.removeAllRanges();sel.addRange(r2);
    }
  }
  // Backspace로 구분선 삭제
...
function _reCheckLineMarkdown(){
  ...
  if(text==='[]'||text==='[ ]'){
    ...
    li.className='re-check-item';
    li.innerHTML='<input type="checkbox" onclick="reToggleCheck(this)"><span></span>';
    ul.appendChild(li);
    block.before(ul);
    block.remove();
    const r2=document.createRange();
    r2.setStart(li.querySelector('span'),0);r2.collapse(true);
    sel.removeAllRanges();sel.addRange(r2);
    return true;
  }
  return false;
