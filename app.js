...
function reHandleImg(e){
  ...
  reader.onload=async ev=>{
    ...
    // 기본 너비 360px, 드래그 리사이즈 가능한 래퍼로 삽입
    const imgHtml=`<span class="re-img-wrap" style="display:inline-block;position:relative;width:360px;max-width:100%;vertical-align:top;margin:.5em 0;cursor:default;" contenteditable="false">`+
      `<img src="${url}" style="width:100%;border-radius:8px;display:block;" alt="이미지" draggable="false">`+
      `<span class="re-img-handle" data-dir="se" style="position:absolute;right:-5px;bottom:-5px;width:12px;height:12px;background:#D91F3E;border-radius:50%;cursor:se-resize;z-index:10;"></span>`+
      `<span class="re-img-handle" data-dir="sw" style="position:absolute;left:-5px;bottom:-5px;width:12px;height:12px;background:#D91F3E;border-radius:50%;cursor:sw-resize;z-index:10;"></span>`+
      `<span class="re-img-handle" data-dir="ne" style="position:absolute;right:-5px;top:-5px;width:12px;height:12px;background:#D91F3E;border-radius:50%;cursor:ne-resize;z-index:10;"></span>`+
      `<span class="re-img-handle" data-dir="nw" style="position:absolute;left:-5px;top:-5px;width:12px;height:12px;background:#D91F3E;border-radius:50%;cursor:nw-resize;z-index:10;"></span>`+
      `</span><span contenteditable="true"> </span>`;
    reExec('insertHTML', imgHtml);
    toast('이미지가 삽입되었습니다. 모서리를 드래그하여 크기를 조절하세요.');
    // 리사이즈 이벤트 연결
...
function _initImgResize(){
  ...
  if(!editor)return;
  // 이미 붙어있으면 중복 등록 방지
  if(editor._resizeHandlerSet)return;
  editor._resizeHandlerSet=true;

  editor.addEventListener('mousedown',function(e){
    const handle=e.target.closest('.re-img-handle');
    if(!handle)return;
    e.preventDefault();
    const wrap=handle.closest('.re-img-wrap');
    if(!wrap)return;
    const dir=handle.dataset.dir;
    const startX=e.clientX;
    const startW=wrap.offsetWidth;
    _resizeState={wrap,dir,startX,startW};
  });
  ...
  document.addEventListener('mousemove',function(e){
    ...
    let newW;
    if(dir==='se'||dir==='ne') newW=startW+dx;
    else newW=startW-dx;
    newW=Math.max(60,Math.min(newW,760));
