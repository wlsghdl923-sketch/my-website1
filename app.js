...
/* ══ 글자색 팔레트 (30가지) ══ */
const TEXT_COLORS=[
  {c:'#191F28',label:'기본'},{c:'#FFFFFF',label:'흰색'},{c:'#D91F3E',label:'빨강'},
  {c:'#FF5252',label:'연빨강'},{c:'#FF7A00',label:'주황'},{c:'#FFCA28',label:'노랑'},
  {c:'#FFD54F',label:'연노랑'},{c:'#2E9E50',label:'초록'},{c:'#69F0AE',label:'연초록'},
  {c:'#0064FF',label:'파랑'},{c:'#40C4FF',label:'하늘'},{c:'#006064',label:'청록'},
  {c:'#80DEEA',label:'연청록'},{c:'#9C27B0',label:'보라'},{c:'#CE93D8',label:'연보라'},
  {c:'#673AB7',label:'남보라'},{c:'#E91E63',label:'분홍'},{c:'#F48FB1',label:'연분홍'},
  {c:'#795548',label:'갈색'},{c:'#A1887F',label:'연갈색'},{c:'#607D8B',label:'청회'},
  {c:'#8B95A1',label:'회색'},{c:'#BDBDBD',label:'연회'},{c:'#4E342E',label:'진갈'},
  {c:'#BF360C',label:'진주황'},{c:'#1B5E20',label:'진초록'},{c:'#01579B',label:'진파랑'},
  {c:'#4A148C',label:'진보라'},{c:'#880E4F',label:'진분홍'},{c:'#37474F',label:'진청회'},
];

/* ══ 배경색 팔레트 (30가지) ══ */
const BG_COLORS=[
  {c:'transparent',label:'없음'},{c:'#FFFF00',label:'노랑'},{c:'#FFE082',label:'연노랑'},
  {c:'#FFCDD2',label:'분홍'},{c:'#F8BBD9',label:'연분홍'},{c:'#FFF3E0',label:'연주황'},
  {c:'#FFE0B2',label:'살구'},{c:'#C8E6C9',label:'연초록'},{c:'#DCEDC8',label:'연두'},
  {c:'#B2EBF2',label:'하늘'},{c:'#E1F5FE',label:'연하늘'},{c:'#E8EAF6',label:'라벤더'},
  {c:'#EDE7F6',label:'연보라'},{c:'#F3E5F5',label:'연보라2'},{c:'#FCE4EC',label:'연분홍2'},
  {c:'#E0F2F1',label:'민트'},{c:'#F9FBE7',label:'연연두'},{c:'#FFF9C4',label:'연노랑2'},
  {c:'#E3F2FD',label:'연파랑'},{c:'#F1F8E9',label:'연초록2'},{c:'#FFFDE7',label:'아이보리'},
  {c:'#FFF8F0',label:'크림'},{c:'#F0F4FF',label:'연파랑2'},{c:'#F9F9F9',label:'연회'},
  {c:'#D5E8D5',label:'파스텔초록'},{c:'#D5D5E8',label:'파스텔보라'},{c:'#E8D5D5',label:'파스텔분홍'},
  {c:'#E8E8D5',label:'파스텔노랑'},{c:'#D5E8E8',label:'파스텔청록'},{c:'#E8DDD5',label:'파스텔살구'},
];

let _lastTextColor='#D91F3E';
let _lastBgColor='#FFE082';

function _buildColorGrid(colors, gridId, applyFn, curColor){
  const g=document.getElementById(gridId);
  if(!g)return;
  g.innerHTML=colors.map(({c,label})=>{
    const isDark=(c==='#191F28'||c==='#4E342E'||c==='#1B5E20'||c==='#01579B'||c==='#4A148C'||c==='#880E4F'||c==='#37474F'||c==='#BF360C'||c==='#673AB7');
    const isWhite=c==='#FFFFFF'||c==='transparent';
    return `<div class="tc-chip${c===curColor?' sel':''}${isWhite?' tc-chip-dark':''}" style="background:${c==='transparent'?'linear-gradient(135deg,#fff 45%,#f00 45%,#f00 55%,#fff 55%)':c};" title="${label}" onclick="${applyFn}('${c}')"></div>`;
  }).join('');
}

function openTextColorPicker(){
  const btn=document.getElementById('reFgColorBtn');
  const rect=btn?btn.getBoundingClientRect():null;
  const overlay=document.getElementById('textColorOverlay');
  const modal=document.getElementById('textColorModal');
  overlay.style.display='block';
  if(rect&&modal){
    modal.style.top=(rect.bottom+6)+'px';
    modal.style.left=Math.min(rect.left,window.innerWidth-270)+'px';
    modal.style.transform='none';
  }
  _buildColorGrid(TEXT_COLORS,'textColorGrid','applyTextColor',_lastTextColor);
}
function closeTextColorPicker(){document.getElementById('textColorOverlay').style.display='none';}
function applyTextColor(color){
  const c=color||'#191F28';
  document.execCommand('foreColor',false,c);
  _lastTextColor=c;
  const preview=document.getElementById('reFgColorPreview');
  if(preview)preview.setAttribute('fill',c);
  closeTextColorPicker();
}

function openTextBgColorPicker(){
  const btn=document.getElementById('reBgColorBtn');
  const rect=btn?btn.getBoundingClientRect():null;
  const overlay=document.getElementById('textBgColorOverlay');
  const modal=document.getElementById('textBgColorModal');
  overlay.style.display='block';
  if(rect&&modal){
    modal.style.top=(rect.bottom+6)+'px';
    modal.style.left=Math.min(rect.left,window.innerWidth-270)+'px';
  ...
  _buildColorGrid(BG_COLORS,'textBgColorGrid','applyTextBgColor',_lastBgColor);
}
function closeTextBgColorPicker(){document.getElementById('textBgColorOverlay').style.display='none';}
function applyTextBgColor(color){
  if(!color||color==='transparent'){
    // 배경색 제거: hiliteColor를 white로 설정 후 제거
    document.execCommand('hiliteColor',false,'transparent');
  }else{
    document.execCommand('hiliteColor',false,color);
    _lastBgColor=color;
    const preview=document.getElementById('reBgColorPreview');
    if(preview)preview.setAttribute('fill',color);
  }
  closeTextBgColorPicker();
}
...
function openCellColorPicker(){
  ...
  if(g)g.innerHTML=CELL_COLORS30.map(c=>`<div class="clr-chip${c===cur?' sel':''}" style="background:${c};${c==='#FFFFFF'?'border-color:#ccc;':''}" title="${c}" onclick="pickCellClr(this,'${c}')"></div>`).join('');
...
const TBL_BORDER_COLORS=[
  '#E5E8EB','#191F28','#D91F3E','#FF7A00','#0064FF',
  '#2E9E50','#9C27B0','#8B95A1','#F44336','#E91E63',
  '#673AB7','#3F51B5','#2196F3','#03A9F4','#00BCD4',
  '#009688','#4CAF50','#8BC34A','#CDDC39','#FFEB3B',
  '#FFC107','#FF9800','#FF5722','#795548','#607D8B',
  '#BDBDBD','#90CAF9','#A5D6A7','#FFCC80','#CE93D8',
  '#E8D5D5','#E8E0D5','#E8E8D5','#D5E8D5','#D5E8E8',
  '#D5D5E8','#E8D5E8','#E2E8D5','#D5E2E8','#E8DDD5',
  '#DDE8D5','#D5DDE8','#E8D5DD','#E5DDE8','#D8E8E5',
];
...
function openTblBorderColorPicker(){
  ...
  if(g)g.innerHTML=TBL_BORDER_COLORS.map(c=>`<div class="clr-chip${c===cur?' sel':''}" style="background:${c};" title="${c}" onclick="pickTblBorderClr(this,'${c}')"></div>`).join('');
...
function _initTableResize(){
  ...
  const THRESH=6,MIN_COL=1,MIN_ROW=10;
  editor.addEventListener('mousemove',function(e){
    ...
    if(x>=rect.width-THRESH)editor.style.cursor='col-resize';
    else if(y>=rect.height-THRESH)editor.style.cursor='row-resize';
  ...
  editor.addEventListener('mousedown',function(e){
    ...
    const rect=cell.getBoundingClientRect();
    const x=e.clientX-rect.left,y=e.clientY-rect.top;
    if(x>=rect.width-THRESH){
      e.preventDefault();e.stopPropagation();
      _tblRsz={type:'col',cell,startX:e.clientX,startW:cell.offsetWidth};
      cell.closest('table')?.classList.add('col-resizing');
    }else if(y>=rect.height-THRESH){
      e.preventDefault();e.stopPropagation();
  ...
  document.addEventListener('mousemove',function(e){
    if(!_tblRsz)return;
    if(_tblRsz.type==='col'){
      const newW=Math.max(MIN_COL,_tblRsz.startW+(e.clientX-_tblRsz.startX));
      _tblRsz.cell.style.width=newW+'px';_tblRsz.cell.style.minWidth='0';_tblRsz.cell.style.maxWidth='none';
    }else{
      const newH=Math.max(MIN_ROW,_tblRsz.startH+(e.clientY-_tblRsz.startY));
      _tblRsz.tr.style.height=newH+'px';
      Array.from(_tblRsz.tr.querySelectorAll('td,th')).forEach(c=>{c.style.height=newH+'px';});
    }
  });
  document.addEventListener('mouseup',function(){
    if(_tblRsz){_tblRsz.cell?.closest('table')?.classList.remove('col-resizing');_tblRsz=null;editor.style.cursor='';}
  });
}
...
.tbl-sel{outline:none!important;}@media print{body{padding:20px;}}
</style></head><body>
...
async function _readDocRawAsync(docId){
  try{
    ...
    return raw;
  }catch(e){console.error('문서 읽기 실패:',e);return '';}
}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('contextmenu',function(e){
      ...
      if(td){e.preventDefault();showTblCtxMenu(e,td);}
    });
  ...
  if(docId){
    ...
    reUpdateToolbar();setTimeout(()=>{_initImgResize();_initTableResize();},200);
...
function reInsertTable(){
  ...
  toast('3×3 표 삽입. 셀을 우클릭하면 편집 메뉴가 열립니다.');
}
...
function reOnKeydown(e){
  if(e.key==='Tab'){
    ...
    if(li){
      ...
      // 일반 텍스트: 불릿 리스트 시작 또는 들여쓰기 단계
      if(e.shiftKey){
        document.execCommand('outdent');
      }else{
        // 현재 블록이 리스트가 아니면 불릿 리스트로 전환
