...
/* ══ 글자색/배경색 팔레트 — mousedown으로 선택 유지하며 즉시 적용 ══ */
const TEXT_COLORS30=[
...
let _savedSel=null; // 색상 팔레트 열기 전 선택 영역 저장
...
function initColorPalettes(){
  ...
  fg.innerHTML=TEXT_COLORS30.map(c=>{
    const isLight=c==='#FFFFFF'||c==='#CFD8DC';
    return `<div class="tc${isLight?' tc-white':''}" style="background:${c};" title="${c}"
      onmousedown="event.preventDefault();applyFgColor('${c}')"></div>`;
  }).join('')+'<div class="tc" style="background:linear-gradient(135deg,#f00,#ff0,#0f0,#0ff,#00f,#f0f);border-radius:3px;" title="기본색" onmousedown="event.preventDefault();applyFgColor(\'#191F28\')"></div>';

  // 배경색 팔레트 생성
  bg.innerHTML=BG_COLORS30.map(c=>{
    const isTransparent=c==='transparent';
    const isLight=c==='#FFFFFF'||c==='#F9F9F9';
    const bgStyle=isTransparent?'background:linear-gradient(135deg,#fff 45%,#f00 45%,#f00 55%,#fff 55%);':'background:'+c+';';
    return `<div class="tc${isLight||isTransparent?' tc-white':''}" style="${bgStyle}" title="${isTransparent?'없음':c}"
...
function toggleColorPalette(id){
  ...
  if(sel&&sel.rangeCount&&!sel.isCollapsed){
    _savedSel={range:sel.getRangeAt(0).cloneRange()};
  }
...
function _restoreSel(){
  ...
  if(_savedSel){
    const sel=window.getSelection();
    sel.removeAllRanges();
    sel.addRange(_savedSel.range);
  }
}

function applyFgColor(color){
  _restoreSel();
  document.execCommand('foreColor',false,color==='#191F28'?'#191F28':color);
  // 버튼 아래 색상 바 업데이트
...
function applyBgColor(color){
  ...
  if(color==='transparent'){
    ...
    // 버튼 색상 업데이트
    const fill=document.getElementById('reBgColorFill');
...
function _initTableResize(){
  ...
  editor.addEventListener('mousedown',function(e){
    ...
    if(x>=rect.width-THRESH){
      ...
      e.preventDefault();e.stopPropagation();
      const tr=cell.closest('tr');_tblRsz={type:'row',tr,startY:e.clientY,startH:tr.offsetHeight};
    }
  });
  document.addEventListener('mousemove',function(e){
    ...
    if(_tblRsz.type==='col'){
      ...
      _tblRsz.cell.style.minWidth='0px';
      _tblRsz.cell.style.maxWidth='none';
      _tblRsz.cell.style.overflow='hidden';
    }else{
...
h1{font-size:1.9rem;font-weight:800;margin:1.2em 0 .4em;}h2{font-size:1.4rem;font-weight:800;margin:1em 0 .35em;}
h3{font-size:1.1rem;font-weight:700;margin:.9em 0 .3em;}p{margin:.4em 0;}
ul,ol{padding-left:1.5em;margin:.4em 0;}li{margin:.15em 0;}
...
async function openRichEditor(docId=''){
  ...
  if(docId){
    ...
    reUpdateToolbar();setTimeout(()=>{_initImgResize();_initTableResize();initColorPalettes();},200);
