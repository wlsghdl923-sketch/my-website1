...
/* ════════════════════════════════════
   글자색 / 배경색 팔레트
   mousedown → 선택 저장 → 적용 → 복원
════════════════════════════════════ */
const TEXT_COLORS30 = [
  '#191F28','#4E5968','#8B95A1','#FFFFFF','#D91F3E','#FF5252',
  '#FF7A00','#FFCA28','#2E9E50','#69F0AE','#0064FF','#40C4FF',
  '#006064','#80DEEA','#9C27B0','#CE93D8','#673AB7','#E91E63',
  '#F48FB1','#795548','#A1887F','#607D8B','#BF360C','#1B5E20',
  '#01579B','#4A148C','#880E4F','#37474F','#4E342E','#CFD8DC',
];
const BG_COLORS30 = [
  'transparent','#FFFF00','#FFE082','#FFCDD2','#F8BBD9','#FFF3E0',
  '#FFE0B2','#C8E6C9','#DCEDC8','#B2EBF2','#E1F5FE','#E8EAF6',
  '#EDE7F6','#F3E5F5','#FCE4EC','#E0F2F1','#F9FBE7','#FFF9C4',
  '#E3F2FD','#F1F8E9','#FFFDE7','#FFF8F0','#F0F4FF','#F9F9F9',
  '#D5E8D5','#D5D5E8','#E8D5D5','#E8E8D5','#D5E8E8','#E8DDD5',
];
let _savedSel = null;

function initColorPalettes() {
  const fg = document.getElementById('reFgPalette');
  const bg = document.getElementById('reBgPalette');
  if (!fg || fg._init) return;
  fg._init = true; bg._init = true;

  fg.innerHTML = TEXT_COLORS30.map(c => {
    const light = (c === '#FFFFFF' || c === '#CFD8DC');
    return `<div class="tc${light?' tc-light':''}" style="background:${c};"
      onmousedown="event.preventDefault();applyFgColor('${c}')"></div>`;
  }).join('');

  bg.innerHTML = BG_COLORS30.map(c => {
    const T = c === 'transparent';
    const light = (c === '#FFFFFF' || c === '#F9F9F9' || T);
    const sty = T
      ? 'background:linear-gradient(135deg,#fff 45%,#f44 45%,#f44 55%,#fff 55%);'
      : `background:${c};`;
    return `<div class="tc${light?' tc-light':''}" style="${sty}"
      onmousedown="event.preventDefault();applyBgColor('${c}')"></div>`;
  }).join('');

  document.addEventListener('mousedown', function(e) {
    if (!e.target.closest('.re-color-wrap'))
...
function toggleColorPalette(id) {
  ...
  if (sel && sel.rangeCount && !sel.isCollapsed)
    _savedSel = { r: sel.getRangeAt(0).cloneRange() };
...
function applyBgColor(color) {
  ...
  if (color !== 'transparent') {
    const f = document.getElementById('reBgColorFill');
    if (f) f.setAttribute('fill', color);
  }
  document.querySelectorAll('.re-color-palette').forEach(p => p.style.display = 'none');
  _savedSel = null;
}
...
function showEditorCtxMenu(e) {
  const menu = document.getElementById('editorCtxMenu');
  if (!menu) return;
  menu.style.display = 'block';
  menu.style.left = Math.min(e.clientX, window.innerWidth - 160) + 'px';
  menu.style.top  = Math.min(e.clientY, window.innerHeight - 60) + 'px';
  const close = ev => {
    if (!ev.target.closest('#editorCtxMenu')) {
...
function openMemoInput() {
  ...
  const st = document.getElementById('memoSelectedText');
  if (st) st.textContent = '"' + _memoSel.text.slice(0, 60) + (_memoSel.text.length > 60 ? '...' : '') + '"';
  document.getElementById('memoInputText').value = '';
  document.getElementById('memoInputOverlay').style.display = 'block';
  setTimeout(() => document.getElementById('memoInputText').focus(), 120);
}
...
function refreshMemoPanel() {
  ...
  list.innerHTML = memos.map((m, i) => {
    const id = m.dataset.memoid || ('m' + i);
    m.dataset.memoid = id;
    const q = (m.textContent || '').slice(0, 40);
    const c = m.dataset.memo || '';
    return `<div class="memo-card" id="mc_${id}">
      <div class="memo-card-quote">"${q}"</div>
      <div class="memo-card-body">${c}</div>
      <div class="memo-card-btns">
        <button onclick="editMemoCard('${id}')">수정</button>
        <button class="del" onclick="deleteMemoCard('${id}')">삭제</button>
      </div>
    </div>`;
  }).join('');
}
...
function highlightMemoCard(memoEl) {
  if (!memoEl) return;
  const id = memoEl.dataset.memoid; if (!id) return;
  document.querySelectorAll('.memo-card').forEach(c => c.style.outline = '');
  const card = document.getElementById('mc_' + id);
  if (card) { card.style.outline = '2px solid #F57F17'; card.scrollIntoView({ behavior:'smooth', block:'nearest' }); }
}
...
async function _loadCoCustomEmojis() {
  try {
    const row = await _sbRest.selectOne('site_data', 'co_custom_emojis');
...
function _coColorGridHTML(selB) {
  return CO_COLORS.map((c,i) => `<div class="co-sw${c.b===selB?' sel':''}" style="background:${c.b};" data-i="${i}" onclick="pickCoColor(this)"></div>`).join('');
}
async function openCalloutPicker() {
  await _loadCoCustomEmojis();
  _coPick = { color:CO_COLORS[0], emoji:'💡' };
  const cg=document.getElementById('coColorRow'), eg=document.getElementById('coEmojiGrid');
  if(cg) cg.innerHTML=_coColorGridHTML(CO_COLORS[0].b);
  if(eg) eg.innerHTML=_coEmojiGridHTML('💡');
...
async function handleCoCustomImg(e){
  ...
  reader.onload=async ev=>{_coCustomEmojis.push(ev.target.result);await _saveCoCustomEmojis();const sel=_coPick.emoji;['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});toast('커스텀 이모지 저장됨');};
  reader.readAsDataURL(file);
}
async function delCoCustomEmoji(idx){
  await _loadCoCustomEmojis();_coCustomEmojis.splice(idx,1);await _saveCoCustomEmojis();
  const sel=_coPick.emoji;['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});toast('삭제됨');
}
...
function tblCmd(cmd){
  ...
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    ...
    for(let r2=ri;r2<ri+rs;r2++){const sc=(r2===ri)?ci+1:ci;for(let c2=sc;c2<ci+cs;c2++){const nc=document.createElement('td');nc.contentEditable='true';nc.style.cssText=TDS;const targetRow=rows[r2];if(targetRow){const sib=Array.from(targetRow.querySelectorAll('td,th'))[c2];targetRow.insertBefore(nc,sib||null);}}}
  }else if(cmd==='splitH'){
...
function _initTableResize() {
  ...
  let _colDrag = null;   // {cells, startX, startW}
  let _rowDrag = null;   // {tr, startY, startH}
  const THRESH = 7;      // 경계 감지 범위 (px)

  // 현재 마우스 위치가 셀 경계인지 판단
  function _getEdge(e) {
    // 컬러 팔레트 위면 무시
    if (e.target.closest && e.target.closest('.re-color-palette')) return null;
    ...
    // 오른쪽 경계 (열)
    if (rx >= rect.width - THRESH) return { type: 'col', cell };
    // 아래쪽 경계 (행)
    if (ry >= rect.height - THRESH) return { type: 'row', cell };
    return null;
  }
  ...
  editor.addEventListener('mousemove', function(e) {
    ...
    const cur = editor.style.cursor;
    const want = edge ? (edge.type === 'col' ? 'col-resize' : 'row-resize') : '';
    if (cur !== want) editor.style.cursor = want;
  });
  ...
  editor.addEventListener('mousedown', function(e) {
    ...
    if (edge.type === 'col') {
      ...
      const rows  = Array.from(table.querySelectorAll('tr'));
      const ci    = Array.from(edge.cell.parentElement.querySelectorAll('td,th')).indexOf(edge.cell);
      const cells = rows.map(r => Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
      _colDrag = { cells, startX: e.clientX, startW: edge.cell.offsetWidth };
    } else {
      const tr = edge.cell.closest('tr');
      _rowDrag = { tr, startY: e.clientY, startH: tr.offsetHeight };
    }
  }, true); // ← capture: true で他のhandlerより先に実行
...
function doPdfExport() {
  ...
  if (withMemo) {
    ...
    if (memos.length) {
      ...
      memos.forEach(m => {
        memoSection += `<div style="margin-bottom:8px;background:#FFFDE7;border-left:3px solid #FFE082;padding:8px 12px;border-radius:0 6px 6px 0;font-size:.85rem;"><span style="font-weight:700;color:#5D4037;">"${m.text}"</span><div style="color:#5D4037;margin-top:4px;">${m.memo}</div></div>`;
      });
      memoSection += '</div>';
    }
  } else {
    clone.querySelectorAll('.re-memo').forEach(m => {
...
body{font-family:'Noto Sans KR',sans-serif;font-size:14px;line-height:1.9;color:#191F28;padding:40px 48px;}
...
async function exportBackup() {
  ...
  try {
    const rows = await _sbRest.select('site_data');
    if (!rows || rows.length === 0) { toast('데이터 없음'); return; }
    const obj = { _v:1, _d:new Date().toISOString(), data:{} };
    rows.forEach(r => { obj.data[r.key] = r.value; });
    const url = URL.createObjectURL(new Blob([JSON.stringify(obj,null,2)], { type:'application/json' }));
    const a = document.createElement('a');
    a.href = url;
    a.download = 'backup_' + new Date().toISOString().slice(0,16).replace('T','_').replace(':','-') + '.json';
...
async function importBackup(event) {
  ...
  try {
    ...
    for (const [k,v] of Object.entries(obj.data))
      await _sbRest.upsert('site_data', { key:k, value:v, updated_at:new Date().toISOString() });
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(h){
        ...
        document.execCommand('insertHTML',false,h.replace(/<aside[^>]*>/gi,'<div class="re-callout" style="'+CS+'">').replace(/<\/aside>/gi,'</div>'));
  ...
  if(docId){
    ...
    reUpdateToolbar();
    setTimeout(()=>{_initImgResize();_initTableResize();initColorPalettes();},300);
    const raw=await _readDocRawAsync(docId);
...
function reInsertTable(){
  const rows=3,cols=3;
  let html='<table data-tbl="1" style="border-collapse:collapse;width:100%;margin:.8em 0;border:1px solid #E5E8EB;" contenteditable="false"><colgroup>';
  ...
  toast('3×3 표 삽입. 셀 경계를 드래그해서 크기 조절, 우클릭으로 편집');
}
