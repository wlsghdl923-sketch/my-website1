...
let _activeCalDocId = null; // 현재 활성화된 캘린더 문서 ID (null=전체)
...
<tbody>${rowsHtml||'<tr><td colspan="99" style="padding:24px;color:#8B95A1;text-align:center;">페이지가 없습니다.</td></tr>'}</tbody></table>
  ...
  // 캘린더 문서 버튼 제거됨 (캘린더는 메인 캘린더에서 직접 사용)
...
/* ══ CAL DOC RENAME (제거됨 - 캘린더는 문서 목록에서 이름 변경) ══ */
...
function saveCalDoc(){
  ...
  _docTargetFolder=null;
  saveAll();renderDocList();closeCalMaker();toast('캘린더 "'+title+'" 가 생성되었습니다.');
}

/* ══ 캘린더 문서 열기 (메인 캘린더로 이동 + 필터 적용) ══ */
...
function renderCalFilterChips(){
  ...
  if(!wrap)return;
  // 모든 카드에서 calendar 타입 문서 수집
  const calDocs=[];
  for(const[key,docs] of Object.entries(CARD_DOCS)){
    (docs||[]).forEach(d=>{if(d.type==='calendar')calDocs.push(d);});
  }
  if(!calDocs.length){wrap.style.display='none';return;}
  wrap.style.display='flex';
  const chipsEl=document.getElementById('calFilterChips');
  if(!chipsEl)return;
  chipsEl.innerHTML=
    `<span class="cal-filter-chip${!_activeCalDocId?' active':''}" onclick="closeCalendarDoc()">전체</span>`+
    calDocs.map(d=>`<span class="cal-filter-chip${_activeCalDocId===d.id?' active':''}" onclick="openCalendarDoc('${d.id}')">${d.title}</span>`).join('');
}
...
function getFilteredEvents(){
  let evts=EVENTS;
  if(_activeCalDocId){
    evts=evts.filter(ev=>ev.calendarId===_activeCalDocId);
  }
  return evts;
}
...
function renderTagFilter(){
  const _fEvtsForTags=getFilteredEvents();
  const wrap=document.getElementById('tagFilterWrap');
  if(!wrap)return;
  const allTags=[...new Set(_fEvtsForTags.flatMap(ev=>ev.tags||[]))];
  if(!allTags.length){wrap.style.display='none';return;}
  wrap.style.display='flex';
...
function renderLegend(){
  ...
  if(!el)return;
  const _fEvts=getFilteredEvents();
  if(!_fEvts.length){el.innerHTML='<div class="leg-empty">등록된 일정이 없습니다.</div>';return;}

  const vis=CAL_CFG.visibleTags;
  const visEvts=vis?_fEvts.filter(ev=>(ev.tags||[]).some(t=>vis.includes(t))||(ev.tags||[]).length===0):_fEvts;
...
function renderMonthly(){
  ...
  const vis=CAL_CFG.visibleTags;
  const baseEvts=(()=>{
    const _baseEvts=getFilteredEvents();
    let evts=vis?_baseEvts.filter(ev=>(ev.tags||[]).some(t=>vis.includes(t))||(ev.tags||[]).length===0):_baseEvts;
...
function renderQuarterly(){
  ...
  const _fEvts=getFilteredEvents();
  const vis=CAL_CFG.visibleTags;
  let allEvts=vis?_fEvts.filter(ev=>(ev.tags||[]).some(t=>vis.includes(t))||(ev.tags||[]).length===0):_fEvts;
  // 프로젝트 필터 적용
...
/* ══ iframe postMessage 제거됨 (캘린더 직접 렌더링) ══ */
...
function saveEvent(){
  ...
  const calId=_activeCalDocId||((editEvIdx!==-1&&EVENTS[editEvIdx])?EVENTS[editEvIdx].calendarId:null);
  const ev={name,project,task,note,color:selColor,start,end,startTime,endTime,tags:[..._editTags],excludeWeekend,process,episode,calendarId:calId||null};
