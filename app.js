...
let _memoTemplates=[];
let _memoSelMarkerColor='',_memoSelMarkerName='';

async function _loadMemoTemplates(){try{const row=await _sbRest.selectOne('site_data','memo_tpl');if(row&&Array.isArray(row.value))_memoTemplates=row.value;else if(row&&typeof row.value==='string')_memoTemplates=JSON.parse(row.value);}catch{}}
async function _saveMemoTemplates(){try{await _sbRest.upsert('site_data',{key:'memo_tpl',value:_memoTemplates,updated_at:new Date().toISOString()});}catch{}}
...
function _initMemoMarkerPalette(){
  ...
  _memoSelMarkerColor='';_memoSelMarkerName='';
  pal.innerHTML=MARKER_COLORS.map(c=>`<div class="mkr-chip" style="background:${c};" data-color="${c}" onclick="pickMemoMarker(this,'${c}')"></div>`).join('');
}

function pickMemoMarker(el,color){
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
  if(_memoSelMarkerColor===color){_memoSelMarkerColor='';_memoSelMarkerName='';document.getElementById('memoSelectedMarker').style.display='none';return;}
  el.classList.add('sel');_memoSelMarkerColor=color;
  const nameInp=document.getElementById('memoMarkerName');
  _memoSelMarkerName=nameInp?nameInp.value.trim():'';
  const sm=document.getElementById('memoSelectedMarker');
  if(sm){sm.style.display='block';sm.innerHTML=`<span style="display:inline-flex;align-items:center;gap:5px;"><span style="width:10px;height:10px;border-radius:50%;background:${color};display:inline-block;"></span>선택됨${_memoSelMarkerName?' — '+_memoSelMarkerName:''}</span>`;}
}
...
async function openMemoInput(){
  ...
  _memoSelMarkerColor='';_memoSelMarkerName='';
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
  const sm=document.getElementById('memoSelectedMarker');if(sm)sm.style.display='none';
...
function _renderMemoTemplateChips(){
  const chips=document.getElementById('memoTemplateChips');if(!chips)return;
  if(!_memoTemplates.length){chips.innerHTML='<span style="font-size:.72rem;color:#bbb;">저장된 내용 없음</span>';return;}
  chips.innerHTML=_memoTemplates.slice(-15).reverse().map((t)=>
    `<span class="memo-tmpl-chip" onclick="document.getElementById('memoInputDesc').value=unescape('${escape(t)}')">${t.length>14?t.slice(0,14)+'…':t}</span>`
  ).join('');
}
...
function saveMemoToSelection(){
  ...
  if(markerNameInp&&_memoSelMarkerColor)_memoSelMarkerName=markerNameInp.value.trim();
  const desc=document.getElementById('memoInputDesc').value||'';
  if(desc.trim()&&!_memoTemplates.includes(desc.trim())){_memoTemplates.push(desc.trim());if(_memoTemplates.length>15)_memoTemplates=_memoTemplates.slice(-15);_saveMemoTemplates();}
  const now=new Date();
  ...
  span.className='re-memo';
  span.setAttribute('contenteditable','false');
  span.dataset.memoid=id;
  span.dataset.memoName=name||'메모';
  span.dataset.memoDesc=desc;
  span.dataset.memoDate=dateStr;
  span.dataset.memoResolved='false';
  span.dataset.memoMarkerColor=_memoSelMarkerColor;
  span.dataset.memoMarkerName=_memoSelMarkerName;
  // 마커 색으로 하이라이트
  const bg=_memoSelMarkerColor?_hexToRgba(_memoSelMarkerColor,0.3):'rgba(255,213,79,.38)';
  const border=_memoSelMarkerColor||'#F57F17';
  span.style.cssText=`background:${bg};border-bottom:2px solid ${border};border-radius:2px;cursor:pointer;display:inline;`;
...
function _hexToRgba(hex,a){const r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);return `rgba(${r},${g},${b},${a})`;}
...
function refreshMemoPanel(){
  ...
  // 마커 집계
  const markerMap={};
  memos.forEach(m=>{const c=m.dataset.memoMarkerColor;const n=m.dataset.memoMarkerName;if(c){const key=c+'|'+(n||'');if(!markerMap[key])markerMap[key]={color:c,name:n||'마커',count:0};markerMap[key].count++;}});
  if(markerPanel){
    if(Object.keys(markerMap).length>0){
      markerPanel.style.display='block';
      if(markerList)markerList.innerHTML=Object.values(markerMap).map(mk=>`<div class="marker-item"><div class="marker-dot" style="background:${mk.color};"></div><div class="marker-label">${mk.color} : ${mk.name}</div><div class="marker-count">${mk.count}</div></div>`).join('');
    }else{markerPanel.style.display='none';}
  }
  // 메모 카드 배치
  const editorWrap=document.getElementById('reEditorWrap');
  const edRect=editor.getBoundingClientRect();
  const panelRect=panel.getBoundingClientRect();
  const CARD_H=115,GAP=6;
  let nextMinY=8;
  const cards=[];
  memos.forEach((m,i)=>{
    const id=m.dataset.memoid||('m'+i);m.dataset.memoid=id;
    const mRect=m.getBoundingClientRect();
    const scroll=editorWrap?editorWrap.scrollTop:0;
    let y=(mRect.top-edRect.top+scroll);
    y=Math.max(nextMinY,y);nextMinY=y+CARD_H+GAP;
    cards.push({id,y,m});
  });
  list.style.minHeight=(cards[cards.length-1].y+CARD_H+40)+'px';
  const validIds=new Set();
  cards.forEach(({id,y,m})=>{
    validIds.add('mc_'+id);
    ...
    const mc=m.dataset.memoMarkerColor||'';
    const mn=m.dataset.memoMarkerName||'';
    ...
    card.style.top=y+'px';
    const markerHtml=mc?`<div class="memo-card-marker"><span style="width:10px;height:10px;border-radius:50%;background:${mc};display:inline-block;"></span><span>${mc} : ${mn||'마커'}</span></div>`:'';
    card.innerHTML=
      ...
      (desc||isResolved?`<div class="memo-card-desc">${isResolved?'<span class="memo-done-badge">✓ 완료</span>'+(desc?'<br><span style="color:#888;font-size:.76rem;">'+desc+'</span>':''):desc}</div>`:'')+
      `<div class="memo-card-btns">`+
      (!isResolved?`<button class="mresolve" onclick="event.stopPropagation();toggleResolveMemoCard('${id}')">✓</button>`:
       `<button class="munresolve" onclick="event.stopPropagation();toggleResolveMemoCard('${id}')">↩</button>`)+
...
function toggleResolveMemoCard(id){
  const el=document.querySelector(`[data-memoid="${id}"]`);if(!el)return;
  const isResolved=el.dataset.memoResolved==='true';
  el.dataset.memoResolved=isResolved?'false':'true';
  if(isResolved){
    // 완료 해제: 원래 마커 색상 복원
    const mc=el.dataset.memoMarkerColor;
    const bg=mc?_hexToRgba(mc,0.3):'rgba(255,213,79,.38)';
    const border=mc||'#F57F17';
    el.style.cssText=`background:${bg};border-bottom:2px solid ${border};border-radius:2px;cursor:pointer;display:inline;opacity:1;`;
  }else{
    // 완료: 연두색
    el.style.cssText='background:rgba(34,197,94,.2);border-bottom:2px solid #16A34A;border-radius:2px;cursor:pointer;display:inline;opacity:.75;';
  }
...
function tblCmd(cmd){
  ...
  const editor=document.getElementById('reEditor');if(editor)setTimeout(()=>_rebuildColHandles(editor),50);
}
...
/* ══════════════════════════════════════════
   표 열 리사이즈 — 핸들 div 직접 삽입
   
   가로선(행)과 완전히 동일한 원리:
   • 핸들 div를 각 셀 오른쪽에 삽입
   • mousedown → 드래그 → document mousemove로 너비 조절
   • contenteditable 충돌 없음
══════════════════════════════════════════ */
let _colRsz=null, _rowRsz=null;

function _rebuildColHandles(editor){
  if(!editor)return;
  ...
  editor.querySelectorAll('table').forEach(table=>{
    ...
    rows.forEach(tr=>{
      ...
      cells.forEach((cell,ci)=>{
        if(ci===cells.length-1)return;
        if(cell.querySelector('.col-rsz-h'))return;
        ...
        h.addEventListener('mousedown',function(e){
          ...
          h.classList.add('active');document.body.style.cursor='col-resize';
        });
        cell.appendChild(h);
      });
    });
...
function _initTableResize(){
  ...
  _rebuildColHandles(editor);

  // 행 높이 — 셀 아래쪽 6px
  let _rowEdge=null;
  editor.addEventListener('mousemove',function(e){
    if(_colRsz||_rowRsz)return;
    if(e.target.classList&&e.target.classList.contains('col-rsz-h')){editor.style.cursor='';return;}
  ...
  editor.addEventListener('mousedown',function(e){
    if(!_rowEdge||e.target.classList.contains('col-rsz-h'))return;
  ...
  document.addEventListener('mousemove',function(e){
    if(_colRsz){
      const newW=Math.max(1,_colRsz.startW+(e.clientX-_colRsz.startX));
      _colRsz.cells.forEach(c=>{c.style.width=newW+'px';c.style.minWidth='0';c.style.maxWidth='none';c.style.overflow='hidden';});
    }
    if(_rowRsz){
  ...
  document.addEventListener('mouseup',function(){
    if(_colRsz){document.querySelectorAll('.col-rsz-h.active').forEach(h=>h.classList.remove('active'));_colRsz=null;document.body.style.cursor='';}
    if(_rowRsz){_rowRsz=null;editor.style.cursor='';}
  });
  // 내용 변경 시 핸들 재생성
  const obs=new MutationObserver(()=>{clearTimeout(obs._t);obs._t=setTimeout(()=>_rebuildColHandles(editor),300);});
...
function doPdfExport(){
  ...
  let memoSection='';
  if(withMemo){
    const memos=[];editor.querySelectorAll('.re-memo').forEach(m=>memos.push({text:(m.textContent||'').replace(/\u200B/g,''),name:m.dataset.memoName||'메모',desc:m.dataset.memoDesc||'',resolved:m.dataset.memoResolved==='true',mc:m.dataset.memoMarkerColor||'',mn:m.dataset.memoMarkerName||''}));
    if(memos.length){memoSection='<div style="margin-top:32px;border-top:2px solid #BFDBFE;padding-top:16px;"><div style="font-weight:800;font-size:.95rem;margin-bottom:10px;color:#1e3a5f;">📝 메모 목록</div>';memos.forEach(m=>{const mcDot=m.mc?`<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${m.mc};margin-right:4px;vertical-align:middle;"></span>`:'' ;memoSection+=`<div style="margin-bottom:8px;background:${m.resolved?'#F0FDF4':'#EFF6FF'};border-left:3px solid ${m.resolved?'#86EFAC':m.mc||'#BFDBFE'};padding:8px 12px;border-radius:0 6px 6px 0;font-size:.85rem;"><div style="font-weight:700;color:#1e3a5f;">${mcDot}${m.name}${m.resolved?' ✓':''}</div><div style="color:#8B95A1;font-size:.78rem;margin:2px 0;">"${m.text}"</div>${m.desc?`<div style="color:#5D4037;margin-top:3px;">${m.desc}</div>`:''}</div>`;});memoSection+='</div>';}
...
function reInsertTable(){
  ...
  toast('3×3 표 삽입. 열 경계를 드래그해서 너비 조절 | 우클릭 편집');
}
