...
async function openRichEditor(docId=''){
  ...
  _asActive=!!docId; _asSetUI('idle');
  // 이벤트 바인딩 (한 번만)
  setTimeout(()=>{ _bindCtxMenu(); _bindTableCtxMenu(); }, 200);
...
function reInsertCheckbox() {
  const editor = document.getElementById('reEditor');
  editor.focus();
  const html = '<ul class="re-checklist"><li class="re-check-item"><input type="checkbox" onclick="this.parentElement.classList.toggle(\'checked\',this.checked)"><span>할 일 항목</span></li></ul><p><br></p>';
  reExec('insertHTML', html);
}
...
function reOnKeydown(e){
  ...
  if(e.key==='Enter'){
    ...
    if(chkItem){
      ...
      newLi.innerHTML='<input type="checkbox" onclick="this.parentElement.classList.toggle(\'checked\',this.checked)"><span></span>';
      chkItem.after(newLi);
...
function _reCheckLineMarkdown(){
  ...
  if(text==='[]'||text==='[ ]'){
    ...
    li.innerHTML='<input type="checkbox" onclick="this.parentElement.classList.toggle(\'checked\',this.checked)"><span></span>';
    ul.appendChild(li);
    block.before(ul);
    block.remove();
    const r2=document.createRange();
