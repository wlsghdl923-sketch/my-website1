...
/* ══ 콜아웃 픽커 ══ */
let _calloutColor={border:'#C7D7FF',bg:'#F0F4FF',text:'#1A3A8F'};
let _calloutEmoji='💡';
function openCalloutPicker(){
  _calloutEmoji='💡';
  document.querySelectorAll('.co-swatch').forEach(s=>s.style.borderColor='transparent');
  const first=document.querySelector('.co-swatch');
  if(first){first.style.borderColor='#333';_calloutColor={border:first.dataset.color,bg:first.dataset.bg,text:first.dataset.text};}
  document.getElementById('calloutPickerOverlay').style.display='block';
}
function closeCalloutPicker(){document.getElementById('calloutPickerOverlay').style.display='none';}
function selectCalloutColor(el){
  document.querySelectorAll('.co-swatch').forEach(s=>s.style.borderColor='transparent');
  el.style.borderColor='#333';
  _calloutColor={border:el.dataset.color,bg:el.dataset.bg,text:el.dataset.text};
}
function setCalloutEmoji(em){_calloutEmoji=em;}
function insertCalloutWithStyle(){
  const emojiHtml=_calloutEmoji?`<span style="font-size:1.2rem;flex-shrink:0;margin-top:1px;">${_calloutEmoji}</span>`:'';
  const html=`<div class="re-callout" contenteditable="false" style="display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid ${_calloutColor.border};background:${_calloutColor.bg};color:${_calloutColor.text};line-height:1.7;">`+
    emojiHtml+
    `<div contenteditable="true" style="flex:1;min-width:0;outline:none;" data-placeholder="내용을 입력하세요">내용을 입력하세요</div>`+
    `</div><p><br></p>`;
  closeCalloutPicker();
...
function showTableContextMenu(e,td){
  ...
  const onClose=()=>{menu.style.display='none';document.removeEventListener('click',onClose);};
  setTimeout(()=>document.addEventListener('click',onClose),10);
}
function tableCmd(cmd){
  const td=_ctxTd;if(!td)return;
  ...
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    ...
    for(let i=0;i<cols;i++){const ntd=document.createElement('td');ntd.contentEditable='true';ntd.style.cssText='padding:10px 14px;border:1px solid #E5E8EB;min-width:60px;';newTr.appendChild(ntd);}
    if(cmd==='addRowAbove')tr.parentNode.insertBefore(newTr,tr);
    ...
    rows.forEach((row,rIdx)=>{
      ...
      const nc=document.createElement(tag);nc.contentEditable='true';
      nc.style.cssText=rIdx===0?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;min-width:60px;':'padding:10px 14px;border:1px solid #E5E8EB;min-width:60px;';
      if(cmd==='addColLeft')c.parentNode.insertBefore(nc,c);
      else c.parentNode.insertBefore(nc,c.nextSibling);
    });
  }
    ...
    if(nextRow){
      const below=Array.from(nextRow.querySelectorAll('td,th'))[ci];
      if(below){
        td.rowSpan=(td.rowSpan||1)+(below.rowSpan||1);
        td.innerHTML+=below.innerHTML;
        below.remove();
      }
    }
  }
...
async function exportBackup(){
  ...
  try{
    ...
    const obj={_backup_version:1,_backup_date:new Date().toISOString(),data:{}};
    ...
    a.href=url;a.download='backup_'+d+'.json';
    document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
    toast('백업 완료 — '+rows.length+'개 항목');
  }catch(e){toast('백업 실패: '+((e&&e.message)||'오류'));console.error(e);}
}
async function importBackup(event){
  ...
  try{
    ...
    if(!obj.data){toast('올바른 백업 파일이 아닙니다.');return;}
  ...
  }catch(e){toast('복원 실패: '+((e&&e.message)||''));console.error(e);}
}
...
aside{display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid #C7D7FF;background:#F0F4FF;color:#1A3A8F;line-height:1.7;}
.re-callout{display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;line-height:1.7;}
  ...
  if(!editor._pasteHandlerSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(html){
        ...
        const clean=html
          .replace(/<aside[^>]*>/gi,'<div class="re-callout" style="'+CS+'">')
    ...
    editor.addEventListener('contextmenu',function(e){
      ...
      if(td){e.preventDefault();showTableContextMenu(e,td);}
    });
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
...
function _initImgResize(){
  ...
  editor.addEventListener('mousedown',function(e){
    const handle=e.target.closest&&e.target.closest('.re-img-handle');
    if(!handle)return;e.preventDefault();
    const wrap=handle.parentElement;if(!wrap)return;
    _resizeState={wrap,dir:handle.dataset.dir,startX:e.clientX,startW:wrap.offsetWidth};
  });
...
function reInsertTable(){
  ...
  const TS='border-collapse:collapse;width:100%;margin:.8em 0;border-radius:8px;overflow:hidden;border:1px solid #E5E8EB;';
  const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;min-width:60px;';
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;min-width:60px;';
  let html=`<table style="${TS}" contenteditable="false"><colgroup>`;
  ...
  for(let r=0;r<rows;r++){
    ...
    for(let c=0;c<cols;c++){
      const isH=r===0;
      html+=`<${isH?'th':'td'} contenteditable="true" style="${isH?THS:TDS}">${isH?'제목 '+(c+1):''}</${isH?'th':'td'}>`;
    }
    html+='</tr>';
  }
