...
function saveMemoInput() {
  ...
  if (_memoEditId) {
    ...
    closeMemoInput(); refreshMemoPanel(); _reDirty=true;_asSchedule(); return;
  }
  ...
  el.innerHTML = _memos.map(m =>
    `<div class="memo-card${m.resolved?' memo-resolved':''}" id="mc_${m.id}" style="border-left-color:${m.color};" onclick="_focusMemo('${m.id}')">
       ...
       <div class="memo-card-btns">
         ...
         ${IS_SHARE?'':`<button onclick="event.stopPropagation();editMemo('${m.id}')">수정</button>`}
         ${IS_SHARE?'':`<button class="btn-del" onclick="event.stopPropagation();deleteMemo('${m.id}')">삭제</button>`}
       </div>
     </div>`
  ).join('');
}
