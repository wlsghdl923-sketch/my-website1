...
function toggleColorPalette(id){
  const sel=window.getSelection();if(sel&&sel.rangeCount&&!sel.isCollapsed)_savedSel={r:sel.getRangeAt(0).cloneRange()};
  const p=document.getElementById(id);if(!p)return;
  const open=p.style.display==='flex';document.querySelectorAll('.re-color-palette').forEach(x=>x.style.display='none');if(!open)p.style.display='flex';
}
...
/* ══ 메모 — Notion 스타일 ══
   • contextmenu에서 즉시 range 저장
   • 오른쪽 패널에 Y좌표 기준으로 카드 배치
   • 삭제: span 내용을 부모로 이동 후 span 제거
══ */
...
function openMemoInput(){
  ...
  if(prev)prev.textContent='"'+_memoSavedText.slice(0,60)+(_memoSavedText.length>60?'...':'')+'"';
  document.getElementById('memoInputText').value='';
  document.getElementById('memoInputOverlay').style.display='block';
  setTimeout(()=>document.getElementById('memoInputText').focus(),100);
}

function closeMemoInput(){
  document.getElementById('memoInputOverlay').style.display='none';
  _memoSavedRange=null;_memoSavedText='';
}

function saveMemoToSelection(){
  const text=(document.getElementById('memoInputText').value||'').trim();
  if(!text){toast('메모 내용을 입력하세요.');return;}
  ...
  span.className='re-memo';span.dataset.memo=text;span.dataset.memoid=id;
...
function debouncedMemoPanel(){clearTimeout(_memoDebTimer);_memoDebTimer=setTimeout(refreshMemoPanel,300);}
...
function refreshMemoPanel(){
  ...
  if(memos.length===0){panel.style.display='none';return;}
  panel.style.display='block';

  // 패널 스크롤 컨테이너 기준 Y 계산
  const panelEl=panel;
  const editorScrollEl=editor.closest('[style*="overflow-y:auto"]')||editor.parentElement;
  const panelRect=panelEl.getBoundingClientRect();

  // 최소 높이를 동적으로 잡기 위한 accumulated bottom
  let nextY=8;
  const cards=[];

  memos.forEach((m,i)=>{
    const id=m.dataset.memoid||('m'+i);m.dataset.memoid=id;
    // 에디터 내 span의 top → 패널 기준 Y
    const mRect=m.getBoundingClientRect();
    const y=Math.max(nextY, mRect.top - panelRect.top + panelEl.scrollTop);
    cards.push({id,y,quote:(m.textContent||'').slice(0,40),memo:m.dataset.memo||''});
  });

  // 카드 겹침 방지: 위에서부터 최소 간격 확보
  const CARD_H=90; // 추정 카드 높이
  for(let i=1;i<cards.length;i++){
    if(cards[i].y<cards[i-1].y+CARD_H+6)cards[i].y=cards[i-1].y+CARD_H+6;
  }

  // 패널 최소 높이 설정
  const last=cards[cards.length-1];
  list.style.height=Math.max(200,(last.y+CARD_H+20))+'px';

  list.innerHTML=cards.map(c=>{
    const now=new Date();
    const dateStr=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')} ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
    return `<div class="memo-card" id="mc_${c.id}" style="top:${c.y}px;" onclick="highlightMemoCard(document.querySelector('[data-memoid=\\'${c.id}\\']'))">
      <div class="memo-card-header">
        <span class="memo-card-user">메모</span>
        <span class="memo-card-time">${dateStr}</span>
      </div>
      <div class="memo-card-quote">${c.quote}</div>
      <div class="memo-card-body">${c.memo}</div>
      <div class="memo-card-btns">
        <button onclick="event.stopPropagation();editMemoCard('${c.id}')">수정</button>
        <button class="mdel" onclick="event.stopPropagation();deleteMemoCard('${c.id}')">삭제</button>
      </div>
    </div>`;
  }).join('');
}
...
function editMemoCard(id){
  ...
  const nw=prompt('메모 수정:',el.dataset.memo||'');if(nw===null)return;
  if(nw.trim()){el.dataset.memo=nw.trim();}
  setTimeout(refreshMemoPanel,50);
}

function deleteMemoCard(id){
  /* 핵심: span의 자식 노드를 부모로 옮긴 뒤 span 제거 */
  const el=document.querySelector(`[data-memoid="${id}"]`);
  if(!el){toast('메모를 찾을 수 없습니다.');return;}
...
function _coEmojiGridHTML(sel){const customs=_coCustomEmojis.map((em,i)=>{const isImg=em.startsWith('data:')||em.startsWith('http');const inner=isImg?`<img src="${em}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;">`:`${em}`;return `<span class="co-em-btn${em===sel?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${inner}<span class="co-em-del" onclick="event.stopPropagation();delCoCustomEmoji(${i})">✕</span></span>`;});const base=CO_EMOJIS.map(em=>`<span class="co-em-btn${em===sel?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${em}</span>`);const none=`<span class="co-em-btn co-em-none${sel===''?' sel':''}" data-em="" onclick="pickCoEmoji(this)">없음</span>`;return [...customs,...base,none].join('');}
...
async function handleCoCustomImg(e){const file=e.target.files[0];e.target.value='';if(!file)return;await _loadCoCustomEmojis();const reader=new FileReader();reader.onload=async ev=>{_coCustomEmojis.push(ev.target.result);await _saveCoCustomEmojis();const sel=_coPick.emoji;['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});toast('커스텀 이모지 저장됨');};reader.readAsDataURL(file);}
...
function tblCmd(cmd){
  ...
  else if(cmd==='delRow'){if(rows.length>1)tr.remove();}
  else if(cmd==='delCol'){if(cols>1)rows.forEach(row=>{const c=Array.from(row.querySelectorAll('td,th'))[ci];if(c)c.remove();});}
  else if(cmd==='delTable'){table.remove();}
  _tblTd=null;clearTableCellSel();
  setTimeout(()=>_addColHandles(document.getElementById('reEditor')),50);
}
...
/* ════════════════════════════════
   표 열/행 리사이즈
   ━━ 세로선: 핸들 div를 각 셀 오른쪽에 직접 삽입 ━━
   ━━ 가로선: 셀 아래쪽 감지 (기존 방식 유지)    ━━
════════════════════════════════ */
let _colRszData=null, _rowRszData=null;

function _addColHandles(editor){
  if(!editor)return;
  // 기존 핸들 제거
  editor.querySelectorAll('.col-rsz').forEach(h=>h.remove());
  // 모든 셀(마지막 열 제외)에 핸들 추가
  editor.querySelectorAll('table').forEach(table=>{
    const rows=Array.from(table.querySelectorAll('tr'));
    rows.forEach(tr=>{
      const cells=Array.from(tr.querySelectorAll('td,th'));
      cells.forEach((cell,ci)=>{
        if(ci===cells.length-1)return; // 마지막 열은 핸들 없음
        ...
        h.addEventListener('mousedown',function(e){
          ...
          _colRszData={cells:colCells,startX:e.clientX,startW:cell.offsetWidth};
...
function _initTableResize(){
  ...
  // 초기 핸들 생성
  _addColHandles(editor);

  // 행 높이 드래그 — 셀 아래쪽 6px 감지
  let _rowEdge=null;
  editor.addEventListener('mousemove',function(e){
    if(_colRszData||_rowRszData)return;
    if(e.target.classList&&e.target.classList.contains('col-rsz'))return;
    const cell=e.target.closest&&e.target.closest('td,th');
    if(!cell){if(editor.style.cursor==='row-resize')editor.style.cursor='';return;}
    const rect=cell.getBoundingClientRect();
    const ry=e.clientY-rect.top;
    if(ry>=rect.height-6&&ry<=rect.height+6){editor.style.cursor='row-resize';_rowEdge=cell;}
    else{if(editor.style.cursor==='row-resize')editor.style.cursor='';_rowEdge=null;}
  });
  editor.addEventListener('mousedown',function(e){
    if(!_rowEdge||e.target.classList.contains('col-rsz'))return;
    e.preventDefault();e.stopPropagation();
    const tr=_rowEdge.closest('tr');
    _rowRszData={tr,startY:e.clientY,startH:tr.offsetHeight};
    _rowEdge=null;
  });

  // 열/행 드래그 처리 (document 레벨)
  document.addEventListener('mousemove',function(e){
    if(_colRszData){
      const newW=Math.max(1,_colRszData.startW+(e.clientX-_colRszData.startX));
      _colRszData.cells.forEach(c=>{c.style.width=newW+'px';c.style.minWidth='0';c.style.maxWidth='none';c.style.overflow='hidden';});
    }
    if(_rowRszData){
      const newH=Math.max(10,_rowRszData.startH+(e.clientY-_rowRszData.startY));
      _rowRszData.tr.style.height=newH+'px';
      Array.from(_rowRszData.tr.querySelectorAll('td,th')).forEach(c=>{c.style.height=newH+'px';});
    }
  });
  document.addEventListener('mouseup',function(){
    if(_colRszData){
      document.querySelectorAll('.col-rsz.dragging').forEach(h=>h.classList.remove('dragging'));
      _colRszData=null;editor.style.cursor='';
    }
    if(_rowRszData){_rowRszData=null;editor.style.cursor='';}
  });

  // 에디터 내용 변경 시 핸들 갱신
  const obs=new MutationObserver(()=>{clearTimeout(obs._t);obs._t=setTimeout(()=>_addColHandles(editor),200);});
...
function doPdfExport(){
  ...
  const clone=editor.cloneNode(true);clone.querySelectorAll('.col-rsz').forEach(h=>h.remove());
  let memoSection='';
  if(withMemo){const memos=[];editor.querySelectorAll('.re-memo').forEach(m=>memos.push({text:m.textContent,memo:m.dataset.memo||''}));
    if(memos.length){memoSection='<div style="margin-top:32px;border-top:2px solid #FFE082;padding-top:16px;"><div style="font-weight:800;font-size:.95rem;margin-bottom:10px;color:#F57F17;">📝 메모 목록</div>';memos.forEach(m=>{memoSection+=`<div style="margin-bottom:8px;background:#FFFDE7;border-left:3px solid #FFE082;padding:8px 12px;border-radius:0 6px 6px 0;font-size:.85rem;"><span style="font-weight:700;color:#5D4037;">"${m.text}"</span><div style="color:#5D4037;margin-top:4px;">${m.memo}</div></div>`;});memoSection+='</div>';}}
...
async function _readDocRawAsync(docId){
  try{const row=await _sbRest.selectOne('site_data','hub_doc_'+docId);if(!row||row.value===undefined||row.value===null)return '';let raw=typeof row.value==='string'?row.value:JSON.stringify(row.value);if(raw.startsWith('"')&&raw.endsWith('"')){try{const d=JSON.parse(raw);if(typeof d==='string')raw=d;}catch{}}return raw;}catch(e){return '';}
}
...
async function openHtmlEditor(docId=''){
  ...
  if(docId){
    ...
    const raw=await _readDocRawAsync(docId);
    document.getElementById('deContent').value=raw||'';updatePreview();
  }else{
    document.getElementById('deTitle').value='';document.getElementById('deContent').value='';
    document.getElementById('docEditorOverlay').classList.add('open');updatePreview();
  }
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('click',function(e){
      ...
      if(memo){e.stopPropagation();highlightMemoCard(memo);}
    });
...
function reHandleImg(e){
  ...
  reader.onload=async ev=>{
    ...
    const HS='position:absolute;width:12px;height:12px;background:#D91F3E;border-radius:50%;z-index:10;border:2px solid #fff;';
    reExec('insertHTML','<span class="re-img-wrap" style="display:inline-block;position:relative;width:360px;max-width:100%;vertical-align:top;margin:.5em 0;" contenteditable="false"><img src="'+url+'" style="width:100%;border-radius:8px;display:block;" draggable="false"><span class="re-img-handle" data-dir="se" style="'+HS+'right:-6px;bottom:-6px;cursor:se-resize;"></span><span class="re-img-handle" data-dir="sw" style="'+HS+'left:-6px;bottom:-6px;cursor:sw-resize;"></span><span class="re-img-handle" data-dir="ne" style="'+HS+'right:-6px;top:-6px;cursor:ne-resize;"></span><span class="re-img-handle" data-dir="nw" style="'+HS+'left:-6px;top:-6px;cursor:nw-resize;"></span></span><span contenteditable="true"> </span>');
    setTimeout(()=>_initImgResize(),100);toast('이미지 삽입 완료');
  };reader.readAsDataURL(file);e.target.value='';
}
let _imgRsz=null;
function _initImgResize(){
  const ed=document.getElementById('reEditor');if(!ed||ed._imgRszSet)return;ed._imgRszSet=true;
  ed.addEventListener('mousedown',function(e){const h=e.target.closest&&e.target.closest('.re-img-handle');if(!h)return;e.preventDefault();const w=h.parentElement;_imgRsz={w,dir:h.dataset.dir,sx:e.clientX,sw:w.offsetWidth};});
  document.addEventListener('mousemove',function(e){if(!_imgRsz)return;const dx=e.clientX-_imgRsz.sx;_imgRsz.w.style.width=Math.max(60,Math.min((_imgRsz.dir==='se'||_imgRsz.dir==='ne')?_imgRsz.sw+dx:_imgRsz.sw-dx,760))+'px';});
  document.addEventListener('mouseup',()=>{_imgRsz=null;});
}
function reInsertTable(){
  ...
  for(let r=0;r<rows;r++){html+='<tr>';
    for(let c=0;c<cols;c++){const isH=r===0;const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;':'padding:10px 14px;border:1px solid #E5E8EB;';
      html+='<'+(isH?'th':'td')+' contenteditable="true" style="'+st+'">'+(isH?'제목'+(c+1):'')+'</'+(isH?'th':'td')+'>';}html+='</tr>';}
...
function reOnKeydown(e){
  if(e.key==='Tab'){
    ...
    else{if(e.shiftKey)document.execCommand('outdent');else{const inList=document.queryCommandState('insertUnorderedList')||document.queryCommandState('insertOrderedList');if(!inList)document.execCommand('insertUnorderedList');else document.execCommand('indent');}}
    return;
  }
