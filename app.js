...
function tblCmd(cmd){
  ...
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    ...
    const nr=rows[ri+1];
    if(nr){const bl=Array.from(nr.querySelectorAll('td,th'))[ci];if(bl){td.rowSpan=(td.rowSpan||1)+(bl.rowSpan||1);td.innerHTML+=bl.innerHTML;bl.remove();}}
...
/* ══ 표 선 드래그 리사이즈 — 세로선 최소 1px, 감도 1:1 ══ */
let _tblRsz=null;
function _initTableResize(){
  ...
  const THRESH=6;    // 경계 감지 범위(px)
  const MIN_COL=1;   // 세로선: 열 최소 너비 1px — 거의 제한 없음
  const MIN_ROW=10;  // 가로선: 행 최소 높이
  ...
  document.addEventListener('mouseup',function(){
    if(_tblRsz){
      _tblRsz.cell?.closest('table')?.classList.remove('col-resizing');
      _tblRsz=null;editor.style.cursor='';
    }
  });
}
...
table{border-collapse:collapse;width:100%;margin:.6em 0;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}
th{background:#F5F6FA;font-weight:700;}img{max-width:100%;border-radius:6px;}
.re-img-wrap{display:inline-block;vertical-align:top;}.re-img-handle{display:none!important;}
