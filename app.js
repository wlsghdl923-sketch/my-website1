...
function renderPartialRestoreList(){
  ...
  for(const cardId of Object.keys(cdocs)){
    ...
    docs.forEach(doc=>{
      ...
      const typeIcon = doc.type==='richtext' ? '📝' : doc.type==='html' ? '🌐' : doc.type==='refdoc' ? '🖼' : doc.type==='calendar' ? '📅' : '📄';
...
function buildTreeHTML(docs,parentId,depth){
  ...
  items.forEach(item=>{
    ...
    if(item.type==='folder'){
      ...
      const clickFn = item.type==='richtext' ? `openRichViewer('${item.id}')` : (item.type==='calendar' ? `openCalendarDoc('${item.id}')` : `openDocViewer('${item.id}')`);
