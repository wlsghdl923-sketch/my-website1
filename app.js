...
.notion-callout{display:block;background:#F0F4FF;border:1px solid #C7D7FF;border-radius:8px;padding:.8em 1em;margin:.8em 0;color:#1A3A8F;line-height:1.7;}
...
function _cleanPasteHtml(html){
  // <aside>...</aside> → <div class="notion-callout">...</div>
  html=html.replace(/<aside([^>]*)>/gi,'<div class="notion-callout"$1>');
  html=html.replace(/<\/aside>/gi,'</div>');
  // 노션 이미지 제거 (끊긴 주소)
  html=html.replace(/<img[^>]+notion[^>]*>/gi,'');
  return html;
}
function openRichEditor(docId=''){
  ...
  if(!editor._pasteHandlerSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(html){
        const clean=_cleanPasteHtml(html);
