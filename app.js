...
function _pdfPreviewUpdate(){
  ...
  _pdfPreviewTimer=setTimeout(_pdfPreviewRender,200);
}
...
function _pdfPreviewRender(){
  ...
  const ratio=phMm/pwMm; // 높이/너비 비율
  const previewW=Math.min(380, document.getElementById('pdfPreviewScroll')?.clientWidth-40||380);
  ...
  let styleStr='';
  if(/^<!DOCTYPE/i.test(bodyHtml.trim())||/^<html/i.test(bodyHtml.trim())){
    const m=bodyHtml.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
    bodyHtml=m?m[1]:bodyHtml;
    const styleMatches=[...content.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)];
    styleStr=styleMatches.map(s=>s[1]).join('\n');
  }
  const iframeScale=contentAreaW/contentWidth;
  const iframeBodyH=contentAreaH/iframeScale; // iframe 내부 기준 한 페이지 높이

  // 먼저 높이를 측정하기 위한 숨겨진 iframe
  const measurer=document.createElement('iframe');
  measurer.style.cssText=`position:fixed;left:-9999px;top:0;width:${contentWidth}px;border:none;visibility:hidden;`;
  document.body.appendChild(measurer);
  const mDoc=measurer.contentDocument;
  mDoc.open();
  mDoc.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700&display=swap" rel="stylesheet"><style>body{font-family:'Noto Sans KR',sans-serif;padding:32px;line-height:1.8;color:#191F28;font-size:15px;margin:0;}h1,h2,h3{font-weight:800;letter-spacing:-.02em;margin:1.2em 0 .5em;}h1{font-size:1.8rem;}h2{font-size:1.4rem;}h3{font-size:1.1rem;}p{margin:.6em 0;}img{max-width:100%;border-radius:8px;}hr{border:none;border-top:1px solid #E5E8EB;margin:1.5em 0;}table{border-collapse:collapse;width:100%;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}${styleStr}</style></head><body>${bodyHtml}</body></html>`);
  mDoc.close();

  setTimeout(()=>{
    const totalH=mDoc.body.scrollHeight;
    const pageCount=Math.max(1,Math.ceil(totalH/iframeBodyH));
    ...
    const iframeSrc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700&display=swap" rel="stylesheet"><style>body{font-family:'Noto Sans KR',sans-serif;padding:32px;line-height:1.8;color:#191F28;font-size:15px;margin:0;}h1,h2,h3{font-weight:800;letter-spacing:-.02em;margin:1.2em 0 .5em;}h1{font-size:1.8rem;}h2{font-size:1.4rem;}h3{font-size:1.1rem;}p{margin:.6em 0;}img{max-width:100%;border-radius:8px;}hr{border:none;border-top:1px solid #E5E8EB;margin:1.5em 0;}table{border-collapse:collapse;width:100%;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}${styleStr}</style></head><body>${bodyHtml}</body></html>`;
    ...
    for(let p=0;p<pageCount;p++){
      ...
      frame.style.height=iframeBodyH+'px';
      frame.style.transform=`scale(${iframeScale})`;
      frame.style.transformOrigin='top left';
      frame.sandbox='allow-same-origin';

      const wrapper=document.createElement('div');
      wrapper.style.cssText=`width:${contentAreaW}px;height:${contentAreaH}px;overflow:hidden;`;
      ...
      // iframe에 콘텐츠 로드 후 스크롤 위치 조정
      frame.addEventListener('load',()=>{
        try{
          frame.contentDocument.body.parentElement.style.overflow='hidden';
          frame.contentWindow.scrollTo(0, p*iframeBodyH);
        }catch(e){}
      });
      frame.srcdoc=iframeSrc;
    }
  },300);
}
...
async function _loadPdfLibs(){
  ...
  try{
    ...
    await load('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.2/jspdf.umd.min.js');
    _pdfLibsLoaded=true;return true;
  }catch(e){toast('PDF 라이브러리 로드 실패. 네트워크를 확인해주세요.');return false;}
}
...
async function execPdfExport(){
  ...
  // 라이브러리 로드
  const ok=await _loadPdfLibs();
  if(!ok){prog.style.display='none';document.getElementById('pdfExportBtn').disabled=false;return;}
  ...
  // HTML 콘텐츠 주입
  let rawHtml=content;
  if(/^<!DOCTYPE/i.test(rawHtml.trim())||/^<html/i.test(rawHtml.trim())){
    // 전체 HTML → body 내용 추출
    const m=rawHtml.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
    const bodyContent=m?m[1]:rawHtml;
    // head의 style 추출
    const styleMatches=[...rawHtml.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)];
    const styles=styleMatches.map(s=>s[1]).join('\n');
    if(styles){const sEl=document.createElement('style');sEl.textContent=styles;container.appendChild(sEl);}
    container.innerHTML+= bodyContent;
  }else{
    container.innerHTML=`<div style="font-family:'Noto Sans KR',sans-serif;padding:32px;line-height:1.8;color:#191F28;font-size:15px;max-width:${contentWidth}px;">
      <style>h1,h2,h3{font-weight:800;letter-spacing:-.02em;margin:1.2em 0 .5em;}h1{font-size:1.8rem;}h2{font-size:1.4rem;}h3{font-size:1.1rem;}p{margin:.6em 0;}img{max-width:100%;border-radius:8px;}hr{border:none;border-top:1px solid #E5E8EB;margin:1.5em 0;}table{border-collapse:collapse;width:100%;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}</style>
      ${content}
    </div>`;
  }
  ...
  if(images.length){
    ...
    await Promise.allSettled([...images].map(img=>new Promise(r=>{
  ...
  try{
    const scale=scalePct/100*2; // 고해상도
    ...
    const imgW=pw-margin*2; // mm
    const imgH=(canvas.height*imgW)/canvas.width; // 비율 유지
    const pageH=ph-margin*2; // 사용 가능 높이 mm
    ...
    if(imgH<=pageH){
      ...
      const sliceH=Math.floor(canvas.height/totalPages); // 캔버스 px 단위 슬라이스
      ...
      for(let i=0;i<totalPages;i++){
        ...
        const pct=70+Math.round((i+1)/totalPages*25);
        progBar.style.width=pct+'%';
        progText.textContent=`PDF 생성 중... (${i+1}/${totalPages} 페이지)`;
      }
    }

    progBar.style.width='100%';
    progText.textContent='다운로드 중...';
    pdf.save(`${title}.pdf`);
    ...
    progText.textContent='오류 발생';progBar.style.width='0%';
    document.getElementById('pdfExportBtn').disabled=false;
  }finally{
