...
let _dataLoaded = false; // Supabase 로드 성공 전까지 저장 차단
let _loadedCounts = {};  // 로드 시점의 데이터 개수 기록 (빈 배열 덮어쓰기 방지)
let _allowEmptySave = false; // 의도적 전체 삭제 시 true

/* ══ localStorage 완전 비사용 — Supabase 단일 소스 ══ */
function ls(k,d){return d;}  // 로컬 읽기 비활성화 — 항상 기본값 반환
function ss(k,v){try{localStorage.setItem(k,JSON.stringify(v));}catch(e){}}  // localStorage 백업 캐시
...
let CAL_DOC_CFG = {};
let _activeCalDocId=null;
let _activeCalDocType=null;
let CAL_FILTER_ORDER=[];  // 캘린더 필터 순서
/* ══ 일정 문서 (독립 캘린더) 관련 전역 ══ */
let _schedDocMode=false;
let _schedDocId=null;
let _savedCtxForSched=null;
...
function handleSbStatusClick(){
  ...
  if(badge&&badge.classList.contains('error')){
    if(!_dataLoaded){toast('서버에서 다시 불러오는 중...');initData().then(()=>{if(_dataLoaded&&typeof renderAll==='function')renderAll();});}
    else{toast('Supabase 재저장 시도 중...');saveAll();}
  }
}

async function _sbSet(key, value){
  if(!_dataLoaded){console.warn('Supabase 로드 미완료 — 저장 차단: '+key);return {ok:false, error:'로드 미완료'};}
  ...
  for(let attempt=0;attempt<3;attempt++){
    try{
      ...
      console.warn('Supabase 저장 시도 '+(attempt+1)+'/3 실패 ['+key+']: '+((e&&e.message)||String(e)));
      if(attempt<2) await new Promise(r=>setTimeout(r,500*(attempt+1)));
    }
  }
  console.error('Supabase 저장 최종 실패 ['+key+']');
  _sbFailCount++;
  return {ok:false, error:'3회 재시도 후 저장 실패'};
}
...
function _localSave(key, value){try{localStorage.setItem(key,typeof value==='string'?value:JSON.stringify(value));}catch(e){}} // localStorage 백업 캐시
...
async function saveAll(){
  if(!_dataLoaded){console.warn('Supabase 로드 미완료 — 저장 차단');return;}
  // ── 일정 문서 모드: 메인 키 대신 번들 키로 저장 ──
  if(_schedDocMode){_scheduleSchedBundleSave();return;}
  // 이미 저장 실행 중이면 대기열에 등록
...
async function _doSaveAll(){
  ...
  // ── 빈 배열 덮어쓰기 방지 ──
  // 로드 시 데이터가 있었는데 지금 비어있으면 저장 차단
  const _critical = {hub_events:EVENTS, hub_cdocs:CARD_DOCS, hub_cards:CARDS, hub_tabs:TABS};
  if(!_allowEmptySave){
    for(const[key,arr] of Object.entries(_critical)){
      const loaded = _loadedCounts[key]||0;
      const now = Array.isArray(arr)?arr.length:(typeof arr==='object'?Object.keys(arr).length:0);
      if(loaded > 0 && now === 0){
        console.error(`⛔ ${key}: 로드 시 ${loaded}건 → 현재 0건 — 빈 배열 저장 차단`);
        _setSbStatus('error','데이터 보호: 빈 배열 저장 차단됨');
        _saveRunning=false;
        return;
      }
    }
  }
  _allowEmptySave=false; // 일회성 플래그 초기화
  ...
  const saves=[
    ...
    _sbSet('hub_cal_filter_order', CAL_FILTER_ORDER),
  ...
  if(failed>0){
    ...
    console.warn(`Supabase 저장 실패 ${failed}건 — 로컬 백업에 임시 저장됨`);
  }else{
  ...
  _saveRunning=false;
  if(_savePending){_savePending=false;saveAll();}
}
...
function _applyMap(map){
  ...
  if(map['hub_cal_filter_order']) { CAL_FILTER_ORDER=map['hub_cal_filter_order']; }
...
async function initData(){
  ...
  try{
    ...
    // ── 로드 시점 데이터 개수 기록 (덮어쓰기 방지용) ──
    _loadedCounts = {
      hub_events: Array.isArray(EVENTS)?EVENTS.length:0,
      hub_cdocs:  typeof CARD_DOCS==='object'?Object.keys(CARD_DOCS).length:0,
      hub_cards:  typeof CARDS==='object'?Object.keys(CARDS).length:0,
      hub_tabs:   typeof TABS==='object'?Object.keys(TABS).length:0,
    };

    // ── 자동 백업 (데이터가 있을 때만) ──
    try{
      if(EVENTS.length>0)   _sbSet('hub_events_backup', EVENTS);
      if(SPECIALS.length>0) _sbSet('hub_specials_backup', SPECIALS);
      if(Object.keys(CARD_DOCS).length>0) _sbSet('hub_cdocs_backup', CARD_DOCS);
      if(Object.keys(CARDS).length>0) _sbSet('hub_cards_backup', CARDS);
      if(Object.keys(TABS).length>0)  _sbSet('hub_tabs_backup', TABS);
    }catch(bkErr){console.warn('자동 백업 실패:',bkErr);}
    ...
    let recovered=false;
    try{
      const lsTabs=localStorage.getItem('hub_tabs');
      if(lsTabs){
        const keys=['hub_tabs','hub_cards','hub_cdocs','hub_events','hub_specials','hub_presets','hub_cal_name','hub_active','hub_cfg','hub_colors','hub_cal_cfg','hub_cal_doc_cfg','hub_processes','hub_assignee_sets','hub_cal_display','hub_pd_order'];
        const map={};
        keys.forEach(k=>{const v=localStorage.getItem(k);if(v)try{map[k]=JSON.parse(v);}catch{}});
        if(Object.keys(map).length>0){_applyMap(map);_dataLoaded=true;recovered=true;}
      }
    }catch{}
    if(recovered){
      _setSbStatus('error','오프라인 모드 (로컬 백업 사용 중)');
      console.warn('Supabase 실패 → 로컬 백업에서 복구됨');
    }else{
...
/* ══ 탭 복귀 시 최신 데이터 자동 새로고침 ══ */
let _lastLoadTs=0;
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='visible' && _dataLoaded && !_saveRunning && Date.now()-_lastLoadTs>30000){
    _lastLoadTs=Date.now();
    initData().then(()=>{if(_dataLoaded&&typeof renderAll==='function')renderAll();});
  }
});
...
async function openMemoInput(selectedText, editId) {
  ...
  document.getElementById('memoContentInput').innerHTML = existing?.content || '';
...
/* ── 메모 에디터 색상 팔레트 ── */
const _MEMO_HILITE_COLORS=['#FFEB3B','#A5F5A0','#81D4FA','#F48FB1','#CE93D8','#FFCC80','#FF8A65','#80CBC4','#E0E0E0','#B0BEC5'];
const _MEMO_FORE_COLORS=['#E53935','#1E88E5','#43A047','#FB8C00','#8E24AA','#00897B','#D81B60','#3949AB','#333333','#757575'];
(function _initMemoPalettes(){
  function _dots(colors,cmd,panelId){
    return colors.map(c=>`<div onmousedown="event.preventDefault();document.execCommand('${cmd}',false,'${c}');document.getElementById('${panelId}').style.display='none'" style="width:22px;height:22px;border-radius:50%;background:${c};cursor:pointer;border:1.5px solid rgba(0,0,0,.12);"></div>`).join('')
     + (cmd==='hiliteColor'?`<div onmousedown="event.preventDefault();document.execCommand('removeFormat');document.getElementById('${panelId}').style.display='none'" title="제거" style="width:22px;height:22px;border-radius:50%;background:#fff;cursor:pointer;border:1.5px dashed #bbb;font-size:10px;display:flex;align-items:center;justify-content:center;color:#999;">✕</div>`:'');
  }
  setTimeout(()=>{
    const h=document.getElementById('memoPaletteHilite');
    const f=document.getElementById('memoPaletteFore');
    if(h) h.innerHTML=_dots(_MEMO_HILITE_COLORS,'hiliteColor','memoPaletteHilite');
    if(f) f.innerHTML=_dots(_MEMO_FORE_COLORS,'foreColor','memoPaletteFore');
  },0);
})();
function _toggleMemoPalette(type){
  const hEl=document.getElementById('memoPaletteHilite');
  const fEl=document.getElementById('memoPaletteFore');
  if(type==='hilite'){fEl.style.display='none';hEl.style.display=hEl.style.display==='flex'?'none':'flex';}
  else{hEl.style.display='none';fEl.style.display=fEl.style.display==='flex'?'none':'flex';}
}
...
function _renderMemoContentChips() {
  ...
  el.innerHTML = _contentPresets.map(c =>
    `<span class="memo-chip" onclick="document.getElementById('memoContentInput').innerHTML=unescape('${escape(c)}')">${c.slice(0,14)}</span>`
  ).join('');
}
...
function saveMemoInput() {
  ...
  let content = document.getElementById('memoContentInput')?.innerHTML?.trim() || '';
  if(content.replace(/<br\s*\/?>/gi,'').replace(/&nbsp;/gi,'').replace(/<[^>]*>/g,'').trim()==='') content='';
  ...
  if (_memoEditId) {
    ...
    if (m) {
      ...
      allSpans.forEach(sp => {
        sp.style.background = _hexA(color, .38);
        sp.style.borderBottomColor = color;
        sp.title = (markerName || '메모') + (content ? ' — ' + content.slice(0,30) : '');
        sp.dataset.memoMarker = markerName;
        sp.dataset.memoContent = content;
      });
    }
  ...
  try {
    ...
    const spans = [];
    _memoInsertingNow = true; /* Observer 비활성화 */
    ...
    textNodes.forEach((tn, i) => {
      ...
      sp.dataset.memoMarker = markerName;
      sp.dataset.memoContent = content;
      sp.dataset.memoDate = dateStr;
      sp.dataset.memoResolved = 'false';
...
function _renderMemoPanelList() {
  ...
  el.innerHTML = _memos.map(m =>
    `<div class="memo-card${m.resolved?' memo-resolved':''}" id="mc_${m.id}" style="border-left-color:${m.color};" onclick="_focusMemo('${m.id}')">
       ...
       <div class="memo-card-quote">"${(m.text||'')}"</div>
       ${m.content ? `<div class="memo-card-content">${m.content}</div>` : ''}
       <div class="memo-card-btns">
         ${_reViewMode?'':`<button class="btn-done" onclick="event.stopPropagation();toggleMemoDone('${m.id}')">✓</button>`}
         ${IS_SHARE||_reViewMode?'':`<button onclick="event.stopPropagation();editMemo('${m.id}')">수정</button>`}
         ${IS_SHARE||_reViewMode?'':`<button class="btn-del" onclick="event.stopPropagation();deleteMemo('${m.id}')">삭제</button>`}
       </div>
     </div>`
  ).join('');
}
...
function toggleMemoDone(id) {
  ...
  const allSpans = m.spanEls || (m.spanEl ? [m.spanEl] : []);
  allSpans.forEach(sp => {
    sp.style.opacity = m.resolved ? '0.55' : '1';
    sp.dataset.memoResolved = m.resolved ? 'true' : 'false';
  });
  refreshMemoPanel();
}
...
// HTML escape 헬퍼 (제목/카드명에 특수문자 들어가도 안전)
function _prEsc(s){ s=String(s==null?'':s); return s.replace(/[&<>"']/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }

// 카드 상세 정보 반환: {name, exists(현재 존재 여부), fromBackup(백업에만 있음)}
function _getCardInfo(cardId){
  // 1) 현재 CARDS에서 찾기
  for(const key of Object.keys(CARDS)){
    const found = (CARDS[key]||[]).find(c=>c.id===cardId);
    if(found) return {name: found.name, exists: true, fromBackup: false};
  }
  // 2) 백업의 hub_cards에서 찾기
  if(_partialBackupData && _partialBackupData.hub_cards){
    const backupCards = _partialBackupData.hub_cards;
    for(const key of Object.keys(backupCards)){
      const list = backupCards[key];
      if(Array.isArray(list)){
        const found = list.find(c=>c.id===cardId);
        if(found) return {name: found.name, exists: false, fromBackup: true};
      }
    }
  }
  return {name: cardId, exists: false, fromBackup: false};
}

// 기존 함수 호환 유지 (다른 곳에서 쓸 가능성 대비)
function _getCardName(cardId){
  const info = _getCardInfo(cardId);
  if(info.exists) return info.name;
  if(info.fromBackup) return info.name + ' (삭제된 카드 — 복원 시 함께 복구됨)';
  return cardId;
}

async function openPartialRestore(event){
  const file = event.target.files[0];
  event.target.value = '';
  if(!file) return;
  if(!file.name.endsWith('.json')){ toast('JSON 파일만 선택할 수 있습니다.'); return; }
  ...
  try{
    ...
    if(!obj.data || typeof obj.data !== 'object'){ toast('올바른 백업 파일이 아닙니다.'); return; }
    _partialBackupData = obj.data;
    const cdocs = _partialBackupData.hub_cdocs;
    if(!cdocs || typeof cdocs !== 'object'){
      toast('백업 파일에 문서 목록(hub_cdocs)이 없습니다.');
      return;
    }
    renderPartialRestoreList();
    document.getElementById('partialRestoreOverlay').classList.add('open');
  }catch(err){
    alert('부분 복원 오류:\n' + err.message + '\n\n' + err.stack);
    toast('파일 분석 실패: '+err.message);
  }
}

function closePartialRestore(){
  document.getElementById('partialRestoreOverlay').classList.remove('open');
  _partialBackupData = null;
}

function renderPartialRestoreList(){
  const container = document.getElementById('partialRestoreList');
  container.innerHTML = '';
  const summaryEl = document.getElementById('partialRestoreSummary');
  if(summaryEl) summaryEl.innerHTML = '';
  if(!_partialBackupData || !_partialBackupData.hub_cdocs) return;

  const newOnlyEl = document.getElementById('partialRestoreNewOnly');
  const newOnly = !!(newOnlyEl && newOnlyEl.checked);
  const cdocs = _partialBackupData.hub_cdocs;

  // ── 1단계: 통계 집계 (필터와 무관, 백업 전체 기준) ──
  let totalDocs = 0, newDocs = 0, existDocs = 0, missingContent = 0;
  let totalCards = 0, deletedCards = 0;
  for(const cardId of Object.keys(cdocs)){
    const docList = cdocs[cardId];
    if(!Array.isArray(docList)) continue;
    const docs = docList.filter(d=>d.type!=='folder');
    if(docs.length === 0) continue;
    totalCards++;
    const cardInfo = _getCardInfo(cardId);
    if(!cardInfo.exists) deletedCards++;
    const currentList = CARD_DOCS[cardId] || [];
    docs.forEach(doc=>{
      totalDocs++;
      const docExists = currentList.some(d=>d.id===doc.id);
      const hasContent = _partialBackupData['hub_doc_'+doc.id] !== undefined || _partialBackupData['hub_doc__'+doc.id] !== undefined;
      if(docExists) existDocs++; else newDocs++;
  ...
  if(summaryEl){
    summaryEl.innerHTML =
      '<b>백업 파일 분석</b><br>' +
      '📦 전체 문서 <b>' + totalDocs + '</b>개 ' +
      '<span style="color:#16a34a;">· 🆕 신규 <b>' + newDocs + '</b>개</span> ' +
      '<span style="color:var(--t2);">· ✅ 이미 있음 <b>' + existDocs + '</b>개</span>' +
      (missingContent>0 ? ' <span style="color:#e94560;">· ⚠ 본문 없음 <b>' + missingContent + '</b>개</span>' : '') +
      '<br>📁 카드 <b>' + totalCards + '</b>개' +
      (deletedCards>0 ? ' <span style="color:#e94560;">(현재 삭제된 카드 ' + deletedCards + '개 — 복원 시 카드 자체도 복구됨)</span>' : '');
  }
  ...
  for(const cardId of Object.keys(cdocs)){
    ...
    if(!Array.isArray(docList)) continue;
    const docs = docList.filter(d=>d.type!=='folder');
    const folders = docList.filter(d=>d.type==='folder');
    if(docs.length === 0) continue;

    const cardInfo = _getCardInfo(cardId);
    const currentList = CARD_DOCS[cardId] || [];

    // "신규만 표시" 모드: 카드 안에 신규 문서가 하나도 없으면 카드 자체 숨김
    if(newOnly){
      const anyNew = docs.some(d=> !currentList.some(cd=>cd.id===d.id));
      if(!anyNew) continue;
    }

    // 카드별 카운트
    let cardNewCount = 0, cardExistCount = 0;
    docs.forEach(d=>{
      if(currentList.some(cd=>cd.id===d.id)) cardExistCount++; else cardNewCount++;
    });

    // 카드 헤더
    const header = document.createElement('div');
    header.style.cssText = 'font-size:.82rem;font-weight:700;margin:14px 0 6px;padding-bottom:5px;border-bottom:1px solid var(--bd);color:var(--t1);display:flex;align-items:center;gap:8px;flex-wrap:wrap;';
    let badgeHtml;
    if(cardInfo.exists){
      badgeHtml = '<span style="font-size:.68rem;font-weight:600;background:#dcfce7;color:#166534;padding:2px 8px;border-radius:10px;">✅ 현재 카드 존재</span>';
    } else if(cardInfo.fromBackup){
      badgeHtml = '<span style="font-size:.68rem;font-weight:600;background:#fee2e2;color:#991b1b;padding:2px 8px;border-radius:10px;">⚠ 삭제된 카드 — 복원 시 함께 복구</span>';
    } else {
      badgeHtml = '<span style="font-size:.68rem;font-weight:600;background:#f3f4f6;color:#6b7280;padding:2px 8px;border-radius:10px;">? 카드 정보 없음</span>';
    }
    const countHtml = '<span style="font-size:.7rem;font-weight:500;color:var(--t2);margin-left:auto;">신규 <b style="color:#16a34a;">' + cardNewCount + '</b> / 이미 있음 <b>' + cardExistCount + '</b> / 백업 ' + docs.length + '</span>';
    header.innerHTML = '<span>📁 ' + _prEsc(cardInfo.name) + '</span>' + badgeHtml + countHtml;
    container.appendChild(header);

    // 폴더 맵
    const folderMap = {};
    folders.forEach(f=>{ folderMap[f.id] = f.title; });

    // 문서 행
    docs.forEach(doc=>{
      const docExists = currentList.some(d=>d.id===doc.id);
      if(newOnly && docExists) return; // "신규만 표시"

      const hasContent = _partialBackupData['hub_doc_'+doc.id] !== undefined || _partialBackupData['hub_doc__'+doc.id] !== undefined;
      const row = document.createElement('label');
      row.style.cssText = 'display:flex;align-items:center;gap:8px;padding:5px 4px;cursor:pointer;font-size:.8rem;border-radius:4px;';
      row.onmouseenter = function(){ this.style.background='var(--bg2,#f5f5f5)'; };
      row.onmouseleave = function(){ this.style.background=''; };

      const cb = document.createElement('input');
      cb.type = 'checkbox';
      cb.className = 'partial-restore-cb';
      cb.dataset.cardId = cardId;
      cb.dataset.docId = doc.id;
      cb.dataset.docExists = docExists ? '1' : '0';
      cb.dataset.hasContent = hasContent ? '1' : '0';
      cb.style.cssText = 'width:16px;height:16px;flex-shrink:0;';
      cb.onchange = updatePartialCount;
      // 기본 체크: 신규+본문 있음 → 자동 체크 / 이미 있음 → 미체크 (실수 덮어쓰기 방지) / 본문 없음 → 비활성
      if(!hasContent){ cb.disabled = true; cb.checked = false; }
      else if(!docExists){ cb.checked = true; }
      else { cb.checked = false; }

      const label = document.createElement('span');
      const folderLabel = doc.parentId && folderMap[doc.parentId] ? '<span style="color:var(--t3);font-size:.72rem;">' + _prEsc(folderMap[doc.parentId]) + ' / </span>' : '';
      const typeIcon = doc.type==='richtext' ? '📝' : doc.type==='html' ? '🌐' : doc.type==='refdoc' ? '🖼' : doc.type==='calendar' ? '🏷️' : doc.type==='sched' ? '📘' : '📄';
      label.innerHTML = typeIcon + ' ' + folderLabel + _prEsc(doc.title || '');
      label.style.cssText = 'flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';

      // 우측 상태 배지 영역
      const statusWrap = document.createElement('span');
      statusWrap.style.cssText = 'display:flex;align-items:center;gap:4px;flex-shrink:0;';
      if(docExists){
        const exBadge = document.createElement('span');
        exBadge.style.cssText = 'font-size:.66rem;font-weight:600;background:#f3f4f6;color:#6b7280;padding:2px 7px;border-radius:8px;';
        exBadge.textContent = '✅ 이미 있음';
        exBadge.title = '현재 데이터에 같은 ID로 존재합니다. 체크해서 복원하면 본문이 백업으로 덮어씌워집니다.';
        statusWrap.appendChild(exBadge);
      } else {
        const newBadge = document.createElement('span');
        newBadge.style.cssText = 'font-size:.66rem;font-weight:600;background:#dcfce7;color:#166534;padding:2px 7px;border-radius:8px;';
        newBadge.textContent = '🆕 신규';
        newBadge.title = '현재 데이터에 없습니다. 복원 시 새로 추가됩니다.';
        statusWrap.appendChild(newBadge);
      }
      if(!hasContent){
        const noData = document.createElement('span');
        noData.textContent = '본문 없음';
        noData.style.cssText = 'font-size:.66rem;font-weight:600;background:#fee2e2;color:#991b1b;padding:2px 7px;border-radius:8px;';
        noData.title = '백업 파일에 이 문서의 본문 데이터가 없어 복원할 수 없습니다.';
        statusWrap.appendChild(noData);
      }
...
function updatePartialCount(){
  const cbs = document.querySelectorAll('.partial-restore-cb:checked');
  let nNew = 0, nOver = 0;
  cbs.forEach(cb=>{
    if(cb.dataset.docExists === '1') nOver++; else nNew++;
  });
  const el = document.getElementById('partialRestoreCount');
  if(!el) return;
  let txt = '선택: <b>' + cbs.length + '</b>개';
  if(cbs.length > 0){
    const parts = [];
    if(nNew > 0) parts.push('<span style="color:#16a34a;">🆕 신규 추가 ' + nNew + '개</span>');
    if(nOver > 0) parts.push('<span style="color:#e94560;">⚠ 본문 덮어쓰기 ' + nOver + '개</span>');
    txt += ' (' + parts.join(' · ') + ')';
  }
  el.innerHTML = txt;
}

function partialRestoreSelectAll(checked){
  document.querySelectorAll('.partial-restore-cb').forEach(cb=>{
...
// 신규(현재 없는 것)만 체크
function partialRestoreSelectNew(){
  document.querySelectorAll('.partial-restore-cb').forEach(cb=>{
    if(cb.disabled){ cb.checked = false; return; }
    cb.checked = (cb.dataset.docExists !== '1');
  });
  updatePartialCount();
}

async function executePartialRestore(){
  const cbs = document.querySelectorAll('.partial-restore-cb:checked');
  if(cbs.length === 0){ toast('복원할 문서를 선택해주세요.'); return; }

  // 신규 / 덮어쓰기 분리 집계
  let nNew = 0, nOver = 0;
  cbs.forEach(cb=>{ if(cb.dataset.docExists === '1') nOver++; else nNew++; });

  let confirmMsg = '선택한 ' + cbs.length + '개 문서를 복원합니다.\n\n';
  if(nNew > 0)  confirmMsg += '🆕 신규 추가: ' + nNew + '개 (현재 없는 문서를 새로 추가)\n';
  if(nOver > 0) confirmMsg += '⚠ 본문 덮어쓰기: ' + nOver + '개 (현재 본문이 백업 내용으로 대체됩니다)\n';
  confirmMsg += '\n나머지 데이터(다른 문서, 카드, 일정, 태그 등)는 그대로 유지됩니다.\n\n진행하시겠습니까?';
  if(!confirm(confirmMsg)) return;

  toast('부분 복원 중...');
  const cdocs = _partialBackupData.hub_cdocs;
  let docCount = 0;
  let recoveredCardCount = 0;

  try{
    // Group selected docs by card
    const selectedByCard = {};
    cbs.forEach(cb=>{
      const cardId = cb.dataset.cardId;
      const docId = cb.dataset.docId;
      if(!selectedByCard[cardId]) selectedByCard[cardId] = [];
      selectedByCard[cardId].push(docId);
    });

    // ★ 삭제된 카드 자동 복구 — 복원 대상 카드가 현재 CARDS에 없으면 백업에서 카드(및 필요시 탭)를 되살림
    const backupCards = (_partialBackupData && _partialBackupData.hub_cards) || {};
    const backupTabs  = (_partialBackupData && _partialBackupData.hub_tabs)  || {};
    let cardsChanged = false;
    let tabsChanged  = false;
    for(const cardId of Object.keys(selectedByCard)){
      // 현재 CARDS에 이미 있는지 확인
      let exists = false;
      for(const key of Object.keys(CARDS)){
        if((CARDS[key]||[]).some(c=>c.id===cardId)){ exists = true; break; }
      }
      if(exists) continue;

      // 백업에서 이 카드가 어느 (g-tabId) 키에 있었는지 찾기
      let foundCard = null, foundKey = null;
      for(const key of Object.keys(backupCards)){
        const list = backupCards[key];
        if(!Array.isArray(list)) continue;
        const c = list.find(x=>x.id===cardId);
        if(c){ foundCard = c; foundKey = key; break; }
      }
      if(!foundCard || !foundKey) continue; // 백업에도 카드 정보가 없으면 스킵 (문서만 복원)

      // 키에서 g(섹션)와 tabId 분리 (예: "docs-자료" → g="docs", tabId="자료")
      const sepIdx = foundKey.indexOf('-');
      if(sepIdx < 0) continue;
      const g = foundKey.slice(0, sepIdx);
      const tabId = foundKey.slice(sepIdx+1);

      // 탭이 없으면 백업에서 탭도 복구
      if(!Array.isArray(TABS[g])) TABS[g] = [];
      if(!TABS[g].some(t=>t.id===tabId)){
        const backupTabList = backupTabs[g];
        let tabObj = null;
        if(Array.isArray(backupTabList)) tabObj = backupTabList.find(t=>t.id===tabId);
        TABS[g].push(tabObj || {id:tabId, name:tabId});
        tabsChanged = true;
      }

      // 카드 복구
      if(!Array.isArray(CARDS[foundKey])) CARDS[foundKey] = [];
      CARDS[foundKey].push(foundCard);
      cardsChanged = true;
      recoveredCardCount++;
    }

    // For each card, merge selected docs + their parent folders into CARD_DOCS
    for(const cardId of Object.keys(selectedByCard)){
      const selectedIds = new Set(selectedByCard[cardId]);
      const backupList = cdocs[cardId] || [];
      const currentList = CARD_DOCS[cardId] || [];
      const existingIds = new Set(currentList.map(d=>d.id));

      // Collect needed folder IDs
      const neededFolderIds = new Set();
      backupList.forEach(d=>{
        if(selectedIds.has(d.id) && d.parentId){
          neededFolderIds.add(d.parentId);
        }
      });

      // Add folders first, then docs
      backupList.forEach(d=>{
        if(existingIds.has(d.id)) return;
        if(d.type==='folder' && neededFolderIds.has(d.id)){
          currentList.push(d);
      ...
      CARD_DOCS[cardId] = currentList;
    }

    // Save updated CARD_DOCS
    ss('hub_cdocs', CARD_DOCS);
    await _sbSet('hub_cdocs', CARD_DOCS);

    // ★ 카드/탭이 복구되었으면 함께 저장
    if(cardsChanged){
      ss('hub_cards', CARDS);
      await _sbSet('hub_cards', CARDS);
    }
    if(tabsChanged){
      ss('hub_tabs', TABS);
      await _sbSet('hub_tabs', TABS);
    }

    // Upsert each selected doc content
    for(const cb of cbs){
      const docId = cb.dataset.docId;
      // Try both key formats (hub_doc_ and hub_doc__)
      const content = _partialBackupData['hub_doc__'+docId] !== undefined
        ? _partialBackupData['hub_doc__'+docId]
        : _partialBackupData['hub_doc_'+docId];
      if(content !== undefined){
        _localSave('hub_doc__'+docId, content);
        await _sbRest.upsert('site_data', {key:'hub_doc__'+docId, value:content, updated_at:new Date().toISOString()});
        docCount++;
      }
    }
    ...
    // Refresh UI
    renderSection('docs');
    renderSection('webtoon');
    closePartialRestore();
    closeSiteSettings();
    const recoveryMsg = recoveredCardCount > 0 ? ' (삭제된 카드 ' + recoveredCardCount + '개도 함께 복구됨)' : '';
    toast('부분 복원 완료 — ' + docCount + '개 문서가 복원되었습니다.' + recoveryMsg);
  }catch(e){
    toast('부분 복원 실패: ' + ((e&&e.message)||'알 수 없는 오류'));
    console.error('executePartialRestore error:', e);
  }
}
...
function buildTreeHTML(docs,parentId,depth){
  ...
  items.forEach(item=>{
    ...
    if(item.type==='folder'){
      ...
      const clickFn = item.type==='richtext' ? `openRichViewer('${item.id}')` : (item.type==='calendar' ? `openCalendarDoc('${item.id}')` : (item.type==='sched' ? `openSchedDoc('${item.id}')` : `openDocViewer('${item.id}')`));
      ...
      html+=`<div class="tree-doc-item" data-idx="${globalIdx}"
        ...
        <div class="tree-doc-actions">
          ...
          <button class="doc-act-btn" onclick="event.stopPropagation();editDoc('${item.id}')" title="편집">✎</button>
...
function deleteDoc(docId){
  ...
  openConfirmDel(`"${d?.title||'문서'}"를 삭제하시겠습니까?`, () => {
    ...
    // 일정 문서면 번들 키도 삭제
    if(d && d.type==='sched'){
      _sbRest.delete('site_data','hub_sched_doc_'+docId).catch(function(){});
    }
...
function deleteFolder(folderId){
  ...
  openConfirmDel(`"${f?.title||'폴더'}" 폴더와 포함된 모든 항목을 삭제하시겠습니까?`, () => {
    ...
    collect(folderId);
    // 삭제 대상 문서 객체를 미리 수집 (type 판단용)
    const delDocs = docs.filter(x=>toDelete.has(x.id));
    ...
    delDocs.forEach(x=>{
      if(x && x.type==='sched'){
        _sbRest.delete('site_data','hub_sched_doc_'+x.id).catch(function(){});
      }
    });
    saveAll();renderDocList();toast('삭제되었습니다.');
  });
...
async function openDocViewer(docId){
  ...
  // 캘린더 타입: 리다이렉트
  if(d.type==='calendar'){openCalendarDoc(docId);return;}
  // 일정 문서 타입: 리다이렉트
  if(d.type==='sched'){openSchedDoc(docId);return;}
...
function editDoc(docId){
  ...
  else if(d.type==='calendar')openCalDocRenameModal(docId);
  else if(d.type==='sched')openSchedDocRenameModal(docId);
...
function _applyReMode(){
  ...
  if(_reViewMode){
    ...
    {const _bh=document.querySelector('.re-blk-handle');if(_bh)_bh.style.display='none';}
...
async function openRichEditor(docId=''){
  ...
  _applyReMode();
  // 이벤트 바인딩 (한 번만)
  setTimeout(()=>{ _bindCtxMenu(); _bindTableCtxMenu(); _bindMemoSync(editor); _bindTblResize(editor); _initTblDrag(editor); _bindBlockDrag(editor); }, 200);
  ...
  if(docId){
    ...
    // PC: 패널 자동 표시 / 모바일: 패널 숨김 디폴트
    const _isMobileDevice = window.innerWidth <= 768;
    if (!_isMobileDevice && !_rePanelsOn) {
      reTogglePanels();
    } else if (_isMobileDevice && _rePanelsOn) {
      reTogglePanels();
    }
    // 편집 모드일 때만 자동저장 활성화
...
function _reScanMemoSpans(editor){
  ...
  Object.entries(groups).forEach(([id, spans]) => {
    const span = spans[0];
    ...
    const markerName = span.dataset.memoMarker ?? (span.title?.split(' — ')[0]||'메모');
    const content = span.dataset.memoContent ?? (span.title?.split(' — ')[1]||'');
    const date = span.dataset.memoDate || '';
    const resolved = span.dataset.memoResolved === 'true' || parseFloat(span.style.opacity||'1')<0.8;
    _memos.push({ id, text, color, markerName, content, resolved, date, spanEl:span, spanEls:spans });
  });
  refreshMemoPanel();
}
...
function _bindMemoSync(editor){
  ...
  /* ── 분할/복제 span을 기존 메모에 병합하는 공용 함수 ── */
  function _syncMemoSpans(){
    if(_memoInsertingNow)return; /* 삽입 중에는 동기화 건너뛰기 */
    const allSpans=editor.querySelectorAll('.re-memo-mark');
    const knownSpans=new Set();
    _memos.forEach(m=>{
      if(m.spanEls)m.spanEls.forEach(s=>knownSpans.add(s));
      if(m.spanEl)knownSpans.add(m.spanEl);
    });

    let changed=false;
    allSpans.forEach(span=>{
      if(knownSpans.has(span))return;
      const oid=span.dataset.memoId;
      const existing=_memos.find(m=>m.id===oid);
      if(existing){
        /* 같은 ID → 브라우저가 쪼갠 조각 → 기존 메모에 병합 */
        if(!existing.spanEls)existing.spanEls=existing.spanEl?[existing.spanEl]:[];
        if(!existing.spanEls.includes(span)){
          existing.spanEls.push(span);
          existing.text=existing.spanEls.map(s=>s.textContent).join('');
        }
        span.onclick=e=>{e.stopPropagation();_focusMemo(oid);};
        changed=true;
      } else {
        /* 완전히 새 span (붙여넣기 복제) → 새 메모 생성 */
        ...
        const orig=oid?_memos.find(m=>m.id===oid):null;
        _memos.push({
          ...
          spanEl:span, spanEls:[span]
        });
        changed=true;
      }
    });

    /* 고아 메모 제거 (DOM에서 사라진 span) */
    const livingSpans=new Set(allSpans);
    _memos=_memos.filter(m=>{
      if(m.spanEls&&m.spanEls.length){
        m.spanEls=m.spanEls.filter(s=>livingSpans.has(s));
        if(m.spanEls.length){m.spanEl=m.spanEls[0];return true;}
      } else if(m.spanEl&&livingSpans.has(m.spanEl)){return true;}
  ...
  // 붙여넣기 후 동기화
  editor.addEventListener('paste',()=>{ setTimeout(_syncMemoSpans,100); });

  // MutationObserver: span 분할/삭제 감지
  if(_reMemoObserver)_reMemoObserver.disconnect();
  _reMemoObserver=new MutationObserver(()=>{ _syncMemoSpans(); });
...
function reExec(cmd,val=null){
  const editor=document.getElementById('reEditor');
  // 콜아웃 내부 등 nested contenteditable에서 포커스를 빼앗지 않기
  const sel=window.getSelection();
  const activeEl=document.activeElement;
  const isInsideEditor=editor.contains(activeEl)||editor.contains(sel?.anchorNode);
  if(!isInsideEditor) editor.focus();
...
/* ══ 블록 드래그-재배치 (PC 전용) ══
   - 편집 모드에서 문단 위에 마우스를 올리면 왼쪽에 ⋮⋮ 손잡이가 뜸
   - 손잡이를 끌어 최상위 블록(본문/제목/목록/콜아웃/표/이미지 등)의 순서를 바꿈
   - 옮기는 순간 메모 감시(Observer)를 잠시 멈춰 메모 꼬임 방지
   - 옮긴 뒤 자동저장으로 순서 보존
   - 뷰어/공유 모드에서는 동작하지 않음 */
let _blkHandle = null;     // 떠다니는 손잡이
let _blkDropLine = null;   // 드롭 위치 표시선
let _blkDragEl = null;     // 현재 끌고 있는 블록

function _blkTopBlocks(editor){
  // 에디터의 최상위 '요소' 자식만 블록으로 취급 (떠도는 텍스트 노드 제외)
  return Array.from(editor.children).filter(el=>el.nodeType===1 && el.tagName!=='BR');
}
function _blkBlockUnderY(editor, clientY){
  const blocks=_blkTopBlocks(editor);
  for(const b of blocks){
    const r=b.getBoundingClientRect();
    if(clientY>=r.top-2 && clientY<=r.bottom+2) return b;
  }
  return null;
}

function _bindBlockDrag(editor){
  if(editor._blkDragBound) return;
  editor._blkDragBound = true;
  const scroll = document.getElementById('reEditorScroll');
  if(!scroll) return;

  if(!_blkHandle){
    _blkHandle = document.createElement('div');
    _blkHandle.className = 're-blk-handle';
    _blkHandle.title = '드래그하여 이동';
    _blkHandle.textContent = '⋮⋮';
    _blkHandle.style.display = 'none';
    scroll.appendChild(_blkHandle);
  }
  if(!_blkDropLine){
    _blkDropLine = document.createElement('div');
    _blkDropLine.className = 're-blk-dropline';
    _blkDropLine.style.display = 'none';
    scroll.appendChild(_blkDropLine);
  }

  let _hoverBlock = null;
  let _dropTarget = null;
  let _dropAfter = false;

  function _positionHandle(block){
    if(!block || _reViewMode){ _blkHandle.style.display='none'; _hoverBlock=null; return; }
    _hoverBlock = block;
    const sr = scroll.getBoundingClientRect();
    const br = block.getBoundingClientRect();
    const top  = br.top  - sr.top  + scroll.scrollTop + 1;
    const left = br.left - sr.left - 22;
    _blkHandle.style.top  = top + 'px';
    _blkHandle.style.left = Math.max(2, left) + 'px';
    _blkHandle.style.display = 'flex';
  }

  // 블록 위에 마우스 → 손잡이 위치 갱신 (드래그 중에는 건너뜀)
  editor.addEventListener('mousemove', e=>{
    if(_blkDragEl || _reViewMode) return;
    const b = _blkBlockUnderY(editor, e.clientY);
    if(b) _positionHandle(b);
  });
  // 에디터 영역을 벗어나면 손잡이 숨김 (드래그 중에는 유지)
  scroll.addEventListener('mouseleave', ()=>{ if(!_blkDragEl) _blkHandle.style.display='none'; });

  function _onDragMove(e){
    if(!_blkDragEl) return;
    e.preventDefault();
    // 가장자리 근처에서 자동 스크롤
    const sr = scroll.getBoundingClientRect();
    if(e.clientY < sr.top + 40) scroll.scrollTop -= 14;
    else if(e.clientY > sr.bottom - 40) scroll.scrollTop += 14;

    const blocks = _blkTopBlocks(editor).filter(b=>b!==_blkDragEl);
    let target=null, after=false;
    for(const b of blocks){
      const r = b.getBoundingClientRect();
      const mid = r.top + r.height/2;
      if(e.clientY < mid){ target=b; after=false; break; }
      target=b; after=true;
    }
    _dropTarget=target; _dropAfter=after;
    if(target){
      const sr2 = scroll.getBoundingClientRect();
      const r = target.getBoundingClientRect();
      const y = (after ? r.bottom : r.top) - sr2.top + scroll.scrollTop;
      const x = r.left - sr2.left;
      _blkDropLine.style.top = y + 'px';
      _blkDropLine.style.left = x + 'px';
      _blkDropLine.style.width = r.width + 'px';
      _blkDropLine.style.display = 'block';
    } else {
      _blkDropLine.style.display = 'none';
    }
  }
  ...
  function _onDragUp(){
    ...
    document.body.style.userSelect = '';
    _blkDropLine.style.display = 'none';
    const dragEl = _blkDragEl;
    _blkDragEl = null;
    if(dragEl) dragEl.classList.remove('re-blk-dragging');
    const target = _dropTarget; const after = _dropAfter;
    _dropTarget = null;
    if(!dragEl || !target || target===dragEl) return;

    // 메모 감시를 잠시 멈추고 DOM 이동 (메모 꼬임 방지)
    const prevGuard = _memoInsertingNow;
    _memoInsertingNow = true;
    try{
      if(after) target.after(dragEl);
      else      target.before(dragEl);
    }catch(err){ console.warn('블록 이동 실패:', err); }
    setTimeout(()=>{ _memoInsertingNow = prevGuard; }, 80);

    // 순서 변경 → 미저장 표시 + 자동저장
    _reDirty = true;
    if(typeof _asSchedule==='function' && _asActive && _reEditDocId) _asSchedule();
  }

  // 손잡이를 눌러 드래그 시작
  _blkHandle.addEventListener('mousedown', e=>{
    if(_reViewMode || !_hoverBlock) return;
    e.preventDefault();
    _blkDragEl = _hoverBlock;
    _blkDragEl.classList.add('re-blk-dragging');
    document.body.style.userSelect = 'none';
    ...
    <div class="pdf-section">
      ...
      ${_memos.map((m, i) =>
        `<div class="pdf-memo">
          ...
          <div class="pdf-memo-quote">"${(m.text||'')}"</div>
...
.re-callout-icon{font-size:1.3rem;flex-shrink:0;line-height:1.7;}
.re-callout-body{flex:1;line-height:1.7;min-height:1.3em;}
...
let _calloutNoBg = false; // 배경색 없음 토글
...
function openCalloutPop(editTarget) {
  ...
  if (editTarget) {
    ...
    // 배경 투명 여부 감지
    const bg = editTarget.style.background || editTarget.style.backgroundColor || '';
    _calloutNoBg = (bg === 'transparent' || bg === '');
  } else {
    _reSaveSelection();
    _calloutNoBg = false;
  }
...
function _renderCalloutColors() {
  ...
  if (!grid) return;
  // 투명(없음) 옵션 추가 - 테두리 색은 유지, 배경만 투명
  let html = `<div class="re-cp-dot${_calloutNoBg?' sel':''}" style="background:#fff;border:2px dashed #CCC;position:relative;" onclick="pickCalloutColor(this,'transparent')" title="배경색 없음"><span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:10px;color:#999;pointer-events:none;">✕</span></div>`;
  html += RE_COLORS_30.map(c =>
    `<div class="re-cp-dot${c===_calloutSelColor&&!_calloutNoBg?' sel':''}" style="background:${c}" onclick="pickCalloutColor(this,'${c}')"></div>`
  ).join('');
  grid.innerHTML = html;
}
function pickCalloutColor(el, color) {
  if(color==='transparent'){
    _calloutNoBg = !_calloutNoBg; // 토글
  } else {
    _calloutSelColor = color;
    _calloutNoBg = false;
  }
  _renderCalloutColors(); // 선택 상태 갱신
  _renderCalloutPreview();
}
function _renderCalloutPreview() {
  const el = document.getElementById('calloutPreview');
  if (!el) return;
  const bgColor = _calloutNoBg ? 'transparent' : _hexA(_calloutSelColor, 0.08);
  const borderColor = _calloutSelColor;
  ...
  const iconHtml = isImg
    ...
    : _calloutSelEmoji;
  el.innerHTML = `<div style="border-left:4px solid ${borderColor};background:${bgColor};border-radius:0 10px 10px 0;padding:14px 18px;display:flex;gap:10px;align-items:flex-start;line-height:1.7;">
...
function execCalloutInsert() {
  ...
  if (_calloutEditTarget) {
    // 수정 모드: 기존 콜아웃 업데이트
    _calloutEditTarget.style.borderLeftColor = borderColor;
  ...
  const editor = document.getElementById('reEditor');
  editor.focus();
  const html = `<div class="re-callout" contenteditable="false" style="border-left-color:${borderColor};background:${bgColor};">
...
function reOnKeydown(e){
  ...
  if(e.ctrlKey||e.metaKey){
    // Ctrl+B → 굵게
    if(!e.shiftKey && e.key==='b'){e.preventDefault();document.execCommand('bold',false,null);reUpdateToolbar();return;}
    // Ctrl+I → 기울임 (브라우저 네이티브도 지원하지만 명시 처리)
    if(!e.shiftKey && e.key==='i'){e.preventDefault();document.execCommand('italic',false,null);reUpdateToolbar();return;}
    // Ctrl+U → 밑줄
    if(!e.shiftKey && e.key==='u'){e.preventDefault();document.execCommand('underline',false,null);reUpdateToolbar();return;}
    // Ctrl+E → 인라인 코드
    if(e.key==='e'){
      e.preventDefault();
      const sel=window.getSelection();
      if(sel&&sel.rangeCount>0&&!sel.isCollapsed){
        ...
        reExec('insertHTML',`<code style="background:#F0F2F5;padding:1px 5px;border-radius:4px;font-size:.9em;font-family:monospace;">${txt}</code>`);
      }
      return;
    }
    // Ctrl+K → 링크 삽입
    if(e.key==='k'){e.preventDefault();reInsertLink();return;}
    // Ctrl+D → 현재 블록 복제
    if(e.key==='d'){
      e.preventDefault();
      const sel2=window.getSelection();
      const node=sel2?.anchorNode;
      const block=(node?.nodeType===3?node.parentElement:node)?.closest?.('p,h1,h2,h3,h4,h5,h6,li,pre,blockquote,div.re-callout');
      if(block){
    ...
    if(e.shiftKey){
      ...
      // Ctrl+Shift+S → 취소선
      if(k==='S'){e.preventDefault();reExec('strikeThrough');return;}
      // Ctrl+Shift+H → 하이라이트(현재 선택된 배경색)
      if(k==='H'){
        e.preventDefault();
        const bar=document.getElementById('reBgColorBar');
        const curBg=bar?bar.style.background||'#FFEB3B':'#FFEB3B';
        reExec('hiliteColor',curBg);
        return;
      }
      // Ctrl+Shift+0 → 본문(p)
      if(e.code==='Digit0'){e.preventDefault();reSetBlock('p');return;}
      // Ctrl+Shift+1 → H1
      if(e.code==='Digit1'){e.preventDefault();reSetBlock('h1');return;}
      // Ctrl+Shift+2 → H2
      if(e.code==='Digit2'){e.preventDefault();reSetBlock('h2');return;}
      // Ctrl+Shift+3 → H3
      if(e.code==='Digit3'){e.preventDefault();reSetBlock('h3');return;}
      // Ctrl+Shift+4 → 체크박스
      if(e.code==='Digit4'){e.preventDefault();reInsertCheckbox();return;}
      // Ctrl+Shift+5 → 불릿 목록
      if(e.code==='Digit5'){e.preventDefault();reExec('insertUnorderedList');return;}
      // Ctrl+Shift+6 → 번호 목록
      if(e.code==='Digit6'){e.preventDefault();reExec('insertOrderedList');return;}
      // Ctrl+Shift+8 → 코드블록
      if(e.code==='Digit8'){e.preventDefault();reSetBlock('pre');return;}
      // Ctrl+Shift+9 → 인용구
      if(e.code==='Digit9'){e.preventDefault();reSetBlock('blockquote');return;}
    }
  }
  if(e.key==='Tab'){
    e.preventDefault();
...
/* ══════════════════════════════════════════════════════════════════
   일정 문서 (독립 캘린더) 모듈
   - 메인 캘린더와 완전 독립: EVENTS/SPECIALS/PRESETS/COLORS/CFG/CAL_CFG/
     PROCESSES/ASSIGNEE_SETS/CAL_DISPLAY/CAL_FILTER_ORDER/작품·공정자 필터
     을 모두 별도로 보관
   - Supabase 키: hub_sched_doc_<docId>
   ══════════════════════════════════════════════════════════════════ */

/* 기본값 번들 생성 (새 일정 문서 or 로드 실패 시) */
function _schedDefaultBundle(){
  return {
    events: [],
    specials: [],
    presets: [],
    colors: JSON.parse(JSON.stringify(DEF_COLORS||{})),
    cfg: JSON.parse(JSON.stringify(DEF_CFG||{})),
    cal_cfg: {defaultView:'m',todayColor:'#2E9E50',holidayColor:'#E84060',defaultColor:'#D91F3E'},
    processes: [...DEF_PROCESSES_DEFAULT],
    assignee_sets: [],
    cal_display: {labelMode:'task'},
    cal_filter_order: [],
    card_docs_local: [], // 내부 작품/공정자 필터 캘린더 문서들
    cal_name: ''
  };
}

/* 현재 전역 변수들을 번들 JSON으로 직렬화 */
function _schedMakeBundle(){
  return {
    events: EVENTS,
    specials: SPECIALS,
    presets: PRESETS,
    ...
    cal_cfg: CAL_CFG,
    processes: PROCESSES,
    assignee_sets: ASSIGNEE_SETS,
    cal_display: CAL_DISPLAY,
    cal_filter_order: CAL_FILTER_ORDER,
    card_docs_local: (CARD_DOCS && CARD_DOCS['_schedLocal']) ? CARD_DOCS['_schedLocal'] : [],
    cal_name: CAL_NAME
  };
}

/* 메인 컨텍스트 백업 (일정 문서 진입 시) */
function _schedBackupCtx(){
  _savedCtxForSched = {
    EVENTS, SPECIALS, PRESETS, COLORS, CFG, CAL_CFG,
    PROCESSES, ASSIGNEE_SETS, CAL_DISPLAY, CAL_FILTER_ORDER,
    CARD_DOCS, CAL_NAME,
    _curCard,
    _activeCalDocId,
    _activeCalDocType,
    _loadedCounts: Object.assign({}, _loadedCounts),
    scrollY: window.scrollY,
    // 복귀 시 UI 상태 복원용
    prevCardDetailDisplay: (document.getElementById('cardDetailView')||{}).style ? document.getElementById('cardDetailView').style.display : '',
    prevMainSiteDisplay:   (document.getElementById('mainSite')||{}).style ? document.getElementById('mainSite').style.display : '',
  };
}

/* 번들을 전역 변수에 주입 */
function _schedApplyBundle(bundle){
  EVENTS = Array.isArray(bundle.events) ? bundle.events : [];
  SPECIALS = Array.isArray(bundle.specials) ? bundle.specials : [];
  PRESETS = Array.isArray(bundle.presets) ? bundle.presets : [];
  COLORS = bundle.colors && typeof bundle.colors === 'object' ? bundle.colors : JSON.parse(JSON.stringify(DEF_COLORS||{}));
  CFG = bundle.cfg && typeof bundle.cfg === 'object' ? bundle.cfg : JSON.parse(JSON.stringify(DEF_CFG||{}));
  CAL_CFG = bundle.cal_cfg && typeof bundle.cal_cfg === 'object' ? bundle.cal_cfg : {defaultView:'m',todayColor:'#2E9E50',holidayColor:'#E84060',defaultColor:'#D91F3E'};
  PROCESSES = Array.isArray(bundle.processes) ? bundle.processes : [...DEF_PROCESSES_DEFAULT];
  ASSIGNEE_SETS = Array.isArray(bundle.assignee_sets) ? bundle.assignee_sets : [];
  CAL_DISPLAY = bundle.cal_display && typeof bundle.cal_display === 'object' ? bundle.cal_display : {labelMode:'task'};
  CAL_FILTER_ORDER = Array.isArray(bundle.cal_filter_order) ? bundle.cal_filter_order : [];
  CARD_DOCS = { _schedLocal: Array.isArray(bundle.card_docs_local) ? bundle.card_docs_local : [] };
  CAL_NAME = bundle.cal_name || '';
  _curCard = '_schedLocal';
  _activeCalDocId = null;
  _activeCalDocType = null;
  // 빈 배열 덮어쓰기 방지는 이 모드에서 무의미 (어차피 번들 저장 경로)
  // 그래도 안전하게 현재 개수 기록
  _loadedCounts = {
    hub_events: EVENTS.length,
    hub_cdocs:  Object.keys(CARD_DOCS).length,
    hub_cards:  1,
    hub_tabs:   1,
  };
}

/* 메인 컨텍스트 복원 (일정 문서 종료 시) */
function _schedRestoreCtx(){
  if(!_savedCtxForSched) return;
  const s = _savedCtxForSched;
  EVENTS = s.EVENTS;
  SPECIALS = s.SPECIALS;
  PRESETS = s.PRESETS;
  COLORS = s.COLORS;
  ...
  ASSIGNEE_SETS = s.ASSIGNEE_SETS;
  CAL_DISPLAY = s.CAL_DISPLAY;
  CAL_FILTER_ORDER = s.CAL_FILTER_ORDER;
  CARD_DOCS = s.CARD_DOCS;
  CAL_NAME = s.CAL_NAME;
  _curCard = s._curCard;
  _activeCalDocId = s._activeCalDocId;
  _activeCalDocType = s._activeCalDocType;
  _loadedCounts = s._loadedCounts;
  _savedCtxForSched = null;
}

/* 디바운스 번들 저장 (saveAll이 이쪽으로 우회됨) */
function _scheduleSchedBundleSave(){
  if(_schedSaveTimer) clearTimeout(_schedSaveTimer);
  _setSbStatus('saving','저장 중...');
  _schedSaveTimer = setTimeout(_doSchedBundleSave, 400);
}
async function _doSchedBundleSave(){
  if(!_schedDocMode || !_schedDocId) return;
  if(!_dataLoaded) return;
  const bundle = _schedMakeBundle();
  try{
    await _sbRest.upsert('site_data', {
      key: 'hub_sched_doc_' + _schedDocId,
      value: bundle,
      updated_at: new Date().toISOString()
    });
    _setSbStatus('saved','서버 저장 완료');
    setTimeout(()=>{
      ...
      if(b&&b.classList.contains('saved')){
        ...
        if(t)t.textContent='저장됨';
      }
    },3000);
  }catch(e){
    console.warn('일정 문서 저장 실패:', (e&&e.message)||String(e));
    _setSbStatus('error','일정 문서 저장 실패');
    toast('일정 문서 저장 실패');
  }
}

/* ══ 일정 문서 생성 모달 ══ */
function openSchedDocMaker(){
  closeDocTypeModal();
  const inp=document.getElementById('sdmTitle');
  if(inp) inp.value='';
  document.getElementById('schedDocMakerOverlay').classList.add('open');
  setTimeout(()=>{ if(inp) inp.focus(); },180);
}
function closeSchedDocMaker(){
  document.getElementById('schedDocMakerOverlay').classList.remove('open');
}
function saveNewSchedDoc(){
  const title=(document.getElementById('sdmTitle').value||'').trim();
  if(!title){toast('이름을 입력해주세요.');return;}
  const id=uid();
  const docObj={id,title,type:'sched',parentId:_docTargetFolder||null};
  if(!CARD_DOCS[_curCard])CARD_DOCS[_curCard]=[];
  ...
  renderDocList();
  closeSchedDocMaker();
  toast('일정 문서 "'+title+'"가 생성되었습니다.');
}

/* ══ 일정 문서 열기 ══ */
async function openSchedDoc(docId){
  // 현재 카드의 CARD_DOCS에서 해당 문서 찾기 (title 확보 목적)
  let docTitle = '일정 문서';
  try{
    for(const[key,docs] of Object.entries(CARD_DOCS)){
      const d = (docs||[]).find(x=>x.id===docId);
      if(d){ docTitle = d.title || '일정 문서'; break; }
    }
  }catch(e){}
  ...
  let bundle = null;
  try{
    const row = await _sbRest.selectOne('site_data','hub_sched_doc_'+docId);
    if(row){
      const v = (typeof row === 'object' && row.value !== undefined) ? row.value : row;
      if(typeof v === 'string'){
        try{ bundle = JSON.parse(v); }catch(e){ bundle = null; }
      } else {
        bundle = v;
      }
    }
  }catch(e){ console.warn('일정 문서 로드 실패:', (e&&e.message)||String(e)); }
  if(!bundle || typeof bundle !== 'object') bundle = _schedDefaultBundle();

  // 3) 전역 변수에 주입
  _schedApplyBundle(bundle);
  // 일정 문서 모드에서는 폴더 타겟 초기화 (이전 메인 모드 잔여값 방지)
  _docTargetFolder = null;

  // 4) UI 전환
  const cdv = document.getElementById('cardDetailView');
  if(cdv) cdv.style.display = 'none';
  const dvv = document.getElementById('docViewerView');
  if(dvv) dvv.style.display = 'none';
  const ms = document.getElementById('mainSite');
  if(ms) ms.style.display = 'block';

  // 메인 섹션(문서, 웹툰, 히어로, 구분선) 숨김
  const hero = document.querySelector('.hero');
  if(hero) hero.style.display = 'none';
  const docsSec = document.getElementById('docs');
  if(docsSec) docsSec.style.display = 'none';
  const webtoonSec = document.getElementById('webtoon');
  if(webtoonSec) webtoonSec.style.display = 'none';
  document.querySelectorAll('.sec-div').forEach(el=>{ el.style.display='none'; });

  // 배너 표시
  const banner = document.getElementById('schedDocBanner');
  if(banner){
    banner.style.display = 'flex';
    const nm = banner.querySelector('.sched-doc-banner-name');
    if(nm) nm.textContent = docTitle;
  }

  // 기존 캘린더 필터 배너(작품/공정자) 숨김 (_activeCalDocId를 null로 초기화했으므로)
  const calBanner = document.getElementById('calDocBanner');
  if(calBanner) calBanner.style.display = 'none';

  // 5) 설정 적용 & 렌더
  try{ if(typeof applyColors==='function') applyColors(); }catch(e){}
  try{ if(typeof applyConfig==='function') applyConfig(); }catch(e){}
  try{ if(typeof applyCalCfg==='function') applyCalCfg(); }catch(e){}

  // 캘린더 이름 표시 (없으면 문서 제목을 기본값으로)
  try{
    const calNameEl = document.getElementById('calNameText');
    if(calNameEl) calNameEl.textContent = CAL_NAME || docTitle;
  }catch(e){}

  try{ renderLegend(); }catch(e){}
  try{ renderMonthly(); }catch(e){}
  try{ renderTagFilter(); }catch(e){}
  try{ renderCalFilterChips(); }catch(e){}
  try{
    if(document.getElementById('viewQ') && document.getElementById('viewQ').style.display !== 'none') renderQuarterly();
  }catch(e){}
...
/* ══ 일정 문서 닫기 ══ */
async function closeSchedDoc(){
  if(!_schedDocMode){ return; }

  // 1) 현재 번들 즉시 저장 (대기 중인 디바운스도 강제 실행)
  if(_schedSaveTimer){ clearTimeout(_schedSaveTimer); _schedSaveTimer=null; }
  try{
    await _doSchedBundleSave();
  }catch(e){ console.warn('일정 문서 종료 저장 실패:', e); }

  const prevCardDisp = _savedCtxForSched ? _savedCtxForSched.prevCardDetailDisplay : '';
  ...
  const docsSec = document.getElementById('docs');
  if(docsSec) docsSec.style.display = '';
  const webtoonSec = document.getElementById('webtoon');
  if(webtoonSec) webtoonSec.style.display = '';
  document.querySelectorAll('.sec-div').forEach(el=>{ el.style.display=''; });

  const banner = document.getElementById('schedDocBanner');
  if(banner) banner.style.display = 'none';

  // 이전에 카드 상세 뷰에 있었으면 그 상태로 복원
  const cdv = document.getElementById('cardDetailView');
  const ms  = document.getElementById('mainSite');
  if(prevCardDisp === 'block' && cdv){
  ...
  try{
    const calNameEl = document.getElementById('calNameText');
    if(calNameEl) calNameEl.textContent = CAL_NAME || '캘린더';
  }catch(e){}
...
/* ══ 일정 문서 이름 변경 ══ */
let _renameSchedDocId = null;
function openSchedDocRenameModal(docId){
  _renameSchedDocId = docId;
  let d=null;
  for(const[key,docs] of Object.entries(CARD_DOCS)){
    d=(docs||[]).find(x=>x.id===docId);
    if(d)break;
  }
  if(!d)return;
  document.getElementById('schedDocRenameIn').value = d.title || '';
  document.getElementById('schedDocRenameOverlay').classList.add('open');
  setTimeout(()=>{ const el=document.getElementById('schedDocRenameIn'); if(el) el.select(); },180);
}
function closeSchedDocRename(){
  document.getElementById('schedDocRenameOverlay').classList.remove('open');
}
function saveSchedDocRename(){
  const v=(document.getElementById('schedDocRenameIn').value||'').trim();
  if(!v){toast('이름을 입력해주세요.');return;}
  let d=null;
  for(const[key,docs] of Object.entries(CARD_DOCS)){
    d=(docs||[]).find(x=>x.id===_renameSchedDocId);
    if(d)break;
  }
  if(!d)return;
  d.title=v;
...
/* ══ CAL MAKER ══ */
function openCalMaker(){
  closeDocTypeModal();
  document.getElementById('cmTitle').value='';
  document.getElementById('cmSubType').value='work';
  document.getElementById('calMakerOverlay').classList.add('open');
  setTimeout(()=>document.getElementById('cmTitle').focus(),180);
}
...
function saveCalDoc(){
  ...
  if(!title){toast('이름을 입력해주세요.');return;}
  const subType=document.getElementById('cmSubType').value||'work';
  const id=uid();
  const docObj={id,title,type:'calendar',calSubType:subType,parentId:_docTargetFolder||null};
  if(!CARD_DOCS[_curCard])CARD_DOCS[_curCard]=[];
  CARD_DOCS[_curCard].push(docObj);
  _docTargetFolder=null;
  CAL_FILTER_ORDER.push(id);
  saveAll();renderDocList();closeCalMaker();renderCalFilterChips();
  toast((subType==='worker'?'공정자':'작품')+' 캘린더 "'+title+'"가 생성되었습니다.');
}

/* ══ 캘린더 문서 열기 ══ */
function openCalendarDoc(docId){
  document.getElementById('cardDetailView').style.display='none';
  document.getElementById('docViewerView').style.display='none';
  document.getElementById('mainSite').style.display='block';
  _activeCalDocId=docId;
  let d=null;
  for(const[key,docs] of Object.entries(CARD_DOCS)){d=(docs||[]).find(x=>x.id===docId);if(d)break;}
  _activeCalDocType=d?.calSubType||'work';
  const calTitle=d?d.title:'캘린더';
  const banner=document.getElementById('calDocBanner');
  if(banner){
    banner.style.display='flex';
    banner.querySelector('.cal-doc-banner-name').textContent=calTitle;
    banner.querySelector('.cal-doc-banner-label').textContent=_activeCalDocType==='worker'?'공정자 캘린더':'작품 캘린더';
  }
  const addBtn=document.getElementById('btnAddEvent');
  if(addBtn)addBtn.innerHTML='＋ '+calTitle+' 일정 추가';
  renderLegend();renderMonthly();renderTagFilter();renderCalFilterChips();
  if(document.getElementById('viewQ')?.style.display!=='none')renderQuarterly();
  document.getElementById('calendar').scrollIntoView({behavior:'smooth'});
  const url=new URL(window.location);url.searchParams.delete('card');url.searchParams.delete('doc');
  history.pushState({},'',url);
}
function closeCalendarDoc(){
  _activeCalDocId=null;_activeCalDocType=null;
  const banner=document.getElementById('calDocBanner');
  if(banner)banner.style.display='none';
  const addBtn=document.getElementById('btnAddEvent');
  if(addBtn)addBtn.innerHTML='＋ 일정 추가';
  renderLegend();renderMonthly();renderTagFilter();renderCalFilterChips();
  if(document.getElementById('viewQ')?.style.display!=='none')renderQuarterly();
}

/* ══ 캘린더 문서 이름 변경 ══ */
let _renameCalDocId=null;
function openCalDocRenameModal(docId){
  _renameCalDocId=docId;
  let d=null;
  for(const[key,docs] of Object.entries(CARD_DOCS)){d=(docs||[]).find(x=>x.id===docId);if(d)break;}
  if(!d)return;
...
function saveCalDocRename(){
  const v=document.getElementById('calDocRenameIn').value.trim();
  if(!v){toast('이름을 입력해주세요.');return;}
  let d=null;
  for(const[key,docs] of Object.entries(CARD_DOCS)){d=(docs||[]).find(x=>x.id===_renameCalDocId);if(d)break;}
  if(!d)return;
  d.title=v;
  saveAll();renderDocList();renderCalFilterChips();
  closeCalDocRename();toast('캘린더 이름이 변경되었습니다.');
}

/* ══ 캘린더 필터 칩 (순서 변경 가능) ══ */
function _getAllCalDocs(){
  const all=[];
  for(const[key,docs] of Object.entries(CARD_DOCS)){
    (docs||[]).forEach(d=>{if(d.type==='calendar')all.push(d);});
  }
  // CAL_FILTER_ORDER 순서 적용
  if(CAL_FILTER_ORDER.length){
    const ordered=[];
    CAL_FILTER_ORDER.forEach(id=>{const d=all.find(x=>x.id===id);if(d)ordered.push(d);});
    all.forEach(d=>{if(!ordered.includes(d))ordered.push(d);});
    return ordered;
  }
  return all;
}
function renderCalFilterChips(){
  const wrap=document.getElementById('calFilterWrap');
  if(!wrap)return;
  const allCals=_getAllCalDocs();
  if(!allCals.length){wrap.style.display='none';return;}
  wrap.style.display='flex';
  const chipsEl=document.getElementById('calFilterChips');
  if(!chipsEl)return;
  let h=`<span class="cal-filter-chip${!_activeCalDocId?' active':''}" onclick="closeCalendarDoc()">전체</span>`;
  const workCals=allCals.filter(d=>d.calSubType!=='worker');
  const workerCals=allCals.filter(d=>d.calSubType==='worker');
  if(workCals.length){
    h+=`<span class="cal-filter-sep">📁 작품</span>`;
    workCals.forEach((d,i)=>{
      h+=`<span class="cal-filter-chip${_activeCalDocId===d.id?' active':''}" draggable="true" data-cfidx="${allCals.indexOf(d)}" data-cfid="${d.id}"
        ondragstart="cfDragStart(event)" ondragover="cfDragOver(event)" ondrop="cfDrop(event)"
        onclick="openCalendarDoc('${d.id}')">${d.title}${workCals.length>1?'<span class=cf-grip>⠿</span>':''}</span>`;
    });
  }
  if(workerCals.length){
    h+=`<span class="cal-filter-sep">👤 공정자</span>`;
    workerCals.forEach((d,i)=>{
      h+=`<span class="cal-filter-chip worker${_activeCalDocId===d.id?' active':''}" draggable="true" data-cfidx="${allCals.indexOf(d)}" data-cfid="${d.id}"
        ondragstart="cfDragStart(event)" ondragover="cfDragOver(event)" ondrop="cfDrop(event)"
        onclick="openCalendarDoc('${d.id}')">${d.title}${workerCals.length>1?'<span class=cf-grip>⠿</span>':''}</span>`;
    });
  }
  chipsEl.innerHTML=h;
}
let _cfDragId=null;
function cfDragStart(e){_cfDragId=e.target.dataset.cfid;e.dataTransfer.effectAllowed='move';}
function cfDragOver(e){e.preventDefault();e.dataTransfer.dropEffect='move';}
function cfDrop(e){
  e.preventDefault();
  const targetId=e.target.closest('[data-cfid]')?.dataset.cfid;
  if(!_cfDragId||!targetId||_cfDragId===targetId)return;
  const allCals=_getAllCalDocs();
  const ids=allCals.map(d=>d.id);
  const fromIdx=ids.indexOf(_cfDragId);const toIdx=ids.indexOf(targetId);
  if(fromIdx<0||toIdx<0)return;
  ids.splice(fromIdx,1);ids.splice(toIdx,0,_cfDragId);
  CAL_FILTER_ORDER=ids;
  saveAll();renderCalFilterChips();
  _cfDragId=null;
}
...
function getFilteredEvents(){
  let evts=EVENTS;
  if(_activeCalDocId){
    if(_activeCalDocType==='worker'){
      let d=null;
      for(const[key,docs] of Object.entries(CARD_DOCS)){d=(docs||[]).find(x=>x.id===_activeCalDocId);if(d)break;}
      const wName=d?d.title:'';
      evts=evts.filter(ev=>(ev.worker||'')===wName||(ev.calendarIds||[]).includes(_activeCalDocId));
    }else{
      evts=evts.filter(ev=>(ev.calendarIds||[]).includes(_activeCalDocId));
    }
  }
  return evts;
}
...
function renderCalTagChips(){
  const wrap=document.getElementById('evCalTagWrap');
  if(!wrap)return;
  const allCals=_getAllCalDocs();
  if(!allCals.length){wrap.innerHTML='<span style="font-size:.78rem;color:var(--t3)">캘린더가 없습니다. 캘린더 만들기에서 추가하세요.</span>';return;}
  wrap.innerHTML=allCals.map(d=>{
    const on=_editCalIds.includes(d.id);
    const isW=d.calSubType==='worker';
    return`<span class="ev-cal-chip${isW?' worker':''}${on?' active':''}" onclick="toggleCalTag('${d.id}')">${isW?'👤 ':''}${d.title}</span>`;
  }).join('');
}
function toggleCalTag(id){
  const i=_editCalIds.indexOf(id);
  if(i>=0)_editCalIds.splice(i,1);else _editCalIds.push(id);
  renderCalTagChips();
}

/* ══ 일정 우클릭 복사 ══ */
let _ctxEvIdx=-1;
function showEvCtxMenu(e,idx){
  e.preventDefault();e.stopPropagation();
  _ctxEvIdx=idx;
  const menu=document.getElementById('evCtxMenu');
  if(!menu)return;
  menu.style.display='block';
  menu.style.left=Math.min(e.clientX,window.innerWidth-160)+'px';
  menu.style.top=Math.min(e.clientY,window.innerHeight-50)+'px';
}
function hideEvCtxMenu(){const m=document.getElementById('evCtxMenu');if(m)m.style.display='none';_ctxEvIdx=-1;}
function copyEvent(){
  if(_ctxEvIdx<0||_ctxEvIdx>=EVENTS.length)return;
  const src=EVENTS[_ctxEvIdx];
  const copy=JSON.parse(JSON.stringify(src));
  copy.name=copy.name+' (복사)';
  if(copy.task)copy.task=copy.task+' (복사)';
  EVENTS.push(copy);
  saveAll();renderLegend();renderMonthly();renderTagFilter();
  if(document.getElementById('viewQ')?.style.display!=='none')renderQuarterly();
  hideEvCtxMenu();
  toast('"'+src.name+'" 일정이 복사되었습니다.');
}
document.addEventListener('click',function(){hideEvCtxMenu();});
...
function renderTagFilter(){
  ...
  if(!wrap)return;
  const _fE=getFilteredEvents();
  const allTags=[...new Set(_fE.flatMap(ev=>ev.tags||[]))];
  if(!allTags.length){wrap.style.display='none';return;}
  wrap.style.display='flex';
...
// CAL_DISPLAY: 캘린더 표기 설정 — task/process/project/project+process/process+task/all/worker/name
...
function getEvLabel(ev){
  ...
  switch(mode){
    ...
    case 'all':        return [proj,proc,task].filter(Boolean).join(SEP);
    case 'worker':     return wkr||task;
...
function openCalDisplayModal(){
  ...
  const htCb=document.getElementById('calHideTime');
  if(htCb)htCb.checked=(CAL_DISPLAY||{}).hideTime||false;
...
function updateCalLabelPreview(mode){
  const sampleEv={name:'글콘티 검수',project:'로도스도 전기',process:'콘티',task:'글콘티 검수',worker:'김작가'};
  const prevCAL=CAL_DISPLAY;CAL_DISPLAY={...CAL_DISPLAY,labelMode:mode};
...
function saveCalDisplaySettings(){
  ...
  CAL_DISPLAY.hideTime=document.getElementById('calHideTime')?.checked||false;
...
function renderLegend(){
  ...
  if(!el)return;
  const _fE=getFilteredEvents();
  if(!_fE.length){el.innerHTML='<div class="leg-empty">등록된 일정이 없습니다.</div>';return;}

  const vis=CAL_CFG.visibleTags;
  const visEvts=vis?_fE.filter(ev=>(ev.tags||[]).some(t=>vis.includes(t))||(ev.tags||[]).length===0):_fE;
  ...
  el.innerHTML=sortedKeys.map(key=>{
    ...
    const items=evts.map(ev=>{
      ...
      const _ht=(CAL_DISPLAY||{}).hideTime||false;
      const timeStr=(!_ht&&ev.startTime)?`<span class="leg-time">${ev.startTime}${ev.endTime?'~'+ev.endTime:''}</span>`:'';
...
function renderMonthly(){
  ...
  const baseEvts=(()=>{
    const _fE=getFilteredEvents();
    let evts=vis?_fE.filter(ev=>(ev.tags||[]).some(t=>vis.includes(t))||(ev.tags||[]).length===0):_fE;
    ...
    if(numL>0){
      ...
      for(let l=0;l<numL;l++){
        ...
        laned.filter(i=>i.lane===l).forEach(item=>{
          ...
          if(item.e.excludeWeekend){
            ...
            segments.forEach(seg=>{
              ...
              segBar.onclick=()=>openEventModal(EVENTS.indexOf(item.e));
                segBar.oncontextmenu=(e)=>showEvCtxMenu(e,EVENTS.indexOf(item.e));
              const sh2=pd(item.e.start)>=week[0].date&&pd(item.e.start)<=week[6].date;
              if(sh2&&seg===segments[0]){
                const timeStr=(!((CAL_DISPLAY||{}).hideTime)&&item.e.startTime)?item.e.startTime+' ':'';
          ...
          bar.oncontextmenu=(e)=>showEvCtxMenu(e,EVENTS.indexOf(item.e));
...
function _updateGanttProjectFilter(){
  ...
  if(!sel)return;
  const projects=[...new Set(getFilteredEvents().map(e=>e.project||'').filter(Boolean))].sort();
...
function renderQuarterly(){
  ...
  const vis=CAL_CFG.visibleTags;
  const _fE=getFilteredEvents();
  let allEvts=vis?_fE.filter(ev=>(ev.tags||[]).some(t=>vis.includes(t))||(ev.tags||[]).length===0):_fE;
  // 프로젝트 필터 적용
  ...
  function buildProcRow(proc,evts,lblHtml,procColor){
    ...
    evts.forEach(ev=>{
      ...
      if(ev.excludeWeekend){
        ...
        r+=`<div class="g-bar" style="${styleStr}" onclick="openEventModal(${realIdx})" title="${ev.name}${ev.worker?' ['+ev.worker+']':''}\n${ev.start} ~ ${ev.end}">${label}</div>`;
      }
    });
    r+=`</div></td></tr>`;
    return r;
...
function openEventModal(idx=-1){
  ...
  const wdl=document.getElementById('evWorkerList');
  if(wdl){const workers=[...new Set(EVENTS.map(e=>e.worker||'').filter(Boolean))];wdl.innerHTML=workers.map(w=>`<option value="${w}">`).join('');}
  if(idx===-1){
    ...
    procSel.value='';
    document.getElementById('evWorker').value='';_editCalIds=[];
    if(_activeCalDocId)_editCalIds=[_activeCalDocId];
    renderCalTagChips();
  }else{
    const ev=EVENTS[idx];
    ...
    document.getElementById('evWorker').value=ev.worker||'';
    _editCalIds=[...(ev.calendarIds||[])];
    renderCalTagChips();
  }
  renderTagChips();
...
function saveEvent(){
  ...
  const worker=document.getElementById('evWorker').value.trim()||'';
  const ev={name,project,task,note,color:selColor,start,end,startTime,endTime,tags:[..._editTags],excludeWeekend,process,worker,calendarIds:[..._editCalIds]};
...
(async function init(){
  ...
  if(!IS_SHARE){
    renderSection('docs');renderSection('webtoon');
    renderLegend();renderTagFilter();renderCalFilterChips();
