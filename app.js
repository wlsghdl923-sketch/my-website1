...
/* ══ 글자색/배경색 팔레트 ══ */
const TEXT_COLORS30=['#191F28','#4E5968','#8B95A1','#FFFFFF','#D91F3E','#FF5252',
  '#FF7A00','#FFCA28','#2E9E50','#69F0AE','#0064FF','#40C4FF','#006064','#80DEEA',
  '#9C27B0','#CE93D8','#673AB7','#E91E63','#F48FB1','#795548','#A1887F','#607D8B',
  '#BF360C','#1B5E20','#01579B','#4A148C','#880E4F','#37474F','#4E342E','#CFD8DC'];
const BG_COLORS30=['transparent','#FFFF00','#FFE082','#FFCDD2','#F8BBD9','#FFF3E0',
  '#FFE0B2','#C8E6C9','#DCEDC8','#B2EBF2','#E1F5FE','#E8EAF6','#EDE7F6','#F3E5F5',
  '#FCE4EC','#E0F2F1','#F9FBE7','#FFF9C4','#E3F2FD','#F1F8E9','#FFFDE7','#FFF8F0',
  '#F0F4FF','#F9F9F9','#D5E8D5','#D5D5E8','#E8D5D5','#E8E8D5','#D5E8E8','#E8DDD5'];
let _savedSel=null;
...
function toggleColorPalette(id){
  ...
  if(sel&&sel.rangeCount&&!sel.isCollapsed)_savedSel={r:sel.getRangeAt(0).cloneRange()};
  const p=document.getElementById(id);if(!p)return;
  const open=p.style.display==='flex';
  document.querySelectorAll('.re-color-palette').forEach(x=>x.style.display='none');
  if(!open)p.style.display='flex';
}
...
/* ══ 메모 — Notion 스타일 ══
   핵심: contextmenu에서 즉시 range 저장
   → 메뉴 클릭 시에도 사라지지 않음
══ */
let _memoSavedRange=null;
let _memoSavedText='';
...
function saveMemoToSelection(){
  ...
  span.className='re-memo';span.dataset.memo=text;span.dataset.memoid=id;span.title='📝 '+text;
  try{_memoSavedRange.surroundContents(span);}
  catch{
    const frag=_memoSavedRange.extractContents();
    span.appendChild(frag);_memoSavedRange.insertNode(span);
  }
  sel.removeAllRanges();
  closeMemoInput();refreshMemoPanel();toast('메모가 저장되었습니다.');
}
...
function refreshMemoPanel(){
  ...
  list.innerHTML=memos.map((m,i)=>{
    ...
    return `<div class="memo-card" id="mc_${id}">
      <div class="memo-card-quote">"${(m.textContent||'').slice(0,40)}"</div>
      <div class="memo-card-body">${m.dataset.memo||''}</div>
      <div class="memo-card-btns">
        <button onclick="editMemoCard('${id}')">수정</button>
        <button class="mdel" onclick="deleteMemoCard('${id}')">삭제</button>
      </div>
    </div>`;
  }).join('');
}
...
function highlightMemoCard(memoEl){
  if(!memoEl)return;const id=memoEl.dataset.memoid;if(!id)return;
  document.querySelectorAll('.memo-card').forEach(c=>c.style.boxShadow='');
  const card=document.getElementById('mc_'+id);
  if(card){card.style.boxShadow='0 0 0 2px #F57F17';card.scrollIntoView({behavior:'smooth',block:'nearest'});}
}
...
const CO_COLORS=[
  {b:'#C7D7FF',bg:'#F0F4FF',t:'#1A3A8F'},{b:'#FFCDD2',bg:'#FFF3F5',t:'#B71C1C'},
  {b:'#C8E6C9',bg:'#F1F8F1',t:'#1B5E20'},{b:'#FFE082',bg:'#FFFDE7',t:'#5D4037'},
  {b:'#FFCC80',bg:'#FFF8F0',t:'#E65100'},{b:'#CE93D8',bg:'#F8F0FF',t:'#4A148C'},
  {b:'#E0E0E0',bg:'#F9F9F9',t:'#212121'},{b:'#B2EBF2',bg:'#E0F7FA',t:'#006064'},
  {b:'#F8BBD9',bg:'#FCE4EC',t:'#880E4F'},{b:'#DCEDC8',bg:'#F9FBE7',t:'#33691E'},
  {b:'#D1C4E9',bg:'#EDE7F6',t:'#311B92'},{b:'#B3E5FC',bg:'#E1F5FE',t:'#01579B'},
  {b:'#FFD180',bg:'#FFFBF0',t:'#BF6000'},{b:'#80CBC4',bg:'#E0F2F1',t:'#004D40'},
];
const CO_EMOJIS=['💡','⚠️','✅','📌','🔥','📝','🎯','💬','🔑','📚','🎨','💎','🚀','🌟','🎵','❓','❗','🔔','📊','🗂️','🌈','⭐','🏆','💪','🧠','🔍','💼','📅','🌐','🔒','🎉','✨','🛠️','📣','🤝','🌱','💫','🔗','📱','🖥️','✍️','🗒️','🔺','🔻','▶️','⏸️','⏹️','🔄','🎬','📸'];
let _coPick={color:CO_COLORS[0],emoji:'💡'};
function _coEmojiGridHTML(sel){
  const customs=_coCustomEmojis.map((em,i)=>{const isImg=em.startsWith('data:')||em.startsWith('http');const inner=isImg?`<img src="${em}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;">`:`${em}`;return `<span class="co-em-btn${em===sel?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${inner}<span class="co-em-del" onclick="event.stopPropagation();delCoCustomEmoji(${i})">✕</span></span>`;});
  const base=CO_EMOJIS.map(em=>`<span class="co-em-btn${em===sel?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${em}</span>`);
  const none=`<span class="co-em-btn co-em-none${sel===''?' sel':''}" data-em="" onclick="pickCoEmoji(this)">없음</span>`;
  return [...customs,...base,none].join('');
}
...
async function openCalloutPicker(){
  await _loadCoCustomEmojis();_coPick={color:CO_COLORS[0],emoji:'💡'};
  const cg=document.getElementById('coColorRow'),eg=document.getElementById('coEmojiGrid');
  if(cg)cg.innerHTML=_coColorGridHTML(CO_COLORS[0].b);if(eg)eg.innerHTML=_coEmojiGridHTML('💡');
...
function _buildCalloutHtml(color,emoji){
  const isImg=emoji&&(emoji.startsWith('data:')||emoji.startsWith('http'));
  const em=emoji?(isImg?`<img src="${emoji}" data-coem="${emoji}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;" contenteditable="false">`:`<span data-coem="${emoji}" contenteditable="false" style="font-size:1.2rem;flex-shrink:0;margin-top:1px;">${emoji}</span>`):'';
  const st=`display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid ${color.b};background:${color.bg};color:${color.t};line-height:1.7;`;
  return `<div class="re-callout" contenteditable="false" style="${st}">${em}<div contenteditable="true" style="flex:1;min-width:0;outline:none;">내용을 입력하세요</div></div><p><br></p>`;
}
...
function applyCoEmojiChange(){
  if(!_coChangeTarget)return;const old=_coChangeTarget.querySelector('[data-coem]');if(old)old.remove();
  if(_coPick.emoji){const isImg=_coPick.emoji.startsWith('data:')||_coPick.emoji.startsWith('http');const el=document.createElement(isImg?'img':'span');el.setAttribute('data-coem',_coPick.emoji);el.contentEditable='false';if(isImg){el.src=_coPick.emoji;el.style.cssText='width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;';}else{el.style.cssText='font-size:1.2rem;flex-shrink:0;margin-top:1px;';el.textContent=_coPick.emoji;}const body=_coChangeTarget.querySelector('div[contenteditable="true"]');if(body)_coChangeTarget.insertBefore(el,body);else _coChangeTarget.prepend(el);}
  closeCoEmojiChange();
}
...
function tblCmd(cmd){
  const td=_tblTd;if(!td)return;
  const tr=td.closest('tr'),table=td.closest('table');if(!tr||!table){_tblTd=null;return;}
  const rows=Array.from(table.querySelectorAll('tr'));const ri=rows.indexOf(tr);
  const cells=Array.from(tr.querySelectorAll('td,th'));const ci=cells.indexOf(td);
  const cols=rows[0]?rows[0].querySelectorAll('td,th').length:1;
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;';
  const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;';
...
/* ══ 표 열/행 리사이즈 ══
   핵심 설계:
   • 세로선: 각 표의 모든 열 경계 X좌표를 수집,
     마우스가 경계 ±5px 이내면 col-resize 커서 + 드래그 시작
   • 이렇게 하면 e.target이 어느 셀인지 관계없이 작동
   • 가로선: 마찬가지로 각 행의 bottom Y좌표 수집
══ */
let _tblColDrag=null, _tblRowDrag=null;

function _getTableBorders(editor){
  /* 에디터 내 모든 표의 열/행 경계를 [{type,x/y,table,cells,rowEl,startW/H}] 형태로 반환 */
  const borders=[];
  editor.querySelectorAll('table').forEach(table=>{
    ...
    firstRowCells.forEach((cell,ci)=>{
      if(ci===firstRowCells.length-1)return; // 마지막 열 오른쪽은 표 경계 → 제외
      const rect=cell.getBoundingClientRect();
      const borderX=rect.right;
      // 같은 열 인덱스의 모든 셀
      const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
      borders.push({type:'col',x:borderX,cells:colCells,startW:cell.offsetWidth});
    });
    // 행 경계
    rows.forEach((tr,ri)=>{
      if(ri===rows.length-1)return;
      const rect=tr.getBoundingClientRect();
      borders.push({type:'row',y:rect.bottom,tr,startH:tr.offsetHeight});
    });
  });
  return borders;
}
...
function _initTableResize(){
  ...
  // 경계 근처이면 커서 변경 + 드래그 정보 캐시
  let _nearBorder=null;
  document.addEventListener('mousemove',function(e){
    if(_tblColDrag){
      const newW=Math.max(1,_tblColDrag.startW+(e.clientX-_tblColDrag.startX));
      _tblColDrag.cells.forEach(c=>{c.style.width=newW+'px';c.style.minWidth='0';c.style.maxWidth='none';c.style.overflow='hidden';});
      return;
    }
    if(_tblRowDrag){
      const newH=Math.max(10,_tblRowDrag.startH+(e.clientY-_tblRowDrag.startY));
      _tblRowDrag.tr.style.height=newH+'px';
      Array.from(_tblRowDrag.tr.querySelectorAll('td,th')).forEach(c=>{c.style.height=newH+'px';});
      return;
    }
    // 경계 감지 — 에디터 안에 있을 때만
    const edRect=editor.getBoundingClientRect();
    if(e.clientX<edRect.left||e.clientX>edRect.right||e.clientY<edRect.top||e.clientY>edRect.bottom){
      if(editor.style.cursor)editor.style.cursor='';_nearBorder=null;return;
    }
    const borders=_getTableBorders(editor);
    let found=null;
    for(const b of borders){
      if(b.type==='col'&&Math.abs(e.clientX-b.x)<=THRESH){found=b;break;}
      if(b.type==='row'&&Math.abs(e.clientY-b.y)<=THRESH){found=b;break;}
    }
    if(found){
      editor.style.cursor=found.type==='col'?'col-resize':'row-resize';
      _nearBorder={...found,startX:e.clientX,startY:e.clientY};
    }else{
  ...
  // 경계 근처에서 mousedown → 드래그 시작
  document.addEventListener('mousedown',function(e){
    if(!_nearBorder)return;
    // 색상 팔레트 위면 무시
    if(e.target.closest&&e.target.closest('.re-color-palette'))return;
    e.preventDefault();
    if(_nearBorder.type==='col'){
      _tblColDrag={cells:_nearBorder.cells,startX:e.clientX,startW:_nearBorder.cells[0].offsetWidth};
    }else{
      _tblRowDrag={tr:_nearBorder.tr,startY:e.clientY,startH:_nearBorder.tr.offsetHeight};
    }
  });

  document.addEventListener('mouseup',function(){
    if(_tblColDrag||_tblRowDrag){
      _tblColDrag=null;_tblRowDrag=null;
      editor.style.cursor='';
    }
  });
}
...
function doPdfExport(){
  closePdfOpt();
  const editor=document.getElementById('reEditor');
  const title=document.getElementById('reTitle')?.value||'문서';
  const withMemo=document.getElementById('pdfOptMemo')?.checked!==false;
  const clone=editor.cloneNode(true);
  let memoSection='';
  if(withMemo){
    const memos=[];editor.querySelectorAll('.re-memo').forEach(m=>memos.push({text:m.textContent,memo:m.dataset.memo||''}));
...
<style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Noto Sans KR',sans-serif;font-size:14px;line-height:1.9;color:#191F28;padding:40px 48px;}
h1{font-size:1.9rem;font-weight:800;margin:1.2em 0 .4em;}h2{font-size:1.4rem;font-weight:800;margin:1em 0 .35em;}h3{font-size:1.1rem;font-weight:700;margin:.9em 0 .3em;}
p{margin:.4em 0;}ul,ol{padding-left:1.5em;margin:.4em 0;}li{margin:.15em 0;}
blockquote{border-left:4px solid #D91F3E;padding:.5em 1em;background:#FFF3F5;border-radius:0 6px 6px 0;margin:.6em 0;}
.re-callout{display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:12px 14px;margin:.6em 0;line-height:1.7;}
table{border-collapse:collapse;width:100%;margin:.6em 0;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}
img{max-width:100%;border-radius:6px;}.re-img-wrap{display:inline-block;vertical-align:top;}.re-img-handle{display:none!important;}
.tbl-sel{outline:none!important;}.re-memo{background:rgba(255,213,79,.4);border-radius:2px;border-bottom:2px solid #F57F17;}
@media print{body{padding:20px;}}
</style></head><body>
<h1 style="border-bottom:2px solid #E5E8EB;padding-bottom:10px;margin-bottom:20px;">${title}</h1>
${clone.innerHTML}${memoSection}
...
async function exportBackup(){
  toast('백업 중...');
  try{const rows=await _sbRest.select('site_data');if(!rows||rows.length===0){toast('데이터 없음');return;}
    const obj={_v:1,_d:new Date().toISOString(),data:{}};rows.forEach(r=>{obj.data[r.key]=r.value;});
    const url=URL.createObjectURL(new Blob([JSON.stringify(obj,null,2)],{type:'application/json'}));
    const a=document.createElement('a');a.href=url;a.download='backup_'+new Date().toISOString().slice(0,16).replace('T','_').replace(':','-')+'.json';document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
    toast('백업 완료 — '+rows.length+'개');}catch{toast('백업 실패');}
}
async function importBackup(event){
  const file=event.target.files[0];event.target.value='';if(!file)return;
  if(!confirm('⚠ 현재 데이터가 덮어씌워집니다.\n\n파일: '+file.name))return;
  toast('복원 중...');
  try{const obj=JSON.parse(await file.text());if(!obj.data){toast('올바른 파일 아님');return;}
    for(const[k,v] of Object.entries(obj.data))await _sbRest.upsert('site_data',{key:k,value:v,updated_at:new Date().toISOString()});
    _applyMap(obj.data);applyColors();applyConfig();applyCalCfg();
    renderSection('docs');renderSection('webtoon');renderLegend();renderTagFilter();
    document.getElementById('calNameText').textContent=CAL_NAME;renderMonthly();
    closeSiteSettings();toast('복원 완료');}catch{toast('복원 실패');}
}
...
async function _readDocRawAsync(docId){
  try{
    ...
    let raw=typeof row.value==='string'?row.value:JSON.stringify(row.value);
    if(raw.startsWith('"')&&raw.endsWith('"')){try{const d=JSON.parse(raw);if(typeof d==='string')raw=d;}catch{}}
    return raw;
  }catch(e){return '';}
}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(h){const CS='display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid #C7D7FF;background:#F0F4FF;color:#1A3A8F;line-height:1.7;';document.execCommand('insertHTML',false,h.replace(/<aside[^>]*>/gi,'<div class="re-callout" style="'+CS+'">').replace(/<\/aside>/gi,'</div>'));}
      else{document.execCommand('insertText',false,e.clipboardData.getData('text/plain'));}
    });
    /* 우클릭 메뉴 — 메모: selection을 contextmenu에서 즉시 저장 */
    editor.addEventListener('contextmenu',function(e){
      ...
      if(sel&&!sel.isCollapsed){
        e.preventDefault();
        // selection 즉시 저장 (메뉴 클릭 시 이미 사라지므로 여기서 저장)
    ...
    editor.addEventListener('dblclick',function(e){
      const callout=e.target.closest&&e.target.closest('.re-callout');
      if(callout)openCoEmojiChange(callout);
    });
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
...
function _initImgResize(){
  const ed=document.getElementById('reEditor');
  if(!ed||ed._imgRszSet)return;ed._imgRszSet=true;
  ed.addEventListener('mousedown',function(e){
    const h=e.target.closest&&e.target.closest('.re-img-handle');
    if(!h)return;e.preventDefault();
    const w=h.parentElement;_imgRsz={w,dir:h.dataset.dir,sx:e.clientX,sw:w.offsetWidth};
  });
...
function reInsertTable(){
  ...
  for(let r=0;r<rows;r++){html+='<tr>';
    for(let c=0;c<cols;c++){
      const isH=r===0;
      const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;':'padding:10px 14px;border:1px solid #E5E8EB;';
      html+='<'+(isH?'th':'td')+' contenteditable="true" style="'+st+'">'+(isH?'제목'+(c+1):'')+'</'+(isH?'th':'td')+'>';
    }html+='</tr>';}
  html+='</tbody></table><p><br></p>';
  reExec('insertHTML',html);
  setTimeout(()=>_initTableResize(),100);
  toast('3×3 표 삽입. 셀 경계를 드래그해서 크기 조절 | 우클릭 편집');
}
...
function reOnKeydown(e){
  if(e.key==='Tab'){
    ...
    else{if(e.shiftKey)document.execCommand('outdent');
      else{const inList=document.queryCommandState('insertUnorderedList')||document.queryCommandState('insertOrderedList');if(!inList)document.execCommand('insertUnorderedList');else document.execCommand('indent');}}
    return;
  }
