...
══════════════════════════════════════════════════ */
const FG_COLORS=[
  '#191F28','#4E5968','#8B95A1','#FFFFFF',
  '#D91F3E','#FF5252','#FF7A00','#FFCA28',
  '#2E9E50','#00C4A7','#0064FF','#40C4FF',
  '#9C27B0','#E91E63','#795548','#607D8B',
];
const BG_COLORS=[
  'transparent',
  '#FFFF00','#FFE082','#FFCDD2',
  '#F8BBD9','#FFF3E0','#C8E6C9','#B2EBF2',
  '#E1F5FE','#EDE7F6','#FCE4EC','#F5F5F5',
  '#DCEDC8','#B3E5FC','#D7CCC8','#CFD8DC',
];
let _cpSavedRange=null;

function initColorPalettes(){
  const fg=document.getElementById('reFgPalette');
  const bg=document.getElementById('reBgPalette');
  if(!fg||fg._init)return;
  fg._init=bg._init=true;

  fg.innerHTML=FG_COLORS.map(c=>{
    const light=(c==='#FFFFFF'||c==='#FFCA28'||c==='#FFFF00');
    return `<div class="cp-swatch${light?' light':''}" style="background:${c}" onclick="applyFgColor('${c}')"></div>`;
  }).join('');

  bg.innerHTML=BG_COLORS.map(c=>{
    if(c==='transparent'){
      return `<div class="cp-swatch light" style="background:linear-gradient(135deg,#fff 45%,#f44 45%,#f44 55%,#fff 55%)" title="색상 제거" onclick="applyBgColor('transparent')"></div>`;
    }
    const light=(c==='#FFFF00'||c==='#FFE082'||c==='#F5F5F5');
    return `<div class="cp-swatch${light?' light':''}" style="background:${c}" onclick="applyBgColor('${c}')"></div>`;
  }).join('');

  // 외부 클릭 닫기 (한 번만)
  if(!document._cpCloseSet){
    document._cpCloseSet=true;
    document.addEventListener('mousedown',function(e){
      if(!e.target.closest('.re-color-wrap')&&!e.target.closest('#hrPickerPanel')){
        document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');
        const hr=document.getElementById('hrPickerPanel');
...
function toggleColorPalette(id){
  // selection 저장 (버튼 클릭으로 포커스 잃기 전에)
  const s=window.getSelection();
  if(s&&s.rangeCount>0&&!s.isCollapsed){
    _cpSavedRange=s.getRangeAt(0).cloneRange();
  }
...
function _restoreAndApply(execFn){
  if(_cpSavedRange){
    const s=window.getSelection();
    s.removeAllRanges();
    s.addRange(_cpSavedRange);
    _cpSavedRange=null;
  }
  execFn();
  document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');
...
/* ══════════════════════════════════════════════════
   구분선 — 7가지 스타일 + 드롭다운 + Backspace 삭제
══════════════════════════════════════════════════ */
function toggleHrPicker(){
  const p=document.getElementById('hrPickerPanel');
  if(!p)return;
  p.style.display=p.style.display==='block'?'none':'block';
}

function insertHr(style){
  const p=document.getElementById('hrPickerPanel');
  if(p)p.style.display='none';

  /* contenteditable 안에서 안전하게 삽입하려면 실제 DOM 조작 */
  ...
  // wrapper div (contenteditable=false → 드래그 선택 제외)
  const wrap=document.createElement('div');
  wrap.className='hr-wrap';
  wrap.contentEditable='false';
  wrap.dataset.hrStyle=style;
  ...
  del.className='hr-del';
  del.textContent='✕';
  del.title='구분선 삭제';
  del.onclick=function(e){e.stopPropagation();removeHr(del);};
  ...
  const styleName={solid:'실선',dashed:'대시',dotted:'점선',double:'이중',thick:'굵은',fade:'페이드',wavy:'물결'}[style]||style;
  toast(`${styleName} 구분선 삽입 | 호버해서 ✕로 삭제`);
}

function removeHr(delBtn){
  const wrap=delBtn.closest('.hr-wrap');
  if(wrap){
...
/* ══════════════════════════════════════════════════
   ...
   근본 해결:
   - 표 전체 contenteditable=false (에디터 이벤트와 분리)
   - 각 td/th contenteditable=true (내용 편집 가능)
   - 핸들(.col-rsz-h)을 .re-tbl-wrap에 절대좌표로 배치
     → td 밖이라 contenteditable 이벤트와 완전히 독립
   - 드래그: document mousemove capture:true
══════════════════════════════════════════════════ */
let _colDrag=null;   // 현재 드래그 상태
let _rowDrag=null;

(function _setupTableResize(){
  // 한 번만 등록
  document.addEventListener('mousemove',function(e){
    if(_colDrag){
      const dx=e.clientX-_colDrag.startX;
      const newW=Math.max(30,_colDrag.startW+dx);
      _colDrag.cells.forEach(c=>{
        c.style.width=newW+'px';
        c.style.minWidth=newW+'px';
      });
      _colDrag.handle.style.left=
        (_colDrag.cellInitLeft + newW - _colDrag.wrapLeft)+'px';
    }
    if(_rowDrag){
      const dy=e.clientY-_rowDrag.startY;
      const newH=Math.max(18,_rowDrag.startH+dy);
      _rowDrag.tr.style.height=newH+'px';
      _rowDrag.tr.querySelectorAll('td,th').forEach(c=>c.style.height=newH+'px');
    }
  },true);

  document.addEventListener('mouseup',function(){
    if(_colDrag){
      _colDrag.handle.classList.remove('dragging');
      _colDrag=null;
...
function _buildTable(rows,cols){
  ...
  let h=`<div class="re-tbl-wrap"><table class="re-tbl" contenteditable="false" style="border-collapse:collapse;width:100%;table-layout:fixed;"><colgroup>`;
  for(let c=0;c<cols;c++) h+=`<col style="width:${W}%">`;
  h+=`</colgroup><tbody>`;
  for(let r=0;r<rows;r++){
    h+=`<tr>`;
    for(let c=0;c<cols;c++){
      const isH=r===0;
      h+=`<${isH?'th':'td'} contenteditable="true" style="padding:8px 10px;border:1px solid #E5E8EB;${isH?'background:#F5F6FA;font-weight:600;':''}">${isH?'제목 '+(c+1):''}</${isH?'th':'td'}>`;
    }
    h+=`</tr>`;
  }
...
function reInsertTable(){
  document.execCommand('insertHTML',false,_buildTable(3,3));
  // DOM이 생긴 뒤 핸들 배치
  requestAnimationFrame(()=>requestAnimationFrame(_initAllTableHandles));
  toast('표 삽입 | 열 경계를 드래그해서 너비 조절');
}

function _initAllTableHandles(){
  const editor=document.getElementById('reEditor');
  if(!editor)return;
  editor.querySelectorAll('.re-tbl-wrap').forEach(wrap=>{
    const table=wrap.querySelector('.re-tbl');
    if(table)_attachHandles(wrap,table);
  });
}

function _attachHandles(wrap,table){
  // 기존 핸들 제거
  wrap.querySelectorAll('.col-rsz-h').forEach(h=>h.remove());
  ...
  firstCells.forEach((cell,ci)=>{
    if(ci>=firstCells.length-1)return; // 마지막 열 제외
    ...
    function setPos(){
      ...
      h.style.left=(cr.right-wr.left)+'px';
      h.style.height=table.offsetHeight+'px';
    }
    setPos();
    wrap.appendChild(h);
    ...
    h.addEventListener('mousedown',function(e){
      ...
      const colCells=allRows.map(r=>
        Array.from(r.querySelectorAll('td,th'))[ci]
      ).filter(Boolean);
      ...
      _colDrag={
        ...
        handle:h,
        wrapLeft:wr.left,
        cellInitLeft:cell.getBoundingClientRect().left - wr.left,
      };
  ...
  table.addEventListener('mousedown',function(e){
    ...
    if(e.clientY>rect.bottom-6){
      ...
      _rowDrag={
        tr:td.closest('tr'),
        startY:e.clientY,
        startH:td.closest('tr').offsetHeight,
      };
...
function _setAutoSaveStatus(status){
  ...
  badge.className='auto-save-badge';
  if(status==='idle'){
    icon.innerHTML='<circle cx="4.5" cy="4.5" r="3.8" fill="none" stroke="#C0CAD6" stroke-width="1.3"/>';
    text.textContent='대기';
  } else if(status==='changed'){
    icon.innerHTML='<circle cx="4.5" cy="4.5" r="3.8" fill="none" stroke="#FF7A00" stroke-width="1.3" stroke-dasharray="5,3"/>';
    text.textContent='미저장';
  } else if(status==='saving'){
    badge.classList.add('saving');
    icon.innerHTML='<circle cx="4.5" cy="4.5" r="3.8" fill="none" stroke="#FF7A00" stroke-width="1.3"/><path d="M4.5 2v2.5l1.5 1.5" stroke="#FF7A00" stroke-width="1.1" stroke-linecap="round"/>';
    text.textContent='저장중';
  } else if(status==='saved'){
    badge.classList.add('saved');
    icon.innerHTML='<circle cx="4.5" cy="4.5" r="3.8" fill="none" stroke="#2E9E50" stroke-width="1.3"/><polyline points="2.5,4.5 4,6 6.5,3" stroke="#2E9E50" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" fill="none"/>';
    text.textContent='저장됨';
    setTimeout(()=>_setAutoSaveStatus('idle'),2500);
  } else if(status==='error'){
    badge.classList.add('error');
    icon.innerHTML='<circle cx="4.5" cy="4.5" r="3.8" fill="none" stroke="#D91F3E" stroke-width="1.3"/><line x1="3" y1="3" x2="6" y2="6" stroke="#D91F3E" stroke-width="1.2"/><line x1="6" y1="3" x2="3" y2="6" stroke="#D91F3E" stroke-width="1.2"/>';
    text.textContent='실패';
  }
}

function _scheduleAutoSave(){
  if(!_autoSaveEnabled||!_reEditDocId)return;
  clearTimeout(_autoSaveTimer);
  _setAutoSaveStatus('changed');
...
function openMdGuide(){
  const ov=document.getElementById('mdGuideOverlay');
...
function reOnKeydown(e){
    ...
    if(sel&&sel.rangeCount>0&&sel.isCollapsed){
      ...
      if(range.startOffset===0){
        ...
        if(node.nodeType===3){
          ...
          prev=node.childNodes[range.startOffset-1]||node.previousSibling;
        }
        if(prev&&prev.classList&&prev.classList.contains('hr-wrap')){
          e.preventDefault();
          removeHr(prev.querySelector('.hr-del'));
          return;
        }
      }
    }
  ...
  // ── Space/Enter: 마크다운 단축키 체크 ──
  if(e.key===' '||e.key==='Enter'){
    if(_checkMarkdownShortcut(e)){return;}
  }
  // ── Enter in pre ──
  if(e.key==='Enter'){
    ...
    if(tag==='pre'){
      e.preventDefault();
      reExec('insertHTML','<br>');
    }
  }
}

function _checkMarkdownShortcut(e){
  const sel=window.getSelection();
  ...
  // 헤딩
  const headMatch=line.match(/^(#{1,4})$/);
  if(headMatch&&e.key===' '){
    e.preventDefault();
  ...
  if(boldMatch&&e.key===' '){
    ...
    const r=document.createRange();
    r.setStart(node,start);r.setEnd(node,start+inner.length);
