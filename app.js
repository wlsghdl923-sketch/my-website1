...
async function execPdfExport(){
  ...
  // iframe으로 렌더링 (body 스타일 정확 적용)
  const renderFrame=document.createElement('iframe');
  renderFrame.style.cssText='position:fixed;left:-9999px;top:0;width:'+contentWidth+'px;border:none;visibility:hidden;';
  document.body.appendChild(renderFrame);
  const fullHtml=_pdfBuildFullHtml(content,contentWidth,'');
  renderFrame.srcdoc=fullHtml;
  await new Promise(r=>{renderFrame.onload=r;});
  const fDoc=renderFrame.contentDocument;
  const images=fDoc.querySelectorAll('img');
  if(images.length){
  ...
  const totalH=fDoc.body.scrollHeight;
  renderFrame.style.height=totalH+'px';
  await new Promise(r=>setTimeout(r,500));
  progText.textContent='캡처 중...';progBar.style.width='35%';
  try{
    const renderScale=scalePct/100*2;
    const canvas=await html2canvas(fDoc.body,{scale:renderScale,useCORS:true,allowTaint:true,backgroundColor:'#ffffff',logging:false,windowWidth:contentWidth,width:contentWidth,height:totalH});
    ...
    console.error('PDF err:',e);try{document.body.removeChild(renderFrame);}catch(ex){}
