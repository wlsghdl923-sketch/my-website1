...
function editDoc(docId){
  ...
  else if(d.type==='calendar')openCalendarDoc(docId);
...
function renderCalFilterChips(){
  ...
  allCals.forEach((d,i)=>{
    ...
    h+=`<span class="cal-filter-chip${isW?' worker':''}${_activeCalDocId===d.id?' active':''}" draggable="true" data-cfidx="${i}" data-cfid="${d.id}"
      ondragstart="cfDragStart(event)" ondragover="cfDragOver(event)" ondrop="cfDrop(event)"
      onclick="openCalendarDoc('${d.id}')">${isW?'👤 ':''}${d.title}${allCals.length>1?'<span class=cf-grip>⠿</span>':''}</span>`;
  });
  chipsEl.innerHTML=h;
}
let _cfDragId=null;
...
function getFilteredEvents(){
  ...
  if(_activeCalDocId){
    if(_activeCalDocType==='worker'){
      ...
      evts=evts.filter(ev=>(ev.worker||'')===wName);
    }else{
