...
async function openDocViewer(docId){
  ...
  // 캘린더 타입: openCalendarDoc으로 리다이렉트
  if(d.type==='calendar'){
    openCalendarDoc(docId);
    return;
  }
...
function saveCalDoc(){
  ...
  _docTargetFolder=null;
  saveAll();renderDocList();closeCalMaker();
...
function openCalendarDoc(docId){
  ...
  if(banner){
    ...
    banner.querySelector('.cal-doc-banner-label').textContent=
      _activeCalDocType==='worker'?'공정자 캘린더':'작품 캘린더';
  }
  ...
  const url=new URL(window.location);
  url.searchParams.delete('card');url.searchParams.delete('doc');
...
function renderCalFilterChips(){
  ...
  if(!wrap)return;
  const workCals=[],workerCals=[];
  for(const[key,docs] of Object.entries(CARD_DOCS)){
    (docs||[]).forEach(d=>{
      if(d.type==='calendar'){
        if(d.calSubType==='worker') workerCals.push(d);
        else workCals.push(d);
      }
    });
  }
  if(!workCals.length&&!workerCals.length){wrap.style.display='none';return;}
  ...
  if(workCals.length) h+=`<span class="cal-filter-sep">작품</span>`+workCals.map(d=>`<span class="cal-filter-chip${_activeCalDocId===d.id?' active':''}" onclick="openCalendarDoc('${d.id}')">${d.title}</span>`).join('');
  if(workerCals.length) h+=`<span class="cal-filter-sep">공정자</span>`+workerCals.map(d=>`<span class="cal-filter-chip worker${_activeCalDocId===d.id?' active':''}" onclick="openCalendarDoc('${d.id}')">${d.title}</span>`).join('');
  chipsEl.innerHTML=h;
}
...
function renderCalTagChips(){
  ...
  const workCals=[];
  for(const[key,docs] of Object.entries(CARD_DOCS)){
    (docs||[]).forEach(d=>{if(d.type==='calendar'&&d.calSubType!=='worker')workCals.push(d);});
  }
  if(!workCals.length){wrap.innerHTML='<span style="font-size:.78rem;color:var(--t3)">작품 캘린더가 없습니다. 캘린더 만들기에서 추가하세요.</span>';return;}
  wrap.innerHTML=workCals.map(d=>{
    const on=_editCalIds.includes(d.id);
    return`<span class="ev-cal-chip${on?' active':''}" onclick="toggleCalTag('${d.id}')">${d.title}</span>`;
  }).join('');
}
...
function openEventModal(idx=-1){
  ...
  if(idx===-1){
    ...
    // 활성 작품 캘린더면 자동 선택
    if(_activeCalDocId&&_activeCalDocType!=='worker')_editCalIds=[_activeCalDocId];
    renderCalTagChips();
  }else{
    const ev=EVENTS[idx];
