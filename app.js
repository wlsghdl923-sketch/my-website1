...
function togglePanels(){
  ...
  if(mp) mp.style.display=_panelsVisible?'block':'none';
  if(rp) rp.style.display=_panelsVisible?'block':'none';
  if(btn) btn.classList.toggle('active',_panelsVisible);
  if(_panelsVisible) setTimeout(refreshMemoPanel,100);
}
...
══════════════════════════════════════════════════════════ */
const MEMO_COLORS=[
  '#FFD54F','#FF8A65','#F06292','#CE93D8',
  '#90CAF9','#80DEEA','#A5D6A7','#EF5350',
  '#7E57C2','#26C6DA','#66BB6A','#8D6E63',
];
let _memos=[];            // [{id,text,markerColor,markerName,content,resolved,y}]
let _memoEditId=null;
let _memoSavedRange=null;

/* 메모 입력 팝업 열기 */
function openMemoInput(text,range){
  _memoSavedRange=range?range.cloneRange():null;
  _memoEditId=null;
  document.getElementById('memoInputTitle').textContent='📝 메모 추가';
  document.getElementById('memoSelectedText').textContent=text?`"${text.slice(0,80)}"`:' ';
  document.getElementById('memoMarkerName').value='';
  document.getElementById('memoContent').value='';
  _renderMemoColorPicker(MEMO_COLORS[0]);
  document.getElementById('memoInputOverlay').style.display='flex';
  setTimeout(()=>document.getElementById('memoMarkerName').focus(),150);
}
...
function openMemoEdit(id){
  ...
  document.getElementById('memoSelectedText').textContent=`"${m.text.slice(0,80)}"`;
  document.getElementById('memoMarkerName').value=m.markerName||'';
  document.getElementById('memoContent').value=m.content||'';
  _renderMemoColorPicker(m.markerColor||MEMO_COLORS[0]);
  document.getElementById('memoInputOverlay').style.display='flex';
}

function _renderMemoColorPicker(selectedColor){
  const el=document.getElementById('memoColorPicker');
  if(!el)return;
  el.innerHTML=MEMO_COLORS.map(c=>
    `<div class="memo-color-dot${c===selectedColor?' selected':''}" style="background:${c};" data-color="${c}" onclick="_selectMemoColor('${c}')"></div>`
  ).join('');
}

function _selectMemoColor(c){
  document.querySelectorAll('#memoColorPicker .memo-color-dot').forEach(d=>{
    d.classList.toggle('selected',d.dataset.color===c);
  });
}

function _getSelectedMemoColor(){
  const sel=document.querySelector('#memoColorPicker .memo-color-dot.selected');
  return sel?sel.dataset.color:MEMO_COLORS[0];
}

function closeMemoInput(){
  document.getElementById('memoInputOverlay').style.display='none';
  _memoSavedRange=null;
}

function submitMemo(){
  const color=_getSelectedMemoColor();
  const markerName=document.getElementById('memoMarkerName').value.trim();
  const content=document.getElementById('memoContent').value.trim();
  if(_memoEditId){
    // 수정
    const m=_memos.find(x=>x.id===_memoEditId);
    if(m){m.markerColor=color;m.markerName=markerName;m.content=content;}
    // 하이라이트 색 업데이트
    const span=document.querySelector(`[data-memo-id="${_memoEditId}"]`);
    if(span){
      span.style.background=_hexAlpha(color,0.4);
  ...
  try{
    ...
    sel.removeAllRanges();
    sel.addRange(_memoSavedRange);
    const text=sel.toString().trim();
    // 선택된 텍스트를 span으로 감싸기
    const span=document.createElement('span');
    span.className='re-memo-span';
    ...
    const y=rect.top-edRect.top+editor.scrollTop;
    _memos.push({id,text,markerColor:color,markerName,content,resolved:false,y,spanEl:span});
  }catch(e){
    // surroundContents 실패 시 (선택이 여러 요소에 걸친 경우)
    const text=_memoSavedRange.toString().trim();
    const y=0;
    _memos.push({id,text:text,markerColor:color,markerName,content,resolved:false,y});
  }
  closeMemoInput();
  refreshMemoPanel();
}
...
function _renderMarkerPanel(){
  const el=document.getElementById('reMarkerList');
  const statusEl=document.getElementById('reMarkerStatus');
  if(!el)return;
  // 마커별 집계
  const map={};
  _memos.forEach(m=>{
    const key=m.markerColor+'|'+(m.markerName||'미분류');
    if(!map[key])map[key]={color:m.markerColor,name:m.markerName||'미분류',count:0,done:0};
    map[key].count++;
    if(m.resolved)map[key].done++;
  });
  el.innerHTML=Object.values(map).map(g=>`
    <div class="marker-row" onclick="_filterByMarker('${g.color}')">
      <div class="marker-dot" style="background:${g.color};"></div>
      <div style="flex:1;min-width:0;">
        <div style="font-weight:700;color:#1e3a5f;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${g.name}</div>
        <div style="font-size:.67rem;color:var(--t2);">${g.count}개 (완료 ${g.done})</div>
      </div>
    </div>
  `).join('')||'<div style="font-size:.72rem;color:var(--t3);text-align:center;padding:8px 0;">메모 없음</div>';

  if(statusEl){
    const total=_memos.length;
    const done=_memos.filter(m=>m.resolved).length;
    statusEl.innerHTML=`<div style="font-size:.7rem;color:var(--t2);">전체 ${total}개 · 완료 ${done}개</div>`;
  }
}

function _filterByMarker(color){
  // 해당 색상 메모만 하이라이트
  document.querySelectorAll('.re-memo-span').forEach(s=>{
    const id=s.dataset.memoId;
    const m=_memos.find(x=>x.id===id);
    if(m&&m.markerColor===color){
      s.scrollIntoView({behavior:'smooth',block:'center'});
      s.classList.add('active');
      setTimeout(()=>s.classList.remove('active'),1500);
    }
  });
}

function _renderMemoCards(){
  const panel=document.getElementById('reMemoPanel');
  const list=document.getElementById('reMemoList');
  if(!panel||!list)return;
  if(!_memos.length){
    list.innerHTML='<div style="padding:16px;font-size:.79rem;color:var(--t3);text-align:center;">메모가 없습니다</div>';
    list.style.position='relative';list.style.minHeight='60px';
    return;
  }
  const editor=document.getElementById('reEditor');
  ...
  _memos.forEach(m=>{
    if(m.spanEl){
      const sr=m.spanEl.getBoundingClientRect();
      if(edRect) m.y=sr.top-edRect.top;
    }
  });
  // 정렬
  const sorted=[..._memos].sort((a,b)=>a.y-b.y);
  list.style.position='relative';
  let minY=8;
  const CARD_H=80;
  const rects=[];
  sorted.forEach((m,i)=>{
    let y=Math.max(minY,m.y||8);
    // 겹침 방지
    rects.forEach(r=>{if(y<r.bottom+6)y=r.bottom+6;});
    rects.push({top:y,bottom:y+CARD_H});
    m._cardY=y;
    minY=y;
  });
  list.style.minHeight=(rects[rects.length-1]?.bottom+20||100)+'px';
  list.innerHTML=sorted.map(m=>`
    <div class="memo-card${m.resolved?' resolved':''}" id="mc_${m.id}" style="top:${m._cardY}px;border-left-color:${m.markerColor};"
      onclick="_activateMemoCard('${m.id}')">
      <div class="memo-card-marker" style="color:${m.markerColor};">
        <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${m.markerColor};margin-right:4px;"></span>
        ${m.markerName||'메모'}${m.resolved?' ✓':''}
      </div>
      <div class="memo-card-text">"${m.text.slice(0,50)}"</div>
      ${m.content?`<div class="memo-card-content">${m.content.slice(0,60)}</div>`:''}
      <div class="memo-card-actions">
        <button onclick="event.stopPropagation();toggleMemoResolved('${m.id}')">${m.resolved?'↩ 미완료':'✓ 완료'}</button>
        <button onclick="event.stopPropagation();openMemoEdit('${m.id}')">수정</button>
        <button onclick="event.stopPropagation();deleteMemo('${m.id}')" style="color:#E53935;">삭제</button>
      </div>
    </div>
  `).join('');
}

function _activateMemoCard(id){
  document.querySelectorAll('.memo-card').forEach(c=>c.style.boxShadow='');
  document.querySelectorAll('.re-memo-span').forEach(s=>s.classList.remove('active'));
  const card=document.getElementById('mc_'+id);
  if(card){card.style.boxShadow='0 0 0 2px var(--P)';card.scrollIntoView({behavior:'smooth',block:'nearest'});}
  const span=document.querySelector(`[data-memo-id="${id}"]`);
...
function toggleMemoResolved(id){
  ...
  if(m.spanEl)m.spanEl.classList.toggle('resolved',m.resolved);
  refreshMemoPanel();
}
...
function deleteMemo(id){
  ...
  if(m.spanEl){
    // span을 벗겨내고 텍스트만 유지
    const parent=m.spanEl.parentNode;
    while(m.spanEl.firstChild)parent.insertBefore(m.spanEl.firstChild,m.spanEl);
    parent.removeChild(m.spanEl);
  }
  _memos.splice(idx,1);
  refreshMemoPanel();
  toast('메모 삭제됨');
...
function openMemoManager(){
  _renderMemoManagerList();
  document.getElementById('memoManagerOverlay').style.display='flex';
}
function closeMemoManager(){
  document.getElementById('memoManagerOverlay').style.display='none';
}
function _renderMemoManagerList(){
  const el=document.getElementById('memoManagerList');
  if(!el)return;
  if(!_memos.length){
    el.innerHTML='<div style="color:var(--t2);font-size:.82rem;text-align:center;padding:12px 0;">메모가 없습니다</div>';
    return;
  }
  el.innerHTML=_memos.map(m=>`
    <div style="display:flex;align-items:flex-start;gap:8px;padding:10px;background:var(--bg);border-radius:8px;border-left:3px solid ${m.markerColor};">
      <div style="flex:1;">
        <div style="font-size:.79rem;font-weight:700;color:${m.markerColor};">
          ${m.markerName||'메모'}${m.resolved?' ✓':''}
        </div>
        <div style="font-size:.76rem;color:var(--t2);margin:2px 0;">"${m.text.slice(0,60)}"</div>
        ${m.content?`<div style="font-size:.78rem;color:var(--t);margin-top:4px;">${m.content}</div>`:''}
      </div>
      <div style="display:flex;flex-direction:column;gap:4px;">
        <button onclick="closeMemoManager();openMemoEdit('${m.id}')" style="font-size:.7rem;padding:2px 7px;border:1px solid var(--bd);border-radius:4px;background:#fff;cursor:pointer;">수정</button>
        <button onclick="deleteMemo('${m.id}');_renderMemoManagerList()" style="font-size:.7rem;padding:2px 7px;border:1px solid #FFCDD2;border-radius:4px;background:#FFF3F5;cursor:pointer;color:#E53935;">삭제</button>
      </div>
    </div>
  `).join('');
}
...
function reOnContextMenu(e){
  ...
  const text=sel.toString().trim();
  if(!text){_closeCtxMenu();return;}
  e.preventDefault();
  _closeCtxMenu();
  const range=sel.getRangeAt(0).cloneRange();
  const menu=document.createElement('div');
  menu.id='memoContextMenu';
  menu.style.left=e.clientX+'px';
  menu.style.top=e.clientY+'px';
  menu.innerHTML=`<button onclick="_ctxAddMemo()">📝 메모 추가</button>`;
  document.body.appendChild(menu);
  _ctxMenu={menu,text,range};
  setTimeout(()=>document.addEventListener('click',_closeCtxMenuOnClick),10);
}
function _ctxAddMemo(){
  if(!_ctxMenu)return;
  openMemoInput(_ctxMenu.text,_ctxMenu.range);
  _closeCtxMenu();
}
function _closeCtxMenu(){
  if(_ctxMenu){_ctxMenu.menu.remove();_ctxMenu=null;}
  document.removeEventListener('click',_closeCtxMenuOnClick);
}
function _closeCtxMenuOnClick(e){
  if(!e.target.closest('#memoContextMenu'))_closeCtxMenu();
}

/* ══════════════════════════════════════════════════════════
   이미지 의견 작성 패널 토글 + 개선
══════════════════════════════════════════════════════════ */
let _rdPanelsVisible=false;
function rdTogglePanels(){
  _rdPanelsVisible=!_rdPanelsVisible;
  const lp=document.getElementById('rdMarkerPanel');
  const rp=document.getElementById('rdMemoPanel');
  const btn=document.getElementById('btnRdPanels');
  if(lp)lp.style.display=_rdPanelsVisible?'block':'none';
  if(rp)rp.style.display=_rdPanelsVisible?'block':'none';
  if(btn)btn.classList.toggle('active',_rdPanelsVisible);
}

/* openRefDocModal — Supabase에서 직접 로드 */
  ...
  if(docId){
    ...
    try{
      ...
      if(row&&row.value){
        ...
        if(val&&val.images){_rdImages=val.images;renderRdImages();}
      }
    }catch(err){
      console.warn('이미지 의견 로드 실패:',err);
    }
  }
  _rdPanelsVisible=false;
  ['rdMarkerPanel','rdMemoPanel'].forEach(id=>{
...
/* renderRdImages — 7:3 비율 + 더블클릭 라이트박스 + 리사이저 */
/* renderRdImages → 새 버전으로 대체 */
...
function renderRdImages(){
  ...
  list.innerHTML=_rdImages.map((img,i)=>`
    <div class="rd-row" id="rdRow-${i}"
      ...
      <div class="rd-row-body" id="rdBody-${i}">
        <div class="rd-img-side" id="rdImgSide-${i}" style="flex:7;">
          <img src="${img.data}" alt="이미지 ${i+1}"
            style="max-width:100%;max-height:400px;object-fit:contain;display:block;cursor:zoom-in;"
            title="더블클릭 → 원본 크기"
            ondblclick="rdOpenLightbox(this.src)">
          <div class="rd-img-side-overlay" onclick="_rdSlotIdx=${i};document.getElementById('rdFileInSlot').click()">
            <span>🔄 클릭하여 이미지 교체</span>
          </div>
        </div>
        <div class="rd-resizer" id="rdResizer-${i}" onmousedown="rdStartResize(event,${i})" title="드래그하여 비율 조절"></div>
        <div class="rd-txt-side" id="rdTxtSide-${i}" style="flex:3;">
          ...
          <textarea id="rdTxtArea-${i}" class="rd-txt-area"
            ...
            placeholder="이 이미지에 대한 의견, 분석, 메모 등을 작성하세요."
...
/* rd 리사이저 드래그 */
let _rdRszState=null;
function rdStartResize(e,i){
  e.preventDefault();
  const img=document.getElementById(`rdImgSide-${i}`);
  const txt=document.getElementById(`rdTxtSide-${i}`);
  const body=document.getElementById(`rdBody-${i}`);
  const rsz=document.getElementById(`rdResizer-${i}`);
  if(!img||!txt||!body)return;
  _rdRszState={imgEl:img,txtEl:txt,rszEl:rsz,startX:e.clientX,imgW:img.offsetWidth,totalW:body.offsetWidth};
  rsz&&rsz.classList.add('active');
  document.body.style.userSelect='none';
  document.body.style.cursor='col-resize';
  function onMove(ev){
    if(!_rdRszState)return;
    const dx=ev.clientX-_rdRszState.startX;
    const newW=Math.max(80,Math.min(_rdRszState.totalW-80,_rdRszState.imgW+dx));
    _rdRszState.imgEl.style.flex='none';
    _rdRszState.imgEl.style.width=newW+'px';
    _rdRszState.txtEl.style.flex='none';
    _rdRszState.txtEl.style.width=(_rdRszState.totalW-newW-5)+'px';
  }
  function onUp(){
    if(_rdRszState){_rdRszState.rszEl&&_rdRszState.rszEl.classList.remove('active');_rdRszState=null;}
...
/* 라이트박스 */
function rdOpenLightbox(src){
  const lb=document.getElementById('rdLightbox');
  const img=document.getElementById('rdLightboxImg');
  if(!lb||!img)return;
  img.src=src;
...
function saveRefDoc(){
  ...
  saveAll();renderDocList();closeRefDocModal();toast('이미지 의견 작성 문서가 저장되었습니다.');
}
