...
const MARKER_COLORS=['#D91F3E','#FF5252','#FF7A00','#FFCA28','#2E9E50','#00BFA5','#0064FF','#40C4FF','#006064','#9C27B0','#CE93D8','#673AB7','#E91E63','#F48FB1','#795548','#A1887F','#607D8B','#BF360C','#1B5E20','#01579B','#4A148C','#880E4F','#37474F','#191F28','#FF8F00','#558B2F','#00838F','#283593','#6A1B9A','#AD1457'];
let _savedSel=null;
...
let _memoTemplates=[];    // 메모 내용 (최대 10개)
let _memoNameHistory=[];  // 메모명 히스토리 (최대 10개)
let _markerPresets=[];    // 색+이름 프리셋
...
async function _loadMemoData(){
  try{
    ...
    const r3=await _sbRest.selectOne('site_data','memo_marker_presets');
...
async function _saveMemoData(){
  try{
    ...
    await _sbRest.upsert('site_data',{key:'memo_marker_presets',value:_markerPresets,updated_at:new Date().toISOString()});
  }catch{}
}
...
async function openMemoInput(){
  ...
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
  _renderMemoNameChips();
  _renderMemoTemplateChips();
  _renderMarkerPresetChips();
...
function _renderMemoTemplateChips(){
  ...
  if(!_memoTemplates.length){el.innerHTML='<span style="font-size:.7rem;color:#bbb;">저장된 내용 없음</span>';return;}
...
function applyMarkerPreset(color,encodedName){
...
function closeMemoInput(){document.getElementById('memoInputOverlay').style.display='none';_memoSavedRange=null;_memoSavedText='';}
...
function saveMemoToSelection(){
  ...
  const markerNameInp=document.getElementById('memoMarkerName');
  _selMarkerName=markerNameInp?markerNameInp.value.trim():'';
  ...
  if(_selMarkerColor&&_selMarkerName){
    const exists=_markerPresets.some(p=>p.color===_selMarkerColor&&p.name===_selMarkerName);
    if(!exists){_markerPresets.push({color:_selMarkerColor,name:_selMarkerName});if(_markerPresets.length>10)_markerPresets=_markerPresets.slice(-10);}
  }
  _saveMemoData();
  const now=new Date();
  ...
  span.dataset.memoid=id;span.dataset.memoName=name||'메모';span.dataset.memoDesc=desc;span.dataset.memoDate=dateStr;span.dataset.memoResolved='false';
  span.dataset.memoMc=_selMarkerColor;span.dataset.memoMn=_selMarkerName;
...
const CARD_MIN_H=90;  // 카드 최소 높이 (겹침 방지용)
const CARD_GAP=6;
...
function refreshMemoPanel(){
  ...
  const markerPanel=document.getElementById('reMarkerPanel');
  const markerList=document.getElementById('reMarkerList');
  ...
  if(!memos.length){panel.style.display='none';if(markerPanel)markerPanel.style.display='none';return;}
  panel.style.display='block';
  // 마커 집계 (색상코드 제거, 마커명만 표시)
  const markerMap={};
  memos.forEach(m=>{const c=m.dataset.memoMc,n=m.dataset.memoMn;if(c){const key=c+'|'+(n||'');if(!markerMap[key])markerMap[key]={color:c,name:n||'마커',count:0};markerMap[key].count++;}});
  if(markerPanel){
    const hasMarkers=Object.keys(markerMap).length>0;
    markerPanel.style.display=hasMarkers?'block':'none';
    if(hasMarkers&&markerList)markerList.innerHTML=Object.values(markerMap).map(mk=>`<div class="marker-row"><div class="marker-dot" style="background:${mk.color};"></div><div class="marker-label">${mk.name}</div><div class="marker-count">${mk.count}</div></div>`).join('');
  }
  // Y좌표 계산
  const edRect=editor.getBoundingClientRect();
  const scroll=wrap?wrap.scrollTop:0;
  const HEADER_H=40; // "메모 목록" 헤더 높이
  ...
  memos.forEach((m,i)=>{
    ...
    y=Math.max(nextMinY,y);
    nextMinY=y+CARD_MIN_H+CARD_GAP;
  ...
  positions.forEach(({id,y,m})=>{
    validIds.add('mc_'+id);
    const isResolved=m.dataset.memoResolved==='true';
    const name=m.dataset.memoName||'메모';
    const desc=m.dataset.memoDesc||'';
    const date=m.dataset.memoDate||'';
    ...
    card.className='memo-card'+(isResolved?' resolved':'');
    card.style.top=y+'px';
    ...
    card.innerHTML=
      ...
      (desc||isResolved?`<div class="memo-card-desc">${isResolved?'<span class="memo-done-badge">✓ 완료</span>'+(desc?'<br><span style="color:#6b7280;font-size:.74rem;">'+desc+'</span>':''):desc}</div>`:'')+
      `<div class="memo-card-btns">`+
      (!isResolved?`<button class="mresolve" onclick="event.stopPropagation();toggleResolveMemo('${id}')">✓</button>`:
       `<button class="munresolve" onclick="event.stopPropagation();toggleResolveMemo('${id}')">↩</button>`)+
      `<button onclick="event.stopPropagation();editMemoCard('${id}')">수정</button>`+
      `<button class="mdel" onclick="event.stopPropagation();deleteMemoCard('${id}')">삭제</button>`+
      `</div>`;
    card.onclick=function(e){if(!e.target.closest('button'))highlightMemoCard(m);};
  });
...
function editMemoCard(id){
  const el=document.querySelector(`[data-memoid="${id}"]`);if(!el)return;
  const nName=prompt('메모 이름:',el.dataset.memoName||'');if(nName===null)return;
  const nDesc=prompt('메모 내용:',el.dataset.memoDesc||'');if(nDesc===null)return;
  el.dataset.memoName=nName;el.dataset.memoDesc=nDesc;
  setTimeout(refreshMemoPanel,50);
}
...
/* ════════════════════════════════════════════════
   표 열 리사이즈 — Google Sheets 방식
   
   핵심: 에디터 위에 겹치는 투명 오버레이 div에
   각 표의 열 경계마다 드래그 핸들을 배치.
   
   오버레이는 pointer-events:none이고
   핸들만 pointer-events:all.
   → contenteditable과 완전히 독립.
   → 마우스가 핸들 위에서만 col-resize 커서 표시.
════════════════════════════════════════════════ */
let _colDragState=null;

function _getOverlay(){return document.getElementById('tblColOverlay');}

function _updateTableColHandles(){
  const overlay=_getOverlay();const editor=document.getElementById('reEditor');const wrap=document.getElementById('reEditorWrap');
  ...
  editor.querySelectorAll('table').forEach(table=>{
    ...
    // 첫 행 기준으로 열 경계 X좌표 계산
    const firstRowCells=Array.from(rows[0].querySelectorAll('td,th'));
    firstRowCells.forEach((cell,ci)=>{
      if(ci===firstRowCells.length-1)return; // 마지막 열 우측은 제외
      const cellRect=cell.getBoundingClientRect();
      // 오버레이 내 X = 셀 오른쪽 경계 - wrap 왼쪽
      const handleX=cellRect.right-wrapRect.left;
      const h=document.createElement('div');
      h.className='tbl-col-handle';
      h.style.left=handleX+'px';
      h.style.top=tblTop+'px';
      h.style.height=tblH+'px';
      h.dataset.ci=ci;
      h.dataset.tableId=table.dataset.tblid||'';
      // 이 열의 모든 셀 참조 저장
      h.addEventListener('mousedown',function(e){
        e.preventDefault();e.stopPropagation();
        const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
        _colDragState={cells:colCells,startX:e.clientX,startW:cell.offsetWidth,handle:h};
...
function _initTableResize(){
  ...
  document.addEventListener('mousemove',function(e){
    if(_colDragState){
      const newW=Math.max(1,_colDragState.startW+(e.clientX-_colDragState.startX));
      _colDragState.cells.forEach(c=>{c.style.width=newW+'px';c.style.minWidth='0';c.style.maxWidth='none';c.style.overflow='hidden';});
      // 핸들 위치도 실시간 갱신
      const firstCell=_colDragState.cells[0];
      if(firstCell&&wrap){const wr=wrap.getBoundingClientRect();const cr=firstCell.getBoundingClientRect();_colDragState.handle.style.left=(cr.right-wr.left)+'px';}
    }
    if(_rowDrag){
  ...
  document.addEventListener('mouseup',function(){
    if(_colDragState){
      ...
      setTimeout(_updateTableColHandles,50);
    }
    if(_rowDrag){_rowDrag=null;editor.style.cursor='';}
  });
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    const wrap=document.getElementById('reEditorWrap');
    if(wrap&&!wrap._scrollSet){wrap._scrollSet=true;wrap.addEventListener('scroll',function(){debouncedMemoPanel();});}
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
