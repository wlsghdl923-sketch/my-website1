...
function _bindCtxMenu() {
  ...
  /* 에디터 클릭 → 표 우클릭 메뉴 위치기억용 */
  editor.addEventListener('contextmenu', function(e) {
    _lastCtxTd = e.target.closest('td,th') || null;
  }, true);
}
...
function _bindTableCtxMenu() {
  const editor = document.getElementById('reEditor');
  if (!editor || editor._tblBound) return;
  editor._tblBound = true;
  editor.addEventListener('contextmenu', function(e) {
...
function _tblMergeSelected(table, fallbackTd) {
  ...
  if (sels.length < 2) {
    toast('병합할 셀을 Shift+클릭으로 2개 이상 선택하세요.');
    return;
  }
  ...
  tds.forEach(td => {
    ...
    minR = Math.min(minR, ri); maxR = Math.max(maxR, ri + rs - 1);
    minC = Math.min(minC, ci); maxC = Math.max(maxC, ci + cs - 1);
  });
  // 첫 번째 셀 확장
  const firstTd = tds[0];
  let mergedContent = '';
  tds.forEach(td => {
    if (td !== firstTd) {
      if (td.textContent.trim()) mergedContent += ' ' + td.textContent.trim();
      td.remove();
    }
  });
  firstTd.colSpan = maxC - minC + 1;
  firstTd.rowSpan = maxR - minR + 1;
  if (mergedContent) firstTd.textContent += mergedContent;
  ...
  if (rs > 1) {
    ...
    for (let r = 1; r < rs; r++) {
      if (rows[ri + r]) {
        ...
        nc.style.cssText = 'padding:8px 12px;border:1px solid #E5E8EB;';
        const refCells = rows[ri + r].querySelectorAll('td,th');
        if (refCells[ci]) refCells[ci].before(nc);
        else rows[ri + r].appendChild(nc);
      }
    }
  }
  toast('셀이 분리되었습니다.');
...
function _bindTblResize(editor) {
  ...
  editor.addEventListener('mousedown', e => {
    ...
    // 하단 경계 5px 이내 → 행 리사이즈
    else if (Math.abs(e.clientY - rect.bottom) < 5) {
      e.preventDefault();
...
function reInsertDivider() {
  const styles = [
    { cls: 'dv-solid', label: '실선 ───' },
    { cls: 'dv-dashed', label: '대시 - - -' },
    { cls: 'dv-dotted', label: '점선 · · ·' },
    { cls: 'dv-double', label: '이중선 ═══' },
    { cls: 'dv-gradient', label: '그라데이션' },
    { cls: 'dv-fade', label: '페이드' },
    { cls: 'dv-stars', label: '✦ 별 ✦' },
  ];
  let choice = prompt('구분선 스타일 번호를 선택하세요:\n' + styles.map((s,i) => `${i+1}. ${s.label}`).join('\n'), '1');
  if (!choice) return;
