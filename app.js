...
function _applyReMode(){
  ...
  if(_reViewMode){
    ...
    editor.style.userSelect='text'; // 텍스트 선택은 허용 (복사용)
...
function _buildShareUrl(){
  ...
  if(noPw){
    url.searchParams.set('open', '1'); // 비밀번호 없이 접근
  }else{
    url.searchParams.delete('open');
  }
  if(customPw && customPwVal){
    url.searchParams.set('spw', customPwVal); // 별도 비밀번호
    url.searchParams.delete('open');
  }else{
