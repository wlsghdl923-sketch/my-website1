...
/* ── PDF 공통: HTML → body/style 추출 ── */
function _pdfExtractHtml(content,contentWidth){
  ...
  const baseStyle=`body{font-family:'Noto Sans KR',sans-serif;padding:32px;line-height:1.8;color:#191F28;font-size:15px;margin:0;word-break:break-word;}h1,h2,h3{font-weight:800;letter-spacing:-.02em;margin:1.2em 0 .5em;}h1{font-size:1.8rem;}h2{font-size:1.4rem;}h3{font-size:1.1rem;}p{margin:.6em 0;}img{max-width:100%;border-radius:8px;}hr{border:none;border-top:1px solid #E5E8EB;margin:1.5em 0;}table{border-collapse:collapse;width:100%;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}`;
  return {bodyHtml, styleStr, baseStyle};
}
...
function _pdfPreviewRender(){
  ...
  const pageBodyH=contentAreaH/scale; // 원본 크기 기준 한 페이지 높이

  const {bodyHtml,styleStr,baseStyle}=_pdfExtractHtml(content,contentWidth);

  // 숨겨진 측정 컨테이너
  const measurer=document.createElement('div');
  measurer.style.cssText=`position:fixed;left:-9999px;top:0;width:${contentWidth}px;z-index:-1;`;
  measurer.innerHTML=`<style>${baseStyle}${styleStr}</style>${bodyHtml}`;
  document.body.appendChild(measurer);

  // 이미지 로딩 + 폰트 대기
  const imgs=measurer.querySelectorAll('img');
  const imgPromises=[...imgs].map(img=>new Promise(r=>{if(img.complete)r();else{img.onload=r;img.onerror=r;}}));
  Promise.allSettled(imgPromises).then(()=>{
    setTimeout(()=>{
      const totalH=measurer.scrollHeight;
      const pageCount=Math.max(1,Math.ceil(totalH/pageBodyH));

      const container=document.getElementById('pdfPreviewPages');
      container.innerHTML='';
      const info=document.getElementById('pdfPageInfo');
      if(info) info.textContent=`${pageCount}페이지 · ${pwMm}×${phMm}mm`;
      ...
      for(let p=0;p<pageCount;p++){
        ...
        clipper.style.cssText=`width:${contentAreaW}px;height:${contentAreaH}px;overflow:hidden;position:relative;`;

        // 축소된 콘텐츠 (원본 너비로 렌더링 → scale 축소)
        const inner=document.createElement('div');
        inner.style.cssText=`width:${contentWidth}px;transform:scale(${scale});transform-origin:top left;position:absolute;top:${-p*contentAreaH}px;left:0;`;
        inner.innerHTML=`<style>${baseStyle}${styleStr}</style>${bodyHtml}`;
  ...
  const tryLoad=async(urls)=>{
    for(const url of urls){
      try{await loadScript(url);return true;}catch(e){
        const el=document.querySelector(`script[src="${url}"]`);if(el)el.remove();
      }
    }
    return false;
  };
  try{
    const h2cOk=await tryLoad([
      'https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js',
      'https://unpkg.com/html2canvas@1.4.1/dist/html2canvas.min.js',
      'https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js'
    ]);
    if(!h2cOk) throw new Error('html2canvas');
    const jpOk=await tryLoad([
    ...
    console.error('PDF 라이브러리 로드 실패:',e);
    toast('PDF 라이브러리 로드 실패. 브라우저 인쇄 방식으로 전환합니다.');
    return false;
  }
}
...
async function execPdfExport(){
  ...
  progText.textContent='라이브러리 로드 중...';
  ...
  progText.textContent='문서 렌더링 중...';progBar.style.width='15%';
  ...
  container.style.cssText=`position:fixed;left:-9999px;top:0;width:${contentWidth}px;background:#fff;z-index:-1;`;
  container.innerHTML=`<style>${baseStyle}${styleStr}</style>${bodyHtml}`;
  ...
  try{
    ...
    toast('PDF 생성 중 오류가 발생했습니다. 브라우저 인쇄로 전환합니다.');
    _pdfFallbackPrint(content,title,pageSize,orient,contentWidth,margin);
...
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700&display=swap" rel="stylesheet">
<style>${baseStyle}${styleStr}
@media print{@page{size:${sz};margin:${margin}mm;}body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}
body{max-width:${contentWidth}px;margin:0 auto;}
</style><title>${title}</title></head><body>${bodyHtml}</body></html>`;
