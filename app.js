...
function _bindTblResize(editor) {
  ...
  // ★ 체크박스: change 이벤트 위임 (click보다 신뢰성 높음)
  editor.addEventListener('change', e => {
    if (e.target.type === 'checkbox' && e.target.closest('.re-check-item')) {
      const li = e.target.closest('.re-check-item');
...
async function openRichEditor(docId=''){
  ...
  if(docId){
    ...
    // 메모가 있으면 패널 자동 표시
    if (_memos.length > 0 && !_rePanelsOn) {
      reTogglePanels();
    }
  }else{
    titleIn.value='';
...
function _renderCalloutEmojis() {
  ...
  if (!grid) return;
  grid.innerHTML = _calloutEmojis.map(e =>
    `<div class="co-emoji-btn${e===_calloutSelEmoji?' sel':''}" onclick="pickCalloutEmoji(this,'${e}')">${e}</div>`
  ).join('');
}
function pickCalloutEmoji(el, emoji) {
  _calloutSelEmoji = emoji;
...
function removeCalloutEmoji() {
  if (_calloutEmojis.length <= 1) { toast('최소 1개는 있어야 합니다.'); return; }
  const idx = _calloutEmojis.indexOf(_calloutSelEmoji);
...
function _renderCalloutPreview() {
  ...
  el.innerHTML = `<div style="border-left:4px solid ${_calloutSelColor};background:${bgColor};border-radius:0 10px 10px 0;padding:14px 18px;display:flex;gap:10px;align-items:flex-start;line-height:1.7;">
    <span style="font-size:1.3rem;">${_calloutSelEmoji}</span>
...
function execCalloutInsert() {
  ...
  const html = `<div class="re-callout" contenteditable="false" style="border-left-color:${_calloutSelColor};background:${bgColor};">
    <span class="re-callout-icon" contenteditable="false">${_calloutSelEmoji}</span>
