...
<\/script>
    ...
    const rowsHtml=(stored.pages||[]).map(page=>{
      ...
      const imgCells=Array.from({length:maxImgCols},(_, ci)=>{
        ...
        return`<td class="rv-td rv-img-td"><img src="${img.data}" alt=""></td>`;
      }).join('');
...
async function openRefDocModal(docId=''){
  ...
  _rdData={leftColumns:[{id:'_lc0',name:'페이지'}],markers:[],pages:[{id:'_p0',leftValues:['1'],images:[{data:''}],opinion:'',tags:[]}],splitRatio:70};
  if(docId){
    ...
    if(stored&&typeof stored==='object'){
      if(stored.leftColumns){
        ...
        _rdData.pages=stored.pages.map((p,i)=>({
          ...
          images:(p.images||[]).map(img=>({data:img.data||''})),
...
function rdAddImgCol(pi){
  _rdSync();
  const p=_rdData.pages[pi];
  if(p.images.length>=3){toast('이미지는 페이지당 최대 3개입니다.');return;}
  p.images.push({data:''});
  renderRdEditor();
}
function rdDelImgCol(pi,ci){
  _rdSync();
  const p=_rdData.pages[pi];
  if(p.images.length<=1){toast('최소 1개의 이미지 영역이 필요합니다.');return;}
  p.images.splice(ci,1);
  renderRdEditor();
}
...
function rdAddPage(){
  ...
  _rdData.pages.push({id:uid(),leftValues:lv,images:[{data:''}],opinion:'',tags:[]});
  renderRdEditor();
...
function handleRdImageSlotNew(e){
  ...
  reader.onload=ev=>{
    const img=new Image();img.onload=async()=>{
      ...
      if(_rdData.pages[pageIdx]&&_rdData.pages[pageIdx].images[colIdx]){
        _rdData.pages[pageIdx].images[colIdx].data=url;
        renderRdEditor();
        toast('업로드 완료');
      }
...
function rdAddMarker(){
  const color=document.getElementById('rdNewMarkerColor').value;
  const name=document.getElementById('rdNewMarkerName').value.trim();
  if(!name){toast('마커 이름을 입력해주세요.');return;}
  _rdData.markers.push({id:uid(),name,color});
...
function rdEditMarkerColor(mi,v){_rdData.markers[mi].color=v;renderRdMarkerList();}
function renderRdMarkerList(){
  const list=document.getElementById('rdMarkerList');
  if(!_rdData.markers.length){list.innerHTML='<p style="font-size:.8rem;color:var(--t3);padding:8px 0;">등록된 마커가 없습니다.</p>';return;}
  list.innerHTML=_rdData.markers.map((m,mi)=>`
    <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--bd);">
      <input type="color" value="${m.color}" onchange="rdEditMarkerColor(${mi},this.value)" style="width:30px;height:26px;border:1px solid var(--bd);border-radius:4px;cursor:pointer;">
      <span style="width:18px;height:18px;border-radius:50%;background:${m.color};flex-shrink:0;"></span>
      <input type="text" value="${m.name}" oninput="rdEditMarkerName(${mi},this.value)" style="flex:1;padding:5px 8px;border:1px solid var(--bd);border-radius:5px;font-size:.82rem;font-family:inherit;">
      <button onclick="rdDelMarker(${mi})" style="padding:3px 8px;border:1px solid #FFCDD2;border-radius:5px;background:#fff;color:#E53935;cursor:pointer;font-size:.72rem;font-weight:700;">삭제</button>
    </div>`).join('');
}
...
function rdStartResize(e){
  ...
  const lcEl=document.querySelector('.rd2-left-zone');
  ...
  const onMove=(ev)=>{
    ...
    document.querySelectorAll('.rd2-img-zone').forEach(el=>el.style.flex='0 0 '+_rdData.splitRatio+'%');
    document.querySelectorAll('.rd2-txt-zone').forEach(el=>el.style.flex='1');
  };
...
function rdHandleOpinionInput(e,pi){
  ...
  const block=node.closest('div,p,li,h1,h2,h3,blockquote')||el;
  ...
  if(/\*\*(.+?)\*\*/.test(fullText)){
    block.innerHTML=fullText.replace(/\*\*(.+?)\*\*/g,'<b>$1</b>');
    _rdPlaceCaret(block);
  }else if(/(?<!\*)\*([^*]+?)\*(?!\*)/.test(fullText)){
    block.innerHTML=fullText.replace(/(?<!\*)\*([^*]+?)\*(?!\*)/g,'<i>$1</i>');
    _rdPlaceCaret(block);
  }else if(/`([^`]+?)`/.test(fullText)){
    block.innerHTML=fullText.replace(/`([^`]+?)`/g,'<code style="background:#f0f0f0;padding:1px 4px;border-radius:3px;font-size:.88em;">$1</code>');
    _rdPlaceCaret(block);
  }
}
...
function renderRdEditor(){
  ...
  const lcHeaders=lc.map((col,li)=>`
    <div class="rd2-lc-hdr-cell">
  ...
  const headerHtml=`<div class="rd2-header-row">
    <div class="rd2-left-zone">
      ${lcHeaders}
      ${lc.length<3?`<button class="rd2-lc-add-btn" onclick="rdAddLeftCol()" title="왼쪽 영역 추가">＋</button>`:''}
    </div>
    <div class="rd2-img-zone" style="flex:0 0 ${split}%;"><span class="rd2-hdr-label">이미지</span></div>
    <div class="rd2-resize-handle" onmousedown="rdStartResize(event)" ontouchstart="rdStartResize(event)"></div>
    <div class="rd2-txt-zone" style="flex:1;"><span class="rd2-hdr-label">의견</span></div>
  </div>`;
  ...
  const pagesHtml=_rdData.pages.map((page,pi)=>{
    ...
    const lcCells=lc.map((col,li)=>`
      <div class="rd2-lc-val-cell">
    ...
    const imgsHtml=page.images.map((img,ci)=>{
      const imgContent=img.data
        ?`<img src="${img.data}" alt="" style="width:100%;height:100%;object-fit:contain;">`
        :`<div class="rd2-img-placeholder" onclick="rdClickImgSlot(${pi},${ci})">＋ 이미지</div>`;
      return`<div class="rd2-img-cell">
        <div class="rd2-img-box" ${img.data?`onclick="rdClickImgSlot(${pi},${ci})"`:''}>
          ${imgContent}
        </div>
    ...
    const toolbar=`<div class="rd2-op-toolbar">
      ...
      <button onmousedown="event.preventDefault()" onclick="rdFmt(${pi},'strikeThrough')" title="취소선"><s>S</s></button>
      <span class="rd2-tb-sep"></span>
      <button onmousedown="event.preventDefault()" onclick="rdFmt(${pi},'formatBlock','<h1>')" title="제목1">H1</button>
      <button onmousedown="event.preventDefault()" onclick="rdFmt(${pi},'formatBlock','<h2>')" title="제목2">H2</button>
      <button onmousedown="event.preventDefault()" onclick="rdFmt(${pi},'formatBlock','<h3>')" title="제목3">H3</button>
      ...
      <select onmousedown="event.stopPropagation()" onchange="rdFmt(${pi},'foreColor',this.value);this.selectedIndex=0;" title="글자색">
        ...
        <option value="#191F28" style="color:#191F28">■ 검정</option>
        <option value="#D91F3E" style="color:#D91F3E">■ 빨강</option>
        <option value="#0064FF" style="color:#0064FF">■ 파랑</option>
        <option value="#2E9E50" style="color:#2E9E50">■ 초록</option>
        <option value="#FF7A00" style="color:#FF7A00">■ 주황</option>
        <option value="#9C27B0" style="color:#9C27B0">■ 보라</option>
      </select>
    </div>`;
    ...
    return`<div class="rd2-page-row" id="rdPageRow-${pi}">
      ...
      <div class="rd2-img-zone" style="flex:0 0 ${split}%;display:flex;align-items:stretch;">
        ${imgsHtml}
        ${addImgBtn}
      </div>
      <div class="rd2-resize-handle rd2-rh-passive"></div>
      <div class="rd2-txt-zone" style="flex:1;">
        ${markerArea}
        ${toolbar}
        <div class="rd2-opinion-area" id="rdOpinion-${pi}" contenteditable="true"
          ...
          data-placeholder="의견을 작성하세요. (# 제목, - 목록, > 인용, **굵게**, *기울임*)">${page.opinion||''}</div>
      </div>
    </div>`;
  }).join('');
