...
function checkMarkdownShortcut(){
  ...
  if(lineText==='---'||lineText==='---\n'){
    const block=node.parentElement;
    if(block){block.innerHTML='';document.execCommand('insertHTML',false,'<hr><p><br></p>');}
    return;
  }
  // - 또는 * → unordered list
  if(/^[-*]\s$/.test(lineText)){
...
function togglePanels(){
  ...
  if(_panelsForceOn){
    const mp=document.getElementById('reMarkerPanel');
    const rp=document.getElementById('reMemoPanel');
    const lr=document.getElementById('markerPanelResizer');
    const rr=document.getElementById('memoPanelResizer');
    if(mp)mp.style.display='block';if(rp)rp.style.display='block';
    if(lr)lr.style.display='block';if(rr)rr.style.display='block';
  }else{
    refreshMemoPanel(); // 메모 있으면 유지, 없으면 숨김
  }
}
...
function refreshMemoPanel(){
  ...
  if(!memos.length&&!_panelsForceOn){
    ...
    if(lr)lr.style.display='none';if(rr)rr.style.display='none';
    return;
  }
  ...
  if(mPanel){
    ...
    if(mList){
      const rows=Object.values(markerMap).map(mk=>`<div class="marker-row"><div class="marker-dot" style="background:${mk.color};"></div><div class="marker-label">${mk.name}</div><div class="marker-count">${mk.count}</div></div>`).join('');
      mList.innerHTML=`<div style="background:#DBEAFE;border-radius:6px;padding:4px 7px;margin-bottom:5px;font-size:.7rem;color:#1e40af;font-weight:700;text-align:center;">전체 ${total}개</div>`+rows;
    }
  ...
  memos.forEach((m,i)=>{
    ...
    card.id='mc_'+id;card.style.borderLeftColor=borderColor;
    ...
    card.innerHTML=
      `<div class="memo-card-header"><span class="memo-card-time">${date}</span></div>`+
      markerHtml+
...
async function openMemoManager(){
  await _loadMemoData();
  _initMmgrPalette();_renderMmgrList();
...
function _renderMmgrTplList(){
  ...
  el.innerHTML=_memoTemplates.map((t,i)=>`<div class="mmgr-item" style="justify-content:space-between;"><div style="flex:1;font-size:.82rem;color:#1e3a5f;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-right:8px;">${t}</div><button class="mbtn-c" style="padding:2px 8px;font-size:.72rem;color:#E53935;flex-shrink:0;" onclick="deleteMemoTemplate(${i})">삭제</button></div>`).join('');
}
...
/* ═══════════════════════════════════════
   ...
   핵심: document에서 capture=true로 mousedown을 잡아
   표 셀의 오른쪽 경계(±8px)에서 드래그 시작.
   contenteditable의 기본 동작보다 먼저 실행됨.
   
   행 리사이즈: 셀 하단 6px 감지
═══════════════════════════════════════ */
let _colRsz=null, _rowRsz=null;
let _tblRszInited=false;

function _initTableColResize(editor){
  if(_tblRszInited)return;_tblRszInited=true;
  const EDGE=8; // 경계 감지 픽셀
  // capture phase — contenteditable보다 먼저
  document.addEventListener('mousedown',function(e){
    ...
    const table=cell.closest('table');const rows=table?Array.from(table.querySelectorAll('tr')):[];
    const cells=Array.from(cell.parentElement.querySelectorAll('td,th'));
    const ci=cells.indexOf(cell);
    // 오른쪽 경계 (현재 셀, 마지막 열 제외)
    if(x>=rect.width-EDGE&&ci<cells.length-1){
      e.preventDefault();e.stopPropagation();
      const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
      _colRsz={cells:colCells,startX:e.clientX,startW:cell.offsetWidth};
      document.body.style.cursor='col-resize';document.body.style.userSelect='none';
      return;
    }
    // 왼쪽 경계 (이전 열, ci>0)
    if(x<=EDGE&&ci>0){
      ...
      const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci-1]).filter(Boolean);
      _colRsz={cells:colCells,startX:e.clientX,startW:prevCell.offsetWidth};
      document.body.style.cursor='col-resize';document.body.style.userSelect='none';
      return;
    }
    // 하단 경계 (행 높이)
    if(y>=rect.height-6){
      ...
      _rowRsz={tr,startY:e.clientY,startH:tr.offsetHeight};
      document.body.style.cursor='row-resize';document.body.style.userSelect='none';
    }
  },true); // ← capture=true

  // 커서 변경 (mousemove preview)
  document.addEventListener('mousemove',function(e){
    if(_colRsz){
      const dx=e.clientX-_colRsz.startX;
      const newW=Math.max(1,_colRsz.startW+dx);
      _colRsz.cells.forEach(c=>{c.style.width=newW+'px';c.style.minWidth='0';c.style.maxWidth='none';});
      return;
    }
    if(_rowRsz){
      const newH=Math.max(10,_rowRsz.startH+(e.clientY-_rowRsz.startY));
      _rowRsz.tr.style.height=newH+'px';
      Array.from(_rowRsz.tr.querySelectorAll('td,th')).forEach(c=>{c.style.height=newH+'px';});
      return;
    }
    // 커서 미리보기
    if(!editor.contains(e.target))return;
    ...
    const cells=Array.from(cell.parentElement.querySelectorAll('td,th'));
    const ci=cells.indexOf(cell);
    if((x>=rect.width-EDGE&&ci<cells.length-1)||(x<=EDGE&&ci>0))editor.style.cursor='col-resize';
    else if(y>=rect.height-6)editor.style.cursor='row-resize';
    else editor.style.cursor='';
  });

  document.addEventListener('mouseup',function(){
    if(_colRsz){_colRsz=null;document.body.style.cursor='';document.body.style.userSelect='';}
    if(_rowRsz){_rowRsz=null;document.body.style.cursor='';document.body.style.userSelect='';}
    if(editor)editor.style.cursor='';
  });
}

function _initRowResize(editor){/* 통합됨 */}
...
async function openRichEditor(docId=''){
  ...
  if(docId){
    ...
    reUpdateToolbar();setTimeout(()=>{_initImgResize();_initRowResize(editor);initColorPalettes();_initMemoMarkerPalette();},300);
