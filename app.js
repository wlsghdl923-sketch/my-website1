...
function tblCmd(cmd){
  ...
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;min-width:20px;';
  const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;min-width:20px;';
...
/* ── 표 선 색상: 기본값 채도/명도와 비슷한 파스텔 계열 30가지 + 추가 15가지 = 45가지 ── */
// 기본값 #E5E8EB = 연한 회청색 (HSL ~210, 10%, 91%)
// 추가 15가지: 비슷한 밝기(85~93%)의 파스텔 색상들
const TBL_BORDER_COLORS=[
  ...
  // 신규 15가지: #E5E8EB와 비슷한 채도·명도의 파스텔 계열
...
/* ══ 표 선 드래그 리사이즈 — 최소 너비 20px, 감도 1:1 ══ */
let _tblRsz=null;
function _initTableResize(){
  ...
  document.addEventListener('mousemove',function(e){
    if(!_tblRsz)return;
    if(_tblRsz.type==='col'){
      // 1:1 감도 — dx 그대로 적용, 최소 20px
      const dx=e.clientX-_tblRsz.startX;
      const newW=Math.max(MIN_COL,_tblRsz.startW+dx);
      _tblRsz.cell.style.width=newW+'px';
      _tblRsz.cell.style.minWidth=newW+'px';
    }else if(_tblRsz.type==='row'){
      const dy=e.clientY-_tblRsz.startY;
      const newH=Math.max(MIN_ROW,_tblRsz.startH+dy);
...
.tbl-sel{outline:none!important;}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(html){
        ...
        const clean=html.replace(/<aside[^>]*>/gi,'<div class="re-callout" style="'+CS+'">').replace(/<\/aside>/gi,'</div>');
  ...
  if(docId){
    ...
    reUpdateToolbar();
    setTimeout(()=>{_initImgResize();_initTableResize();},200);
    const raw=await _readDocRawAsync(docId);
...
function reHandleImg(e){
  ...
  reader.onload=async ev=>{
    ...
    const imgHtml='<span class="re-img-wrap" style="display:inline-block;position:relative;width:360px;max-width:100%;vertical-align:top;margin:.5em 0;" contenteditable="false">'+
      '<img src="'+url+'" style="width:100%;border-radius:8px;display:block;" alt="이미지" draggable="false">'+
      '<span class="re-img-handle" data-dir="se" style="'+HS+'right:-6px;bottom:-6px;cursor:se-resize;"></span>'+
      '<span class="re-img-handle" data-dir="sw" style="'+HS+'left:-6px;bottom:-6px;cursor:sw-resize;"></span>'+
      '<span class="re-img-handle" data-dir="ne" style="'+HS+'right:-6px;top:-6px;cursor:ne-resize;"></span>'+
      '<span class="re-img-handle" data-dir="nw" style="'+HS+'left:-6px;top:-6px;cursor:nw-resize;"></span>'+
      '</span><span contenteditable="true"> </span>';
    reExec('insertHTML',imgHtml);
...
function _initImgResize(){
  ...
  ed.addEventListener('mousedown',function(e){
    ...
    _imgRsz={w,dir:h.dataset.dir,sx:e.clientX,sw:w.offsetWidth};
  });
  document.addEventListener('mousemove',function(e){
    if(!_imgRsz)return;
...
function reInsertTable(){
  ...
  for(let r=0;r<rows;r++){
    ...
    for(let c=0;c<cols;c++){
      const isH=r===0;
      const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;min-width:20px;':'padding:10px 14px;border:1px solid #E5E8EB;min-width:20px;';
