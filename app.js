...
function exportDocPdf(){
  ...
  // @media print 스타일 + 제목 주입
  const printStyle=`<style>@media print{@page{size:A4;margin:20mm;}body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style>`;
  let printHtml=html;
  if(/<\/head>/i.test(printHtml)){
    printHtml=printHtml.replace(/<\/head>/i,printStyle+'</head>');
  }else{
    printHtml=printStyle+printHtml;
  }
  const w=window.open('','_blank');
  if(!w){toast('팝업이 차단되었습니다. 팝업 허용 후 다시 시도해주세요.');return;}
  w.document.open();
  ...
  toast('PDF 인쇄 대화상자가 열립니다. "PDF로 저장"을 선택하세요.');
}
