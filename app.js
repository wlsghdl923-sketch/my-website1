...
/* ══ 커스텀 이모지 저장소 ══ */
let _coCustomEmojis = JSON.parse(localStorage.getItem('co_custom_emojis')||'[]');
function _saveCoEmojis(){localStorage.setItem('co_custom_emojis',JSON.stringify(_coCustomEmojis));}

/* ══ 콜아웃 픽커 ══ */
let _coColor={border:'#C7D7FF',bg:'#F0F4FF',text:'#1A3A8F'};
let _coEmoji='💡';

function openCalloutPicker(){
  _coEmoji='💡';_coColor={border:'#C7D7FF',bg:'#F0F4FF',text:'#1A3A8F'};
  _renderCoEmojis();
  // 첫번째 색상 선택 표시
  const first=document.querySelector('.co-sw');
  if(first){document.querySelectorAll('.co-sw').forEach(s=>s.classList.remove('sel'));first.classList.add('sel');}
  document.getElementById('coOverlay').style.display='block';
}
function closeCalloutPicker(){document.getElementById('coOverlay').style.display='none';}

function selectCoColor(el){
  document.querySelectorAll('.co-sw').forEach(s=>s.classList.remove('sel'));
  el.classList.add('sel');
  _coColor={border:el.dataset.border,bg:el.dataset.bg,text:el.dataset.text};
}
function selectCoEmoji(el){
  document.querySelectorAll('.co-emoji-btn').forEach(s=>s.classList.remove('sel'));
  el.classList.add('sel');
  _coEmoji=el.dataset.em||'';
}
function _renderCoEmojis(){
  const grid=document.getElementById('coEmojiGrid');
  if(!grid)return;
  // 커스텀 이모지 삭제 버튼 포함해서 재렌더
  const customs=_coCustomEmojis.map((em,i)=>{
    ...
    return `<span class="co-emoji-btn co-emoji-custom" onclick="selectCoEmoji(this)" data-em="${em}" style="position:relative;">
      ${inner}
      <span class="co-emoji-del" onclick="event.stopPropagation();delCoEmoji(${i})" title="삭제">✕</span>
    </span>`;
  }).join('');
  // 기존 커스텀 제거 후 재삽입
  document.querySelectorAll('#coEmojiGrid .co-emoji-custom').forEach(el=>el.remove());
  grid.insertAdjacentHTML('afterbegin',customs);
}
function handleCoEmojiImg(e){
  const file=e.target.files[0];e.target.value='';if(!file)return;
  const reader=new FileReader();
  reader.onload=ev=>{
    const dataUrl=ev.target.result;
    _coCustomEmojis.push(dataUrl);
    _saveCoEmojis();
    _renderCoEmojis();
    toast('커스텀 이모지가 추가되었습니다.');
  };
  reader.readAsDataURL(file);
}
function delCoEmoji(idx){
...
function insertCalloutFinal(){
  const emojiHtml=_coEmoji
    ?(_coEmoji.startsWith('data:')||_coEmoji.startsWith('http')
      ?`<img src="${_coEmoji}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;">`
      :`<span style="font-size:1.2rem;flex-shrink:0;margin-top:1px;">${_coEmoji}</span>`)
    :'';
  const style=`display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid ${_coColor.border};background:${_coColor.bg};color:${_coColor.text};line-height:1.7;`;
  const html=`<div class="re-callout" contenteditable="false" style="${style}">`+
    emojiHtml+
...
function tblCmd(cmd){
  const td=_tblCtxTd;if(!td)return;
  ...
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;min-width:60px;';
  const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;min-width:60px;';

  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    const newTr=document.createElement('tr');
    for(let i=0;i<cols;i++){const c=document.createElement('td');c.contentEditable='true';c.style.cssText=TDS;newTr.appendChild(c);}
    cmd==='addRowAbove'?tr.parentNode.insertBefore(newTr,tr):tr.parentNode.insertBefore(newTr,tr.nextSibling);
  ...
  }else if(cmd==='splitCell'){td.removeAttribute('colspan');td.removeAttribute('rowspan');td.colSpan=1;td.rowSpan=1;
...
/* ══ 셀/행/열 색상 ══ */
const CELL_COLORS=['#FFFFFF','#F5F6FA','#FFECEC','#FFF3E0','#FFFDE7','#F1F8E9',
  '#E8F5E9','#E0F7FA','#E1F5FE','#E8EAF6','#F3E5F5','#FCE4EC',
  '#FFCDD2','#FFE0B2','#FFF9C4','#DCEDC8','#B2DFDB','#B3E5FC',
  '#C5CAE9','#E1BEE7','#F8BBD9','#D7CCC8','#CFD8DC','#FF8A65',
  '#FFD54F','#AED581','#4DB6AC','#4FC3F7','#7986CB','#F06292'];
let _cellColorTarget='cell';
let _selectedCellColor='';

function openCellColorPicker(target){
  _cellColorTarget=target;
  const titles={cell:'셀 배경색',row:'행 배경색',col:'열 배경색'};
  document.getElementById('cellColorTitle').textContent=titles[target]||'색상 선택';
  const grid=document.getElementById('cellColorGrid');
  grid.innerHTML=CELL_COLORS.map(c=>
    `<div style="width:26px;height:26px;border-radius:5px;background:${c};cursor:pointer;border:2px solid ${c==='#FFFFFF'?'#E5E8EB':'transparent'};" title="${c}"
     onclick="_selectedCellColor='${c}';document.querySelectorAll('#cellColorGrid div').forEach(d=>d.style.borderColor=d.title===${JSON.stringify(c)}?'#D91F3E':(d.title==='#FFFFFF'?'#E5E8EB':'transparent'));"></div>`
  ).join('');
...
function applyCellColor(color){
  ...
  const ci=tr?Array.from(tr.querySelectorAll('td,th')).indexOf(td):-1;
  if(_cellColorTarget==='cell'){td.style.background=color;}
  else if(_cellColorTarget==='row'){if(tr)Array.from(tr.querySelectorAll('td,th')).forEach(c=>c.style.background=color);}
  else if(_cellColorTarget==='col'){rows.forEach(row=>{const c=Array.from(row.querySelectorAll('td,th'))[ci];if(c)c.style.background=color;});}
  closeCellColorPicker();
}
function applyCellColorConfirm(){if(_selectedCellColor)applyCellColor(_selectedCellColor);}

/* ══ 표 열 너비 드래그 ══ */
let _colResizeState=null;
function _initColResize(){
  const editor=document.getElementById('reEditor');
  if(!editor||editor._colResizeSet)return;
  editor._colResizeSet=true;
  editor.addEventListener('mousedown',function(e){
    if(!e.target.classList.contains('col-resize'))return;
    e.preventDefault();
    const th=e.target.closest('td,th');
    if(!th)return;
    _colResizeState={th,startX:e.clientX,startW:th.offsetWidth};
  });
  document.addEventListener('mousemove',function(e){
    if(!_colResizeState)return;
    const{th,startX,startW}=_colResizeState;
    const newW=Math.max(40,startW+(e.clientX-startX));
    th.style.width=newW+'px';
  });
  document.addEventListener('mouseup',function(){_colResizeState=null;});
  // 기존 표에 핸들 추가
  editor.querySelectorAll('td,th').forEach(cell=>{
    if(!cell.querySelector('.col-resize')){
...
.doc-wrap{max-width:760px;margin:0 auto;background:#fff;border-radius:14px;box-shadow:0 4px 24px rgba(0,0,0,.10);padding:56px 64px 80px;}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._pasteHandlerSet){
    editor._pasteHandlerSet=true;
  ...
  if(docId){
    ...
    reUpdateToolbar();
    setTimeout(()=>{_initImgResize();_initColResize();},200);
    const raw=await _readDocRawAsync(docId);
...
function reHandleImg(e){
  ...
  reader.onload=async ev=>{
    ...
    toast('이미지 삽입 완료. 모서리를 드래그하여 크기 조절하세요.');
...
function _initImgResize(){
  const editor=document.getElementById('reEditor');
  if(!editor||editor._resizeHandlerSet)return;
  editor._resizeHandlerSet=true;
  editor.addEventListener('mousedown',function(e){
    const h=e.target.closest&&e.target.closest('.re-img-handle');
    if(!h)return;e.preventDefault();
    const w=h.parentElement;if(!w)return;
    _resizeState={wrap:w,dir:h.dataset.dir,startX:e.clientX,startW:w.offsetWidth};
  });
  document.addEventListener('mousemove',function(e){
    if(!_resizeState)return;
    const{wrap,dir,startX,startW}=_resizeState;
    const dx=e.clientX-startX;
    wrap.style.width=Math.max(60,Math.min((dir==='se'||dir==='ne')?startW+dx:startW-dx,760))+'px';
  });
  document.addEventListener('mouseup',function(){_resizeState=null;});
}
function reInsertTable(){
  ...
  for(let r=0;r<rows;r++){
    ...
    for(let c=0;c<cols;c++){
      const isH=r===0;
      const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;min-width:60px;':'padding:10px 14px;border:1px solid #E5E8EB;min-width:60px;';
