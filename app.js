...
/* ════════ 메모 — Notion 스타일 ════════
   핵심 설계:
   1. contextmenu에서 range 즉시 저장 (selection 사라지기 전에)
   2. span에 contenteditable=false 지정 → 내부 편집 차단
   3. span 저장 후 커서를 span 바로 뒤 텍스트 노드로 이동
   4. 메모 패널: span의 Y좌표 기준으로 카드 배치
   5. 완료 버튼(✓) → resolved 상태 → 연두색 "완료"
   6. 메모명 + 메모내용 + 템플릿 칩 재사용
════════ */
let _memoSavedRange=null, _memoSavedText='';
let _memoTemplates=[];

async function _loadMemoTemplates(){
  try{const row=await _sbRest.selectOne('site_data','memo_tpl');if(row&&Array.isArray(row.value))_memoTemplates=row.value;else if(row&&typeof row.value==='string')_memoTemplates=JSON.parse(row.value);}catch{}
}
async function _saveMemoTemplates(){
  try{await _sbRest.upsert('site_data',{key:'memo_tpl',value:_memoTemplates,updated_at:new Date().toISOString()});}catch{}
}
...
function _renderMemoTemplateChips(currentDesc){
  ...
  chips.innerHTML=_memoTemplates.slice(-15).reverse().map((t,i)=>
...
function saveMemoToSelection(){
  ...
  const desc=document.getElementById('memoInputDesc').value||''; // 공백도 허용
  // 템플릿 저장 (desc가 비어있지 않을 때만)
  ...
  // span 생성 — contenteditable=false 로 메모 범위 고정
  const span=document.createElement('span');
  span.className='re-memo';
  span.setAttribute('contenteditable','false');  // ← 핵심: 내부 편집 차단
  ...
  try{
    _memoSavedRange.surroundContents(span);
  }catch{
    const frag=_memoSavedRange.extractContents();
    span.appendChild(frag);
    _memoSavedRange.insertNode(span);
  }
  // 커서를 span 바로 뒤로 이동 — 이후 타이핑은 메모 밖
  try{
    const afterRange=document.createRange();
    afterRange.setStartAfter(span);afterRange.collapse(true);
    sel.removeAllRanges();sel.addRange(afterRange);
    // span 뒤에 zero-width space 삽입하여 커서 위치 확정
...
function refreshMemoPanel(){
  ...
  memos.forEach((m,i)=>{
    ...
    // editor 기준 Y (스크롤 감안)
    const scrolled=editor.scrollTop||(document.querySelector('.re-body')?.scrollTop||0);
    let y=mRect.top-edRect.top+scrolled;
  ...
  cards.forEach(({id,y,m})=>{
    ...
    const quote=(m.textContent||'').replace(/\u200B/g,'').slice(0,30);
    ...
    card.innerHTML=
      ...
      (desc||isResolved?`<div class="memo-card-desc">${isResolved?'<span class="memo-done-badge">✓ 완료</span>':desc}</div>`:'')+
      `<div class="memo-card-btns">`+
      (!isResolved?`<button class="mresolve" onclick="event.stopPropagation();resolveMemoCard('${id}')">✓</button>`:'')+
...
function tblCmd(cmd){
  ...
  // 핸들 갱신
  const editor=document.getElementById('reEditor');if(editor)setTimeout(()=>_refreshColHandles(editor),50);
}
...
/* ════════════════════════════════════════
   표 열 리사이즈 — 핸들 div 직접 삽입 방식
   
   ■ 핵심 원리:
     각 셀(마지막 열 제외)의 오른쪽에 .col-rsz-handle div를 삽입.
     핸들을 드래그하면 해당 열의 모든 셀 너비를 직접 조절.
     
   ■ contenteditable 충돌 방지:
     핸들 자체는 contenteditable=false.
     핸들의 mousedown에서 e.preventDefault() + e.stopPropagation()
     → 에디터 포커스 이동 없음.
     
   ■ 좌로 많이 드래그 가능:
     MIN_W=1로 최솟값 없이 자유롭게.
     
   ■ 가로선(행) 리사이즈:
     에디터 mousemove에서 셀 하단 6px 이내 감지.
════════════════════════════════════════ */
let _colRszState=null;
let _rowRszState=null;

function _refreshColHandles(editor){
  if(!editor)return;
  // 기존 핸들 제거
  editor.querySelectorAll('.col-rsz-handle').forEach(h=>h.remove());
  // 각 표의 각 셀에 핸들 추가
  editor.querySelectorAll('table').forEach(table=>{
    ...
    allRows.forEach(tr=>{
      ...
      cells.forEach((cell,ci)=>{
        if(ci===cells.length-1)return; // 마지막 열은 건너뜀
        // 이미 핸들 있으면 건너뜀
        if(cell.querySelector('.col-rsz-handle'))return;
        ...
        handle.addEventListener('mousedown',function(e){
          ...
          e.stopPropagation();
          // 같은 열 인덱스의 모든 셀 수집
          const colCells=allRows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
          _colRszState={
            cells:colCells,
            startX:e.clientX,
...
function _initTableResize(){
  ...
  // 초기 핸들 생성
  _refreshColHandles(editor);

  // 행 높이 드래그 — 셀 아래쪽 6px 감지
  let _rowEdgeCell=null;
  editor.addEventListener('mousemove',function(e){
    if(_colRszState||_rowRszState)return;
    if(e.target.classList&&e.target.classList.contains('col-rsz-handle')){editor.style.cursor='';return;}
    const cell=e.target.closest&&e.target.closest('td,th');
    if(!cell){if(editor.style.cursor==='row-resize')editor.style.cursor='';_rowEdgeCell=null;return;}
    const rect=cell.getBoundingClientRect();
    const ry=e.clientY-rect.top;
    if(ry>=rect.height-6){editor.style.cursor='row-resize';_rowEdgeCell=cell;}
    else{if(editor.style.cursor==='row-resize')editor.style.cursor='';_rowEdgeCell=null;}
  });
  editor.addEventListener('mousedown',function(e){
    if(!_rowEdgeCell||e.target.classList.contains('col-rsz-handle'))return;
    const rect=_rowEdgeCell.getBoundingClientRect();
    const ry=e.clientY-rect.top;
    if(ry<rect.height-6)return;
    e.preventDefault();e.stopPropagation();
    const tr=_rowEdgeCell.closest('tr');
    _rowRszState={tr,startY:e.clientY,startH:tr.offsetHeight};
  });

  // 열/행 드래그 처리
  document.addEventListener('mousemove',function(e){
    if(_colRszState){
      const dx=e.clientX-_colRszState.startX;
      const newW=Math.max(1,_colRszState.startW+dx);
      _colRszState.cells.forEach(c=>{
    ...
    if(_rowRszState){
      const newH=Math.max(10,_rowRszState.startH+(e.clientY-_rowRszState.startY));
      _rowRszState.tr.style.height=newH+'px';
      Array.from(_rowRszState.tr.querySelectorAll('td,th')).forEach(c=>{c.style.height=newH+'px';});
    }
  });
  document.addEventListener('mouseup',function(){
    if(_colRszState){
      document.querySelectorAll('.col-rsz-handle.rsz-active').forEach(h=>h.classList.remove('rsz-active'));
      _colRszState=null;document.body.style.cursor='';document.body.style.userSelect='';
    }
    if(_rowRszState){_rowRszState=null;editor.style.cursor='';}
  });

  // 에디터 내용 변경 시 핸들 재생성
  const obs=new MutationObserver(()=>{clearTimeout(obs._t);obs._t=setTimeout(()=>_refreshColHandles(editor),300);});
  obs.observe(editor,{childList:true,subtree:true,attributes:false});
}

/* ════════ PDF ════════ */
function exportDocAsPdf(){
  const editor=document.getElementById('reEditor');if(!editor){toast('에디터를 열어주세요.');return;}
  if(editor.querySelectorAll('.re-memo').length>0)document.getElementById('pdfOptOverlay').style.display='block';
  else doPdfExport();
}
function closePdfOpt(){document.getElementById('pdfOptOverlay').style.display='none';}
function doPdfExport(){
  closePdfOpt();const editor=document.getElementById('reEditor');const title=document.getElementById('reTitle')?.value||'문서';
  const withMemo=document.getElementById('pdfOptMemo')?.checked!==false;
  const clone=editor.cloneNode(true);
  clone.querySelectorAll('.col-rsz-handle').forEach(h=>h.remove());
  let memoSection='';
  if(withMemo){
    const memos=[];editor.querySelectorAll('.re-memo').forEach(m=>memos.push({text:(m.textContent||'').replace(/\u200B/g,''),name:m.dataset.memoName||'메모',desc:m.dataset.memoDesc||'',resolved:m.dataset.memoResolved==='true'}));
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('contextmenu',function(e){
      ...
      if(sel&&!sel.isCollapsed){
        e.preventDefault();
        _memoSavedRange=sel.getRangeAt(0).cloneRange();
        _memoSavedText=sel.toString().trim();
        showEditorCtxMenu(e);
      }
    });
    editor.addEventListener('click',function(e){
      ...
      if(e.target.classList&&e.target.classList.contains('col-rsz-handle'))return;
      const td=e.target.closest&&e.target.closest('td,th');
