...
<\/script>
  ...
  }else if(d.type==='refdoc'){    let stored={columns:[],markers:[],pages:[],splitRatio:70};try{stored=JSON.parse(raw)||stored;}catch{}
    // 구버전 호환
    if(stored.images&&!stored.pages){
      stored={columns:[{id:'c1',name:'이미지'}],markers:[],splitRatio:70,
        pages:stored.images.map((img,i)=>({id:img.id||'p'+i,name:String(i+1),images:[{data:img.data,name:''}],opinion:img.text||'',tags:[]}))};
    }
    const cols=stored.columns||[{id:'c1',name:'이미지'}];
    const markers=stored.markers||[];
    const split=stored.splitRatio||70;
    const colHeaders=cols.map(c=>`<th class="rv-th">${(c.name||'').replace(/</g,'&lt;')}</th>`).join('');
    const rowsHtml=(stored.pages||[]).map(page=>{
      const imgCells=cols.map((col,ci)=>{
        const img=page.images[ci]||{data:'',name:''};
        const imgName=img.name?`<div class="rv-img-name">${img.name.replace(/</g,'&lt;')}</div>`:'';
        return`<td class="rv-td rv-img-td">${imgName}${img.data?`<img src="${img.data}" alt="">`:'<span class="rv-no-img">이미지 없음</span>'}</td>`;
      }).join('');
      ...
      const opinionHtml=(page.opinion||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
      return`<tr>
        <td class="rv-td rv-pg-td">${(page.name||'').replace(/</g,'&lt;')}</td>
        ${imgCells}
        <td class="rv-td rv-op-td"><div class="rv-tags">${pageTags}</div><div class="rv-opinion">${opinionHtml||'<span style="color:#8B95A1">내용 없음</span>'}</div></td>
      </tr>`;
    }).join('');
...
.rv-img-td{text-align:center;background:#ECEEF2;min-width:180px;}
...
.rv-opinion{white-space:pre-wrap;}
@media(max-width:640px){.rv-img-td{min-width:120px;} .rv-op-td{min-width:140px;}}
</style></head>
<body><h1>${d.title}</h1>
<table class="rv-table"><thead><tr><th class="rv-th">페이지</th>${colHeaders}<th class="rv-th" style="width:${100-split}%">의견</th></tr></thead>
...
let _rdData={columns:[{id:uid(),name:'원본 이미지'}],markers:[],pages:[],splitRatio:70};
let _editRefDocId='';
let _rdUploadTarget=null; // {pageIdx, colIdx}
let _rdResizing=false;

const RD_DEF_COLORS=['#FF0000','#E53935','#D81B60','#C2185B','#AB47BC','#9C27B0','#7B1FA2','#5C6BC0','#3F51B5','#1E88E5','#0288D1','#0097A7','#00897B','#2E9E50','#43A047','#689F38','#9E9D24','#F9A825','#FFB300','#FF8F00','#FF7043','#8D6E63','#795548','#6D4C41','#78909C','#607D8B','#455A64','#333333','#E91E63','#00BCD4'];

function _rdSync(){
  /* 현재 화면의 입력값을 _rdData로 동기화 */
  _rdData.pages.forEach((p,pi)=>{
    const nameIn=document.getElementById(`rdPgName-${pi}`);if(nameIn)p.name=nameIn.value;
    const opIn=document.getElementById(`rdOpinion-${pi}`);if(opIn)p.opinion=opIn.value;
    p.images.forEach((img,ci)=>{
      const imgNameIn=document.getElementById(`rdImgName-${pi}-${ci}`);if(imgNameIn)img.name=imgNameIn.value;
    });
  });
  _rdData.columns.forEach((col,ci)=>{
    const colIn=document.getElementById(`rdColName-${ci}`);if(colIn)col.name=colIn.value;
  });
}
...
async function openRefDocModal(docId=''){
  ...
  _rdData={columns:[{id:uid(),name:'원본 이미지'}],markers:[],pages:[{id:uid(),name:'1',images:[{data:'',name:''}],opinion:'',tags:[]}],splitRatio:70};
  if(docId){
    ...
    if(stored&&typeof stored==='object'){
      if(stored.columns){
        _rdData=stored;
        // 하위 호환: 구버전 images 배열 형태 → 새 형태로 변환
        if(stored.images&&!stored.pages){
          _rdData.columns=[{id:uid(),name:'이미지'}];
          _rdData.pages=stored.images.map((img,i)=>({id:img.id||uid(),name:String(i+1),images:[{data:img.data,name:''}],opinion:img.text||'',tags:[]}));
...
/* 컬럼 추가/삭제 (최대 3개) */
function rdAddColumn(){
  _rdSync();
  if(_rdData.columns.length>=3){toast('이미지 영역은 최대 3개까지 가능합니다.');return;}
  const newColId=uid();
  _rdData.columns.push({id:newColId,name:'이미지 '+ (_rdData.columns.length+1)});
  _rdData.pages.forEach(p=>p.images.push({data:'',name:''}));
  renderRdEditor();
}
function rdDelColumn(ci){
  _rdSync();
  if(_rdData.columns.length<=1){toast('최소 1개의 이미지 영역이 필요합니다.');return;}
  if(!confirm(`"${_rdData.columns[ci].name}" 영역을 삭제하시겠습니까?`))return;
  _rdData.columns.splice(ci,1);
  _rdData.pages.forEach(p=>p.images.splice(ci,1));
  renderRdEditor();
}
...
function rdAddPage(){
  _rdSync();
  const newImages=_rdData.columns.map(()=>({data:'',name:''}));
  _rdData.pages.push({id:uid(),name:String(_rdData.pages.length+1),images:newImages,opinion:'',tags:[]});
  renderRdEditor();
  // 스크롤 맨 아래로
  const container=document.getElementById('rdPageList').parentElement;
  setTimeout(()=>container.scrollTop=container.scrollHeight,100);
}
function rdDelPage(pi){
  _rdSync();
  if(!confirm(`페이지 "${_rdData.pages[pi].name}" 삭제?`))return;
...
function rdStartResize(e){
  ...
  const onMove=(ev)=>{
    ...
    _rdData.splitRatio=Math.max(30,Math.min(85,pct));
...
function renderRdEditor(){
  const cols=_rdData.columns;
  const split=_rdData.splitRatio||70;

  // 컬럼 헤더
  const headerHtml=`
    <div class="rd2-header-row">
      <div class="rd2-pg-col"><span class="rd2-hdr-label">페이지</span></div>
      <div class="rd2-img-zone" style="width:${split}%;display:flex;">
        ${cols.map((col,ci)=>`
          <div class="rd2-col-cell" style="flex:1;">
            <input class="rd2-col-name-in" id="rdColName-${ci}" value="${col.name}" placeholder="영역명">
            <button class="rd2-col-del" onclick="rdDelColumn(${ci})" title="영역 삭제">✕</button>
          </div>`).join('')}
        ${cols.length<3?`<button class="rd2-col-add-btn" onclick="rdAddColumn()" title="이미지 영역 추가">＋</button>`:''}
      </div>
      <div class="rd2-resize-handle" onmousedown="rdStartResize(event)" ontouchstart="rdStartResize(event)"></div>
      <div class="rd2-txt-zone" style="width:${100-split}%;"><span class="rd2-hdr-label">의견</span></div>
    </div>`;
  document.getElementById('rdColHeader').innerHTML=headerHtml;

  // 페이지 행들
  const pagesHtml=_rdData.pages.map((page,pi)=>{
    const imgsHtml=cols.map((col,ci)=>{
      const img=page.images[ci]||{data:'',name:''};
      ...
      return`<div class="rd2-img-cell" style="flex:1;">
        <input class="rd2-img-name-in" id="rdImgName-${pi}-${ci}" value="${img.name||''}" placeholder="이미지명">
    ...
    const markerArea=_rdData.markers.length
      ?`<div class="rd2-tag-area">${markerBtns}</div>`
      :'';

    return`<div class="rd2-page-row" id="rdPageRow-${pi}">
      <div class="rd2-pg-col">
        <input class="rd2-pg-name-in" id="rdPgName-${pi}" value="${page.name||''}" placeholder="페이지">
        <button class="rd2-pg-del" onclick="rdDelPage(${pi})" title="페이지 삭제">✕</button>
      </div>
      <div class="rd2-img-zone" style="width:${split}%;display:flex;">${imgsHtml}</div>
      <div class="rd2-resize-handle rd2-rh-passive"></div>
      <div class="rd2-txt-zone" style="width:${100-split}%;">
        ${markerArea}
        <textarea class="rd2-opinion-area" id="rdOpinion-${pi}" placeholder="이미지에 대한 의견을 작성하세요.">${page.opinion||''}</textarea>
      </div>
    </div>`;
  }).join('');
...
function saveRefDoc(){
  ...
  _rdSync();
  const stored={type:'refdoc',columns:_rdData.columns,markers:_rdData.markers,pages:_rdData.pages,splitRatio:_rdData.splitRatio};
  if(!CARD_DOCS[_curCard])CARD_DOCS[_curCard]=[];
  if(_editRefDocId){
