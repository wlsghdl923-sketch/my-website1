...
async function _readDocRawAsync(docId){
  try{
    ...
    console.error('문서 읽기 실패:',e);
    return '';
  }
}
function _readDocRaw(docId){return '';} // 동기버전 — 하위호환용, 실제로는 위 async 함수 사용
...
async function openRichEditor(docId=''){
  ...
  if(!editor._pasteHandlerSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(html){
        const CALLOUT_STYLE='background:#F0F4FF;border:1px solid #C7D7FF;border-radius:8px;padding:12px 16px;margin:8px 0;color:#1A3A8F;line-height:1.7;display:block;';
        const clean=html.replace(/<aside[^>]*>/gi,'<div style="'+CALLOUT_STYLE+'">').replace(/<\/aside>/gi,'</div>');
