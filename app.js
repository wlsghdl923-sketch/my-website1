...
function _asSchedule(){
  ...
  clearTimeout(_autoSaveTimer);_asUI('pending');
...
async function _loadMemoPresets(){
  try{
    ...
    if(r1&&r1.value){let v=r1.value;if(typeof v==='string')try{v=JSON.parse(v);}catch{}if(Array.isArray(v))_markerPresets=v;}
    const r2=await _sbRest.selectOne('site_data','hub_memo_contents');
    if(r2&&r2.value){let v=r2.value;if(typeof v==='string')try{v=JSON.parse(v);}catch{}if(Array.isArray(v))_contentPresets=v;}
...
/* 우클릭 컨텍스트 메뉴 */
let _ctxMenuInited=false;
function _initMemoCtxMenu(){
  if(_ctxMenuInited)return;
  _ctxMenuInited=true;
  ...
  editor.addEventListener('contextmenu',function(e){
    ...
    if(!sel||sel.isCollapsed||!sel.toString().trim()){
      const m=document.getElementById('memoCtxMenu');if(m)m.style.display='none';
      return;
    }
    e.preventDefault();
...
function memoApplyMarker(color,encName){
  ...
  document.querySelectorAll('#memoColorPicker .memo-color-dot').forEach(d=>d.classList.toggle('sel',d.dataset.color===color));
  const ni=document.getElementById('memoMarkerNameInput');if(ni)ni.value=name;
}
...
function saveMemoInput(){
  ...
  if(_memoEditId){
    ...
    if(m){
      ...
      if(m.spanEl){m.spanEl.style.background=_hexA(color,.38);m.spanEl.style.borderBottomColor=color;}
    }
  ...
  try{
    ...
    /* ── 핵심: span 삽입 후 커서를 span 바깥으로 이동 ── */
    const span=document.createElement('span');
    span.className='re-memo-mark';
    span.dataset.memoId=id;
    span.contentEditable='false';  // 핵심: 편집 불가로 만들어 커서가 span 안으로 들어가지 않게 함
    ...
    toast('텍스트 하이라이트는 단일 블록 내에서 가능합니다.');
  }
  closeMemoInput();
  refreshMemoPanel();
}
...
function refreshMemoPanel(){
  _renderMarkerPanel();_renderMemoPanelList();
}
...
function deleteMemo(id){
  ...
  const m=_memos[idx];
  if(m.spanEl){
    // span 제거 시 내부 textNode만 남김
    const p=m.spanEl.parentNode;
    if(p){
      // contentEditable=false span이므로 outerText로 대체
      const txt=document.createTextNode(m.spanEl.textContent);
      p.insertBefore(txt,m.spanEl);
...
function mmSwitchTab(n){
  _mmTab=n;
  const s1=document.getElementById('mmSec1');
  const s2=document.getElementById('mmSec2');
  const t1=document.getElementById('mmTab1');
  const t2=document.getElementById('mmTab2');
  if(s1){s1.style.display=n===1?'flex':'none';}
  if(s2){s2.style.display=n===2?'flex':'none';}
  if(t1){t1.className=n===1?'mbtn-s':'mbtn-c';t1.style.fontSize='.79rem';t1.style.padding='7px 14px';}
  if(t2){t2.className=n===2?'mbtn-s':'mbtn-c';t2.style.fontSize='.79rem';t2.style.padding='7px 14px';}
...
function mmPickColor(el){
  ...
  el.classList.add('sel');_mmSelColor=el.dataset.color;
}
...
async function mmDeleteMarker(i){
  _markerPresets.splice(i,1);await _saveMemoPresets();_renderMmMarkerList();toast('마커 삭제됨');
}
async function mmAddContent(){
  ...
  _contentPresets.push(val);await _saveMemoPresets();_renderMmContentList();
  document.getElementById('mmContentInput').value='';toast('메모 내용 추가됨');
}
...
async function mmDeleteContent(i){
  _contentPresets.splice(i,1);await _saveMemoPresets();_renderMmContentList();toast('삭제됨');
}
...
async function openRichEditor(docId=''){
  ...
  if(docId){
    ...
    // 로딩 표시
    editor.innerHTML='<p style="color:#8B95A1;font-size:.88rem;text-align:center;padding:40px 0;">불러오는 중...</p>';
    try{
      // Supabase에서 직접 로드
      const row=await _sbRest.selectOne('site_data',`hub_doc_${docId}`);
      if(row&&row.value!=null){
        let raw=row.value;
        // 문자열이 JSON 이중 인코딩된 경우 복구
        if(typeof raw==='string'){
          if(raw.startsWith('"')&&raw.endsWith('"')){
            try{const d2=JSON.parse(raw);if(typeof d2==='string')raw=d2;}catch{}
          }
        }else if(typeof raw==='object'){
          // 객체로 저장된 경우(refdoc 등)는 무시
          raw='';
        }
      ...
      editor.innerHTML='';
      toast('문서 불러오기에 실패했습니다. 다시 시도해주세요.');
    }
  }else{
    titleIn.value='';
    editor.innerHTML='';
...
function reOnKeydown(e){
  ...
  // Enter 처리: 메모 span 안에 있으면 span 밖으로 커서 이동
  if(e.key==='Enter'){
    const sel=window.getSelection();
    if(sel&&sel.rangeCount>0){
      ...
      if(memoSpan){
        e.preventDefault();
        // span 바깥으로 커서 이동 후 새 단락 삽입
        const range=document.createRange();
        range.setStartAfter(memoSpan);
        range.collapse(true);
        sel.removeAllRanges();
        sel.addRange(range);
