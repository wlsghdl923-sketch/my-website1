...
/* ══════════════════════════════════════════════════════════
   자동저장 (Auto-Save) — 2초 debounce
   reOnInput()에서 호출, 저장 중/완료/오류 상태 표시
══════════════════════════════════════════════════════════ */
let _autoSaveTimer = null;
let _autoSaveEnabled = false;
const AUTO_SAVE_DELAY = 2000; // 2초

function _autoSaveSetStatus(state) {
  ...
  if (state === 'idle') {
    ...
    if (icon) icon.innerHTML = '<circle cx="6" cy="6" r="5" fill="none" stroke="#FF7A00" stroke-width="1.5"/><path d="M6 3v3l2 1" stroke="#FF7A00" stroke-width="1.2" stroke-linecap="round"/>';
    if (text) text.textContent = '저장중...';
  } else if (state === 'saved') {
    el.classList.add('saved');
    if (icon) icon.innerHTML = '<circle cx="6" cy="6" r="5" fill="none" stroke="#2E9E50" stroke-width="1.5"/><polyline points="3.5,6 5,7.5 8.5,4" stroke="#2E9E50" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" fill="none"/>';
    if (text) text.textContent = '저장됨 ✓';
    setTimeout(() => _autoSaveSetStatus('idle'), 3000);
  } else if (state === 'error') {
...
function _autoSaveSchedule() {
  ...
  _autoSaveSetStatus('pending');
  _autoSaveTimer = setTimeout(_autoSaveNow, AUTO_SAVE_DELAY);
}
...
async function _autoSaveNow() {
  ...
  try {
    ...
    _autoSaveSetStatus('saved');
  } catch (e) {
    console.warn('자동저장 실패:', e);
    _autoSaveSetStatus('error');
  }
}

function _autoSaveEnable() {
  _autoSaveEnabled = true;
  _autoSaveSetStatus('idle');
}

function _autoSaveDisable() {
...
/* ══════════════════════════════════════════════════════════
   패널 토글 (글쓰기 에디터)
══════════════════════════════════════════════════════════ */
let _editorPanelsOn = false;

function toggleEditorPanels() {
  _editorPanelsOn = !_editorPanelsOn;
  ...
  if (lp) lp.style.display = _editorPanelsOn ? 'block' : 'none';
  if (rp) rp.style.display = _editorPanelsOn ? 'block' : 'none';
  if (btn) btn.classList.toggle('panel-on', _editorPanelsOn);
}
...
/* ══════════════════════════════════════════════════════════
   이미지 의견 작성 — openRefDocModal (Supabase 로드)
══════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════
   이미지 의견 작성 — renderRdImages (7:3 + 더블클릭 라이트박스 + 리사이저)
══════════════════════════════════════════════════════════ */
      ...
      <div class="rd-row-body" id="rdBody-${i}">
        ...
        <div class="rd-img-side" id="rdImgSide-${i}" style="flex:7;">
          <img src="${img.data}" alt="이미지 ${i+1}"
            style="max-width:100%;max-height:480px;object-fit:contain;display:block;cursor:zoom-in;"
            title="더블클릭하면 원본 크기로 봅니다"
            ondblclick="rdOpenLightbox(this.src)">
          <button class="rd-img-replace-btn" onclick="_rdSlotIdx=${i};document.getElementById('rdFileInSlot').click()">🔄 이미지 교체</button>
        </div>
        <!-- 리사이저 -->
        <div class="rd-resizer" id="rdRsz-${i}" onmousedown="rdStartResize(event,${i})" title="좌우 드래그로 비율 조절"></div>
        <!-- 텍스트 영역 (flex:3) -->
...
function rdStartResize(e, i) {
  ...
  _rdRszActive = { imgEl, txtEl, rsz, startX: e.clientX, imgStartW: imgEl.offsetWidth, totalW: body.offsetWidth };
  if (rsz) rsz.classList.add('rz-active');
  document.body.style.userSelect = 'none';
  document.body.style.cursor = 'col-resize';
  function onMove(ev) {
    if (!_rdRszActive) return;
    const dx = ev.clientX - _rdRszActive.startX;
    const newImgW = Math.max(60, Math.min(_rdRszActive.totalW - 60, _rdRszActive.imgStartW + dx));
    const newTxtW = _rdRszActive.totalW - newImgW - 5;
    _rdRszActive.imgEl.style.flex = 'none';
    _rdRszActive.imgEl.style.width = newImgW + 'px';
    _rdRszActive.txtEl.style.flex = 'none';
    _rdRszActive.txtEl.style.width = newTxtW + 'px';
  }
  function onUp() {
    if (_rdRszActive?.rsz) _rdRszActive.rsz.classList.remove('rz-active');
    _rdRszActive = null;
...
/* openRefDocModal → async 버전으로 교체됨 */
...
function openRichEditor(docId=''){
  ...
  // 자동저장 설정
  if(docId){_autoSaveEnable();}else{_autoSaveDisable();}
  setTimeout(()=>{
