...
/* ── 색상 팔레트 ── */
const TEXT_COLORS30=['#191F28','#4E5968','#8B95A1','#FFFFFF','#D91F3E','#FF5252','#FF7A00','#FFCA28','#2E9E50','#69F0AE','#0064FF','#40C4FF','#006064','#80DEEA','#9C27B0','#CE93D8','#673AB7','#E91E63','#F48FB1','#795548','#A1887F','#607D8B','#BF360C','#1B5E20','#01579B','#4A148C','#880E4F','#37474F','#4E342E','#CFD8DC'];
const BG_COLORS30=['transparent','#FFFF00','#FFE082','#FFCDD2','#F8BBD9','#FFF3E0','#FFE0B2','#C8E6C9','#DCEDC8','#B2EBF2','#E1F5FE','#E8EAF6','#EDE7F6','#F3E5F5','#FCE4EC','#E0F2F1','#F9FBE7','#FFF9C4','#E3F2FD','#F1F8E9','#FFFDE7','#FFF8F0','#F0F4FF','#F9F9F9','#D5E8D5','#D5D5E8','#E8D5D5','#E8E8D5','#D5E8E8','#E8DDD5'];
const MARKER_COLORS=['#D91F3E','#FF5252','#FF7A00','#FFCA28','#00BFA5','#0064FF','#40C4FF','#006064','#9C27B0','#CE93D8','#673AB7','#E91E63','#F48FB1','#795548','#A1887F','#607D8B','#BF360C','#1B5E20','#01579B','#4A148C','#880E4F','#37474F','#191F28','#FF8F00','#558B2F','#00838F','#283593','#6A1B9A','#AD1457','#F57F17'];
let _savedSel=null;
function initColorPalettes(){
  const fg=document.getElementById('reFgPalette'),bg=document.getElementById('reBgPalette');
  if(!fg||fg._init)return;fg._init=true;bg._init=true;
  fg.innerHTML=TEXT_COLORS30.map(c=>`<div class="tc${c==='#FFFFFF'||c==='#CFD8DC'?' tc-light':''}" style="background:${c};" onmousedown="event.preventDefault();applyFgColor('${c}')"></div>`).join('');
  bg.innerHTML=BG_COLORS30.map(c=>{const T=c==='transparent';return `<div class="tc${T||c==='#F9F9F9'?' tc-light':''}" style="${T?'background:linear-gradient(135deg,#fff 45%,#f44 45%,#f44 55%,#fff 55%);':'background:'+c+';'}" onmousedown="event.preventDefault();applyBgColor('${c}')"></div>`;}).join('');
  document.addEventListener('mousedown',function(e){if(!e.target.closest('.re-color-wrap'))document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');});
}
function toggleColorPalette(id){const sel=window.getSelection();if(sel&&sel.rangeCount&&!sel.isCollapsed)_savedSel={r:sel.getRangeAt(0).cloneRange()};const p=document.getElementById(id);if(!p)return;const open=p.style.display==='flex';document.querySelectorAll('.re-color-palette').forEach(x=>x.style.display='none');if(!open)p.style.display='flex';}
function _restoreSel(){if(_savedSel){const s=window.getSelection();s.removeAllRanges();s.addRange(_savedSel.r);}}
function applyFgColor(c){_restoreSel();document.execCommand('foreColor',false,c);const b=document.getElementById('reFgColorBar');if(b)b.style.background=c;document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');_savedSel=null;}
function applyBgColor(c){_restoreSel();document.execCommand('hiliteColor',false,c==='transparent'?'transparent':c);if(c!=='transparent'){const f=document.getElementById('reBgColorFill');if(f)f.setAttribute('fill',c);}document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');_savedSel=null;}

/* ── 마크다운 가이드 ── */
function openMdGuide(){document.getElementById('mdGuideOverlay').style.display='block';}
function closeMdGuide(){document.getElementById('mdGuideOverlay').style.display='none';}

/* ── 마크다운 단축키 (확장) ── */
function checkMarkdownShortcut(){
  const editor=document.getElementById('reEditor');if(!editor)return;
  ...
  // 블록 변환
  const blockPatterns=[
    {re:/^#{4}\s$/,tag:'h4'},{re:/^#{3}\s$/,tag:'h3'},
    {re:/^#{2}\s$/,tag:'h2'},{re:/^#\s$/,tag:'h1'},
    {re:/^>\s$/,tag:'blockquote'},{re:/^```$/,tag:'pre'},
  ];
  ...
  if(/^---$/.test(lineText.trim())){
    ...
    if(block){block.textContent='';document.execCommand('insertHTML',false,'<hr><p><br></p>');}
    return;
  }
  // 목록
  if(/^[-*+]\s$/.test(lineText)){
  ...
  if(/^\[ \]\s$/.test(lineText)||/^\[\]\s$/.test(lineText)){
    node.textContent=text.slice(offset);
    document.execCommand('insertHTML',false,'<label contenteditable="false"><input type="checkbox"> </label>');
    return;
  }
  // 인라인 강조 — **텍스트** → bold (줄 안)
  const boldMatch=lineText.match(/\*\*(.+)\*\*\s?$/);
  if(boldMatch){
    const startIdx=text.lastIndexOf('**'+boldMatch[1]+'**');
    if(startIdx>=0){
      const r2=document.createRange();
      r2.setStart(node,startIdx);r2.setEnd(node,startIdx+boldMatch[0].length);
  ...
  // *텍스트* → italic
  const itMatch=lineText.match(/(?<!\*)\*([^*]+)\*\s?$/);
  if(itMatch&&!boldMatch){
    const startIdx=text.lastIndexOf('*'+itMatch[1]+'*');
    if(startIdx>=0&&(startIdx===0||text[startIdx-1]!=='*')){
      const r2=document.createRange();
      r2.setStart(node,startIdx);r2.setEnd(node,startIdx+itMatch[0].length);
      sel.removeAllRanges();sel.addRange(r2);
  ...
  // ~~텍스트~~ → strikethrough
  const strikeMatch=lineText.match(/~~(.+)~~\s?$/);
  if(strikeMatch){
    const startIdx=text.lastIndexOf('~~'+strikeMatch[1]+'~~');
    if(startIdx>=0){
      const r2=document.createRange();
      r2.setStart(node,startIdx);r2.setEnd(node,startIdx+strikeMatch[0].length);
      sel.removeAllRanges();sel.addRange(r2);
...
/* ── 패널 토글 ── */
let _panelsForceOn=false;
function togglePanels(){
  _panelsForceOn=!_panelsForceOn;
  const btn=document.getElementById('btnTogglePanels');
  if(btn)btn.classList.toggle('active',_panelsForceOn);
  if(_panelsForceOn){
    ['reMarkerPanel','reMemoPanel','markerPanelResizer','memoPanelResizer'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='block';});
  }else{refreshMemoPanel();}
}

/* ════ 메모 ════ */
let _memoSavedRange=null,_memoSavedText='';
let _memoTemplates=[],_markerPresets=[];
let _selMarkerColor='',_selMarkerName='';
let _memoEditId=null;

async function _loadMemoData(){
  try{
    const r1=await _sbRest.selectOne('site_data','memo_tpl');
    if(r1&&Array.isArray(r1.value))_memoTemplates=r1.value;
    else if(r1&&typeof r1.value==='string')_memoTemplates=JSON.parse(r1.value);
    const r3=await _sbRest.selectOne('site_data','memo_mkr_presets');
    if(r3&&Array.isArray(r3.value))_markerPresets=r3.value;
    else if(r3&&typeof r3.value==='string')_markerPresets=JSON.parse(r3.value);
  }catch{}
}
async function _saveMemoData(){
  try{
    await _sbRest.upsert('site_data',{key:'memo_tpl',value:_memoTemplates,updated_at:new Date().toISOString()});
    await _sbRest.upsert('site_data',{key:'memo_mkr_presets',value:_markerPresets,updated_at:new Date().toISOString()});
  }catch{}
}

function _initMemoMarkerPalette(){
  const pal=document.getElementById('memoMarkerPalette');if(!pal||pal._init)return;pal._init=true;
  pal.innerHTML=MARKER_COLORS.map(c=>`<div class="mkr-chip" style="background:${c};" data-c="${c}" onclick="pickMemoMarker(this,'${c}')"></div>`).join('');
}
function _initMmgrPalette(){
  const pal=document.getElementById('mmgrPalette');if(!pal||pal._init)return;pal._init=true;
  pal.innerHTML=MARKER_COLORS.map(c=>`<div class="mkr-chip" style="background:${c};" data-c="${c}" onclick="pickMmgrMarker(this,'${c}')"></div>`).join('');
}
let _mmgrSelColor='';
function pickMmgrMarker(el,color){if(_mmgrSelColor===color){_mmgrSelColor='';document.querySelectorAll('#mmgrPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));return;}document.querySelectorAll('#mmgrPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));el.classList.add('sel');_mmgrSelColor=color;}
function pickMemoMarker(el,color){
  if(_selMarkerColor===color){_selMarkerColor='';document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));return;}
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
  el.classList.add('sel');_selMarkerColor=color;
}

function showEditorCtxMenu(e){
  const menu=document.getElementById('editorCtxMenu');if(!menu)return;
  menu.style.display='block';menu.style.left=Math.min(e.clientX,window.innerWidth-160)+'px';menu.style.top=Math.min(e.clientY,window.innerHeight-60)+'px';
  const close=ev=>{if(!ev.target.closest('#editorCtxMenu')){menu.style.display='none';document.removeEventListener('mousedown',close);}};
  setTimeout(()=>document.addEventListener('mousedown',close),10);
}

async function openMemoInput(){
  document.getElementById('editorCtxMenu').style.display='none';
  if(!_memoSavedRange){toast('텍스트를 먼저 드래그하여 선택하세요.');return;}
  _memoEditId=null;
  await _loadMemoData();
  document.getElementById('memoInputTitle').textContent='📝 메모 추가';
  document.getElementById('memoInputSaveBtn').onclick=saveMemoToSelection;
  const prev=document.getElementById('memoSelectedPreview');
  if(prev)prev.textContent='"'+(_memoSavedText||'').slice(0,50)+((_memoSavedText||'').length>50?'…':'')+'"';
  document.getElementById('memoMarkerName').value='';
  document.getElementById('memoInputDesc').value='';
  _selMarkerColor='';_selMarkerName='';
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
  _renderMemoTemplateChips();_renderMarkerPresetChips();
  document.getElementById('memoInputOverlay').style.display='block';
  setTimeout(()=>document.getElementById('memoMarkerName').focus(),100);
}

async function openMemoInputFromTable(){
  document.getElementById('tblCtxMenu').style.display='none';
  _memoEditId=null;
  await _loadMemoData();
  document.getElementById('memoInputTitle').textContent='📝 메모 추가 (표)';
  document.getElementById('memoInputSaveBtn').onclick=saveMemoToSelection;
  const prev=document.getElementById('memoSelectedPreview');
  if(prev)prev.textContent=_memoSavedText?'"'+_memoSavedText.slice(0,50)+'"':'(표 안 텍스트 드래그 후 우클릭)';
  document.getElementById('memoMarkerName').value='';
  document.getElementById('memoInputDesc').value='';
  _selMarkerColor='';_selMarkerName='';
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
...
async function openMemoEdit(id){
  const el=document.querySelector(`[data-memoid="${id}"]`);if(!el)return;
  _memoEditId=id;
  await _loadMemoData();
  document.getElementById('memoInputTitle').textContent='📝 메모 수정';
  document.getElementById('memoInputSaveBtn').onclick=()=>applyMemoEdit(id);
  const prev=document.getElementById('memoSelectedPreview');
  if(prev)prev.textContent='"'+(el.textContent||'').replace(/\u200B/g,'').slice(0,50)+'"';
  _selMarkerColor=el.dataset.memoMc||'';_selMarkerName=el.dataset.memoMn||'';
  document.getElementById('memoMarkerName').value=_selMarkerName;
  document.getElementById('memoInputDesc').value=el.dataset.memoDesc||'';
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.toggle('sel',c.dataset.c===_selMarkerColor));
  _renderMemoTemplateChips();_renderMarkerPresetChips();
  document.getElementById('memoInputOverlay').style.display='block';
  setTimeout(()=>document.getElementById('memoMarkerName').focus(),100);
}

function applyMemoEdit(id){
  const el=document.querySelector(`[data-memoid="${id}"]`);if(!el){closeMemoInput();return;}
  const mn=document.getElementById('memoMarkerName').value.trim();
  const desc=document.getElementById('memoInputDesc').value||'';
  _selMarkerName=mn;
  if(desc.trim()&&!_memoTemplates.includes(desc.trim())){_memoTemplates.push(desc.trim());if(_memoTemplates.length>15)_memoTemplates=_memoTemplates.slice(-15);}
  if(_selMarkerColor&&mn&&!_markerPresets.some(p=>p.color===_selMarkerColor&&p.name===mn)){_markerPresets.push({color:_selMarkerColor,name:mn});if(_markerPresets.length>20)_markerPresets=_markerPresets.slice(-20);}
  _saveMemoData();
  el.dataset.memoDesc=desc;el.dataset.memoMc=_selMarkerColor;el.dataset.memoMn=mn;
  const bg=_selMarkerColor?_hexRgba(_selMarkerColor,.3):'rgba(255,213,79,.38)';
  const bord=_selMarkerColor||'#F57F17';
  el.style.cssText=`background:${bg};border-bottom:2px solid ${bord};border-radius:2px;cursor:pointer;display:inline;`;
  closeMemoInput();setTimeout(refreshMemoPanel,80);toast('메모가 수정되었습니다.');
}

function _renderMemoTemplateChips(){
  const el=document.getElementById('memoTemplateChips');if(!el)return;
  if(!_memoTemplates.length){el.innerHTML='<span style="font-size:.69rem;color:#bbb;">저장된 내용 없음</span>';return;}
  el.innerHTML=_memoTemplates.slice(-15).reverse().map(t=>`<span class="memo-tmpl-chip" onclick="document.getElementById('memoInputDesc').value=unescape('${escape(t)}')">${t.length>13?t.slice(0,13)+'…':t}</span>`).join('');
}
function _renderMarkerPresetChips(){
  const el=document.getElementById('memoMarkerPresets');if(!el)return;
  if(!_markerPresets.length){el.innerHTML='';return;}
  el.innerHTML=_markerPresets.slice(-12).reverse().map(p=>`<span class="mkr-preset-chip" onclick="applyMarkerPreset('${p.color}','${encodeURIComponent(p.name)}')"><span class="mkr-preset-dot" style="background:${p.color};"></span>${p.name}</span>`).join('');
}
function applyMarkerPreset(color,enc){
  const name=decodeURIComponent(enc);_selMarkerColor=color;
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.toggle('sel',c.dataset.c===color));
  const ni=document.getElementById('memoMarkerName');if(ni)ni.value=name;
}

function closeMemoInput(){
  document.getElementById('memoInputOverlay').style.display='none';
  if(!_memoEditId){_memoSavedRange=null;_memoSavedText='';}
  _memoEditId=null;
}

function saveMemoToSelection(){
  if(!_memoSavedRange){toast('메모를 달 텍스트가 없습니다.');return;}
  const mn=document.getElementById('memoMarkerName').value.trim();_selMarkerName=mn;
  const desc=document.getElementById('memoInputDesc').value||'';
  if(desc.trim()&&!_memoTemplates.includes(desc.trim())){_memoTemplates.push(desc.trim());if(_memoTemplates.length>15)_memoTemplates=_memoTemplates.slice(-15);}
  if(_selMarkerColor&&mn&&!_markerPresets.some(p=>p.color===_selMarkerColor&&p.name===mn)){_markerPresets.push({color:_selMarkerColor,name:mn});if(_markerPresets.length>20)_markerPresets=_markerPresets.slice(-20);}
  _saveMemoData();
  ...
  const sel=window.getSelection();sel.removeAllRanges();sel.addRange(_memoSavedRange);
  const span=document.createElement('span');
  span.className='re-memo';span.setAttribute('contenteditable','false');
  span.dataset.memoid=id;span.dataset.memoDesc=desc;span.dataset.memoDate=dateStr;
  span.dataset.memoResolved='false';span.dataset.memoMc=_selMarkerColor;span.dataset.memoMn=mn;
  const bg=_selMarkerColor?_hexRgba(_selMarkerColor,.3):'rgba(255,213,79,.38)';
  const bord=_selMarkerColor||'#F57F17';
  span.style.cssText=`background:${bg};border-bottom:2px solid ${bord};border-radius:2px;cursor:pointer;display:inline;`;
  try{_memoSavedRange.surroundContents(span);}
  catch{const frag=_memoSavedRange.extractContents();span.appendChild(frag);_memoSavedRange.insertNode(span);}
  try{const r=document.createRange();r.setStartAfter(span);r.collapse(true);sel.removeAllRanges();sel.addRange(r);document.execCommand('insertText',false,'\u200B');}catch{}
  _memoSavedRange=null;_memoSavedText='';_memoEditId=null;
  document.getElementById('memoInputOverlay').style.display='none';
  setTimeout(refreshMemoPanel,80);toast('메모가 저장되었습니다.');
}

function _hexRgba(hex,a){try{const r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);return `rgba(${r},${g},${b},${a})`;}catch{return `rgba(255,213,79,${a})`;}}

/* 메모 패널 */
let _memoDebTimer=null;
function debouncedMemoPanel(){clearTimeout(_memoDebTimer);_memoDebTimer=setTimeout(refreshMemoPanel,200);}

function refreshMemoPanel(){
  const editor=document.getElementById('reEditor');
  const panel=document.getElementById('reMemoPanel');const list=document.getElementById('reMemoList');
  const mPanel=document.getElementById('reMarkerPanel');const mList=document.getElementById('reMarkerList');const mStatus=document.getElementById('reMarkerStatus');
  const lr=document.getElementById('markerPanelResizer');const rr=document.getElementById('memoPanelResizer');
  if(!editor||!panel||!list)return;
  const memos=Array.from(editor.querySelectorAll('.re-memo'));
  if(!memos.length&&!_panelsForceOn){
    panel.style.display='none';if(mPanel)mPanel.style.display='none';
    if(lr)lr.style.display='none';if(rr)rr.style.display='none';return;
  }
  panel.style.display='block';if(rr)rr.style.display='block';
  const markerMap={};let doneCount=0,todoCount=0;
  memos.forEach(m=>{
    const c=m.dataset.memoMc,n=m.dataset.memoMn;
    if(c){const key=c+'|'+(n||'');if(!markerMap[key])markerMap[key]={color:c,name:n||'마커',count:0};markerMap[key].count++;}
    if(m.dataset.memoResolved==='true')doneCount++;else todoCount++;
  });
  const total=memos.length;
  if(mPanel){
    mPanel.style.display='block';if(lr)lr.style.display='block';
    if(mList){const rows=Object.values(markerMap).map(mk=>`<div class="marker-row"><div class="marker-dot" style="background:${mk.color};"></div><div class="marker-label">${mk.name}</div><div class="marker-count">${mk.count}</div></div>`).join('');mList.innerHTML=`<div style="background:#DBEAFE;border-radius:6px;padding:4px 7px;margin-bottom:5px;font-size:.7rem;color:#1e40af;font-weight:700;text-align:center;">전체 ${total}개</div>`+rows;}
    if(mStatus)mStatus.innerHTML=`<div style="display:flex;justify-content:space-between;font-size:.7rem;padding:2px 0;"><span>완료</span><span class="status-done">${doneCount}</span></div><div style="display:flex;justify-content:space-between;font-size:.7rem;padding:2px 0;"><span>미완료</span><span class="status-todo">${todoCount}</span></div>`;
  }
  list.innerHTML='';
  memos.forEach((m,i)=>{
    const id=m.dataset.memoid||('m'+i);m.dataset.memoid=id;
    const isRes=m.dataset.memoResolved==='true';
    const desc=m.dataset.memoDesc||'',date=m.dataset.memoDate||'';
    const mc=m.dataset.memoMc||'',mn=m.dataset.memoMn||'';
    const quote=(m.textContent||'').replace(/\u200B/g,'').slice(0,28);
    const borderColor=mc||(isRes?'#86EFAC':'#BFDBFE');
    const card=document.createElement('div');
    card.className='memo-card'+(isRes?' resolved':'');card.id='mc_'+id;card.style.borderLeftColor=borderColor;
    const markerHtml=mc?`<div class="memo-card-marker"><div class="memo-card-marker-dot" style="background:${mc};"></div><div class="memo-card-marker-name">${mn||'마커'}</div></div>`:'';
    card.innerHTML=`<div class="memo-card-header"><span class="memo-card-time">${date}</span></div>`+markerHtml+
      `<div class="memo-card-quote">"${quote}"</div>`+
      (desc||isRes?`<div class="memo-card-desc">${isRes?'<span class="memo-done-badge">✓ 완료</span>'+(desc?'<br><span style="color:#6b7280;font-size:.73rem;">'+desc+'</span>':''):desc}</div>`:'')+
      `<div class="memo-card-btns">`+
      (!isRes?`<button class="mresolve" onclick="event.stopPropagation();toggleResolveMemo('${id}')">✓</button>`:`<button class="munresolve" onclick="event.stopPropagation();toggleResolveMemo('${id}')">↩</button>`)+
      `<button class="medit" onclick="event.stopPropagation();openMemoEdit('${id}')">수정</button>`+
      `<button class="mdel" onclick="event.stopPropagation();deleteMemoCard('${id}')">삭제</button></div>`;
    card.onclick=function(e){if(!e.target.closest('button')){highlightMemoCard(m);m.scrollIntoView({behavior:'smooth',block:'center'});}};
    list.appendChild(card);
  });
}

function highlightMemoCard(memoEl){document.querySelectorAll('.re-memo').forEach(m=>m.classList.remove('active'));document.querySelectorAll('.memo-card').forEach(c=>c.classList.remove('active'));if(!memoEl)return;memoEl.classList.add('active');const card=document.getElementById('mc_'+memoEl.dataset.memoid);if(card)card.classList.add('active');}
function toggleResolveMemo(id){
  const el=document.querySelector(`[data-memoid="${id}"]`);if(!el)return;
  const isRes=el.dataset.memoResolved==='true';el.dataset.memoResolved=isRes?'false':'true';
  if(isRes){const mc=el.dataset.memoMc;const bg=mc?_hexRgba(mc,.3):'rgba(255,213,79,.38)';const bord=mc||'#F57F17';el.style.cssText=`background:${bg};border-bottom:2px solid ${bord};border-radius:2px;cursor:pointer;display:inline;`;}
  else{el.style.cssText='background:rgba(34,197,94,.18);border-bottom:2px solid #16A34A;border-radius:2px;cursor:pointer;display:inline;';}
  setTimeout(refreshMemoPanel,50);
}
function deleteMemoCard(id){
  ...
  p.insertBefore(frag,el);p.removeChild(el);
  const card=document.getElementById('mc_'+id);if(card)card.remove();
  setTimeout(refreshMemoPanel,50);toast('메모가 삭제되었습니다.');
}

/* 메모 관리 */
let _mmgrTab=1;
function switchMmgrTab(n){
  _mmgrTab=n;
  ['mmgrSec1','mmgrSec2'].forEach((id,i)=>{const el=document.getElementById(id);if(el)el.style.display=i+1===n?'block':'none';});
  ['mmgrTab1','mmgrTab2'].forEach((id,i)=>{const el=document.getElementById(id);if(el){el.className=i+1===n?'mbtn-s':'mbtn-c';el.style.fontSize='.78rem';}});
  if(n===2)_renderMmgrTplList();
}
async function openMemoManager(){
  await _loadMemoData();_initMmgrPalette();_renderMmgrList();
  _mmgrTab=1;switchMmgrTab(1);
  document.getElementById('memoManagerOverlay').style.display='block';
  setTimeout(()=>document.getElementById('mmgrMarkerName').focus(),100);
}
function closeMemoManager(){document.getElementById('memoManagerOverlay').style.display='none';}
function _renderMmgrList(){
  const el=document.getElementById('mmgrList');if(!el)return;
  if(!_markerPresets.length){el.innerHTML='<div style="font-size:.78rem;color:#bbb;text-align:center;padding:8px;">저장된 마커 없음</div>';return;}
  el.innerHTML=_markerPresets.map((p,i)=>`<div class="mmgr-item"><div class="mmgr-dot" style="background:${p.color};"></div><div style="flex:1;font-size:.82rem;font-weight:600;color:#1e3a5f;">${p.name}</div><button class="mbtn-c" style="padding:2px 8px;font-size:.72rem;color:#E53935;" onclick="deleteMarkerPreset(${i})">삭제</button></div>`).join('');
}
function _renderMmgrTplList(){
  const el=document.getElementById('mmgrTplList');if(!el)return;
  if(!_memoTemplates.length){el.innerHTML='<div style="font-size:.78rem;color:#bbb;text-align:center;padding:8px;">저장된 메모 내용 없음</div>';return;}
  el.innerHTML=_memoTemplates.map((t,i)=>`<div class="mmgr-item"><div style="flex:1;font-size:.82rem;color:#1e3a5f;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${t}</div><button class="mbtn-c" style="padding:2px 8px;font-size:.72rem;color:#E53935;flex-shrink:0;" onclick="deleteMemoTemplate(${i})">삭제</button></div>`).join('');
}
async function addMarkerPreset(){
  const name=document.getElementById('mmgrMarkerName').value.trim();
  if(!_mmgrSelColor){toast('색을 먼저 선택하세요.');return;}if(!name){toast('마커명을 입력하세요.');return;}
  if(_markerPresets.some(p=>p.color===_mmgrSelColor&&p.name===name)){toast('이미 있는 조합입니다.');return;}
  _markerPresets.push({color:_mmgrSelColor,name});if(_markerPresets.length>20)_markerPresets=_markerPresets.slice(-20);
  await _saveMemoData();_renderMmgrList();document.getElementById('mmgrMarkerName').value='';toast('마커 추가됨');
}
async function deleteMarkerPreset(idx){_markerPresets.splice(idx,1);await _saveMemoData();_renderMmgrList();toast('마커 삭제됨');}
async function addMemoTemplate(){
  const val=document.getElementById('mmgrTplInput').value.trim();
  if(!val){toast('내용을 입력하세요.');return;}
  if(_memoTemplates.includes(val)){toast('이미 있는 내용입니다.');return;}
  _memoTemplates.push(val);if(_memoTemplates.length>20)_memoTemplates=_memoTemplates.slice(-20);
  await _saveMemoData();_renderMmgrTplList();document.getElementById('mmgrTplInput').value='';toast('메모 내용 추가됨');
}
async function deleteMemoTemplate(idx){_memoTemplates.splice(idx,1);await _saveMemoData();_renderMmgrTplList();toast('삭제됨');}

/* 패널 리사이즈 */
let _panelRszState=null;
function startPanelResize(e,side){
  e.preventDefault();e.stopPropagation();
  const panel=document.getElementById(side==='left'?'reMarkerPanel':'reMemoPanel');
  if(!panel)return;
  _panelRszState={side,startX:e.clientX,startW:panel.offsetWidth,panel};
  document.body.style.cursor='col-resize';document.body.style.userSelect='none';
}
document.addEventListener('mousemove',function(e){
  if(!_panelRszState)return;
  const{side,startX,startW,panel}=_panelRszState;
  const dx=side==='left'?(e.clientX-startX):(startX-e.clientX);
  panel.style.width=Math.max(120,Math.min(320,startW+dx))+'px';
});
document.addEventListener('mouseup',function(){
  if(!_panelRszState)return;_panelRszState=null;document.body.style.cursor='';document.body.style.userSelect='';
});

/* ═══ 콜아웃 ═══ */
let _coCustomEmojis=[];
async function _loadCoCustomEmojis(){try{const row=await _sbRest.selectOne('site_data','co_custom_emojis');if(row&&Array.isArray(row.value))_coCustomEmojis=row.value;else if(row&&typeof row.value==='string')_coCustomEmojis=JSON.parse(row.value);}catch{}}
async function _saveCoCustomEmojis(){await _sbRest.upsert('site_data',{key:'co_custom_emojis',value:_coCustomEmojis,updated_at:new Date().toISOString()});}
const CO_COLORS=[{b:'#C7D7FF',bg:'#F0F4FF',t:'#1A3A8F'},{b:'#FFCDD2',bg:'#FFF3F5',t:'#B71C1C'},{b:'#C8E6C9',bg:'#F1F8F1',t:'#1B5E20'},{b:'#FFE082',bg:'#FFFDE7',t:'#5D4037'},{b:'#FFCC80',bg:'#FFF8F0',t:'#E65100'},{b:'#CE93D8',bg:'#F8F0FF',t:'#4A148C'},{b:'#E0E0E0',bg:'#F9F9F9',t:'#212121'},{b:'#B2EBF2',bg:'#E0F7FA',t:'#006064'},{b:'#F8BBD9',bg:'#FCE4EC',t:'#880E4F'},{b:'#DCEDC8',bg:'#F9FBE7',t:'#33691E'},{b:'#D1C4E9',bg:'#EDE7F6',t:'#311B92'},{b:'#B3E5FC',bg:'#E1F5FE',t:'#01579B'},{b:'#FFD180',bg:'#FFFBF0',t:'#BF6000'},{b:'#80CBC4',bg:'#E0F2F1',t:'#004D40'}];
const CO_EMOJIS=['💡','⚠️','✅','📌','🔥','📝','🎯','💬','🔑','📚','🎨','💎','🚀','🌟','🎵','❓','❗','🔔','📊','🗂️','🌈','⭐','🏆','💪','🧠','🔍','💼','📅','🌐','🔒','🎉','✨','🛠️','📣','🤝','🌱','💫','🔗','📱','🖥️','✍️','🗒️','🔺','🔻','▶️','⏸️','⏹️','🔄','🎬','📸'];
let _coPick={color:CO_COLORS[0],emoji:'💡'};
function _coEmojiGridHTML(sel){const c=_coCustomEmojis.map((em,i)=>{const isImg=em.startsWith('data:')||em.startsWith('http');const inner=isImg?`<img src="${em}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;">`:`${em}`;return `<span class="co-em-btn${em===sel?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${inner}<span class="co-em-del" onclick="event.stopPropagation();delCoCustomEmoji(${i})">✕</span></span>`;});const base=CO_EMOJIS.map(em=>`<span class="co-em-btn${em===sel?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${em}</span>`);const none=`<span class="co-em-btn co-em-none${sel===''?' sel':''}" data-em="" onclick="pickCoEmoji(this)">없음</span>`;return [...c,...base,none].join('');}
function _coColorGridHTML(selB){return CO_COLORS.map((c,i)=>`<div class="co-sw${c.b===selB?' sel':''}" style="background:${c.b};" data-i="${i}" onclick="pickCoColor(this)"></div>`).join('');}
async function openCalloutPicker(){await _loadCoCustomEmojis();_coPick={color:CO_COLORS[0],emoji:'💡'};const cg=document.getElementById('coColorRow'),eg=document.getElementById('coEmojiGrid');if(cg)cg.innerHTML=_coColorGridHTML(CO_COLORS[0].b);if(eg)eg.innerHTML=_coEmojiGridHTML('💡');document.getElementById('coPickerOverlay').style.display='block';}
function closeCalloutPicker(){document.getElementById('coPickerOverlay').style.display='none';}
function pickCoColor(el){document.querySelectorAll('#coColorRow .co-sw').forEach(s=>s.classList.remove('sel'));el.classList.add('sel');_coPick.color=CO_COLORS[parseInt(el.dataset.i)];}
function pickCoEmoji(el){const g=el.closest('#coEmojiGrid,#coEmojiChangeGrid');if(g)g.querySelectorAll('.co-em-btn').forEach(s=>s.classList.remove('sel'));el.classList.add('sel');_coPick.emoji=el.dataset.em||'';}
async function handleCoCustomImg(e){const file=e.target.files[0];e.target.value='';if(!file)return;await _loadCoCustomEmojis();const r=new FileReader();r.onload=async ev=>{_coCustomEmojis.push(ev.target.result);await _saveCoCustomEmojis();const sel=_coPick.emoji;['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});toast('커스텀 이모지 저장됨');};r.readAsDataURL(file);}
async function delCoCustomEmoji(idx){await _loadCoCustomEmojis();_coCustomEmojis.splice(idx,1);await _saveCoCustomEmojis();const sel=_coPick.emoji;['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});toast('삭제됨');}
function _buildCalloutHtml(color,emoji){const isImg=emoji&&(emoji.startsWith('data:')||emoji.startsWith('http'));const em=emoji?(isImg?`<img src="${emoji}" data-coem="${emoji}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;" contenteditable="false">`:`<span data-coem="${emoji}" contenteditable="false" style="font-size:1.2rem;flex-shrink:0;margin-top:1px;">${emoji}</span>`):'';const st=`display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid ${color.b};background:${color.bg};color:${color.t};line-height:1.7;`;return `<div class="re-callout" contenteditable="false" style="${st}">${em}<div contenteditable="true" style="flex:1;min-width:0;outline:none;">내용을 입력하세요</div></div><p><br></p>`;}
function doInsertCallout(){closeCalloutPicker();reExec('insertHTML',_buildCalloutHtml(_coPick.color,_coPick.emoji));}
let _coChangeTarget=null;
async function openCoEmojiChange(calloutEl){_coChangeTarget=calloutEl;await _loadCoCustomEmojis();const cur=calloutEl.querySelector('[data-coem]');const curEm=cur?cur.dataset.coem:'';_coPick.emoji=curEm;const g=document.getElementById('coEmojiChangeGrid');if(g)g.innerHTML=_coEmojiGridHTML(curEm);document.getElementById('coEmojiChangeOverlay').style.display='block';}
function closeCoEmojiChange(){document.getElementById('coEmojiChangeOverlay').style.display='none';_coChangeTarget=null;}
function applyCoEmojiChange(){if(!_coChangeTarget)return;const old=_coChangeTarget.querySelector('[data-coem]');if(old)old.remove();if(_coPick.emoji){const isImg=_coPick.emoji.startsWith('data:')||_coPick.emoji.startsWith('http');const el=document.createElement(isImg?'img':'span');el.setAttribute('data-coem',_coPick.emoji);el.contentEditable='false';if(isImg){el.src=_coPick.emoji;el.style.cssText='width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;';}else{el.style.cssText='font-size:1.2rem;flex-shrink:0;margin-top:1px;';el.textContent=_coPick.emoji;}const body=_coChangeTarget.querySelector('div[contenteditable="true"]');if(body)_coChangeTarget.insertBefore(el,body);else _coChangeTarget.prepend(el);}closeCoEmojiChange();}

/* ═══ 표 ═══ */
let _tblTd=null;
function selectTableCell(td){clearTableCellSel();td.classList.add('tbl-sel');_tblTd=td;}
function clearTableCellSel(){document.querySelectorAll('.tbl-sel').forEach(c=>c.classList.remove('tbl-sel'));}
function showTblCtxMenu(e,td){selectTableCell(td);const menu=document.getElementById('tblCtxMenu');menu.style.display='block';menu.style.left=Math.min(e.clientX,window.innerWidth-200)+'px';menu.style.top=Math.min(e.clientY,window.innerHeight-400)+'px';const close=ev=>{if(!ev.target.closest('#tblCtxMenu')){menu.style.display='none';document.removeEventListener('mousedown',close);}};setTimeout(()=>document.addEventListener('mousedown',close),10);}
function tblCmd(cmd){
  const td=_tblTd;if(!td)return;const tr=td.closest('tr'),table=td.closest('table');if(!tr||!table){_tblTd=null;return;}
  const rows=Array.from(table.querySelectorAll('tr'));const ri=rows.indexOf(tr);const cells=Array.from(tr.querySelectorAll('td,th'));const ci=cells.indexOf(td);const cols=rows[0]?rows[0].querySelectorAll('td,th').length:1;
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;';const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;';
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){const nr=document.createElement('tr');for(let i=0;i<cols;i++){const c=document.createElement('td');c.contentEditable='true';c.style.cssText=TDS;nr.appendChild(c);}cmd==='addRowAbove'?tr.parentNode.insertBefore(nr,tr):tr.parentNode.insertBefore(nr,tr.nextSibling);}
  else if(cmd==='addColLeft'||cmd==='addColRight'){rows.forEach((row,ri2)=>{const c=Array.from(row.querySelectorAll('td,th'))[ci];if(!c)return;const tag=ri2===0?'th':'td';const nc=document.createElement(tag);nc.contentEditable='true';nc.style.cssText=ri2===0?THS:TDS;cmd==='addColLeft'?c.parentNode.insertBefore(nc,c):c.parentNode.insertBefore(nc,c.nextSibling);});}
  else if(cmd==='mergeRight'){const nx=td.nextElementSibling;if(nx){td.colSpan=(td.colSpan||1)+(nx.colSpan||1);td.innerHTML+=nx.innerHTML;nx.remove();}}
  else if(cmd==='mergeDown'){const nr=rows[ri+1];if(nr){const bl=Array.from(nr.querySelectorAll('td,th'))[ci];if(bl){td.rowSpan=(td.rowSpan||1)+(bl.rowSpan||1);td.innerHTML+=bl.innerHTML;bl.remove();}}}
  else if(cmd==='splitCell'){const cs=td.colSpan||1,rs=td.rowSpan||1;if(cs<=1&&rs<=1){toast('병합된 셀이 아닙니다.');_tblTd=null;return;}td.colSpan=1;td.rowSpan=1;for(let r2=ri;r2<ri+rs;r2++){const sc=(r2===ri)?ci+1:ci;for(let c2=sc;c2<ci+cs;c2++){const nc=document.createElement('td');nc.contentEditable='true';nc.style.cssText=TDS;const tRow=rows[r2];if(tRow){const sib=Array.from(tRow.querySelectorAll('td,th'))[c2];tRow.insertBefore(nc,sib||null);}}}}
  else if(cmd==='splitH'){const colspan=td.colSpan||1;const newTr=document.createElement('tr');cells.forEach((cell,i)=>{if(i===ci){const nc=document.createElement('td');nc.contentEditable='true';nc.style.cssText=cell.style.cssText;nc.colSpan=colspan;newTr.appendChild(nc);}else{cell.rowSpan=(cell.rowSpan||1)+1;}});tr.parentNode.insertBefore(newTr,tr.nextSibling);}
  else if(cmd==='delRow'){if(rows.length>1)tr.remove();}else if(cmd==='delCol'){if(cols>1)rows.forEach(row=>{const c=Array.from(row.querySelectorAll('td,th'))[ci];if(c)c.remove();});}else if(cmd==='delTable'){table.remove();}
  _tblTd=null;clearTableCellSel();
  setTimeout(()=>_addColHandlesToAllTables(),50);
}
const CELL_COLORS30=['#FFFFFF','#F5F6FA','#FFECEC','#FFF3E0','#FFFDE7','#F1F8E9','#E8F5E9','#E0F7FA','#E1F5FE','#E8EAF6','#F3E5F5','#FCE4EC','#FFCDD2','#FFE0B2','#FFF9C4','#DCEDC8','#B2DFDB','#B3E5FC','#C5CAE9','#E1BEE7','#F8BBD9','#D7CCC8','#CFD8DC','#FFAB91','#FFD54F','#AED581','#4DB6AC','#4FC3F7','#7986CB','#F06292'];
let _selCellClr='';
function openCellColorPicker(){if(!_tblTd){toast('셀을 먼저 클릭하여 선택하세요.');return;}const cur=_tblTd.style.background||'';const g=document.getElementById('cellClrGrid');if(g)g.innerHTML=CELL_COLORS30.map(c=>`<div class="clr-chip${c===cur?' sel':''}" style="background:${c};${c==='#FFFFFF'?'border-color:#ccc;':''}" onclick="pickCellClr(this,'${c}')"></div>`).join('');_selCellClr=cur;document.getElementById('cellClrOverlay').style.display='block';}
function closeCellClr(){document.getElementById('cellClrOverlay').style.display='none';}
function pickCellClr(el,color){document.querySelectorAll('#cellClrGrid .clr-chip').forEach(c=>c.classList.remove('sel'));el.classList.add('sel');_selCellClr=color;}
function applyCellClr(color){if(_tblTd)_tblTd.style.background=color||'';closeCellClr();_tblTd=null;clearTableCellSel();}
const TBL_BORDER_COLORS=['#E5E8EB','#191F28','#D91F3E','#FF7A00','#0064FF','#2E9E50','#9C27B0','#8B95A1','#F44336','#E91E63','#673AB7','#3F51B5','#2196F3','#03A9F4','#00BCD4','#009688','#4CAF50','#8BC34A','#CDDC39','#FFEB3B','#FFC107','#FF9800','#FF5722','#795548','#607D8B','#BDBDBD','#90CAF9','#A5D6A7','#FFCC80','#CE93D8','#E8D5D5','#E8E0D5','#E8E8D5','#D5E8D5','#D5E8E8','#D5D5E8','#E8D5E8','#E2E8D5','#D5E2E8','#E8DDD5','#DDE8D5','#D5DDE8','#E8D5DD','#E5DDE8','#D8E8E5'];
let _selTblBorderClr='#E5E8EB';
function openTblBorderColorPicker(){if(!_tblTd){toast('셀을 먼저 클릭하여 선택하세요.');return;}const table=_tblTd.closest('table');const cur=(table&&table.style.borderColor)||'#E5E8EB';const g=document.getElementById('tblBorderClrGrid');if(g)g.innerHTML=TBL_BORDER_COLORS.map(c=>`<div class="clr-chip${c===cur?' sel':''}" style="background:${c};" onclick="pickTblBorderClr(this,'${c}')"></div>`).join('');_selTblBorderClr=cur;document.getElementById('tblBorderClrOverlay').style.display='block';}
function closeTblBorderClr(){document.getElementById('tblBorderClrOverlay').style.display='none';}
function pickTblBorderClr(el,c){document.querySelectorAll('#tblBorderClrGrid .clr-chip').forEach(x=>x.classList.remove('sel'));el.classList.add('sel');_selTblBorderClr=c;}
function applyTblBorderClr(color){if(!_tblTd)return;const table=_tblTd.closest('table');if(!table)return;table.style.border='1px solid '+color;table.querySelectorAll('td,th').forEach(cell=>{cell.style.border='1px solid '+color;});closeTblBorderClr();}

/* ═══════════════════════════════════════════
   표 열 리사이즈 — 핵심 설계
   
   각 셀 오른쪽에 .col-rsz-h div를 삽입.
   이 div는 contenteditable=false, position:absolute.
   셀은 position:relative.
   
   핸들 mousedown → 드래그 → 열 너비 변경.
   contenteditable과 완전 분리됨.
═══════════════════════════════════════════ */
let _colDrag=null,_rowDrag=null;

function _addColHandlesToAllTables(){
  const editor=document.getElementById('reEditor');if(!editor)return;
  // 기존 핸들 제거
  editor.querySelectorAll('.col-rsz-h').forEach(h=>h.remove());
  editor.querySelectorAll('table').forEach(table=>{
    ...
    rows.forEach(tr=>{
      ...
      cells.forEach((cell,ci)=>{
        cell.style.position='relative'; // 핸들의 기준점
        if(ci===cells.length-1)return; // 마지막 열 제외
        if(cell.querySelector('.col-rsz-h'))return; // 이미 있으면 건너뜀
        const h=document.createElement('div');
        h.className='col-rsz-h';
        h.contentEditable='false';
        h.addEventListener('mousedown',function(e){
          e.preventDefault();
          e.stopPropagation();
          // 같은 열 인덱스의 모든 셀 수집
          const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
          _colDrag={cells:colCells,startX:e.clientX,startW:cell.offsetWidth};
...
document.addEventListener('mousemove',function(e){
  if(_colDrag){
    ...
    const newW=Math.max(20,_colDrag.startW+dx);
    _colDrag.cells.forEach(c=>{
  ...
  if(_rowDrag){
    const newH=Math.max(10,_rowDrag.startH+(e.clientY-_rowDrag.startY));
    _rowDrag.tr.style.height=newH+'px';
    Array.from(_rowDrag.tr.querySelectorAll('td,th')).forEach(c=>{c.style.height=newH+'px';});
  }
});
document.addEventListener('mouseup',function(){
  if(_colDrag){
    document.querySelectorAll('.col-rsz-h.active').forEach(h=>h.classList.remove('active'));
    _colDrag=null;document.body.style.cursor='';document.body.style.userSelect='';
  }
  if(_rowDrag){_rowDrag=null;}
});

// 에디터에서 행 높이 드래그 (셀 하단 6px)
function _initRowResize(editor){
  if(!editor||editor._rowRszSet)return;editor._rowRszSet=true;
  let _rowEdge=null;
  editor.addEventListener('mousemove',function(e){
    if(_colDrag||_rowDrag)return;
    if(e.target.classList&&e.target.classList.contains('col-rsz-h')){editor.style.cursor='col-resize';return;}
    const cell=e.target.closest&&e.target.closest('td,th');
    if(!cell){if(editor.style.cursor==='row-resize')editor.style.cursor='';_rowEdge=null;return;}
    const rect=cell.getBoundingClientRect();
    const ry=e.clientY-rect.top;
    if(ry>=rect.height-6){editor.style.cursor='row-resize';_rowEdge=cell;}
    else{if(editor.style.cursor==='row-resize')editor.style.cursor='';_rowEdge=null;}
  });
  editor.addEventListener('mousedown',function(e){
    if(!_rowEdge)return;
    if(e.target.classList&&e.target.classList.contains('col-rsz-h'))return;
    const rect=_rowEdge.getBoundingClientRect();
    if(e.clientY-rect.top<rect.height-6)return;
    e.preventDefault();e.stopPropagation();
    const tr=_rowEdge.closest('tr');
    _rowDrag={tr,startY:e.clientY,startH:tr.offsetHeight};
    document.body.style.cursor='row-resize';document.body.style.userSelect='none';
  });
  // MutationObserver: 표 변경 시 핸들 재생성
  const obs=new MutationObserver(()=>{clearTimeout(obs._t);obs._t=setTimeout(_addColHandlesToAllTables,300);});
  obs.observe(editor,{childList:true,subtree:true,attributes:false});
}

/* ═══ PDF ═══ */
function exportDocAsPdf(){const editor=document.getElementById('reEditor');if(!editor){toast('에디터를 열어주세요.');return;}if(editor.querySelectorAll('.re-memo').length>0)document.getElementById('pdfOptOverlay').style.display='block';else doPdfExport();}
function closePdfOpt(){document.getElementById('pdfOptOverlay').style.display='none';}
function doPdfExport(){
  closePdfOpt();const editor=document.getElementById('reEditor');const title=document.getElementById('reTitle')?.value||'문서';const withMemo=document.getElementById('pdfOptMemo')?.checked!==false;
  const clone=editor.cloneNode(true);clone.querySelectorAll('.col-rsz-h').forEach(h=>h.remove());
  let memoSection='';
  if(withMemo){
    const memos=[];editor.querySelectorAll('.re-memo').forEach(m=>memos.push({text:(m.textContent||'').replace(/\u200B/g,''),desc:m.dataset.memoDesc||'',resolved:m.dataset.memoResolved==='true',mc:m.dataset.memoMc||'',mn:m.dataset.memoMn||''}));
    if(memos.length){memoSection='<div style="margin-top:32px;border-top:2px solid #BFDBFE;padding-top:16px;"><div style="font-weight:800;font-size:.95rem;margin-bottom:10px;color:#1e3a5f;">📝 메모 목록</div>';memos.forEach(m=>{const dot=m.mc?`<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${m.mc};margin-right:4px;vertical-align:middle;"></span>`:'';memoSection+=`<div style="margin-bottom:8px;background:${m.resolved?'#F0FDF4':'#EFF6FF'};border-left:3px solid ${m.resolved?'#86EFAC':m.mc||'#BFDBFE'};padding:8px 12px;border-radius:0 6px 6px 0;font-size:.85rem;"><div style="font-weight:700;color:#1e3a5f;">${dot}${m.mn||'메모'}${m.resolved?' ✓':''}</div><div style="color:#6b7280;font-size:.78rem;margin:2px 0;">"${m.text}"</div>${m.desc?`<div style="color:#374151;margin-top:3px;">${m.desc}</div>`:''}</div>`;});memoSection+='</div>';}
  }else{clone.querySelectorAll('.re-memo').forEach(m=>{const frag=document.createDocumentFragment();while(m.firstChild)frag.appendChild(m.firstChild);m.replaceWith(frag);});}
  const win=window.open('','_blank');
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title}</title><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700;800&display=swap" rel="stylesheet"><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Noto Sans KR',sans-serif;font-size:14px;line-height:1.9;color:#191F28;padding:40px 48px;}h1{font-size:2rem;font-weight:800;margin:1em 0 .3em;}h2{font-size:1.5rem;font-weight:700;margin:.9em 0 .25em;}h3{font-size:1.2rem;font-weight:700;margin:.8em 0 .2em;}p{margin:.4em 0;}ul,ol{padding-left:1.5em;margin:.4em 0;}li{margin:.15em 0;}blockquote{border-left:4px solid #D91F3E;padding:.5em 1em;background:#FFF3F5;border-radius:0 6px 6px 0;margin:.6em 0;}hr{border:none;border-top:2px solid #E5E8EB;margin:1em 0;}.re-callout{display:flex;gap:10px;border-radius:8px;padding:12px 14px;margin:.6em 0;}table{border-collapse:collapse;width:100%;margin:.6em 0;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}img{max-width:100%;}.col-rsz-h{display:none!important;}@media print{body{padding:20px;}}</style></head><body><h1 style="border-bottom:2px solid #E5E8EB;padding-bottom:10px;margin-bottom:20px;">${title}</h1>${clone.innerHTML}${memoSection}<script>window.onload=function(){window.print();}<\/script></body></html>`);
  win.document.close();
}

/* ═══ 백업 ═══ */
async function exportBackup(){toast('백업 중...');try{const rows=await _sbRest.select('site_data');if(!rows||rows.length===0){toast('데이터 없음');return;}const obj={_v:1,_d:new Date().toISOString(),data:{}};rows.forEach(r=>{obj.data[r.key]=r.value;});const url=URL.createObjectURL(new Blob([JSON.stringify(obj,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='backup_'+new Date().toISOString().slice(0,16).replace('T','_').replace(':','-')+'.json';document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);toast('백업 완료 — '+rows.length+'개');}catch{toast('백업 실패');}}
async function importBackup(event){const file=event.target.files[0];event.target.value='';if(!file)return;if(!confirm('⚠ 현재 데이터가 덮어씌워집니다.\n\n파일: '+file.name))return;toast('복원 중...');try{const obj=JSON.parse(await file.text());if(!obj.data){toast('올바른 파일 아님');return;}for(const[k,v] of Object.entries(obj.data))await _sbRest.upsert('site_data',{key:k,value:v,updated_at:new Date().toISOString()});_applyMap(obj.data);applyColors();applyConfig();applyCalCfg();renderSection('docs');renderSection('webtoon');renderLegend();renderTagFilter();document.getElementById('calNameText').textContent=CAL_NAME;renderMonthly();closeSiteSettings();toast('복원 완료');}catch{toast('복원 실패');}}
...
<style>
*{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Noto Sans KR',sans-serif;background:#ECEEF2;color:#191F28;font-size:15px;-webkit-font-smoothing:antialiased;padding:32px 20px 80px;}
.doc-wrap{max-width:760px;margin:0 auto;background:#fff;border-radius:14px;box-shadow:0 4px 24px rgba(0,0,0,.1);padding:56px 64px 80px;}
...
aside,.re-callout{display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid #C7D7FF;background:#F0F4FF;color:#1A3A8F;line-height:1.7;}
...
async function _readDocRawAsync(docId){
  try{const row=await _sbRest.selectOne('site_data','hub_doc_'+docId);if(!row||row.value==null)return '';let raw=typeof row.value==='string'?row.value:JSON.stringify(row.value);if(raw.startsWith('"')&&raw.endsWith('"')){try{const d=JSON.parse(raw);if(typeof d==='string')raw=d;}catch{}}return raw;}catch{return '';}
}
function _readDocRaw(docId){return '';}
async function openHtmlEditor(docId=''){
  _editDocId=docId;closeDocTypeModal();
  if(docId){const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);document.getElementById('deTitle').value=d?.title||'';document.getElementById('deContent').value='불러오는 중...';document.getElementById('docEditorOverlay').classList.add('open');const raw=await _readDocRawAsync(docId);document.getElementById('deContent').value=raw||'';updatePreview();}
  else{document.getElementById('deTitle').value='';document.getElementById('deContent').value='';document.getElementById('docEditorOverlay').classList.add('open');updatePreview();}
  setTimeout(()=>document.getElementById('deTitle').focus(),200);
}
...
async function openRichEditor(docId=''){
  _reEditDocId=docId;closeDocTypeModal();
  const editor=document.getElementById('reEditor');
  const titleIn=document.getElementById('reTitle');
  if(!editor._evSet){
    editor._evSet=true;
    editor.addEventListener('paste',function(e){e.preventDefault();const h=e.clipboardData.getData('text/html');if(h){const CS='display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid #C7D7FF;background:#F0F4FF;color:#1A3A8F;line-height:1.7;';document.execCommand('insertHTML',false,h.replace(/<aside[^>]*>/gi,'<div class="re-callout" style="'+CS+'">').replace(/<\/aside>/gi,'</div>'));}else{document.execCommand('insertText',false,e.clipboardData.getData('text/plain'));}});
    editor.addEventListener('contextmenu',function(e){
      const td=e.target.closest&&e.target.closest('td,th');
      if(td){e.preventDefault();const sel=window.getSelection();if(sel&&!sel.isCollapsed){_memoSavedRange=sel.getRangeAt(0).cloneRange();_memoSavedText=sel.toString().trim();}else{_memoSavedRange=null;_memoSavedText='';}showTblCtxMenu(e,td);return;}
      const sel=window.getSelection();
      if(sel&&!sel.isCollapsed){e.preventDefault();_memoSavedRange=sel.getRangeAt(0).cloneRange();_memoSavedText=sel.toString().trim();showEditorCtxMenu(e);}
    });
    editor.addEventListener('click',function(e){
      document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');
      ['editorCtxMenu','tblCtxMenu'].forEach(id=>{const m=document.getElementById(id);if(m)m.style.display='none';});
      if(e.target.classList&&e.target.classList.contains('col-rsz-h'))return;
      const td=e.target.closest&&e.target.closest('td,th');
      if(td)selectTableCell(td);else clearTableCellSel();
      const memo=e.target.closest&&e.target.closest('.re-memo');
      if(memo&&!e.target.closest('button')){e.stopPropagation();highlightMemoCard(memo);memo.scrollIntoView({behavior:'smooth',block:'center'});}
    });
    editor.addEventListener('dblclick',function(e){const callout=e.target.closest&&e.target.closest('.re-callout');if(callout)openCoEmojiChange(callout);});
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
    titleIn.value=d?.title||'';
    editor.innerHTML='<p style="color:#8B95A1;font-size:.88rem;">불러오는 중...</p>';
    document.getElementById('richEditorOverlay').classList.add('open');
    reUpdateToolbar();setTimeout(()=>{_initImgResize();_initRowResize(editor);_addColHandlesToAllTables();initColorPalettes();_initMemoMarkerPalette();},300);
    const raw=await _readDocRawAsync(docId);editor.innerHTML=raw||'';
    setTimeout(refreshMemoPanel,80);
  }else{
    titleIn.value='';editor.innerHTML='';
    document.getElementById('richEditorOverlay').classList.add('open');
    reUpdateToolbar();setTimeout(()=>{_initImgResize();_initRowResize(editor);_addColHandlesToAllTables();initColorPalettes();_initMemoMarkerPalette();},300);
    refreshMemoPanel();
  }
  setTimeout(()=>{if(!docId)titleIn.focus();else editor.focus();},200);
}
...
function reInsertImage(){document.getElementById('reImgFileIn').click();}
function reHandleImg(e){
  const file=e.target.files[0];if(!file)return;const r=new FileReader();
  r.onload=async ev=>{toast('이미지 업로드 중...');const url=await _uploadImgToStorage(ev.target.result,`richtext/${uid()}_${Date.now()}.jpg`);const HS='position:absolute;width:12px;height:12px;background:#D91F3E;border-radius:50%;z-index:10;border:2px solid #fff;';reExec('insertHTML','<span class="re-img-wrap" style="display:inline-block;position:relative;width:360px;max-width:100%;vertical-align:top;margin:.5em 0;" contenteditable="false"><img src="'+url+'" style="width:100%;border-radius:8px;display:block;" draggable="false"><span class="re-img-handle" data-dir="se" style="'+HS+'right:-6px;bottom:-6px;cursor:se-resize;"></span><span class="re-img-handle" data-dir="sw" style="'+HS+'left:-6px;bottom:-6px;cursor:sw-resize;"></span><span class="re-img-handle" data-dir="ne" style="'+HS+'right:-6px;top:-6px;cursor:ne-resize;"></span><span class="re-img-handle" data-dir="nw" style="'+HS+'left:-6px;top:-6px;cursor:nw-resize;"></span></span><span contenteditable="true"> </span>');setTimeout(()=>_initImgResize(),100);toast('이미지 삽입 완료');};r.readAsDataURL(file);e.target.value='';
}
let _imgRsz=null;
function _initImgResize(){const ed=document.getElementById('reEditor');if(!ed||ed._imgRszSet)return;ed._imgRszSet=true;ed.addEventListener('mousedown',function(e){const h=e.target.closest&&e.target.closest('.re-img-handle');if(!h)return;e.preventDefault();const w=h.parentElement;_imgRsz={w,dir:h.dataset.dir,sx:e.clientX,sw:w.offsetWidth};});document.addEventListener('mousemove',function(e){if(!_imgRsz)return;const dx=e.clientX-_imgRsz.sx;_imgRsz.w.style.width=Math.max(60,Math.min((_imgRsz.dir==='se'||_imgRsz.dir==='ne')?_imgRsz.sw+dx:_imgRsz.sw-dx,760))+'px';});document.addEventListener('mouseup',()=>{_imgRsz=null;});}
function reInsertTable(){
  const rows=3,cols=3;
  let html='<table style="border-collapse:collapse;width:100%;table-layout:fixed;margin:.8em 0;border:1px solid #E5E8EB;" contenteditable="false"><colgroup>';
  for(let c=0;c<cols;c++)html+='<col style="width:'+Math.floor(100/cols)+'%">';
  html+='</colgroup><tbody>';
  for(let r=0;r<rows;r++){html+='<tr>';for(let c=0;c<cols;c++){const isH=r===0;html+='<'+(isH?'th':'td')+' contenteditable="true" style="padding:10px 14px;border:1px solid #E5E8EB;'+(isH?'background:#F5F6FA;font-weight:700;text-align:left;':'')+'">'+(isH?'제목'+(c+1):'')+'</'+(isH?'th':'td')+'>';}html+='</tr>';}
  html+='</tbody></table><p><br></p>';
  reExec('insertHTML',html);
  // 핸들 삽입
  setTimeout(()=>_addColHandlesToAllTables(),150);
  toast('3×3 표 삽입 | 열 경계 드래그로 너비 조절 | 우클릭 편집');
}
...
function reOnKeydown(e){
  if(e.key==='Tab'){e.preventDefault();const sel=window.getSelection();if(!sel||!sel.rangeCount)return;const node=sel.getRangeAt(0).commonAncestorContainer;const li=(node.nodeType===1?node:node.parentElement)?.closest('li');if(li){e.shiftKey?document.execCommand('outdent'):document.execCommand('indent');}else{if(e.shiftKey)document.execCommand('outdent');else{const inList=document.queryCommandState('insertUnorderedList')||document.queryCommandState('insertOrderedList');if(!inList)document.execCommand('insertUnorderedList');else document.execCommand('indent');}}return;}
  if(e.key==='Enter'){const tag=(document.queryCommandValue('formatBlock')||'').toLowerCase();if(tag==='pre'){e.preventDefault();reExec('insertHTML','\n');return;}}
  // 인라인 마크다운: ** ** → bold, *_ → italic, ~~ → strike 는 keyup에서 처리
}
function reOnKeyup(e){
  if(e.key===' '||e.key==='Enter')checkMarkdownShortcut();
}
