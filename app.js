...
function saveMemoInput() {
  ...
  try {
    /* span 생성 - contentEditable=false 로 커서가 내부로 진입하지 않게 */
    const span = document.createElement('span');
    span.className          = 're-memo-mark';
    span.dataset.memoId     = id;
    span.contentEditable    = 'false';
    span.style.cssText      = `background:${_hexA(color,.38)};border-bottom:2px solid ${color};border-radius:2px;cursor:pointer;display:inline;`;
    span.title              = (markerName || '메모') + (content ? ' — ' + content.slice(0,30) : '');
    span.onclick = e => { e.stopPropagation(); _focusMemo(id); };

    /* 선택 영역 추출 후 span에 넣기 */
    const frag = _memoCtxRange.extractContents();
    span.appendChild(frag);
    _memoCtxRange.insertNode(span);

    /* 커서를 span 바로 오른쪽으로 이동 (span 내부로 들어가지 않게) */
...
function deleteMemo(id) {
  ...
  if (m.spanEl) {
    ...
    if (p) {
      const t = document.createTextNode(m.spanEl.textContent);
      p.insertBefore(t, m.spanEl);
      p.removeChild(m.spanEl);
    }
  }
  _memos.splice(idx, 1);
...
function buildTreeHTML(docs,parentId,depth){
  ...
  items.forEach(item=>{
    ...
    if(item.type==='folder'){
      ...
      const clickFn = isRich ? `openRichViewer('${item.id}')` : `openDocViewer('${item.id}')`;
      html+=`<div class="tree-doc-item" draggable="true" data-idx="${globalIdx}"
        ...
        <div class="tree-doc-title">${isRich?'✍ ':''}${item.title}</div>
...
const RD_COLORS=['#E53935','#D81B60','#C2185B','#AB47BC','#7B1FA2','#5C6BC0','#3F51B5','#1E88E5','#039BE5','#0097A7','#00897B','#2E9E50','#43A047','#689F38','#9E9D24','#F9A825','#FFB300','#FF8F00','#FF7A00','#FF5722','#8D6E63','#795548','#6D4C41','#78909C','#607D8B','#455A64','#263238','#E91E63','#00BCD4','#333333'];

function _rdSync(){
  _rdData.pages.forEach((p,pi)=>{
    (p.leftValues||[]).forEach((_,li)=>{
      const inp=document.getElementById('rdLV-'+pi+'-'+li);
      if(inp) p.leftValues[li]=inp.value;
    });
    p.images.forEach((img,ci)=>{
      const lbl=document.getElementById('rdImgLbl-'+pi+'-'+ci);
      if(lbl) img.label=lbl.value;
    });
    const opEl=document.getElementById('rdOpinion-'+pi);
    if(opEl) p.opinion=opEl.innerHTML;
  });
  _rdData.leftColumns.forEach((col,li)=>{
    const inp=document.getElementById('rdLCName-'+li);
    if(inp) col.name=inp.value;
  });
}
...
async function openRefDocModal(docId=''){
  ...
  if(docId){
    ...
    if(stored&&typeof stored==='object'){
      if(stored.leftColumns){
        _rdData=stored;
        // label 필드 보장
        _rdData.pages.forEach(p=>{(p.images||[]).forEach(img=>{if(!img.label)img.label='';});});
      } else if(stored.columns&&stored.pages){
        _rdData.leftColumns=[{id:uid(),name:'페이지'}];
        _rdData.markers=stored.markers||[];
        _rdData.splitRatio=stored.splitRatio||70;
        _rdData.pages=stored.pages.map((p,i)=>({
          id:p.id||uid(),leftValues:[p.name||String(i+1)],
          images:(p.images||[]).map(img=>({data:img.data||'',label:img.label||img.name||''})),
          opinion:p.opinion||'',tags:p.tags||[]
        }));
      } else if(stored.images){
        _rdData.leftColumns=[{id:uid(),name:'페이지'}];
        _rdData.markers=[];_rdData.splitRatio=70;
        _rdData.pages=stored.images.map((img,i)=>({
          id:img.id||uid(),leftValues:[String(i+1)],
          images:[{data:img.data||'',label:''}],
          opinion:img.text||'',tags:[]
        }));
      }
    }
  }
...
/* ── 왼쪽 컬럼 관리 ── */
function rdAddLeftCol(){
  _rdSync();
  if(_rdData.leftColumns.length>=3){toast('왼쪽 영역은 최대 3개까지 가능합니다.');return;}
  _rdData.leftColumns.push({id:uid(),name:'항목 '+(_rdData.leftColumns.length+1)});
  _rdData.pages.forEach(p=>{if(!p.leftValues)p.leftValues=[];p.leftValues.push('');});
  renderRdEditor();
}
function rdDelLeftCol(li){
  _rdSync();
  if(_rdData.leftColumns.length<=1){toast('최소 1개의 왼쪽 영역이 필요합니다.');return;}
  _rdData.leftColumns.splice(li,1);
  _rdData.pages.forEach(p=>{if(p.leftValues)p.leftValues.splice(li,1);});
  renderRdEditor();
}

/* ── 페이지별 이미지 관리 ── */
function rdAddImgCol(pi){
  _rdSync();
  if(_rdData.pages[pi].images.length>=3){toast('이미지는 페이지당 최대 3개입니다.');return;}
  _rdData.pages[pi].images.push({data:'',label:''});
  renderRdEditor();
}
function rdDelImgCol(pi,ci){
  _rdSync();
  if(_rdData.pages[pi].images.length<=1){toast('최소 1개의 이미지가 필요합니다.');return;}
  _rdData.pages[pi].images.splice(ci,1);
  renderRdEditor();
}
...
function rdAddPage(){
  _rdSync();
  const lv=_rdData.leftColumns.map(()=>'');
  lv[0]=String(_rdData.pages.length+1);
  _rdData.pages.push({id:uid(),leftValues:lv,images:[{data:'',label:''}],opinion:'',tags:[]});
  renderRdEditor();
  const container=document.querySelector('#refDocOverlay .modal > div:last-child');
  if(container) setTimeout(()=>container.scrollTop=container.scrollHeight,100);
}
function rdDelPage(pi){
  _rdSync();
  if(!confirm('이 페이지를 삭제하시겠습니까?'))return;
  _rdData.pages.splice(pi,1);
  renderRdEditor();
}

/* ── 이미지 업로드 (파일 선택) ── */
function rdClickImgSlot(pi,ci){
  _rdUploadTarget={pageIdx:pi,colIdx:ci};
  document.getElementById('rdFileIn').click();
}
async function _rdUploadAndSet(dataUrl,pi,ci){
  toast('이미지 업로드 중...');
  const url=await _uploadImgToStorage(dataUrl);
  if(_rdData.pages[pi]&&_rdData.pages[pi].images[ci]){
    _rdData.pages[pi].images[ci].data=url;
    renderRdEditor();
    toast('업로드 완료');
  }
}
function handleRdImageSlotNew(e){
  const file=e.target.files[0];if(!file||!_rdUploadTarget)return;
  const{pageIdx,colIdx}=_rdUploadTarget;
  _rdSync();
  const reader=new FileReader();
  reader.onload=ev=>{
    const img=new Image();img.onload=async()=>{
      const c=document.createElement('canvas');const MAX=1200;const sc=Math.min(MAX/img.width,MAX/img.height,1);
      c.width=Math.round(img.width*sc);c.height=Math.round(img.height*sc);
      c.getContext('2d').drawImage(img,0,0,c.width,c.height);
      await _rdUploadAndSet(c.toDataURL('image/jpeg',.85),pageIdx,colIdx);
    };img.src=ev.target.result;
  };reader.readAsDataURL(file);
  e.target.value='';_rdUploadTarget=null;
}

/* ── Ctrl+V 이미지 붙여넣기 ── */
function rdHandlePaste(e,pi,ci){
    ...
    if(item.type.indexOf('image')!==-1){
      ...
      reader.onload=ev=>{
        const img=new Image();img.onload=async()=>{
          ...
          c.width=Math.round(img.width*sc);c.height=Math.round(img.height*sc);
          c.getContext('2d').drawImage(img,0,0,c.width,c.height);
          await _rdUploadAndSet(c.toDataURL('image/jpeg',.85),pi,ci);
        };img.src=ev.target.result;
...
/* ── 마커 편집 (30색 스와치) ── */
function openRdMarkerEditor(){
  _rdSync();renderRdMarkerList();
  document.getElementById('rdMarkerOverlay').classList.add('open');
}
...
function rdPickNewColor(c){
  _rdNewMarkerColor=c;
  document.querySelectorAll('.rd2-mk-new-swatch').forEach(el=>{
    el.classList.toggle('sel',el.dataset.c===c);
  });
}
function rdAddMarker(){
  const name=document.getElementById('rdNewMarkerName').value.trim();
  if(!name){toast('마커 이름을 입력해주세요.');return;}
  _rdData.markers.push({id:uid(),name,color:_rdNewMarkerColor});
  document.getElementById('rdNewMarkerName').value='';
  renderRdMarkerList();
}
function rdDelMarker(mi){
  const m=_rdData.markers[mi];
  _rdData.pages.forEach(p=>{p.tags=(p.tags||[]).filter(t=>t!==m.id);});
  _rdData.markers.splice(mi,1);
  renderRdMarkerList();
}
...
function renderRdMarkerList(){
  ...
  // 기존 마커 목록
  const existHtml=_rdData.markers.length?_rdData.markers.map((m,mi)=>`
    <div class="rd2-mk-item">
      <span class="rd2-mk-dot" style="background:${m.color}"></span>
      <input type="text" value="${m.name}" oninput="rdEditMarkerName(${mi},this.value)" class="rd2-mk-name-in">
      <div class="rd2-mk-colors">${RD_COLORS.map(c=>'<span class="rd2-mk-swatch'+(m.color===c?' sel':'')+'" style="background:'+c+'" onclick="rdPickMarkerColor('+mi+',\''+c+'\')"></span>').join('')}</div>
      <button onclick="rdDelMarker(${mi})" class="rd2-mk-del-btn">삭제</button>
    </div>`).join(''):'<p style="font-size:.8rem;color:var(--t3);padding:8px 0;">등록된 마커가 없습니다.</p>';
  // 새 마커 추가 UI
  const newSwatches=RD_COLORS.map(c=>'<span class="rd2-mk-new-swatch rd2-mk-swatch'+(_rdNewMarkerColor===c?' sel':'')+'" data-c="'+c+'" style="background:'+c+'" onclick="rdPickNewColor(\''+c+'\')"></span>').join('');
  list.innerHTML=existHtml+`
    <div class="rd2-mk-add-area">
      <div class="rd2-mk-add-label">새 마커 추가</div>
      <div class="rd2-mk-colors">${newSwatches}</div>
      <div style="display:flex;gap:6px;margin-top:6px;">
        <input type="text" id="rdNewMarkerName" placeholder="마커 이름" class="rd2-mk-name-in" style="flex:1;">
        <button class="mbtn-s" onclick="rdAddMarker()">추가</button>
      </div>
    </div>`;
}

/* ── 태그 토글 ── */
function rdToggleTag(pi,markerId){
  _rdSync();
  const p=_rdData.pages[pi];if(!p.tags)p.tags=[];
  const idx=p.tags.indexOf(markerId);
  if(idx===-1)p.tags.push(markerId);else p.tags.splice(idx,1);
  renderRdEditor();
}

/* ── 리사이즈 핸들 ── */
function rdStartResize(e){
  e.preventDefault();_rdResizing=true;
  const modal=document.querySelector('#refDocOverlay .modal');
  const lcEl=document.querySelector('.rd2-header-row .rd2-left-zone');
  const lcWpx=lcEl?lcEl.getBoundingClientRect().width:70;
  const onMove=(ev)=>{
    if(!_rdResizing)return;
    const rect=modal.getBoundingClientRect();
    const clientX=ev.touches?ev.touches[0].clientX:ev.clientX;
    const availW=rect.width-lcWpx-6;
    const imgW=clientX-rect.left-lcWpx;
    const pct=(imgW/availW)*100;
    _rdData.splitRatio=Math.max(25,Math.min(85,pct));
    document.querySelectorAll('.rd2-img-zone').forEach(el=>el.style.width=_rdData.splitRatio+'%');
    document.querySelectorAll('.rd2-txt-zone').forEach(el=>el.style.width=(100-_rdData.splitRatio)+'%');
  };
  const onUp=()=>{_rdResizing=false;document.removeEventListener('mousemove',onMove);document.removeEventListener('mouseup',onUp);document.removeEventListener('touchmove',onMove);document.removeEventListener('touchend',onUp);};
...
function rdFmt(pi,cmd,val){
  const el=document.getElementById('rdOpinion-'+pi);
  if(el)el.focus();
  rdExec(cmd,val);
}
function rdHandleOpinionInput(e,pi){
  ...
  const block=node.closest('div,p,li,h1,h2,h3,h4,blockquote')||e.target;
  const text=block.textContent||'';
  // 블록 마크다운 패턴 (스페이스로 트리거)
  const patterns=[
    {re:/^### $/,tag:'h3'},{re:/^## $/,tag:'h2'},{re:/^# $/,tag:'h1'},
    {re:/^- $/,cmd:'insertUnorderedList'},{re:/^\* $/,cmd:'insertUnorderedList'},
    {re:/^1\. $/,cmd:'insertOrderedList'},
    {re:/^> $/,tag:'blockquote'},
    {re:/^---$/,cmd:'insertHorizontalRule'}
  ];
  for(const p of patterns){
    if(p.re.test(text)){
      block.textContent='';
      if(p.cmd) rdExec(p.cmd); else if(p.tag) rdExec('formatBlock','<'+p.tag+'>');
      return;
    }
  }
  // 인라인 마크다운
  const html=block.innerHTML||'';
  let changed=false,newHtml=html;
  if(/\*\*(.+?)\*\*/.test(newHtml)){newHtml=newHtml.replace(/\*\*(.+?)\*\*/g,'<b>$1</b>');changed=true;}
  if(!changed&&/(?<!\*)\*([^*]+?)\*(?!\*)/.test(newHtml)){newHtml=newHtml.replace(/(?<!\*)\*([^*]+?)\*(?!\*)/g,'<i>$1</i>');changed=true;}
  if(!changed&&/~~(.+?)~~/.test(newHtml)){newHtml=newHtml.replace(/~~(.+?)~~/g,'<s>$1</s>');changed=true;}
  if(!changed&&/`([^`]+?)`/.test(newHtml)){newHtml=newHtml.replace(/`([^`]+?)`/g,'<code style="background:#f0f0f0;padding:1px 4px;border-radius:3px;font-size:.88em">$1</code>');changed=true;}
  if(changed){block.innerHTML=newHtml;_rdPlaceCaret(block);}
}
function _rdPlaceCaret(el){
...
function rdHandleOpinionKeydown(e,pi){
  if(e.key==='Tab'){e.preventDefault();rdExec('insertText','    ');}
  if(e.ctrlKey||e.metaKey){
    if(e.key==='b'){e.preventDefault();rdExec('bold');}
    if(e.key==='i'){e.preventDefault();rdExec('italic');}
    if(e.key==='u'){e.preventDefault();rdExec('underline');}
    if(e.key==='d'){e.preventDefault();rdExec('strikeThrough');}
  }
  // Enter 후 blockquote 탈출: 빈 줄에서 Enter → 일반 문단
  if(e.key==='Enter'&&!e.shiftKey){
    const sel=window.getSelection();if(!sel.rangeCount)return;
    let nd=sel.getRangeAt(0).startContainer;
    if(nd.nodeType===3)nd=nd.parentNode;
    const bq=nd.closest('blockquote');
    if(bq&&!(nd.textContent||'').trim()){
      e.preventDefault();
...
function renderRdEditor(){
  const lc=_rdData.leftColumns;
  const split=_rdData.splitRatio||70;
  const txtW=100-split;

  // ── 헤더 행 ──
  // 왼쪽 컬럼 헤더: 세로로 쌓기
  const lcHdrRows=lc.map((col,li)=>`
    <div class="rd2-lc-hdr-row">
      <input class="rd2-lc-name-in" id="rdLCName-${li}" value="${col.name||''}" placeholder="항목명">
      ${lc.length>1?'<button class="rd2-lc-del" onclick="rdDelLeftCol('+li+')" title="삭제">✕</button>':''}
    </div>`).join('');

  const headerHtml=`<div class="rd2-header-row">
    <div class="rd2-left-zone rd2-left-zone-v">
      ${lcHdrRows}
      ${lc.length<3?'<button class="rd2-lc-add-btn" onclick="rdAddLeftCol()" title="왼쪽 영역 추가">＋</button>':''}
    </div>
    <div class="rd2-img-zone" style="width:${split}%"><span class="rd2-hdr-label">이미지</span></div>
    <div class="rd2-resize-gutter" onmousedown="rdStartResize(event)" ontouchstart="rdStartResize(event)"><div class="rd2-resize-line"></div></div>
    <div class="rd2-txt-zone" style="width:${txtW}%"><span class="rd2-hdr-label">의견</span></div>
  </div>`;
  document.getElementById('rdColHeader').innerHTML=headerHtml;

  // ── 페이지 행들 ──
  const pagesHtml=_rdData.pages.map((page,pi)=>{
    if(!page.leftValues) page.leftValues=lc.map(()=>'');
    while(page.leftValues.length<lc.length) page.leftValues.push('');

    // 왼쪽 값 셀: 세로로 쌓기
    const lcRows=lc.map((col,li)=>`
      <div class="rd2-lc-val-row">
        <input class="rd2-lc-val-in" id="rdLV-${pi}-${li}" value="${page.leftValues[li]||''}" placeholder="${col.name||''}">
      </div>`).join('');

    // 이미지 셀들
    const imgsHtml=page.images.map((img,ci)=>{
      const imgContent=img.data
        ?'<img src="'+img.data+'" alt="">'
        :'<div class="rd2-img-placeholder" onclick="rdClickImgSlot('+pi+','+ci+')">＋ 이미지<br><span style="font-size:.65rem;font-weight:400;color:var(--t3)">클릭 또는 Ctrl+V</span></div>';
      return'<div class="rd2-img-cell">'
        +'<input class="rd2-img-lbl-in" id="rdImgLbl-'+pi+'-'+ci+'" value="'+(img.label||'')+'" placeholder="이미지명">'
        +'<div class="rd2-img-box" '+(img.data?'onclick="rdClickImgSlot('+pi+','+ci+')"':'')+' onpaste="rdHandlePaste(event,'+pi+','+ci+')" tabindex="0">'
        +imgContent
        +'</div>'
        +(page.images.length>1?'<button class="rd2-img-del" onclick="rdDelImgCol('+pi+','+ci+')" title="이미지 삭제">✕</button>':'')
        +'</div>';
    }).join('');
    const addImgBtn=page.images.length<3?'<button class="rd2-img-add-btn" onclick="rdAddImgCol('+pi+')" title="이미지 추가">＋</button>':'';

    // 마커 태그
    const markerBtns=_rdData.markers.map(m=>{
      const active=(page.tags||[]).includes(m.id);
      return'<button class="rd2-tag-btn'+(active?' active':'')+'" style="--mc:'+m.color+'" onclick="rdToggleTag('+pi+',\''+m.id+'\')">'+(active?'●':'○')+' '+m.name+'</button>';
    }).join('');
    const markerArea=_rdData.markers.length?'<div class="rd2-tag-area">'+markerBtns+'</div>':'';

    // 의견 툴바
    const tb='<div class="rd2-op-toolbar">'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'bold\')" title="굵게 Ctrl+B"><b>B</b></button>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'italic\')" title="기울임 Ctrl+I"><i>I</i></button>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'underline\')" title="밑줄 Ctrl+U"><u>U</u></button>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'strikeThrough\')" title="취소선 Ctrl+D"><s>S</s></button>'
      +'<span class="rd2-tb-sep"></span>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'formatBlock\',\'<h2>\')" title="제목">H</button>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'formatBlock\',\'<p>\')" title="본문">P</button>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'insertUnorderedList\')" title="글머리 기호">•</button>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'insertOrderedList\')" title="번호 매기기">1.</button>'
      +'<button onmousedown="event.preventDefault()" onclick="rdFmt('+pi+',\'formatBlock\',\'<blockquote>\')" title="인용">❝</button>'
      +'<span class="rd2-tb-sep"></span>'
      +'<select onmousedown="event.stopPropagation()" onchange="rdFmt('+pi+',\'foreColor\',this.value);this.selectedIndex=0;" title="글자색">'
      +'<option value="" disabled selected>색</option>'
      +'<option value="#191F28">■ 검정</option>'
      +'<option value="#D91F3E">■ 빨강</option>'
      +'<option value="#0064FF">■ 파랑</option>'
      +'<option value="#2E9E50">■ 초록</option>'
      +'<option value="#FF7A00">■ 주황</option>'
      +'<option value="#9C27B0">■ 보라</option>'
      +'</select>'
      +'</div>';

    return'<div class="rd2-page-row" id="rdPageRow-'+pi+'">'
      +'<div class="rd2-left-zone rd2-left-zone-v">'
      +lcRows
      +'<button class="rd2-pg-del" onclick="rdDelPage('+pi+')" title="페이지 삭제">✕</button>'
      +'</div>'
      +'<div class="rd2-img-zone" style="width:'+split+'%;display:flex;align-items:stretch;">'
      +imgsHtml+addImgBtn
      +'</div>'
      +'<div class="rd2-resize-gutter rd2-rg-passive"><div class="rd2-resize-line"></div></div>'
      +'<div class="rd2-txt-zone" style="width:'+txtW+'%;">'
      +markerArea+tb
      +'<div class="rd2-opinion-area" id="rdOpinion-'+pi+'" contenteditable="true"'
      +' oninput="rdHandleOpinionInput(event,'+pi+')"'
      +' onkeydown="rdHandleOpinionKeydown(event,'+pi+')"'
      +' data-placeholder="의견을 작성하세요. (# 제목, - 목록, > 인용, **굵게**, ~~취소~~)">'+(page.opinion||'')+'</div>'
      +'</div>'
      +'</div>';
  }).join('');
...
function saveRefDoc(){
  ...
  _rdSync();
  const stored={type:'refdoc',leftColumns:_rdData.leftColumns,markers:_rdData.markers,pages:_rdData.pages,splitRatio:_rdData.splitRatio};
  if(!CARD_DOCS[_curCard])CARD_DOCS[_curCard]=[];
  if(_editRefDocId){
  ...
  saveAll();renderDocList();closeRefDocModal();toast('이미지 의견 문서가 저장되었습니다.');
}
/* ══ HTML EDITOR ══ */
...
async function openRichEditor(docId=''){
  ...
  _applyReMode();
  // 이벤트 바인딩 (한 번만)
  setTimeout(()=>{ _bindCtxMenu(); _bindTableCtxMenu(); _bindMemoSync(editor); _bindTblResize(editor); }, 200);
...
function reOnKeydown(e){
  ...
  if(e.key==='Backspace'){
    ...
    if(sel&&sel.rangeCount>0&&sel.isCollapsed){
      ...
      // 커서가 빈 p의 시작에 있고 이전 형제가 hr인 경우
      const block=(node.nodeType===3?node.parentElement:node)?.closest?.('p,div,h1,h2,h3,h4,h5,h6,li,blockquote,pre');
      if(block&&r.startOffset===0){
