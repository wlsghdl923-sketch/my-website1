...
/* ── 라이브러리 로드 (jsdelivr → unpkg → cdnjs 순차 시도) ── */
async function _loadPdfLibs(){
  ...
  const tryLoad=async(urls)=>{
    for(const url of urls){try{await loadScript(url);return true;}catch(e){const el=document.querySelector('script[src="'+url+'"]');if(el)el.remove();}}
    return false;
  };
  try{
    const h2cOk=await tryLoad(['https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js','https://unpkg.com/html2canvas@1.4.1/dist/html2canvas.min.js','https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js']);
    if(!h2cOk) throw new Error('html2canvas');
    const jpOk=await tryLoad(['https://cdn.jsdelivr.net/npm/jspdf@2.5.2/dist/jspdf.umd.min.js','https://unpkg.com/jspdf@2.5.2/dist/jspdf.umd.min.js','https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.2/jspdf.umd.min.js']);
    if(!jpOk) throw new Error('jsPDF');
    _pdfLibsLoaded=true;return true;
  }catch(e){console.error('PDF lib fail:',e);toast('PDF 라이브러리 로드 실패. 브라우저 인쇄로 전환합니다.');return false;}
}

/* ── PDF 생성 (iframe 렌더링 → html2canvas 캡처) ── */
async function execPdfExport(){
  const content=document.getElementById('deContent').value;
  ...
  prog.style.display='block';progText.textContent='라이브러리 로드 중...';progBar.style.width='5%';
  document.getElementById('pdfExportBtn').disabled=true;
  const libsOk=await _loadPdfLibs();
  if(!libsOk){prog.style.display='none';document.getElementById('pdfExportBtn').disabled=false;_pdfFallbackPrint(content,title,pageSize,orient,contentWidth,margin);return;}
  progText.textContent='문서 렌더링 중...';progBar.style.width='10%';
  const sizes={a4:[210,297],a3:[297,420],letter:[215.9,279.4],legal:[215.9,355.6]};
  let [pw,ph]=sizes[pageSize]||sizes.a4;
  if(orient==='landscape')[pw,ph]=[ph,pw];

  // ★ 메인 DOM에 div로 렌더링 (html2canvas가 iframe body를 캡처 못하는 문제 해결)
  let bodyHtml=content, userStyle='';
  if(/^<!DOCTYPE/i.test(content.trim())||/^<html/i.test(content.trim())){
    const bm=content.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
    bodyHtml=bm?bm[1]:content;
    const sm=[...content.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)];
    userStyle=sm.map(s=>s[1]).join('\n');
  }

  const container=document.createElement('div');
  container.style.cssText='position:fixed;left:-9999px;top:0;width:'+contentWidth+'px;background:#fff;z-index:-1;overflow:visible;';
  // 래퍼 div에 body 역할 스타일 직접 적용
  container.innerHTML='<style>.pdf-r{font-family:"Noto Sans KR",sans-serif;padding:32px;line-height:1.8;color:#191F28;font-size:15px;word-break:break-word;box-sizing:border-box;width:'+contentWidth+'px;background:#fff;}'
    +'.pdf-r h1,.pdf-r h2,.pdf-r h3{font-weight:800;letter-spacing:-.02em;margin:1.2em 0 .5em;}'
    +'.pdf-r h1{font-size:1.8rem;}.pdf-r h2{font-size:1.4rem;}.pdf-r h3{font-size:1.1rem;}'
    +'.pdf-r p{margin:.6em 0;}.pdf-r img{max-width:100%;border-radius:8px;}'
    +'.pdf-r hr{border:none;border-top:1px solid #E5E8EB;margin:1.5em 0;}'
    +'.pdf-r table{border-collapse:collapse;width:100%;}'
    +'.pdf-r td,.pdf-r th{padding:8px 12px;border:1px solid #E5E8EB;}'
    +'.pdf-r th{background:#F5F6FA;font-weight:700;}'
    +userStyle
    +'</style>'
    +'<div class="pdf-r">'+bodyHtml+'</div>';
  document.body.appendChild(container);

  // 이미지 로딩
  const images=container.querySelectorAll('img');
  if(images.length){
    progText.textContent='이미지 로딩 중 ('+images.length+'개)...';progBar.style.width='20%';
    await Promise.allSettled([...images].map(img=>new Promise(r=>{if(img.complete)r();else{img.onload=r;img.onerror=r;}})));
  }
  // 폰트 + 레이아웃 안정화 대기
  await new Promise(r=>setTimeout(r,600));
  progText.textContent='캡처 중...';progBar.style.width='35%';

  const renderTarget=container.querySelector('.pdf-r');
  try{
    const renderScale=scalePct/100*2;
    const canvas=await html2canvas(renderTarget,{
      scale:renderScale, useCORS:true, allowTaint:true,
      backgroundColor:'#ffffff', logging:false, windowWidth:contentWidth
    });
    progText.textContent='PDF 생성 중...';progBar.style.width='70%';
    ...
    if(imgH<=pageH){
      ...
      const sliceH=Math.floor(canvas.height/totalPages);
      for(let i=0;i<totalPages;i++){
        if(i>0)pdf.addPage();
        const srcY=i*sliceH;const srcH=Math.min(sliceH,canvas.height-srcY);const destH=(srcH*imgW)/canvas.width;
        ...
        pdf.addImage(sc.toDataURL('image/jpeg',0.92),'JPEG',margin,margin,imgW,destH);
        progBar.style.width=(70+Math.round((i+1)/totalPages*25))+'%';
        progText.textContent='PDF 생성 중... ('+(i+1)+'/'+totalPages+')';
      }
    }
    progBar.style.width='100%';progText.textContent='다운로드 중...';
    pdf.save(title+'.pdf');
    setTimeout(()=>{progText.textContent='✅ 완료!';setTimeout(()=>closePdfModal(),800);},300);
  }catch(e){
    console.error('PDF err:',e);
    toast('PDF 생성 실패. 브라우저 인쇄로 전환합니다.');
    _pdfFallbackPrint(content,title,pageSize,orient,contentWidth,margin);
    prog.style.display='none';document.getElementById('pdfExportBtn').disabled=false;
  }finally{
...
/* ── 폴백: 브라우저 인쇄 → PDF ── */
function _pdfFallbackPrint(content,title,pageSize,orient,contentWidth,margin){
  ...
  const printHtml=_pdfBuildFullHtml(content,contentWidth,
    '@media print{@page{size:'+sz+';margin:'+margin+'mm;}body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}body{max-width:'+contentWidth+'px;margin:0 auto;}'
  );
  ...
  toast('"PDF로 저장"을 선택해주세요.');closePdfModal();
}
/* ══ CAL MAKER ══ */
let _cmType='single';
