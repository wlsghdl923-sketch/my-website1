...
function _getCardName(cardId){
  ...
  // 2) 없으면 백업 파일의 hub_cards에서 찾기 (삭제된 카드 식별용)
  if(_partialBackupData && _partialBackupData.hub_cards){
    ...
    for(const key of Object.keys(backupCards)){
      ...
      if(Array.isArray(list)){
        ...
        if(found) return found.name + ' (삭제된 카드 — 복원 시 함께 복구됨)';
      }
    }
  }
  return cardId;
...
function renderPartialRestoreList(){
  ...
  for(const cardId of Object.keys(cdocs)){
    ...
    // Card header
    const cardName = _getCardName(cardId) || cardId;
    const header = document.createElement('div');
    header.style.cssText = 'font-size:.82rem;font-weight:700;margin:12px 0 6px;padding-bottom:4px;border-bottom:1px solid var(--bd);color:var(--t1);';
    ...
    docs.forEach(doc=>{
      ...
      const folderLabel = doc.parentId && folderMap[doc.parentId] ? '<span style="color:var(--t3);font-size:.72rem;">' + folderMap[doc.parentId] + ' / </span>' : '';
      const typeIcon = doc.type==='richtext' ? '📝' : doc.type==='html' ? '🌐' : doc.type==='refdoc' ? '🖼' : doc.type==='calendar' ? '🏷️' : doc.type==='sched' ? '📘' : '📄';
      label.innerHTML = typeIcon + ' ' + folderLabel + doc.title;
      label.style.cssText = 'flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';

      if(!hasContent){
        const noData = document.createElement('span');
        noData.textContent = '(본문 없음)';
        noData.style.cssText = 'font-size:.7rem;color:#e94560;flex-shrink:0;';
        row.appendChild(cb);
...
function updatePartialCount(){
  ...
  document.getElementById('partialRestoreCount').textContent = '선택: ' + cbs.length + '개';
}

function partialRestoreSelectAll(checked){
  document.querySelectorAll('.partial-restore-cb').forEach(cb=>{ cb.checked=checked; });
  updatePartialCount();
}
...
async function executePartialRestore(){
  ...
  if(!confirm('선택한 ' + cbs.length + '개 문서를 복원하시겠습니까?\n현재 데이터는 유지되고, 선택한 문서만 추가됩니다.')) return;
