...
async function handleCoCustomImg(e){
  ...
  reader.onload=async ev=>{
    ...
    toast('커스텀 이모지 저장됨 (Supabase — 어떤 기기에서도 사용 가능)');
...
function tblCmd(cmd){
  ...
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;min-width:40px;';
  const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;min-width:40px;';
  ...
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    ...
    // ── 셀 가로 분할 (선택된 셀 아래에 새 행 추가, 나머지 셀은 rowspan +1) ──
...
function openCellColorPicker(){
  ...
  _selCellClr=cur;
  document.getElementById('cellClrOverlay').style.display='block';
}
...
/* ── 표 선 색상 (30가지) ── */
const TBL_BORDER_COLORS=['#E5E8EB','#191F28','#D91F3E','#FF7A00','#0064FF',
  '#2E9E50','#9C27B0','#8B95A1','#F44336','#E91E63','#673AB7','#3F51B5',
  '#2196F3','#03A9F4','#00BCD4','#009688','#4CAF50','#8BC34A','#CDDC39',
  '#FFEB3B','#FFC107','#FF9800','#FF5722','#795548','#607D8B','#BDBDBD',
...
function openTblBorderColorPicker(){
  ...
  _selTblBorderClr=cur;
  document.getElementById('tblBorderClrOverlay').style.display='block';
}
...
function applyTblBorderClr(color){
  ...
  table.style.borderColor=color;table.style.border='1px solid '+color;
  table.querySelectorAll('td,th').forEach(cell=>{
    cell.style.borderColor=color;cell.style.border='1px solid '+color;
  });
  closeTblBorderClr();
}
...
function _initTableResize(){
  ...
  editor.addEventListener('mousemove',function(e){
    ...
    const THRESH=5;
    if(x>rect.width-THRESH&&x<rect.width+THRESH){editor.style.cursor='col-resize';}
    else if(y>rect.height-THRESH&&y<rect.height+THRESH){editor.style.cursor='row-resize';}
  ...
  document.addEventListener('mousemove',function(e){
    ...
    if(_tblRsz.type==='col'){
      ...
      const newW=Math.max(40,_tblRsz.startW+dx);
      ...
      const newH=Math.max(30,_tblRsz.startH+dy);
  ...
  document.addEventListener('mouseup',function(){
    if(_tblRsz){
      if(_tblRsz.cell)_tblRsz.cell.closest('table')?.classList.remove('col-resizing');
      _tblRsz=null;
      editor.style.cursor='';
    }
  });
...
img{max-width:100%;border-radius:6px;}.re-img-wrap{display:inline-block;vertical-align:top;}.re-img-handle{display:none;}
...
function reInsertTable(){
  ...
  for(let r=0;r<rows;r++){
    ...
    for(let c=0;c<cols;c++){
      const isH=r===0;
      const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;min-width:40px;':'padding:10px 14px;border:1px solid #E5E8EB;min-width:40px;';
  ...
  toast('3×3 표 삽입 완료. 셀을 우클릭하면 편집 메뉴가 열립니다.');
}
