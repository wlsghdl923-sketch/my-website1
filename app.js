...
function _asSetUI(state) {
  ...
  switch (state) {
    case 'idle':
      if (icon) icon.innerHTML = '<circle cx="6" cy="6" r="5" fill="none" stroke="#C0CAD6" stroke-width="1.5"/>';
      if (text) text.textContent = '대기중';
      break;
    case 'pending':
      el.classList.add('as-pending');
      if (icon) icon.innerHTML = '<circle cx="6" cy="6" r="5" fill="none" stroke="#FF7A00" stroke-width="1.5" stroke-dasharray="4 2"/>';
      if (text) text.textContent = '변경됨';
      break;
    case 'saving':
      el.classList.add('as-saving');
      if (icon) icon.innerHTML = '<circle cx="6" cy="6" r="5" fill="none" stroke="#FF7A00" stroke-width="1.5"/><path d="M6 3v3l1.5 1" stroke="#FF7A00" stroke-width="1.2" stroke-linecap="round"/>';
      if (text) text.textContent = '저장중...';
      break;
    case 'saved':
      el.classList.add('as-saved');
      if (icon) icon.innerHTML = '<circle cx="6" cy="6" r="5" fill="none" stroke="#2E9E50" stroke-width="1.5"/><polyline points="3.5,6 5,7.5 8.5,3.5" stroke="#2E9E50" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" fill="none"/>';
      if (text) text.textContent = '저장됨 ✓';
      setTimeout(() => _asSetUI('idle'), 3000);
      break;
    case 'error':
      el.classList.add('as-error');
      if (icon) icon.innerHTML = '<circle cx="6" cy="6" r="5" fill="none" stroke="#E53935" stroke-width="1.5"/><line x1="4" y1="4" x2="8" y2="8" stroke="#E53935" stroke-width="1.3"/><line x1="8" y1="4" x2="4" y2="8" stroke="#E53935" stroke-width="1.3"/>';
      if (text) text.textContent = '저장실패';
      break;
  }
}
...
function _asSchedule() {
  ...
  _asSetUI('pending');
  _autoSaveTimer = setTimeout(_asNow, 2000);
}

async function _asNow() {
  if (!_reEditDocId) return;
...
function reTogglePanels() {
  ...
  if (btn) btn.classList.toggle('panel-on', _rePanelsOn);
  if (_rePanelsOn) refreshMemoPanel();
}

/* ════════════════════════════════════════════════════════════
   3. 메모 시스템
   - 우클릭 → editorContextMenu → openMemoFromContext()
   - openMemoPopup(text, range) → 팝업 표시
   - saveMemoFromPopup() → span 삽입 + 패널 갱신
   - refreshMemoPanel() → 마커 패널 + 메모 목록 갱신
════════════════════════════════════════════════════════════ */
const MEMO_COLORS = [
  '#FFD54F','#FF8A65','#F06292','#CE93D8',
  '#90CAF9','#80DEEA','#A5D6A7','#EF5350',
  '#7E57C2','#26C6DA','#66BB6A','#FF7043',
];
let _memos = [];          // {id, text, color, markerName, content, resolved, spanEl}
let _memoEditId = null;
let _memoContextRange = null;

/* 우클릭 이벤트 → editorContextMenu */
document.addEventListener('DOMContentLoaded', () => {
  ...
  editor.addEventListener('contextmenu', (e) => {
    ...
    e.preventDefault();
    _memoContextRange = sel.getRangeAt(0).cloneRange();
...
function openMemoFromContext() {
  const menu = document.getElementById('editorContextMenu');
  if (menu) menu.style.display = 'none';
  if (!_memoContextRange) return;
  const text = _memoContextRange.toString().trim();
  openMemoPopup(text, _memoContextRange);
}

function openMemoPopup(text, range, editId = null) {
  _memoEditId = editId;
  _memoContextRange = range || null;
  document.getElementById('memoPopupTitle').textContent = editId ? '📝 메모 수정' : '📝 메모 추가';
  document.getElementById('memoPopupQuote').textContent = text ? `"${text.slice(0, 60)}"` : '';
  // 기존 값 복원 (수정 시)
  const existing = editId ? _memos.find(m => m.id === editId) : null;
  document.getElementById('memoMarkerName').value = existing?.markerName || '';
  document.getElementById('memoContentInput').value = existing?.content || '';
  // 색상 선택기
  const picker = document.getElementById('memoColorPicker');
  const selColor = existing?.color || MEMO_COLORS[0];
  picker.innerHTML = MEMO_COLORS.map(c =>
    `<div class="memo-color-dot${c === selColor ? ' sel' : ''}" style="background:${c};" onclick="_memoPickColor(this,'${c}')"></div>`
  ).join('');
  document.getElementById('memoPopupOverlay').style.display = 'block';
  setTimeout(() => document.getElementById('memoMarkerName').focus(), 100);
}

function _memoPickColor(el, color) {
  document.querySelectorAll('#memoColorPicker .memo-color-dot').forEach(d => d.classList.remove('sel'));
  el.classList.add('sel');
}
function _memoGetColor() {
  const sel = document.querySelector('#memoColorPicker .memo-color-dot.sel');
  return sel ? sel.style.background : MEMO_COLORS[0];
}

function closeMemoPopup() {
  document.getElementById('memoPopupOverlay').style.display = 'none';
  _memoEditId = null;
}

function saveMemoFromPopup() {
  const color = _memoGetColor();
  ...
  if (_memoEditId) {
    ...
    if (m) {
      ...
      if (m.spanEl) {
        m.spanEl.style.background = _hexAlpha(color, 0.4);
  ...
  // 신규
  if (!_memoContextRange) { closeMemoPopup(); return; }
  const text = _memoContextRange.toString().trim();
  if (!text) { closeMemoPopup(); return; }
  const id = 'memo_' + Date.now();
  try {
    const sel = window.getSelection();
    sel.removeAllRanges();
    sel.addRange(_memoContextRange);
    const span = document.createElement('span');
    span.className = 're-memo-mark';
    span.dataset.memoId = id;
    span.style.cssText = `background:${_hexAlpha(color, 0.4)};border-bottom:2px solid ${color};border-radius:2px;cursor:pointer;`;
    span.title = `메모: ${markerName || '메모'} — ${content || text.slice(0, 30)}`;
    span.onclick = (e) => { e.stopPropagation(); _activateMemoCard(id); };
    _memoContextRange.surroundContents(span);
    sel.removeAllRanges();
    _memos.push({ id, text, color, markerName, content, resolved: false, spanEl: span });
    closeMemoPopup();
    refreshMemoPanel();
    toast('메모가 추가되었습니다.');
  } catch(e) {
    // 여러 요소 걸친 경우 fallback
    _memos.push({ id, text, color, markerName, content, resolved: false, spanEl: null });
    closeMemoPopup();
    refreshMemoPanel();
    toast('메모가 추가되었습니다. (텍스트 하이라이트는 단일 요소 내에서 가능합니다)');
  }
}
...
function _hexAlpha(hex, a) {
  try {
    ...
    const g = parseInt(hex.slice(3,5)||'D5',16);
...
function _activateMemoCard(id) {
  document.querySelectorAll('.re-memo-mark').forEach(s => s.classList.remove('memo-active'));
  document.querySelectorAll('.memo-card-item').forEach(c => c.classList.remove('memo-active'));
  const span = document.querySelector(`[data-memo-id="${id}"]`);
  if (span) { span.classList.add('memo-active'); span.scrollIntoView({behavior:'smooth',block:'center'}); }
  const card = document.getElementById(`mc_${id}`);
  if (card) { card.classList.add('memo-active'); card.scrollIntoView({behavior:'smooth',block:'nearest'}); }
}
...
function _renderMarkerPanel() {
  ...
  const summary = document.getElementById('reMarkerSummary');
  if (!list) return;
  const groups = {};
  _memos.forEach(m => {
    const k = m.color + '|' + (m.markerName || '미분류');
    if (!groups[k]) groups[k] = { color: m.color, name: m.markerName || '미분류', count: 0, done: 0 };
    groups[k].count++;
    if (m.resolved) groups[k].done++;
  });
  list.innerHTML = Object.values(groups).map(g => `
    <div class="marker-panel-row" onclick="_filterByMarker('${g.color}')">
      <div style="width:9px;height:9px;border-radius:50%;background:${g.color};flex-shrink:0;"></div>
      <div style="flex:1;min-width:0;">
        <div style="font-weight:700;color:#1A3A8F;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${g.name}</div>
        <div style="font-size:.67rem;color:#4E5968;">${g.count}개 · 완료 ${g.done}</div>
      </div>
    </div>`).join('') || '<div style="font-size:.72rem;color:#8B95A1;text-align:center;padding:8px 0;">메모 없음</div>';
  if (summary) {
    const total = _memos.length, done = _memos.filter(m=>m.resolved).length;
    summary.textContent = `전체 ${total}개 · 완료 ${done}개`;
  }
}

function _filterByMarker(color) {
  _memos.filter(m => m.color === color).forEach(m => {
    if (m.spanEl) { m.spanEl.classList.add('memo-active'); m.spanEl.scrollIntoView({behavior:'smooth',block:'center'}); }
  });
  setTimeout(() => _memos.forEach(m => { if(m.spanEl) m.spanEl.classList.remove('memo-active'); }), 1500);
}
...
function _renderMemoList() {
  ...
  if (!list) return;
  if (!_memos.length) {
    list.innerHTML = '<div style="font-size:.78rem;color:#8B95A1;text-align:center;padding:16px 0;">메모가 없습니다</div>';
    return;
  }
  list.innerHTML = _memos.map(m => `
    <div class="memo-card-item${m.resolved?' memo-resolved':''}" id="mc_${m.id}" onclick="_activateMemoCard('${m.id}')">
      ...
      <div class="memo-card-actions">
        <button class="btn-resolve" onclick="event.stopPropagation();toggleMemoResolved('${m.id}')">${m.resolved?'↩ 취소':'✓ 완료'}</button>
...
function toggleMemoResolved(id) {
  ...
  if (m.spanEl) m.spanEl.classList.toggle('memo-resolved', m.resolved);
  refreshMemoPanel();
}
...
function _renderMemoManagerList() {
  ...
  if (!el) return;
  if (!_memos.length) { el.innerHTML = '<div style="text-align:center;color:#8B95A1;font-size:.82rem;padding:16px 0;">메모가 없습니다</div>'; return; }
  el.innerHTML = _memos.map(m => `
    <div style="display:flex;gap:8px;align-items:flex-start;padding:10px;background:#F8F9FA;border-radius:8px;border-left:3px solid ${m.color};">
      <div style="flex:1;min-width:0;">
        <div style="font-size:.79rem;font-weight:700;color:${m.color};">${m.markerName||'메모'}${m.resolved?' ✓':''}</div>
        <div style="font-size:.75rem;color:#4E5968;margin:2px 0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">"${m.text.slice(0,60)}"</div>
        ${m.content?`<div style="font-size:.78rem;color:#191F28;">${m.content}</div>`:''}
      </div>
      <div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0;">
        <button onclick="closeMemoManager();editMemo('${m.id}')" style="font-size:.7rem;padding:2px 7px;border:1px solid #E5E8EB;border-radius:4px;background:#fff;cursor:pointer;">수정</button>
...
function clearAllMemos() {
  if (!confirm(`메모 ${_memos.length}개를 모두 삭제하시겠습니까?`)) return;
  _memos.forEach(m => {
    if (m.spanEl) {
      const p = m.spanEl.parentNode;
      if (p) { while(m.spanEl.firstChild) p.insertBefore(m.spanEl.firstChild, m.spanEl); p.removeChild(m.spanEl); }
...
/* ════════════════════════════════════════════════════════════
   4. 이미지 의견 작성 — 패널 토글
════════════════════════════════════════════════════════════ */
let _rdPanelsOn = false;

function rdTogglePanels() {
  _rdPanelsOn = !_rdPanelsOn;
  ...
  if (lp) lp.style.display = _rdPanelsOn ? 'block' : 'none';
  if (rp) rp.style.display = _rdPanelsOn ? 'block' : 'none';
  if (btn) btn.classList.toggle('panel-on', _rdPanelsOn);
}

/* ════════════════════════════════════════════════════════════
   5. 이미지 의견 작성 — openRefDocModal (Supabase에서 로드)
════════════════════════════════════════════════════════════ */
async function openRefDocModal(docId = '') {
  ...
  // 패널 초기화
  _rdPanelsOn = false;
  ['rdMarkerPanel','rdMemoPanel'].forEach(id => { const el=document.getElementById(id); if(el)el.style.display='none'; });
  const rdBtn = document.getElementById('btnRdPanels');
  if (rdBtn) rdBtn.classList.remove('panel-on');

  if (docId) {
    const d = (CARD_DOCS[_curCard] || []).find(x => x.id === docId);
    if (d) document.getElementById('rdTitle').value = d.title || '';
    try {
...
/* ════════════════════════════════════════════════════════════
   6. 이미지 의견 작성 — renderRdImages (7:3 + 더블클릭 + 리사이저)
════════════════════════════════════════════════════════════ */
      ...
      <div class="rd-row-body" id="rdBody-${i}">
        <div class="rd-img-side" id="rdImgSide-${i}" style="flex:7;">
          <img src="${img.data}" alt="이미지 ${i+1}"
            style="max-width:100%;max-height:500px;object-fit:contain;display:block;cursor:zoom-in;"
            title="더블클릭하면 원본 크기로 봅니다"
            ondblclick="rdShowLightbox(this.src)">
          <div class="rd-img-replace-overlay" onclick="_rdSlotIdx=${i};document.getElementById('rdFileInSlot').click()">
            <span>🔄 클릭하여 이미지 교체</span>
          </div>
        </div>
        <div class="rd-resizer" id="rdRsz-${i}" onmousedown="rdStartResize(event,${i})" title="드래그하여 이미지/텍스트 비율 조절"></div>
...
function rdStartResize(e, i) {
  e.preventDefault();
  const imgEl = document.getElementById(`rdImgSide-${i}`);
  const txtEl = document.getElementById(`rdTxtSide-${i}`);
  const body = document.getElementById(`rdBody-${i}`);
  const rsz = document.getElementById(`rdRsz-${i}`);
  if (!imgEl||!txtEl||!body) return;
  _rdRsz = {imgEl,txtEl,rsz,sx:e.clientX,iw:imgEl.offsetWidth,tot:body.offsetWidth};
  if(rsz)rsz.classList.add('rsz-on');
  document.body.style.cursor='col-resize';
  document.body.style.userSelect='none';
  const mv=(ev)=>{if(!_rdRsz)return;const dx=ev.clientX-_rdRsz.sx;const nw=Math.max(60,Math.min(_rdRsz.tot-60,_rdRsz.iw+dx));_rdRsz.imgEl.style.flex='none';_rdRsz.imgEl.style.width=nw+'px';_rdRsz.txtEl.style.flex='none';_rdRsz.txtEl.style.width=(_rdRsz.tot-nw-5)+'px';};
  const up=()=>{if(_rdRsz?.rsz)_rdRsz.rsz.classList.remove('rsz-on');_rdRsz=null;document.body.style.cursor='';document.body.style.userSelect='';document.removeEventListener('mousemove',mv);document.removeEventListener('mouseup',up);};
  document.addEventListener('mousemove',mv);
  document.addEventListener('mouseup',up);
}

function rdShowLightbox(src) {
...
function openRichEditor(docId=''){
  ...
  const pb=document.getElementById('btnTogglePanels');if(pb)pb.classList.remove('panel-on');
  // 자동저장: 기존 문서만 활성화
  _autoSaveActive=!!docId;
  _asSetUI('idle');
  setTimeout(()=>{
