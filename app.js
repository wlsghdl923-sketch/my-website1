...
/* ════════════════════════════════
   메모 — Notion 스타일
   핵심:
   • contextmenu에서 range 즉시 저장
   • 저장 후 커서를 span 뒤로 이동 (이후 일반 텍스트 입력 가능)
   • 패널 Y = span.top - editorCol.top + editorCol.scrollTop
   • panel.scrollTop = editorCol.scrollTop (동기화)
   • 완료 버튼(✓) → 연두색 배지
   • 메모명 + 메모설명 + 템플릿 칩
════════════════════════════════ */
...
async function _loadMemoTemplates(){
  try{const row=await _sbRest.selectOne('site_data','memo_templates');if(row&&Array.isArray(row.value))_memoTemplates=row.value;else if(row&&typeof row.value==='string')_memoTemplates=JSON.parse(row.value);}catch{}
}
async function _saveMemoTemplates(){
  await _sbRest.upsert('site_data',{key:'memo_templates',value:_memoTemplates,updated_at:new Date().toISOString()});
}
...
function _renderMemoTemplateChips(){
  ...
  if(!_memoTemplates.length){chips.innerHTML='<span style="font-size:.72rem;color:#ccc;">저장된 내용 없음</span>';return;}
  chips.innerHTML=_memoTemplates.slice(-12).reverse().map((t,i)=>
    `<span class="memo-tmpl-chip" data-tmpl="${i}">${t.length>16?t.slice(0,16)+'…':t}</span>`
  ).join('');
  chips.onclick=function(e){const chip=e.target.closest('.memo-tmpl-chip');if(!chip)return;const idx=parseInt(chip.dataset.tmpl);const arr=[..._memoTemplates].slice(-12).reverse();if(arr[idx]!==undefined)document.getElementById('memoInputDesc').value=arr[idx];};
}
...
function saveMemoToSelection(){
  // name/desc 모두 공백이어도 OK
  const name=(document.getElementById('memoInputName').value||'').trim();
  const desc=(document.getElementById('memoInputDesc').value||'').trim();
  if(!_memoSavedRange){closeMemoInput();return;}
  // 템플릿 저장
  if(desc&&!_memoTemplates.includes(desc)){_memoTemplates.push(desc);if(_memoTemplates.length>20)_memoTemplates=_memoTemplates.slice(-20);_saveMemoTemplates();}
  ...
  span.className='re-memo';span.dataset.memoid=id;span.dataset.memoName=name||'메모';span.dataset.memoDesc=desc;span.dataset.memoDate=dateStr;span.dataset.memoResolved='';
  ...
  // 커서를 span 뒤로 이동 → 이후 일반 텍스트 입력 가능
  try{const r=document.createRange();r.setStartAfter(span);r.collapse(true);sel.removeAllRanges();sel.addRange(r);}catch{}
...
function refreshMemoPanel(){
  ...
  const editorCol=document.getElementById('reEditorCol');
  if(!editor||!panel||!list||!editorCol)return;
  const memos=Array.from(editor.querySelectorAll('.re-memo'));
  if(!memos.length){panel.style.display='none';return;}
  panel.style.display='block';

  // panel과 editorCol의 상단 기준점
  const colRect=editorCol.getBoundingClientRect();
  const CARD_H=110;
  let nextMinY=8;
  const positions=[];
  ...
  memos.forEach((m,i)=>{
    ...
    // Y = span의 뷰포트 위치 - editorCol 뷰포트 위치 + scroll = editorCol 내 절대 Y
    let y=mRect.top-colRect.top+editorCol.scrollTop;
    y=Math.max(nextMinY,y);nextMinY=y+CARD_H+6;
    positions.push({id,y,m});
  });

  // 리스트 최소 높이 설정
  const last=positions[positions.length-1];
  list.style.minHeight=(last.y+CARD_H+40)+'px';
  ...
  positions.forEach(({id,y,m})=>{
    ...
    const quote=(m.textContent||'').slice(0,30);
    ...
    card.style.top=y+'px';
    card.innerHTML=`<div class="memo-card-header"><span class="memo-card-name">${name}</span><span class="memo-card-time">${date}</span></div>`+
      `<div class="memo-card-quote">"${quote}"</div>`+
      (desc?`<div class="memo-card-desc">${isResolved?'<span class="memo-done-badge">✓ 완료</span>':desc}</div>`:(isResolved?'<div class="memo-card-desc"><span class="memo-done-badge">✓ 완료</span></div>':''))+
      `<div class="memo-card-btns">${!isResolved?`<button class="mresolve" onclick="resolveMemoCard('${id}')">✓</button>`:''}<button onclick="editMemoCard('${id}')">수정</button><button class="mdel" onclick="deleteMemoCard('${id}')">삭제</button></div>`;
  ...
  // 스크롤 동기화
  panel.scrollTop=editorCol.scrollTop;
}
...
function highlightMemoCard(memoEl){
  ...
  const card=document.getElementById('mc_'+memoEl.dataset.memoid);if(card)card.classList.add('active');
}
function resolveMemoCard(id){
  const el=document.querySelector(`[data-memoid="${id}"]`);if(!el)return;
  el.dataset.memoResolved='true';el.classList.add('memo-resolved');
...
function deleteMemoCard(id){
  ...
  const frag=document.createDocumentFragment();while(el.firstChild)frag.appendChild(el.firstChild);p.insertBefore(frag,el);p.removeChild(el);
...
/* ═══════════════════════════════════════════
   표 열/행 리사이즈
   ━━━ 세로선(열) ━━━
   document.addEventListener capture=true 로
   contenteditable 포커스보다 먼저 mousedown 처리
   e.clientX ↔ cell.getBoundingClientRect().right 비교
   ① 현재 셀 오른쪽 ±THRESH
   ② 현재 셀 왼쪽 ±THRESH (= 이전 열 오른쪽)
   두 경우를 모두 체크하여 경계 감지
═══════════════════════════════════════════ */
let _colDrag=null, _rowDrag=null, _cursorMemo=null;

function _getCellAt(x,y,editor){
  const el=document.elementFromPoint(x,y);
  if(!el||!editor.contains(el))return null;
  return el.closest&&el.closest('td,th');
}

function _findColEdge(x,y,editor){
  const THRESH=7;
  const cell=_getCellAt(x,y,editor);
  if(!cell)return null;
  ...
  // ① 이 셀의 오른쪽 경계 (ci < last)
  if(relX>=rect.width-THRESH&&ci<cells.length-1){
    const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci]).filter(Boolean);
    return{cells:colCells,startW:cell.offsetWidth};
  }
  // ② 이 셀의 왼쪽 경계 (= 이전 셀 오른쪽, ci > 0)
  if(relX<=THRESH&&ci>0){
    const prevCell=cells[ci-1];
    const colCells=rows.map(r=>Array.from(r.querySelectorAll('td,th'))[ci-1]).filter(Boolean);
    return{cells:colCells,startW:prevCell.offsetWidth};
  }
  return null;
}

function _findRowEdge(x,y,editor){
  const THRESH=7;
  const cell=_getCellAt(x,y,editor);
  if(!cell)return null;
  const rect=cell.getBoundingClientRect();
  const relY=y-rect.top;
  const tr=cell.closest('tr');if(!tr)return null;
  const table=tr.closest('table');
  const rows=Array.from(table.querySelectorAll('tr'));
  const ri=rows.indexOf(tr);
  // 아래쪽 경계
  if(relY>=rect.height-THRESH&&ri<rows.length-1)return{tr,startH:tr.offsetHeight};
  // 위쪽 경계 (= 이전 행 아래쪽)
  if(relY<=THRESH&&ri>0)return{tr:rows[ri-1],startH:rows[ri-1].offsetHeight};
  return null;
}
...
function _initTableResize(){
  ...
  document.addEventListener('mousemove',function(e){
    if(_colDrag){
      const newW=Math.max(1,_colDrag.startW+(e.clientX-_colDrag.startX));
      _colDrag.cells.forEach(c=>{c.style.width=newW+'px';c.style.minWidth='0';c.style.maxWidth='none';c.style.overflow='hidden';});
      return;
    }
    if(_rowDrag){
    ...
    if(e.clientX<edRect.left||e.clientX>edRect.right||e.clientY<edRect.top||e.clientY>edRect.bottom){if(_cursorMemo){editor.style.cursor='';_cursorMemo=null;}return;}
    if(_findColEdge(e.clientX,e.clientY,editor)){if(_cursorMemo!=='col'){editor.style.cursor='col-resize';_cursorMemo='col';}}
    else if(_findRowEdge(e.clientX,e.clientY,editor)){if(_cursorMemo!=='row'){editor.style.cursor='row-resize';_cursorMemo='row';}}
    else{if(_cursorMemo){editor.style.cursor='';_cursorMemo=null;}}
  });

  // Capture phase mousedown — contenteditable보다 먼저 실행
  document.addEventListener('mousedown',function(e){
    const col=_findColEdge(e.clientX,e.clientY,editor);
    if(col){e.preventDefault();_colDrag={...col,startX:e.clientX};return;}
    const row=_findRowEdge(e.clientX,e.clientY,editor);
    if(row){e.preventDefault();_rowDrag={...row,startY:e.clientY};return;}
  },true);

  document.addEventListener('mouseup',function(){
    _colDrag=null;_rowDrag=null;editor.style.cursor='';_cursorMemo=null;
  });
}
...
function doPdfExport(){
  ...
  let memoSection='';
  if(withMemo){const memos=[];editor.querySelectorAll('.re-memo').forEach(m=>memos.push({text:m.textContent,name:m.dataset.memoName||'메모',desc:m.dataset.memoDesc||'',resolved:m.dataset.memoResolved==='true'}));
    if(memos.length){memoSection='<div style="margin-top:32px;border-top:2px solid #FFE082;padding-top:16px;"><div style="font-weight:800;font-size:.95rem;margin-bottom:10px;color:#F57F17;">📝 메모 목록</div>';memos.forEach(m=>{memoSection+=`<div style="margin-bottom:8px;background:${m.resolved?'#F6FBF6':'#FFFDE7'};border-left:3px solid ${m.resolved?'#2E9E50':'#FFE082'};padding:8px 12px;border-radius:0 6px 6px 0;font-size:.85rem;"><div style="font-weight:700;color:#4E5968;">${m.name}${m.resolved?' ✓':''}</div><div style="color:#8B95A1;font-size:.78rem;margin:2px 0;">"${m.text}"</div>${m.desc?`<div style="color:#5D4037;margin-top:3px;">${m.desc}</div>`:''}</div>`;});memoSection+='</div>';}}
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title}</title><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700;800&display=swap" rel="stylesheet"><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Noto Sans KR',sans-serif;font-size:14px;line-height:1.9;color:#191F28;padding:40px 48px;}h1{font-size:1.9rem;font-weight:800;margin:1.2em 0 .4em;}h2{font-size:1.4rem;font-weight:800;margin:1em 0 .35em;}h3{font-size:1.1rem;font-weight:700;margin:.9em 0 .3em;}p{margin:.4em 0;}ul,ol{padding-left:1.5em;margin:.4em 0;}li{margin:.15em 0;}blockquote{border-left:4px solid #D91F3E;padding:.5em 1em;background:#FFF3F5;border-radius:0 6px 6px 0;margin:.6em 0;}.re-callout{display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:12px 14px;margin:.6em 0;line-height:1.7;}table{border-collapse:collapse;width:100%;margin:.6em 0;}td,th{padding:8px 12px;border:1px solid #E5E8EB;}th{background:#F5F6FA;font-weight:700;}img{max-width:100%;border-radius:6px;}.re-img-wrap{display:inline-block;vertical-align:top;}.re-img-handle{display:none!important;}.tbl-sel{outline:none!important;}.re-memo{background:rgba(255,213,79,.35);border-radius:2px;border-bottom:2px solid #F57F17;}.re-memo.memo-resolved{background:rgba(180,220,180,.45);border-bottom-color:#2E9E50;}@media print{body{padding:20px;}}</style></head><body><h1 style="border-bottom:2px solid #E5E8EB;padding-bottom:10px;margin-bottom:20px;">${title}</h1>${clone.innerHTML}${memoSection}<script>window.onload=function(){window.print();}<\/script></body></html>`);
  win.document.close();
}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    // 스크롤 동기화 (에디터 → 메모 패널)
    const editorCol=document.getElementById('reEditorCol');
    if(editorCol&&!editorCol._scrollSet){
      editorCol._scrollSet=true;
      editorCol.addEventListener('scroll',function(){
        const panel=document.getElementById('reMemoPanel');
        if(panel&&panel.style.display!=='none')panel.scrollTop=editorCol.scrollTop;
        debouncedMemoPanel();
      });
    }
  }
  if(docId){
    ...
    reUpdateToolbar();setTimeout(()=>{_initImgResize();_initTableResize();initColorPalettes();},300);
    const raw=await _readDocRawAsync(docId);editor.innerHTML=raw||'';
    setTimeout(refreshMemoPanel,100);
  }else{
...
function reInsertTable(){
  const rows=3,cols=3;let html='<table style="border-collapse:collapse;width:100%;margin:.8em 0;border:1px solid #E5E8EB;" contenteditable="false"><colgroup>';
  ...
  toast('3×3 표 삽입. 열 경계를 드래그하여 너비 조절 | 우클릭 편집');
}
