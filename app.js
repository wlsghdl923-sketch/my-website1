...
function rdTogglePanels(){
  ...
  const btn=document.getElementById('rdPanelBtn');
...
function rdCloseLightbox(){
  const lb=document.getElementById('rdLightbox');
  if(lb)lb.style.display='none';
}

/* ══ openRefDocModal — async로 Supabase에서 데이터 로드 ══ */
async function openRefDocModal(docId=''){
  ...
  if(docId){
    ...
    try{
      ...
      console.warn('이미지 의견 로드 실패:',e);
      // fallback: _readDocObj 시도
      const stored=_readDocObj(docId);
      if(stored&&stored.images){_rdImages=stored.images;renderRdImages();}
    }
  }
...
/* ══ renderRdImages — 이미지:텍스트 7:3 + 더블클릭 라이트박스 ══ */
  ...
  list.innerHTML=_rdImages.map((img,i)=>`
    <div class="rd-row" id="rdRow-${i}"
      ...
      <div class="rd-row-body" id="rdBody-${i}">
        <!-- 이미지 영역 (기본 70%) -->
        <div class="rd-img-side" id="rdImgSide-${i}" style="flex:7;min-width:120px;">
          <img src="${img.data}" alt="이미지 ${i+1}"
            style="width:100%;height:100%;object-fit:contain;display:block;max-height:400px;cursor:zoom-in;"
            title="더블클릭하면 원본 크기로 보기"
            ondblclick="rdOpenLightbox('${img.data}')"
            onclick="_rdSlotIdx=${i};/* 단클릭은 교체 안함 */"
          >
          <div class="rd-img-replace" onclick="_rdSlotIdx=${i};document.getElementById('rdFileInSlot').click()">🔄 클릭하여 교체</div>
        </div>
        <!-- 리사이저 -->
        <div class="rd-resizer" onmousedown="rdStartResize(event,${i})" title="드래그하여 비율 조절">
          <div class="rd-resizer-bar"></div>
        </div>
        <!-- 텍스트 영역 (기본 30%) -->
        <div class="rd-txt-side" id="rdTxtSide-${i}" style="flex:3;min-width:80px;">
...
function rdStartResize(e,i){
  ...
  const imgSide=document.getElementById(`rdImgSide-${i}`);
  const txtSide=document.getElementById(`rdTxtSide-${i}`);
  if(!body||!imgSide||!txtSide)return;
  _rdRszState={
    startX:e.clientX,
    totalW:body.offsetWidth,
    imgStartW:imgSide.offsetWidth,
    imgEl:imgSide,
    txtEl:txtSide,
  };
  ...
  const onMove=function(ev){
    ...
    const newImgW=Math.max(80,Math.min(_rdRszState.totalW-80,_rdRszState.imgStartW+dx));
    const newTxtW=_rdRszState.totalW-newImgW-5;
    // flex 대신 직접 width 지정
    _rdRszState.imgEl.style.flex='none';
    _rdRszState.imgEl.style.width=newImgW+'px';
    _rdRszState.txtEl.style.flex='none';
    _rdRszState.txtEl.style.width=newTxtW+'px';
  };
  const onUp=function(){
    _rdRszState=null;
...
/* ══ 패널 토글 (richtext editor용) ══ */
/* togglePanels는 기존에 있을 수 있음, 없으면 빈 함수 */
if(typeof togglePanels==='undefined'){
  window.togglePanels=function(){
    ...
    if(!mp&&!rp)return;
    const show=mp?mp.style.display==='none':true;
    if(mp)mp.style.display=show?'block':'none';
    if(rp)rp.style.display=show?'block':'none';
...
/* openRefDocModal → 위의 async 버전으로 대체됨 */
