...
async function openMemoInputFromTable(){
  ...
  document.getElementById('memoInputSaveBtn').onclick=saveMemoToSelection;
  const prev=document.getElementById('memoSelectedPreview');
  if(prev)prev.textContent=_memoSavedText?'"'+_memoSavedText.slice(0,50)+'"':'(표 안 텍스트를 드래그하여 선택 후 우클릭)';
  document.getElementById('memoMarkerName').value='';
  document.getElementById('memoInputDesc').value='';
  _selMarkerColor='';_selMarkerName='';
  document.querySelectorAll('#memoMarkerPalette .mkr-chip').forEach(c=>c.classList.remove('sel'));
...
function applyMemoEdit(id){
  ...
  if(_selMarkerColor&&mn&&!_markerPresets.some(p=>p.color===_selMarkerColor&&p.name===mn)){_markerPresets.push({color:_selMarkerColor,name:mn});if(_markerPresets.length>15)_markerPresets=_markerPresets.slice(-15);}
  _saveMemoData();
...
function _renderMarkerPresetChips(){
  ...
  el.innerHTML=_markerPresets.slice(-10).reverse().map(p=>`<span class="mkr-preset-chip" onclick="applyMarkerPreset('${p.color}','${encodeURIComponent(p.name)}')"><span class="mkr-preset-dot" style="background:${p.color};"></span>${p.name}</span>`).join('');
}
...
function closeMemoInput(){document.getElementById('memoInputOverlay').style.display='none';
  // 표 메모용이 아닌 경우만 range 초기화
  if(!_memoEditId){_memoSavedRange=null;_memoSavedText='';}
  _memoEditId=null;}

function saveMemoToSelection(){
  if(!_memoSavedRange){
    toast('메모를 달 텍스트가 없습니다. 텍스트를 먼저 드래그하여 선택하세요.');
    return;
  }
  const mn=document.getElementById('memoMarkerName').value.trim();
  _selMarkerName=mn;
...
function refreshMemoPanel(){
  ...
  if(!memos.length){
    ...
    if(lResizer)lResizer.style.display='none';if(rResizer)rResizer.style.display='none';
    return;
  }
  ...
  if(mPanel){
    mPanel.style.display='block';if(lResizer)lResizer.style.display='block';
    if(mList){
      const markerRows=Object.values(markerMap).map(mk=>`<div class="marker-row"><div class="marker-dot" style="background:${mk.color};"></div><div class="marker-label">${mk.name}</div><div class="marker-count">${mk.count}</div></div>`).join('');
      mList.innerHTML=`<div style="background:#DBEAFE;border-radius:6px;padding:4px 7px;margin-bottom:5px;font-size:.7rem;color:#1e40af;font-weight:700;text-align:center;">전체 ${totalCount}개</div>`+markerRows;
    }
  ...
  /* 메모 카드 — flow 배치 (position:static, 겹침 없음) */
  list.innerHTML='';
  memos.forEach((m,i)=>{
    ...
    card.innerHTML=
      ...
      `</div>`;
    card.onclick=function(e){if(!e.target.closest('button')){highlightMemoCard(m);scrollToMemoCard(m);}};
    list.appendChild(card);
  });
}

function scrollToMemoCard(memoEl){if(!memoEl)return;memoEl.scrollIntoView({behavior:'smooth',block:'center'});}
...
document.addEventListener('mousemove',function(e){
  ...
  const newW=Math.max(120,Math.min(320,startW+dx));
...
/* 표 세로선 — Sheets 방식 오버레이 */
let _colDragState=null;

function _updateTableColHandles(){
  const overlay=document.getElementById('tblColOverlay');
  const editor=document.getElementById('reEditor');
  const editorWrap=document.getElementById('reEditorWrap');
  if(!overlay||!editor||!editorWrap)return;
  ...
  editor.querySelectorAll('table').forEach(table=>{
    ...
    const tblTop=tblRect.top-edRect.top;
    ...
    firstCells.forEach((cell,ci)=>{
      ...
      // 오버레이는 editorWrap 내 absolute
      const handleX=cellRect.right-edRect.left;
      ...
      h.addEventListener('mousedown',function(e){
        ...
        _colDragState={cells:colCells,startX:e.clientX,startW:cell.offsetWidth,handle:h,cell};
...
function _initTableResize(){
  ...
  setTimeout(_updateTableColHandles,200);
  // 행 높이
  ...
  document.addEventListener('mousemove',function(e){
    if(_colDragState){
      ...
      _colDragState.cells.forEach(c=>{c.style.width=newW+'px';c.style.minWidth='0';c.style.maxWidth='none';c.style.overflow='hidden';});
      // 핸들 실시간 이동
      const edRect=editor.getBoundingClientRect();
      const cr=_colDragState.cell.getBoundingClientRect();
      _colDragState.handle.style.left=(cr.right-edRect.left)+'px';
    }
    if(_rowDrag){
  ...
  document.addEventListener('mouseup',function(){
    if(_colDragState){_colDragState.handle.classList.remove('dragging');_colDragState=null;document.body.style.cursor='';document.body.style.userSelect='';setTimeout(_updateTableColHandles,60);}
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('contextmenu',function(e){
      ...
      if(td){
        ...
        else{_memoSavedRange=null;_memoSavedText='';}
        showTblCtxMenu(e,td);
        return;
      }
      const sel=window.getSelection();
    ...
    editor.addEventListener('click',function(e){
      ...
      if(memo&&!e.target.closest('button')){e.stopPropagation();highlightMemoCard(memo);scrollToMemoCard(memo);}
    });
...
function reInsertTable(){
  ...
  reExec('insertHTML',html);setTimeout(()=>_initTableResize(),100);
