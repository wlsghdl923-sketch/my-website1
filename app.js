...
function initColorPalettes(){
  ...
  fg.innerHTML=TEXT_COLORS30.map(c=>{
    const L=c==='#FFFFFF'||c==='#CFD8DC'||c==='#FFE082';
    return `<div class="tc${L?' tc-light':''}" style="background:${c};" onmousedown="event.preventDefault();applyFgColor('${c}')"></div>`;
  }).join('');
  bg.innerHTML=BG_COLORS30.map(c=>{
    const T=c==='transparent';
    const L=c==='#FFFFFF'||c==='#F9F9F9'||T;
    return `<div class="tc${L?' tc-light':''}" style="${T?'background:linear-gradient(135deg,#fff 45%,#f44 45%,#f44 55%,#fff 55%);':'background:'+c+';'}" onmousedown="event.preventDefault();applyBgColor('${c}')"></div>`;
  }).join('');
  document.addEventListener('mousedown',function(e){
    if(!e.target.closest('.re-color-wrap'))document.querySelectorAll('.re-color-palette').forEach(p=>p.style.display='none');
  });
}
...
function openMemoInput(){
  ...
  const sel=window.getSelection();
  if(!sel||sel.isCollapsed){toast('텍스트를 먼저 드래그하여 선택하세요.');return;}
  _memoSel={r:sel.getRangeAt(0).cloneRange(),text:sel.toString().trim()};
  const selTxt=document.getElementById('memoSelectedText');
  if(selTxt)selTxt.textContent=_memoSel.text;
...
function saveMemoToSelection(){
  ...
  if(!_memoSel){closeMemoInput();return;}
  const sel=window.getSelection();
  sel.removeAllRanges();sel.addRange(_memoSel.r);
  const id='memo_'+Date.now();
  const span=document.createElement('span');
  span.className='re-memo';
  span.dataset.memo=text;
  span.dataset.memoid=id;
  span.title='📝 '+text;
  try{
    _memoSel.r.surroundContents(span);
  }catch{
    const frag=_memoSel.r.extractContents();
    span.appendChild(frag);_memoSel.r.insertNode(span);
  }
  sel.removeAllRanges();
  closeMemoInput();
  refreshMemoPanel();
...
function refreshMemoPanel(){
  ...
  list.innerHTML=memos.map((m,i)=>{
    const id=m.dataset.memoid||('memo_'+i);
    m.dataset.memoid=id;
    return `<div class="memo-card" id="mc_${id}" onclick="highlightMemoCard(document.querySelector('[data-memoid=\\'${id}\\']'))">
      <div class="memo-card-text">"${(m.textContent||'').slice(0,40)}"</div>
      <div class="memo-card-content">${m.dataset.memo||''}</div>
      <div class="memo-card-actions">
        <button class="memo-card-btn" onclick="event.stopPropagation();editMemoCard('${id}')">수정</button>
        <button class="memo-card-btn del" onclick="event.stopPropagation();deleteMemoCard('${id}')">삭제</button>
      </div>
    </div>`;
  }).join('');
}
...
function highlightMemoCard(memoEl){
  if(!memoEl)return;
  const id=memoEl.dataset.memoid;
  if(!id)return;
...
function editMemoCard(id){
  const memoEl=document.querySelector(`[data-memoid="${id}"]`);
  if(!memoEl)return;
  const newText=prompt('메모 수정:',memoEl.dataset.memo||'');
  if(newText===null)return;
  if(newText.trim()){memoEl.dataset.memo=newText.trim();memoEl.title='📝 '+newText.trim();}
  refreshMemoPanel();
}
...
function deleteMemoCard(id){
  const memoEl=document.querySelector(`[data-memoid="${id}"]`);
  if(!memoEl)return;
  const parent=memoEl.parentNode;
  while(memoEl.firstChild)parent.insertBefore(memoEl.firstChild,memoEl);
  parent.removeChild(memoEl);
  refreshMemoPanel();toast('메모가 삭제되었습니다.');
}

/* ══ 커스텀 이모지 (Supabase) ══ */
let _coCustomEmojis=[];
async function _loadCoCustomEmojis(){
  try{const row=await _sbRest.selectOne('site_data','co_custom_emojis');
    if(row&&Array.isArray(row.value))_coCustomEmojis=row.value;
    else if(row&&typeof row.value==='string')_coCustomEmojis=JSON.parse(row.value);
  }catch{}
}
async function _saveCoCustomEmojis(){
  await _sbRest.upsert('site_data',{key:'co_custom_emojis',value:_coCustomEmojis,updated_at:new Date().toISOString()});
}
const CO_COLORS=[
...
function _coEmojiGridHTML(sel){
  const customs=_coCustomEmojis.map((em,i)=>{
    const isImg=em.startsWith('data:')||em.startsWith('http');
    const inner=isImg?`<img src="${em}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;">`:`${em}`;
    return `<span class="co-em-btn${em===sel?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${inner}<span class="co-em-del" onclick="event.stopPropagation();delCoCustomEmoji(${i})">✕</span></span>`;
  });
...
async function handleCoCustomImg(e){
  const file=e.target.files[0];e.target.value='';if(!file)return;await _loadCoCustomEmojis();
  const reader=new FileReader();reader.onload=async ev=>{_coCustomEmojis.push(ev.target.result);await _saveCoCustomEmojis();const sel=_coPick.emoji;['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});toast('커스텀 이모지 저장됨');};reader.readAsDataURL(file);
}
async function delCoCustomEmoji(idx){
  await _loadCoCustomEmojis();_coCustomEmojis.splice(idx,1);await _saveCoCustomEmojis();const sel=_coPick.emoji;['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{const g=document.getElementById(id);if(g)g.innerHTML=_coEmojiGridHTML(sel);});toast('삭제됨');
}
...
async function openCoEmojiChange(calloutEl){
  _coChangeTarget=calloutEl;await _loadCoCustomEmojis();
  const cur=calloutEl.querySelector('[data-coem]');const curEm=cur?cur.dataset.coem:'';_coPick.emoji=curEm;
  const g=document.getElementById('coEmojiChangeGrid');if(g)g.innerHTML=_coEmojiGridHTML(curEm);
  document.getElementById('coEmojiChangeOverlay').style.display='block';
}
...
function applyCoEmojiChange(){
  if(!_coChangeTarget)return;
  const old=_coChangeTarget.querySelector('[data-coem]');if(old)old.remove();
  if(_coPick.emoji){
    const isImg=_coPick.emoji.startsWith('data:')||_coPick.emoji.startsWith('http');
    const el=document.createElement(isImg?'img':'span');el.setAttribute('data-coem',_coPick.emoji);el.contentEditable='false';
    if(isImg){el.src=_coPick.emoji;el.style.cssText='width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;';}
    else{el.style.cssText='font-size:1.2rem;flex-shrink:0;margin-top:1px;';el.textContent=_coPick.emoji;}
    const body=_coChangeTarget.querySelector('div[contenteditable="true"]');
    if(body)_coChangeTarget.insertBefore(el,body);else _coChangeTarget.prepend(el);
  }
  closeCoEmojiChange();
}
document.addEventListener('dblclick',function(e){
  ...
  const editor=document.getElementById('reEditor');
  if(callout&&editor&&editor.contains(callout))openCoEmojiChange(callout);
});
...
function tblCmd(cmd){
  ...
  if(!tr||!table){_tblTd=null;return;}
  ...
  const TDS='padding:10px 14px;border:1px solid #E5E8EB;position:relative;';
  const THS='padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;position:relative;';
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    const nr=document.createElement('tr');
    for(let i=0;i<cols;i++){const c=document.createElement('td');c.contentEditable='true';c.style.cssText=TDS;nr.appendChild(c);}
    cmd==='addRowAbove'?tr.parentNode.insertBefore(nr,tr):tr.parentNode.insertBefore(nr,tr.nextSibling);
  }else if(cmd==='addColLeft'||cmd==='addColRight'){
    rows.forEach((row,ri2)=>{
      const c=Array.from(row.querySelectorAll('td,th'))[ci];if(!c)return;
      const tag=ri2===0?'th':'td';const nc=document.createElement(tag);
      nc.contentEditable='true';nc.style.cssText=ri2===0?THS:TDS;
      cmd==='addColLeft'?c.parentNode.insertBefore(nc,c):c.parentNode.insertBefore(nc,c.nextSibling);
    });
  ...
  else if(cmd==='splitCell'){
    const cs=td.colSpan||1,rs=td.rowSpan||1;if(cs<=1&&rs<=1){toast('병합된 셀이 아닙니다.');_tblTd=null;return;}
    td.colSpan=1;td.rowSpan=1;
    for(let r2=ri;r2<ri+rs;r2++){const startC=(r2===ri)?ci+1:ci;for(let c2=startC;c2<ci+cs;c2++){const nc=document.createElement('td');nc.contentEditable='true';nc.style.cssText=TDS;const targetRow=rows[r2];if(targetRow){const sib=Array.from(targetRow.querySelectorAll('td,th'))[c2];targetRow.insertBefore(nc,sib||null);}}}
  }else if(cmd==='splitH'){
    const colspan=td.colSpan||1;const newTr=document.createElement('tr');
    cells.forEach((cell,i)=>{if(i===ci){const nc=document.createElement('td');nc.contentEditable='true';nc.style.cssText=cell.style.cssText;nc.colSpan=colspan;newTr.appendChild(nc);}else{cell.rowSpan=(cell.rowSpan||1)+1;}});
  ...
  setTimeout(_addColHandles,50);
}

/* ── 셀 색상 ── */
const CELL_COLORS30=['#FFFFFF','#F5F6FA','#FFECEC','#FFF3E0','#FFFDE7','#F1F8E9',
  '#E8F5E9','#E0F7FA','#E1F5FE','#E8EAF6','#F3E5F5','#FCE4EC','#FFCDD2','#FFE0B2',
  '#FFF9C4','#DCEDC8','#B2DFDB','#B3E5FC','#C5CAE9','#E1BEE7','#F8BBD9','#D7CCC8',
  '#CFD8DC','#FFAB91','#FFD54F','#AED581','#4DB6AC','#4FC3F7','#7986CB','#F06292'];
let _selCellClr='';
function openCellColorPicker(){
  if(!_tblTd){toast('셀을 먼저 클릭하여 선택하세요.');return;}
  const cur=_tblTd.style.background||'';
  const g=document.getElementById('cellClrGrid');
  if(g)g.innerHTML=CELL_COLORS30.map(c=>`<div class="clr-chip${c===cur?' sel':''}" style="background:${c};${c==='#FFFFFF'?'border-color:#ccc;':''}" onclick="pickCellClr(this,'${c}')"></div>`).join('');
  _selCellClr=cur;document.getElementById('cellClrOverlay').style.display='block';
}
...
const TBL_BORDER_COLORS=['#E5E8EB','#191F28','#D91F3E','#FF7A00','#0064FF','#2E9E50','#9C27B0','#8B95A1',
  '#F44336','#E91E63','#673AB7','#3F51B5','#2196F3','#03A9F4','#00BCD4','#009688','#4CAF50','#8BC34A',
  '#CDDC39','#FFEB3B','#FFC107','#FF9800','#FF5722','#795548','#607D8B','#BDBDBD','#90CAF9','#A5D6A7',
  '#FFCC80','#CE93D8','#E8D5D5','#E8E0D5','#E8E8D5','#D5E8D5','#D5E8E8','#D5D5E8','#E8D5E8','#E2E8D5',
  '#D5E2E8','#E8DDD5','#DDE8D5','#D5DDE8','#E8D5DD','#E5DDE8','#D8E8E5'];
let _selTblBorderClr='#E5E8EB';
function openTblBorderColorPicker(){
  if(!_tblTd){toast('셀을 먼저 클릭하여 선택하세요.');return;}
  const table=_tblTd.closest('table');const cur=(table&&table.style.borderColor)||'#E5E8EB';
  const g=document.getElementById('tblBorderClrGrid');
  if(g)g.innerHTML=TBL_BORDER_COLORS.map(c=>`<div class="clr-chip${c===cur?' sel':''}" style="background:${c};" onclick="pickTblBorderClr(this,'${c}')"></div>`).join('');
  _selTblBorderClr=cur;document.getElementById('tblBorderClrOverlay').style.display='block';
}
...
function applyTblBorderClr(color){
  if(!_tblTd)return;const table=_tblTd.closest('table');if(!table)return;
  table.style.border='1px solid '+color;table.querySelectorAll('td,th').forEach(cell=>{cell.style.border='1px solid '+color;});
  closeTblBorderClr();
}

/* ══ 표 열 너비 — 핸들 오버레이 방식 ══
   각 셀 오른쪽에 .col-rsz-handle div를 삽입하고
   이를 드래그하여 해당 셀(및 같은 열)의 너비를 조절한다.
   contenteditable 충돌 없이 완전히 독립적으로 작동한다.
══ */
let _colRsz=null;

function _addColHandles(){
  const editor=document.getElementById('reEditor');
  if(!editor)return;
  // 기존 핸들 전부 제거
  ...
  editor.querySelectorAll('td,th').forEach(cell=>{
    ...
    h.addEventListener('mousedown',function(e){
      ...
      const ci=Array.from(cell.parentElement.querySelectorAll('td,th')).indexOf(cell);
...
function _initTableResize(){
  ...
  // 행 높이 드래그: 셀 아래쪽 6px 감지 (기존 방식 유지)
  editor.addEventListener('mousemove',function(e){
    ...
    if(ry>=rect.height-6)editor.style.cursor='row-resize';
    else if(editor.style.cursor==='row-resize')editor.style.cursor='';
  });
  let _rowRsz=null;
  editor.addEventListener('mousedown',function(e){
    if(e.target.classList.contains('col-rsz-handle'))return;
    const cell=e.target.closest&&e.target.closest('td,th');if(!cell)return;
    const rect=cell.getBoundingClientRect();
    const ry=e.clientY-rect.top;
    if(ry>=rect.height-6){
      e.preventDefault();e.stopPropagation();
      const tr=cell.closest('tr');_rowRsz={tr,startY:e.clientY,startH:tr.offsetHeight};
    }
  });
  document.addEventListener('mousemove',function(e){
    if(_colRsz){
      // 열 너비 조절: 1px 이상으로 자유롭게
  ...
  document.addEventListener('mouseup',function(){
    if(_colRsz){document.querySelectorAll('.col-rsz-handle.active').forEach(h=>h.classList.remove('active'));_colRsz=null;}
...
function exportDocAsPdf(){
  const editor=document.getElementById('reEditor');
  if(!editor){toast('에디터를 열어주세요.');return;}
  const memos=editor.querySelectorAll('.re-memo');
  if(memos.length>0){
    document.getElementById('pdfOptOverlay').style.display='block';
  }else{
    doPdfExport();
  }
}
...
function doPdfExport(){
  ...
  const includeMemo=document.getElementById('pdfOptMemo')?.checked!==false;
  ...
  if(includeMemo){
    const memos=[];
    editor.querySelectorAll('.re-memo').forEach(m=>{memos.push({text:m.textContent,memo:m.dataset.memo||''});});
    if(memos.length>0){
      memoSection='<div style="margin-top:32px;border-top:2px solid #FFE082;padding-top:16px;"><div style="font-weight:800;font-size:.95rem;margin-bottom:10px;color:#F57F17;">📝 메모 목록</div>';
      memos.forEach(m=>{memoSection+=`<div style="margin-bottom:8px;background:#FFFDE7;border-left:3px solid #FFE082;padding:8px 12px;border-radius:0 6px 6px 0;font-size:.85rem;"><span style="font-weight:700;color:#5D4037;">"${m.text}"</span><div style="color:#5D4037;margin-top:4px;">${m.memo}</div></div>`;});
      memoSection+='</div>';
    }
  }else{
    // 메모 하이라이트 제거
    clone.querySelectorAll('.re-memo').forEach(m=>{const frag=document.createDocumentFragment();while(m.firstChild)frag.appendChild(m.firstChild);m.replaceWith(frag);});
  }
...
.tbl-sel{outline:none!important;}.re-memo{background:#FFD54F;border-radius:3px;border-bottom:2px solid #F57F17;}
...
async function exportBackup(){
  toast('백업 중...');
  try{
    const rows=await _sbRest.select('site_data');if(!rows||rows.length===0){toast('데이터 없음');return;}
    const obj={_v:1,_d:new Date().toISOString(),data:{}};rows.forEach(r=>{obj.data[r.key]=r.value;});
    const url=URL.createObjectURL(new Blob([JSON.stringify(obj,null,2)],{type:'application/json'}));
    const a=document.createElement('a');a.href=url;a.download='backup_'+new Date().toISOString().slice(0,16).replace('T','_').replace(':','-')+'.json';
    document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
    toast('백업 완료 — '+rows.length+'개');
  }catch(e){toast('백업 실패');}
}
async function importBackup(event){
  ...
  toast('복원 중...');
  try{
    const obj=JSON.parse(await file.text());if(!obj.data){toast('올바른 파일 아님');return;}
...
async function openRichEditor(docId=''){
  ...
  if(!editor._evSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(html){
        const CS='display:flex;align-items:flex-start;gap:10px;border-radius:8px;padding:14px 16px;margin:.8em 0;border:1.5px solid #C7D7FF;background:#F0F4FF;color:#1A3A8F;line-height:1.7;';
        document.execCommand('insertHTML',false,html.replace(/<aside[^>]*>/gi,'<div class="re-callout" style="'+CS+'">').replace(/<\/aside>/gi,'</div>'));
    ...
    editor.addEventListener('contextmenu',function(e){
      ...
      const sel=window.getSelection();
      if(sel&&!sel.isCollapsed){e.preventDefault();showEditorCtxMenu(e);}
    });
    editor.addEventListener('click',function(e){
      ...
      if(memo)highlightMemoCard(memo);
    });
  }
  if(docId){
    const d=(CARD_DOCS[_curCard]||[]).find(x=>x.id===docId);
...
function _initImgResize(){
  ...
  document.addEventListener('mousemove',function(e){
    if(!_imgRsz)return;
    const dx=e.clientX-_imgRsz.sx;
    _imgRsz.w.style.width=Math.max(60,Math.min((_imgRsz.dir==='se'||_imgRsz.dir==='ne')?_imgRsz.sw+dx:_imgRsz.sw-dx,760))+'px';
  });
...
function reInsertTable(){
  ...
  for(let r=0;r<rows;r++){
    ...
    for(let c=0;c<cols;c++){
      const isH=r===0;
      const st=isH?'padding:10px 14px;border:1px solid #E5E8EB;background:#F5F6FA;font-weight:700;text-align:left;position:relative;':'padding:10px 14px;border:1px solid #E5E8EB;position:relative;';
  ...
  setTimeout(()=>{_initTableResize();_addColHandles();},100);
  toast('3×3 표 삽입. 열 경계 드래그로 너비 조절, 우클릭으로 편집');
}
...
function reOnKeydown(e){
  ...
  if(e.key==='Enter'){
    ...
    if(tag==='pre'){e.preventDefault();reExec('insertHTML','\n');}
  }
}
