...
function _renderCalloutColors() {
  ...
  if (!grid) return;
  // 투명(없음) 옵션 추가
  let html = `<div class="re-cp-dot${'transparent'===_calloutSelColor?' sel':''}" style="background:#fff;border:2px dashed #CCC;position:relative;" onclick="pickCalloutColor(this,'transparent')" title="색상 없음"><span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:10px;color:#999;pointer-events:none;">✕</span></div>`;
...
function _renderCalloutPreview() {
  ...
  if (!el) return;
  const isTransparent = _calloutSelColor === 'transparent';
  const bgColor = isTransparent ? 'transparent' : _hexA(_calloutSelColor, 0.08);
  const borderColor = isTransparent ? '#E5E8EB' : _calloutSelColor;
...
function reOnKeydown(e){
  ...
  if(e.ctrlKey||e.metaKey){
    // Ctrl+B → 굵게
    if(e.key==='b'){e.preventDefault();reExec('bold');return;}
    // Ctrl+I → 기울임
    if(e.key==='i'){e.preventDefault();reExec('italic');return;}
    // Ctrl+U → 밑줄
    if(e.key==='u'){e.preventDefault();reExec('underline');return;}
    // Ctrl+E → 인라인 코드
    if(e.key==='e'){
      e.preventDefault();
      const sel=window.getSelection();
