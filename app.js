...
function reInsertCallout(){
  const CS='background:#F0F4FF;border:1px solid #C7D7FF;border-radius:8px;padding:12px 16px;margin:8px 0;color:#1A3A8F;line-height:1.7;display:block;';
  document.execCommand('insertHTML',false,'<div style="'+CS+'">💡 내용을 입력하세요</div><p><br></p>');
}
...
aside{display:block;background:#F0F4FF;border:1px solid #C7D7FF;border-radius:8px;padding:12px 16px;margin:.8em 0;color:#1A3A8F;line-height:1.7;}
[style*="F0F4FF"]{display:block;background:#F0F4FF;border:1px solid #C7D7FF;border-radius:8px;padding:12px 16px;margin:.8em 0;color:#1A3A8F;line-height:1.7;}
...
@media(max-width:600px){.doc-wrap{padding:28px 20px 60px;}body{padding:16px 0 60px;}}
</style></head>
...
async function openRichEditor(docId=''){
  ...
  if(!editor._pasteHandlerSet){
    ...
    editor.addEventListener('paste',function(e){
      ...
      if(html){
        const CS='background:#F0F4FF;border:1px solid #C7D7FF;border-radius:8px;padding:12px 16px;margin:8px 0;color:#1A3A8F;line-height:1.7;display:block;';
        const clean=html.replace(/<aside[^>]*>/gi,'<div style="'+CS+'">').replace(/<\/aside>/gi,'</div>');
...
function _initImgResize(){
  ...
  document.addEventListener('mousemove',function(e){
    ...
    const newW=(dir==='se'||dir==='ne')?startW+dx:startW-dx;
    wrap.style.width=Math.max(60,Math.min(newW,760))+'px';
  });
  ...
  editor.addEventListener('touchstart',function(e){
    ...
    if(!handle)return;
    const wrap=handle.parentElement;
    _resizeState={wrap,dir:handle.dataset.dir,startX:e.touches[0].clientX,startW:wrap.offsetWidth};
  },{passive:true});
  ...
  document.addEventListener('touchend',function(){_resizeState=null;});
}
