...
function _bindTblResize(editor) {
  ...
  // ★ 체크박스: 이모지 기반으로 변경됨 — 이벤트 위임 (re-check-box 클릭)
  editor.addEventListener('click', e => {
    const box = e.target.closest('.re-check-box');
    if (box && box.closest('.re-editor')) {
      e.preventDefault();
      e.stopPropagation();
      reToggleCheck(box);
    }
...
<body><h1>${d.title}</h1><div class="container">${rowsHtml||'<p style="padding:24px;color:#8B95A1;">이미지가 없습니다.</p>'}</div></body></html>`;
  // calendar 타입은 위에서 early return으로 처리됨
    ...
    // richtext는 리치 에디터로 직접 열기 (흰공간 + 패널 + 메모 표시)
...
function reInsertCheckbox() {
  const editor = document.getElementById('reEditor');
  editor.focus();
  const html = '<div class="re-check-item"><span class="re-check-box" contenteditable="false" onclick="reToggleCheck(this)">☐</span> <span class="re-check-text">할 일 항목</span></div>';
  reExec('insertHTML', html);
}
...
function reOnKeydown(e){
  ...
  if(e.key==='Enter'){
    ...
    if(chkItem){
      ...
      newItem.innerHTML='<span class="re-check-box" contenteditable="false" onclick="reToggleCheck(this)">☐</span> <span class="re-check-text"></span>';
...
function _reCheckLineMarkdown(){
  ...
  if(text==='[]'||text==='[ ]'){
    ...
    item.innerHTML='<span class="re-check-box" contenteditable="false" onclick="reToggleCheck(this)">☐</span> <span class="re-check-text"></span>';
    block.before(item);
    block.remove();
    const r2=document.createRange();
