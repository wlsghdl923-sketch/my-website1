...
function folderDragOver(e,folderId){
  e.preventDefault();e.stopPropagation();
  // 문서 또는 폴더 드래그 모두 허용
  if(!_dragFolderId&&_dragDocIdx===null)return;
  document.querySelectorAll('.tree-folder').forEach(el=>el.classList.remove('folder-drag-over'));
  if(_dragFolderId&&_dragFolderId===folderId)return;
  document.getElementById('tf-'+folderId)?.classList.add('folder-drag-over');
}
...
function folderDrop(e,targetFolderId,targetDepth){
  ...
  if(_dragDocIdx!==null){
    const docs=CARD_DOCS[_curCard]||[];
    const moved=docs[_dragDocIdx];
    if(moved&&moved.id!==targetFolderId&&moved.type!=='folder'){
      moved.parentId=targetFolderId;
      if(!_openFolders[_curCard])_openFolders[_curCard]=new Set();
      _openFolders[_curCard].add(targetFolderId);
      saveAll();renderDocList();toast('문서가 폴더로 이동되었습니다.');
    }
    _dragDocIdx=null;return;
  }
