...
══════════════════════════════════════ */
// 이모지 목록
const CO_BASE_EMOJIS=['💡','⚠️','✅','📌','🔥','📝','🎯','💬','🔑','📚',
  '🎨','💎','🚀','🌟','🎵','❓','❗','🔔','📊','🗂️','🌈','⭐','🏆','💪',
  '🧠','🔍','💼','📅','🌐','🔒','🎉','✨','🛠️','📣','🤝','🌱','💫','🔗',
  '📱','🖥️','✍️','🗒️','🔺','🔻','▶️','⏸️','⏹️','🔄'];
const CO_COLORS=[
  {b:'#C7D7FF',bg:'#F0F4FF',t:'#1A3A8F'},
  {b:'#FFCDD2',bg:'#FFF3F5',t:'#B71C1C'},
  {b:'#C8E6C9',bg:'#F1F8F1',t:'#1B5E20'},
  {b:'#FFE082',bg:'#FFFDE7',t:'#5D4037'},
  {b:'#FFCC80',bg:'#FFF8F0',t:'#E65100'},
  {b:'#CE93D8',bg:'#F8F0FF',t:'#4A148C'},
  {b:'#E0E0E0',bg:'#F9F9F9',t:'#212121'},
  {b:'#B2EBF2',bg:'#E0F7FA',t:'#006064'},
  {b:'#F8BBD9',bg:'#FCE4EC',t:'#880E4F'},
  {b:'#DCEDC8',bg:'#F9FBE7',t:'#33691E'},
  {b:'#D1C4E9',bg:'#EDE7F6',t:'#311B92'},
  {b:'#B3E5FC',bg:'#E1F5FE',t:'#01579B'},
  {b:'#FFD180',bg:'#FFFBF0',t:'#BF6000'},
  {b:'#80CBC4',bg:'#E0F2F1',t:'#004D40'},
];

let _coPick={color:CO_COLORS[0],emoji:'💡'};
let _coCustomEmojis=[];
try{_coCustomEmojis=JSON.parse(localStorage.getItem('co_emojis')||'[]');}catch{}

function _saveCo(){localStorage.setItem('co_emojis',JSON.stringify(_coCustomEmojis));}

function _coEmojiHTML(forGrid, selEm){
  const all=[...CO_BASE_EMOJIS];
  const customs=_coCustomEmojis.map((em,i)=>{
    ...
    return `<span class="co-em-btn co-em-custom${em===selEm?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">
      ${inner}
      <span class="co-em-del" onclick="event.stopPropagation();delCoCustomEmoji(${i})" title="삭제">✕</span>
    </span>`;
  });
  const base=all.map(em=>`<span class="co-em-btn${em===selEm?' sel':''}" data-em="${em}" onclick="pickCoEmoji(this)">${em}</span>`);
...
function _coColorHTML(selB){
  return CO_COLORS.map((c,i)=>
    `<div class="co-sw${c.b===selB?' sel':''}" style="background:${c.b};" data-i="${i}" onclick="pickCoColor(this)"></div>`
  ).join('');
}
...
function openCalloutPicker(){
  ...
  if(cg)cg.innerHTML=_coColorHTML(_coPick.color.b);
  if(eg)eg.innerHTML=_coEmojiHTML(true,'💡');
...
function pickCoColor(el){
  ...
  _coPick.color=CO_COLORS[parseInt(el.dataset.i)];
}
function pickCoEmoji(el){
  ...
  el.classList.add('sel');
  _coPick.emoji=el.dataset.em||'';
}
function handleCoCustomImg(e){
  ...
  reader.onload=ev=>{
    ...
    ['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{
      ...
      if(g&&g.closest('[style*="block"]'))g.innerHTML=_coEmojiHTML(true,_coPick.emoji);
    });
    toast('커스텀 이모지 추가됨');
  };
  reader.readAsDataURL(file);
}
function delCoCustomEmoji(idx){
  _coCustomEmojis.splice(idx,1);_saveCo();
  ['coEmojiGrid','coEmojiChangeGrid'].forEach(id=>{
    const g=document.getElementById(id);
    if(g)g.innerHTML=_coEmojiHTML(true,_coPick.emoji);
  });
  toast('삭제됨');
}
function _buildCalloutHtml(color,emoji){
  ...
  const emojiHtml=emoji
    ?(isImg?`<img src="${emoji}" style="width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;">`
            :`<span style="font-size:1.2rem;flex-shrink:0;margin-top:1px;">${emoji}</span>`)
    :'';
  ...
  return `<div class="re-callout" contenteditable="false" style="${st}">
    ${emojiHtml}
    <div contenteditable="true" style="flex:1;min-width:0;outline:none;">내용을 입력하세요</div>
  </div><p><br></p>`;
}
...
function openCoEmojiChange(calloutEl){
  ...
  if(g)g.innerHTML=_coEmojiHTML(true,curEm);
...
function applyCoEmojiChange(){
  ...
  if(isImg){newEl.src=_coPick.emoji;newEl.style.cssText='width:22px;height:22px;object-fit:cover;border-radius:4px;flex-shrink:0;margin-top:2px;';}
  else{newEl.style.cssText='font-size:1.2rem;flex-shrink:0;margin-top:1px;';newEl.textContent=_coPick.emoji;}
  const old=_coChangeTarget.querySelector('[data-coem]');
  if(old)old.replaceWith(newEl);
  else{const body=_coChangeTarget.querySelector('div[contenteditable]');if(body)_coChangeTarget.insertBefore(newEl,body);}
  closeCoEmojiChange();
}
...
document.addEventListener('dblclick',function(e){
  ...
  if(callout&&editor&&editor.contains(callout)){
    openCoEmojiChange(callout);
  }
});
...
function showTblCtxMenu(e,td){
  ...
  const close=ev=>{menu.style.display='none';document.removeEventListener('mousedown',close);};
...
function tblCmd(cmd){
  ...
  if(cmd==='addRowAbove'||cmd==='addRowBelow'){
    ...
    if(nx&&(nx.tagName==='TD'||nx.tagName==='TH')){td.colSpan=(td.colSpan||1)+(nx.colSpan||1);td.innerHTML+=nx.innerHTML;nx.remove();}
    ...
    td.colSpan=sc;td.rowSpan=sr;td.innerHTML='';
    let subHtml='<table style="width:100%;border-collapse:collapse;">';
    for(let r2=0;r2<sr;r2++){subHtml+='<tr>';for(let c2=0;c2<sc;c2++){subHtml+='<td contenteditable="true" style="border:1px dashed #ccc;padding:6px;min-width:30px;"></td>';}subHtml+='</tr>';}
    subHtml+='</table>';
...
function openCellColorPicker(){
  if(!_tblTd){toast('셀을 먼저 우클릭하세요.');return;}
  const g=document.getElementById('cellClrGrid');
  if(g)g.innerHTML=CELL_COLORS30.map(c=>
    `<div class="clr-chip${c===_selCellClr?' sel':''}" style="background:${c};border-color:${c==='#FFFFFF'?'#ccc':'transparent'};" title="${c}" onclick="pickCellClr(this,'${c}')"></div>`
  ).join('');
...
function applyCellClr(color){
  if(_tblTd){_tblTd.style.background=color||'';closeCellClr();}
}
...
function _initImgResize(){
  ...
  document.addEventListener('mousemove',function(e){
    ...
    const nw=((_imgRsz.dir==='se'||_imgRsz.dir==='ne')?_imgRsz.sw+dx:_imgRsz.sw-dx);
    _imgRsz.w.style.width=Math.max(60,Math.min(nw,760))+'px';
  });
...
function reInsertTable(){
  ...
  for(let c=0;c<cols;c++)html+='<col style="width:'+cw+'%">';
  html+='</colgroup><tbody>';
